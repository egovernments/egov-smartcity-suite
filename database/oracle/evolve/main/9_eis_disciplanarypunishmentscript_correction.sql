...
update eg_script set script='from org.egov.exceptions import EGOVRuntimeException        
...
def user_approve():        
    try:        
        ...
        if wfItem.getState().getValue()==''NEW'' or wfItem.getState().getValue()==''APPROVER_REJECTED'':    
            ...
            if hirPos:    
                wfItem.setStatus(EisManagersUtill.getCommonsService().getStatusByModuleAndCode(''DisciplinaryPunishment'',''Created''))    
            ...
            wfItem.setStatus(EisManagersUtill.getCommonsService().getStatusByModuleAndCode(''DisciplinaryPunishment'',''Approved''))     
...
def user_reject():        
    try:        
        ...
        if wfItem.getState().getValue()==''CREATED'':   
            ...
            wfItem.setStatus(EisManagersUtill.getCommonsService().getStatusByModuleAndCode(''DisciplinaryPunishment'',''Rejected''))      
...
result,validationErrors' where name='DisciplinaryPunishment.workflow';

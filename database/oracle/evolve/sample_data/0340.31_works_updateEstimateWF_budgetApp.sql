...
if(action.getName()==''Approve'' or action.getName()==''Forward'') and wfItem.getCurrentState().getNextAction()==''Pending Admin Sanction''  and wfmatrix.getAdditionalRule()==''budgetApp'':
	try:
...
if(action.getName()==''Approve'' or action.getName()==''Forward'') and wfItem.getCurrentState().getNextAction()==''Pending Admin Sanction''  and wfmatrix.getAdditionalRule()==''depositCodeApp'':
	try:
...
if(validationErrors==None):
	...
	if(action.getName()==''Approve'' or action.getName()==''Forward'') and wfItem.getCurrentState().getValue()==''END''  and wfItem.getCurrentState().getPrevious().getValue()==''ADMIN_SANCTIONED''  and wfmatrix.getAdditionalRule()==''budgetApp'':
		try:
...
update eg_script set script=script||('
	if(action.getName()==''Approve'' or action.getName()==''Forward'') and wfItem.getCurrentState().getValue()==''END''  and wfItem.getCurrentState().getPrevious().getValue()==''ADMIN_SANCTIONED''  and wfmatrix.getAdditionalRule()==''depositCodeApp'':
		try:
			wfItem.setBudgetApprNo(persistenceService.getBudgetAppropriationNumber(wfItem))

...
def save():
    ...
    wfItem.changeState(element[1],eisCommonsManager.getPositionByUserId(wfItem.getCreatedBy().getId()),''MRIN Created'')
...
def reject():
	...
	prevNext=find_prev_next(wfItem.getCurrentState().getValue())	
        if wfItem.getCurrentState().getValue()==StoresConstants.WF_NEW_STATE or prevNext[0]==StoresConstants.WF_NEW_STATE:
            wfItem.changeState(''CANCELLED'',eisCommonsManager.getPositionByUserId(wfItem.getCreatedBy().getId()),''MRIN is Canceled'')
            getCurrentDate(wfItem)
	    ...
	    wfItem.changeState(prevNext[0],wfItem.getState().getPrevious().getOwner(),''Issue is rejected from approver.'')
	    getCurrentDate(wfItem)
	    result=persistenceService.persist(wfItem)
	return (result,None)
...
def submit_for_approval():
	...
	if further_prevNext[1]==StoresConstants.WF_END_STATE:
	    wfItem.changeState(prevNext[1],eisCommonsManager.getPositionByUserId(wfItem.getModifiedBy().getId()),''Issue is Approved'')
	    getCurrentDate(wfItem)
	    ...
	    if position != 0:
                wfItem.changeState(prevNext[1],position,''Issue is in Pending Approval'')
                getCurrentDate(wfItem)
                result=persistenceService.persist(wfItem)

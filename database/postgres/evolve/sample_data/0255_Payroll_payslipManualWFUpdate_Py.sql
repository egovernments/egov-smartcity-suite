...
from org.egov.pims.service import EisUtilService
...
def user_approve():    
    try:    
        ...
        if hirPos:                
            ...
            elif wfItem.getState().getValue()==''HOD_APPROVED'':
        ...
        raise ValidationException,[ValidationError(''ExceptionInWorkflow--'',e.getMessage())]' where name='EmpPayroll.workflow';
...
def user_reject():    
	...
	creatorPos=persistenceService.getEisService().getPositionsForUser(wfItem.getCreatedBy().getId(),Date())[0]	
        lowerPos=creatorPos
        if wfItem.getState().getValue()==''CLEARK_APPROVED'':
            wfItem.changeState(''REJECT'',''PENDING_APPROVAL'',lowerPos,comments)    
        elif wfItem.getState().getValue()==''HOD_APPROVED'':
            wfItem.changeState(''REJECT'',''PENDING_APPROVAL'',lowerPos,comments)    
        elif wfItem.getState().getValue()==''ACCOUNTANT_APPROVED'':    
            wfItem.changeState(''REJECT'',''PENDING_APPROVAL'',lowerPos,comments)    
        elif wfItem.getState().getValue()==''REJECT'':    

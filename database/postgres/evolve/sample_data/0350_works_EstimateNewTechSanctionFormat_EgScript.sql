...
SET SCRIPT='if not estimate.getFinancialDetails().isEmpty() and  estimate.getFinancialDetails().get(0): 
	result=estimate.getExecutingDepartment().getDeptCode()+"/TS/"+sequenceGenerator.getNextNumber("TECHNICALSANCTION_NUMBER",1).getFormattedNumber().zfill(4)+"/"+estimate.getFinancialDetails().get(0).getFunction().getCode()
else:
	result=estimate.getExecutingDepartment().getDeptCode()+"/TS/"+sequenceGenerator.getNextNumber("TECHNICALSANCTION_NUMBER",1).getFormattedNumber().zfill(4)'
where NAME='works.techsanctionnumber.generator'; 

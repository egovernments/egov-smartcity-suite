...
public class AdvertisementRateService {
    ...
    public AdvertisementRatesDetails getRatesBySubcategoryUomClassFinancialYearAndMeasurement(final Long subCategoryId,
            final Long unitOfMeasureId,
        ...
        if (units != null && subCategoryId != null && unitOfMeasureId != null && rateClassId != null)
            rate = rateDetailRepository.getRatesBySubcategoryUomClassFinancialYearAndMeasurement(units, subCategoryId,
                    unitOfMeasureId,
    ...
    public AdvertisementRatesDetails getRatesBySubcategoryUomClassAndMeasurementByFinancialYearInDecendingOrder(
            final Long subCategoryId, final Long unitOfMeasureId,
            final Long rateClassId, final Double units) {
        List<AdvertisementRatesDetails> rate = null;

        if (units != null && subCategoryId != null && unitOfMeasureId != null && rateClassId != null)
            rate = rateDetailRepository
                    .getRatesBySubcategoryUomClassMeasurementLessthanCurrentFinancialYearAndFinancialYearInDecendingOrder(units,
                            subCategoryId, unitOfMeasureId,
                            rateClassId);
        if (rate != null && !rate.isEmpty())
            return rate.get(0);
    ...
    public List<ScheduleOfRateSearch> getScheduleOfRateSearchResult(final String category, final String subCategory,
            final String unitofmeasure,
            final String classtype, final String finyear) {

        StringBuilder queryString = prepareQuery(category, subCategory, unitofmeasure, classtype, finyear);
        ...
        List<ScheduleOfRateSearch> scheduleOfRates = new ArrayList<>();
        rateList.forEach(result -> {
            ...
            scheduleOfRates.add(scheduleOfRateSearch);
        });

        return scheduleOfRates;
    }

    private StringBuilder prepareQuery(final String category, final String subCategory, final String unitofmeasure,
            final String classtype, final String finyear) {
        StringBuilder query = new StringBuilder();
        query.append("Select A From AdvertisementRatesDetails A where ");
        Boolean var = Boolean.FALSE;
        if (!StringUtils.isEmpty(category)) {
            query.append(" A.advertisementRate.category.id=:category ");
            var = Boolean.TRUE;
        }
        ...
        if (!StringUtils.isEmpty(subCategory)) {
            if (var)
                query.append(AND);
            query.append(" A.advertisementRate.subCategory.id=:subCategory ");
            var = Boolean.TRUE;
        }
        ...
        if (!StringUtils.isEmpty(unitofmeasure)) {
            if (var)
                query.append(AND);
            query.append(" A.advertisementRate.unitofmeasure.id=:unitofmeasure ");
            var = Boolean.TRUE;
        }
        ...
        if (!StringUtils.isEmpty(classtype)) {
            if (var)
                query.append(AND);
            query.append(" A.advertisementRate.classtype.id=:classtype ");
            var = Boolean.TRUE;
        ...
        if (!StringUtils.isEmpty(finyear)) {
            if (var)
                query.append(AND);
            query.append(" A.advertisementRate.financialyear.id=:finyear ");

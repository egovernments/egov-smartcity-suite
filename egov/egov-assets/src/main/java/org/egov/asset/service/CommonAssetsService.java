...
public class CommonAssetsService {
	...
	private static final Logger LOGGER = Logger
			.getLogger(CommonAssetsService.class);
	...
	public AssetCategory getAssetCategoryById(final Long categoryId) {
		...
		try {
			assetCategory = assetCategoryService.find(
					"from AssetCategory where id=?", categoryId);
		} catch (final HibernateException he) {
			LOGGER.error("Error while getting AssetCategoryById.");
			throw new EGOVRuntimeException(
					"Hibernate Exception : getting AssetCategoryById."
							+ he.getMessage(), he);
		}
	...
	public List<AssetCategory> getAllAssetCategory() {
		...
		try {
			...
			throw new EGOVRuntimeException(
					"Hibernate Exception : getting AllAssetCategoryList."
							+ he.getMessage(), he);
		}
	...
	public Asset getAssetByDeptId(final Integer deptId) {
		...
		try {
			...
			throw new EGOVRuntimeException(
					"Hibernate Exception : getting getAssetByDeptId."
							+ he.getMessage(), he);
		}
	...
	public Asset getAssetById(final Long assetId) {
		...
		try {
			...
			throw new EGOVRuntimeException(
					"Hibernate Exception : getting AssetById."
							+ he.getMessage(), he);
		}
	...
	public Asset getAssetByCode(final String code) {
		...
		try {
			...
			throw new EGOVRuntimeException(
					"Hibernate Exception : getting AssetByCode."
							+ he.getMessage(), he);
		}
	...
	public List<Asset> getAssetsByCategoryId(final Long categoryId) {
		...
		try {
			...
			throw new EGOVRuntimeException(
					"Hibernate Exception : getting AssetsByCategoryId."
							+ he.getMessage(), he);
		}
	...
	public AssetCategory getAssetCategoryByCode(final String code) {
		...
		try {
			assetCategory = assetCategoryService.find(
					"from AssetCategory where code=?", code);
		} catch (final HibernateException he) {
			LOGGER.error("Error while getting AssetCategoryByCode.");
			throw new EGOVRuntimeException(
					"Hibernate Exception : getting AssetCategoryByCode."
							+ he.getMessage(), he);
		}
	...
	public List<Asset> getAllAssets() {
		...
		try {
			...
			throw new EGOVRuntimeException(
					"Hibernate Exception : getting AllAssetList."
							+ he.getMessage(), he);
		}
	...
	public Asset createOrUpdateAsset(final Map<String, Object> parameters) {
		...
		if (parameters.get(ASSET_ID) != null)
			asset = assetService.findById((Long) parameters.get(ASSET_ID),
					false);
		// Check if passed invalid AssetId for update
		if (asset == null)
			throw new ValidationException(Arrays.asList(new ValidationError(
					"asset.id.doesNotExist", "asset.id.doesNotExist")));
		// **************************Update Logic End***********************
		...
		try {
			...
			if (parameters.get(ASSET_CODE) == null && asset.getCode() == null)
				throw new ValidationException(
						Arrays.asList(new ValidationError(
								"asset.code.mandatory", "asset.code.mandatory")));
			...
			if (parameters.get(ASSET_CATEGORY_ID) == null
				...
				final AssetCategory assetCategory = (AssetCategory) genericService
						.find("from AssetCategory where id=?",
								(Long) parameters.get(ASSET_CATEGORY_ID));
				if (assetCategory == null)
					throw new ValidationException(
							Arrays.asList(new ValidationError(
									"asset.category.doesNotExist",
									"asset.category.doesNotExist")));
			...
			if (parameters.get(ASSET_STATUS_ID) == null
				...
				final EgwStatus status = (EgwStatus) genericService.find(
						"from EgwStatus where id=?",
						(Integer) parameters.get(ASSET_STATUS_ID));
				if (status == null)
					throw new ValidationException(
							Arrays.asList(new ValidationError(
									"asset.status.doesNotExist",
									"asset.status.doesNotExist")));
			...
			if (parameters.get(ASSET_AREA_ID) != null) {
				final Boundary area = (Boundary) genericService.find(
						BOUNDARY_IMPL_QUERY,
						(Integer) parameters.get(ASSET_AREA_ID));
				if (area == null)
					throw new ValidationException(
							Arrays.asList(new ValidationError(
									"asset.area.doesNotExist",
									"asset.area.doesNotExist")));
				asset.setArea(area);
			}
			...
			if (parameters.get(ASSET_LOCATION_ID) != null) {
				final Boundary location = (Boundary) genericService.find(
						BOUNDARY_IMPL_QUERY,
						(Integer) parameters.get(ASSET_LOCATION_ID));
				if (location == null)
					throw new ValidationException(
							Arrays.asList(new ValidationError(
									"asset.location.doesNotExist",
									"asset.location.doesNotExist")));
			...
			if (parameters.get(ASSET_STREET_ID) != null) {
				final Boundary street = (Boundary) genericService.find(
						BOUNDARY_IMPL_QUERY,
						(Integer) parameters.get(ASSET_STREET_ID));
				if (street == null)
					throw new ValidationException(
							Arrays.asList(new ValidationError(
									"asset.street.doesNotExist",
									"asset.street.doesNotExist")));
			...
			if (parameters.get(ASSET_WARD_ID) != null) {
				final Boundary ward = (Boundary) genericService.find(
						BOUNDARY_IMPL_QUERY,
						(Integer) parameters.get(ASSET_WARD_ID));
				if (ward == null)
					throw new ValidationException(
							Arrays.asList(new ValidationError(
									"asset.ward.doesNotExist",
									"asset.ward.doesNotExist")));
				asset.setWard(ward);
			}
			...
			if (parameters.get(ASSET_MODE_OF_ACQUISITION) != null)
				asset.setModeOfAcquisition((String) parameters
						.get(ASSET_MODE_OF_ACQUISITION));
			...
			throw new EGOVRuntimeException(
					"Hibernate Exception : in createAsset." + he.getMessage(),
					he);
		}
	...
	public List<Asset> searchAssets(final Map<String, Object> parameters) {
		...
		final List<Object> params2 = (List<Object>) queryAndParam1
				.get("params");
	 ...
	 */
	public PaginatedList searchAssetPage(final Map<String, Object> parameters,
			final int pageNumber, final int pageSize) {
		final Map queryAndParams1 = getsearchAssetQuery(parameters);
		final String query = (String) queryAndParams1.get("query");
		final List<Object> params1 = (List<Object>) queryAndParams1
				.get("params");
		final Page page = assetService.findPageBy(query, pageNumber, pageSize,
				params1.toArray());
		final Long count = (Long) genericService.find("select count(*) "
				+ query, params1.toArray());
	 ...
	 * @return PaginatedList.
	 */
	public PaginatedList findAssetPage(final Map<String, Object> parameters,
			final int pageNumber, final int pageSize) {
	...
	private Map findAssetQuery(final Map<String, Object> parameters) {
		...
		if (!StringUtils.isEmpty((String) parameters.get(ASSET_CATEGORY_CODE))) {
			final AssetCategory assetCategory = (AssetCategory) genericService
					.find(" from AssetCategory where upper(code)=?",
							((String) parameters.get(ASSET_CATEGORY_CODE))
									.toUpperCase());
			if (assetCategory == null)
				throw new ValidationException("invalid.assetcategory.code",
						"Invalid Assetcategory Code");
			else {
		...
		if (!StringUtils
				.isEmpty((String) parameters.get(ASSET_DEPARTMENT_CODE))) {
			final Department dept = (Department) genericService.find(
					" from Department where upper(code)=?",
					((String) parameters.get(ASSET_DEPARTMENT_CODE))
							.toUpperCase());
			if (dept == null)
				throw new ValidationException("invalid.department.code",
		...
		if (!StringUtils.isEmpty((String) parameters.get(ASSET_WARD_NAME))) {
			final Boundary boundary = (Boundary) genericService
					.find(" from Boundary where upper(name)=? and boundaryType=(select id from BoundaryType where upper(name)=? and heirarchyType=(select id from HierarchyType where upper(name)=?))",
							((String) parameters.get(ASSET_WARD_NAME))
									.toUpperCase(), "WARD", "ADMINISTRATION");
			if (boundary == null)
				throw new ValidationException("invalid.ward.name",
						"Invalid Ward Name");
			else {
		...
		if (parameters.get(ASSET_DESCRIPTION) != null
				&& !((String) parameters.get(ASSET_DESCRIPTION)).trim()
						.equalsIgnoreCase("")) {
			sql.append(" and UPPER(asset.description) like '%'||?||'%'");
			paramsList.add(((String) parameters.get(ASSET_DESCRIPTION))
					.toUpperCase());
			counter++;
		}
		if (parameters.get(ASSET_STATUS_LIST) != null) {
			sql.append(" and asset.status.id in (?").append(counter)
					.append(")");
			final ArrayList<Integer> statuses = new ArrayList<Integer>();
			for (final String statusDesc : (String[]) parameters
					.get(ASSET_STATUS_LIST)) {
				final EgwStatus status = (EgwStatus) genericService
						.find(" from EgwStatus where upper(moduletype)='ASSET' and upper(description)=?",
								statusDesc.toUpperCase());
				if (status == null)
					throw new ValidationException("invalid.status.description",
	 ...
	 * @return PaginatedList
	 */
	public PaginatedList getAssetCategoryPage(final int pageNumber,
			final int pageSize) {
		final Map queryAndParamsCat = getAssetCategorySearchQuery(null);
		final String query = (String) queryAndParamsCat.get("query");
		final List<Object> paramsCat = (List<Object>) queryAndParamsCat
				.get("params");
		final Page page = assetCategoryService.findPageBy(query, pageNumber,
				pageSize, paramsCat.toArray());
		final Long count = (Long) genericService.find("select count(*) "
				+ query, paramsCat.toArray());
		return new EgovPaginatedList(page, count.intValue());
	 ...
	 */
	public List<AssetCategory> searchAssetCategory(
			final Map<String, Object> parameters) {
		final Map queryAndParamsCat = getAssetCategorySearchQuery(parameters);
		final String query = (String) queryAndParamsCat.get("query");
		final List<Object> paramsCat2 = (List<Object>) queryAndParamsCat
				.get("params");
	 ...
	 * @return PaginatedList
	 */
	public PaginatedList searchAssetCategoryPage(
			final Map<String, Object> parameters, final int pageNumber,
			final int pageSize) {
		final Map queryAndParamsCat = getAssetCategorySearchQuery(parameters);
		final String query = (String) queryAndParamsCat.get("query");
		final List<Object> paramsCat1 = (List<Object>) queryAndParamsCat
				.get("params");
		final Page page = assetCategoryService.findPageBy(query, pageNumber,
				pageSize, paramsCat1.toArray());
		final Long count = (Long) genericService.find("select count(*) "
				+ query, paramsCat1.toArray());
		return new EgovPaginatedList(page, count.intValue());
	}
	...
	private Map getsearchAssetQuery(final Map<String, Object> parameters) {
		...
		if (parameters.get(ASSET_CODE) != null
				&& !((String) parameters.get(ASSET_CODE)).trim()
						.equalsIgnoreCase("")) {
			sql.append(" and UPPER(asset.code) like '%'||?||'%'");
			paramsList1
					.add(((String) parameters.get(ASSET_CODE)).toUpperCase());
			counter++;
		}
		if (parameters.get(ASSET_DESCRIPTION) != null
				&& !((String) parameters.get(ASSET_DESCRIPTION)).trim()
						.equalsIgnoreCase("")) {
			sql.append(" and UPPER(asset.description) like '%'||?||'%'");
			paramsList1.add(((String) parameters.get(ASSET_DESCRIPTION))
					.toUpperCase());
			counter++;
		}
		if (parameters.get(ASSET_WARD_ID) != null) {
		...
		if (parameters.get(ASSET_STATUS_ID_LIST) != null) {
			final List<Integer> statusIds = (List<Integer>) parameters
					.get(ASSET_STATUS_ID_LIST);
	...
	private Map getAssetCategorySearchQuery(final Map<String, Object> parameters) {
		...
		if (parameters.get(ASSET_CATEGORY_CODE) != null
				&& !((String) parameters.get(ASSET_CATEGORY_CODE)).trim()
						.equalsIgnoreCase("")) {
			sql.append(" and UPPER(assetcategory.code) like '%'||?||'%'");
			paramsListCat.add(((String) parameters.get(ASSET_CATEGORY_CODE))
					.toUpperCase());
		}
		if (parameters.get(ASSET_CATEGORY_NAME) != null
				&& !((String) parameters.get(ASSET_CATEGORY_NAME)).trim()
						.equalsIgnoreCase("")) {
			sql.append(" and UPPER(assetcategory.name) like '%'||?||'%'");
			paramsListCat.add(((String) parameters.get(ASSET_CATEGORY_NAME))
					.toUpperCase());
		}
		queryAndParamsCat.put("query", sql.toString());
		queryAndParamsCat.put("params", paramsListCat);
	...
	public String getAllChilds(final Long parentCatgId) {
		...
		final List<Long> assetChildCategoryList = genericService
				.findAllByNamedQuery("ParentChildCategories", parentCatgId);
	...
	public void setAssetCategoryService(
			final AssetCategoryService assetCategoryService) {

...
@Result(name = "success", type = "redirect", location = "assetCategory.action")
...
public class AssetCategoryAction extends BaseFormAction {
	...
	private static final Logger LOGGER = Logger
			.getLogger(AssetCategoryAction.class);
	...
	public void prepare() {
		...
		isAutoGeneratedCode = appService
				.getUniqueAppConfigValue("IS_ASSET_CATEGORYCODE_AUTOGENERATED");
		if (id != null && id != -1)
			assetCategory = assetCategoryService.findById(id, false);
		super.prepare();

		setupDropdownDataExcluding("assetAccountCode", "accDepAccountCode",
				"revAccountCode", "depExpAccountCode");
		addDropdownData("assetTypeList", Arrays.asList(AssetType.values()));
		addDropdownData("depreciationMethodList",
				Arrays.asList(DepreciationMethod.values()));
		try {
			final String purposeId = appService.getUniqueAppConfigValue(
					module_asset, assetAccCodePURPOSEID);
			final List<CChartOfAccounts> assetAccounts = chartOfAccountsDAO
			...
			LOGGER.error("Error while loading dropdown data - assetAccountCodeList."
					+ e.getMessage());
			addFieldError("assetAccountCodeList",
					"Unable to load asset account information");
			throw new EGOVRuntimeException(
					"Unable to load asset account information", e);
		}

		try {
			final String purposeId = appService.getUniqueAppConfigValue(
					module_asset, accDepPURPOSEID);
			final List<CChartOfAccounts> accumulatedDeps = chartOfAccountsDAO
					.getAccountCodeByPurpose(Integer.valueOf(purposeId));
			addDropdownData("accDepAccountCodeList", accumulatedDeps);
		} catch (final EGOVException e) {
			LOGGER.error("Error while loading dropdown data - accDepAccountCodeList."
					+ e.getMessage());
			addFieldError("accDepAccountCodeList",
					"Unable to load accumulated depreciation information");
			throw new EGOVRuntimeException(
					"Unable to load accumulated depreciation information", e);
		}

		try {
			final String purposeId = appService.getUniqueAppConfigValue(
					module_asset, revResAccPURPOSEID);
			final List<CChartOfAccounts> revAccounts = chartOfAccountsDAO
					.getAccountCodeByPurpose(Integer.valueOf(purposeId));
			addDropdownData("revAccountCodeList", revAccounts);
		} catch (final EGOVException e) {
			LOGGER.error("Error while loading dropdown data - revAccountCodeList."
					+ e.getMessage());
			addFieldError("revAccountCodeList",
					"Unable to load revaluation account information");
			throw new EGOVRuntimeException(
					"Unable to load revaluation account information", e);
		}

		try {
			final String purposeId = appService.getUniqueAppConfigValue(
					module_asset, depExpAccPURPOSEID);
			final List<CChartOfAccounts> depExpenseAccounts = chartOfAccountsDAO
					.getAccountCodeByPurpose(Integer.valueOf(purposeId));
			addDropdownData("depExpAccountCodeList", depExpenseAccounts);
		} catch (final EGOVException e) {
			LOGGER.error("Error while loading dropdown data - depExpAccountCodeList."
					+ e.getMessage());
			addFieldError("depExpAccountCodeList",
					"Unable to load depreciation expense account information");
			throw new EGOVRuntimeException(
					"Unable to load depreciation expense account information",
					e);
		}
	 ...
	 */
	@Action(value = "/assetcategory/assetCategory-newform", results = @Result(name = NEW, location = "/WEB-INF/jsp/assets/assetcategory/assetCategory-new.jsp"))
	...
	@Action(value = "/assetcategory/assetCategory-edit", results = @Result(name = SEARCH, location = "/WEB-INF/jsp/assets/assetcategory/assetCategory-edit.jsp"))
	public String edit() {
		userMode = EDIT;
	...
	@Action(value = "/assetcategory/assetCategory-view", results = @Result(name = SEARCH, location = "/WEB-INF/jsp/assets/assetcategory/assetCategory-search.jsp"))
	public String view() {
		userMode = VIEW;
		dataDisplayStyle = "none";
		return SEARCH;
	...
	@Action(value = "/assetcategory/assetCategory-list", results = @Result(name = EDIT, location = "/WEB-INF/jsp/assets/assetcategory/assetCategory-search.jsp"))
	public String list() {
		if (!assetType.equalsIgnoreCase("-1") && (id == null || id == -1))
			assetCategoryList = assetCategoryService
					.findAllBy("from AssetCategory ac order by name asc");
		else if (!assetType.equalsIgnoreCase("-1") && (id == null || id == -1))
			assetCategoryList = assetCategoryService
					.findAllBy("from AssetCategory ac where ac.assetType="
	...
	public String save() throws NumberFormatException, EGOVException {
		...
		if (parentId != null && parentId != -1L)
			assetCategory.setParent(assetCategoryService.findById(parentId,
					false));
		...
		addActionMessage("\'" + assetCategory.getCode() + "\' "
				+ getText("asset.category.save.success"));
		userMode = EDIT;
	...
	protected void addDepMetaDatas() {
		...
		for (final DepreciationMetaData lDepreciationMetaData : depMetaDatas)
			if (validDepMetaData(lDepreciationMetaData)) {
				lDepreciationMetaData
						.setFinancialYear((CFinancialYear) getPersistenceService()
								.find("from CFinancialYear where id = ?",
										lDepreciationMetaData
	...
	protected boolean validDepMetaData(
			final DepreciationMetaData tDepreciationMetaData) {
		if (tDepreciationMetaData != null
				&& tDepreciationMetaData.getFinancialYear() != null
		...
		try {
			...
			if (categoryList != null && !categoryList.isEmpty()) {
				...
				while (assetCatIterator.hasNext()) {
					...
					parentMap.put(
							Long.valueOf(assetCategoryObject[0].toString()),
							assetCategoryObject[1].toString());
				}
			}

...
public class AjaxAssetAction extends BaseFormAction {
	...
	private static final Logger LOGGER = Logger
			.getLogger(AjaxAssetAction.class);
	...
	public String populateCategoryDetails() {
		assetCategory = (AssetCategory) getPersistenceService().find(
	...
	public String populateParentAssetCategoryList() {
		if (assetType == null || assetType.equalsIgnoreCase("-1"))
			assetCategoryList = getPersistenceService().findAllBy(
					"from AssetCategory where parent is null");
		else
			assetCategoryList = getPersistenceService().findAllBy(
					"from AssetCategory where parent is null and assetType=?",
					assetType);
		return PARENT_CATEGORIES;
	}
	...
	public String populateSubCategories() {
		if (parentCatId != -1 && parentCatId != null)
			assetSubCategoryList = getPersistenceService().findAllBy(
					"from AssetCategory where parent.id=?", parentCatId);
		else
	...
	public void setAssetSubCategoryList(
			final List<AssetCategory> assetSubCategoryList) {

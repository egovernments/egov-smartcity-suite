...
public abstract class AssetBaseSearchAction extends SearchFormAction {
	...
	private static final Logger LOGGER = Logger
			.getLogger(AssetBaseSearchAction.class);
	...
	public void prepare() {
		super.prepare();
		setupDropdownDataExcluding("area", "location", "street", "ward",
				"zone", "status");
	...
	protected void loadPreviousData() {
		...
		if (null != asset.getWard() && asset.getWard().getId() != -1) {
			addDropdownData("streetList",
					assetCommonUtil.populateStreets(asset.getWard().getId()));
			addDropdownData("areaList",
					assetCommonUtil.populateArea(asset.getWard().getId()));
		}
		if (null != asset.getArea() && asset.getArea().getId() != -1)
			addDropdownData("locationList",
					assetCommonUtil.populateLocations(asset.getArea().getId()));

...
<script>
	...
	var ADOPTESSADDRESS = "adoptionDetail.adopteeAddress";
	...
	function loadDesignationFromMatrix()
    {
      ...
      // var dept=document.getElementById('dept').options[document.getElementById('dept').selectedIndex].text;
       loadDesignationByDeptAndType('BirthRegistration',null,currentState,amountRule,additionalRule,pendingActions); 
    }
	...
	function retainCitizen(citizen,flag)
	{
		...
		populate(citizen+".middleName").value = NOTAVAILABLE;
		populate(citizen+".lastName").value = NOTAVAILABLE;
 	...
 	function formatDateBnd(strDate)
   {
	...
	function bodyOnLoad()
	{
		...
		/* registrationDate should be readonly for online registration   */
		var isMainUnit = populate("ismainregunit").value;
		 ...
		 if(parentAddressFlag[0].checked)
		    retainAddress(PARENTADDRESS);
		    
		 //Adoption details   
		 if(BIRTHADOPTIONFLAG==1){  
		  	if(populate("isChildAdopted").checked){
               dom.get("adoptionDetails").style.display='block';
	     	   enableAlltheAdoptionFields();
	        }
	        else{
	      	  dom.get("adoptionDetails").style.display='none';
	     	  disableAlltheAdoptionFields();
	        }
	        if(populate("mode").value=="edit" || populate("mode").value=="modify"){
	            enableAlltheAdoptionFields();
	        	populate("isChildAdopted").disabled = true;
	        	populate("adoptionDate").disabled = true;
	        }
	      }
	     ...
	     if(populate("mode").value=="nameinclusion")
	     {
	        
	   		for (var i =0; i < (document.forms[0].length-1);i++) {
				if (document.forms[0].elements[i].name != 'close' && document.forms[0].elements[i].id != 'nameinclusion' )
					document.forms[0].elements[i].disabled = true;		
			}
			populate("nameChange.receiptNo" ).disabled = false;
			populate("nameChange.firstName" ).disabled = false;
			populate("nameChange.middleName" ).disabled = false;
			populate("nameChange.lastName" ).disabled = false;
			populate("nameChange.remarks" ).disabled = false;
			populate("changename" ).disabled = false;
	     } 
	...
	/**
	 *   On submit to enable all the fields
	 */
      ...
      if(populate("mode").value=='offline' &&  HOSPITALUSERFLAG !=1 ){
         ...
         if(isNum(regNumber) && birthDate!=""){
        	...
        	makeJSONCall(["minValue","maxValue","uniqueRegFlag"], 
		    	'${pageContext.request.contextPath}/common/ajaxCommon!populateBirthRegNumRange.action',
 ...
 */
	...
	function validateForm(obj)
	{
	    changedString="";
	 	dom.get("birthRegistration_error").style.display = 'none';
	 	form = document.getElementById("birthRegistration");
        clearErrorMessages(form);
        clearErrorLabels(form);
		var isError = false;
		var error = "Required";
	 	if(!validateForm_birthRegistration() ){
	  	   isError = true;
		}
	    ...
	    if(populate("registrationUnit").value == -1){
	 	     addError(populate("registrationUnit"),error);
			 isError= true;
	 	}
	 	var mode = populate("mode").value;
	 	if(mode=="offline" && populate("registrationNo").value=="")
	 	{
	 		 addError(populate("registrationNo"),error);
			 isError= true;
	 	}
	    ...
	    {  
		    if(getSelectedInformantType()==-1)
		    {
		         warn('<s:text name="informat.relations.child.required"/>');
				 isError= true;
		    }
		 }
	 	if(BIRTHADOPTIONFLAG==1){
		 	if(!checkAdoptionDetails())
		  		isError= true;
		}
	...
	function populateInformant()
    {
		...
		clearErrorMessages(form);
        clearErrorLabels(form);
        makeReadOnlyCitizen(INFORMANT); /* informant name fields shouldbe readonly if father or mother is selected*/
		if(getSelectedInformantType()=='<s:property value="@org.egov.bnd.utils.BndConstants@FATHER"/>'){
		  	...
		  	if(!validateCitizen(citizen,'<s:text name="father.lbl"/>')){
		  		makeEmptyCitizen(INFORMANT,"false"); /* reset informant values if father name is changed to null*/
	      		return false;
	      	}
			...
			if(!validateCitizen(citizen,'<s:text name="mother.lbl"/>')){
				makeEmptyCitizen(INFORMANT,"false"); /* reset informant values if mother name is changed to null*/
		       return false;
		    }
		    populateInformantDetails(citizen);
		}
 ...
 */
  ...
  {
         /*if(populate(address+".addressID")!=null)
   ...
   function checkAdoptionDetails()
   {
      ...
      {
         if(!validateCitizen(ADOPTEEMOTHER,'<s:text name="adoptee.mother.lbl"/>'))
	      	isError = true;
	      	
		 if(!validateCitizen(ADOPTEEFATHER,'<s:text name="adoptee.father.lbl"/>'))
			isError = true;
	 
	 	  if(trimAll(populate("adoptionDetail.adopteeAddress.streetAddress1").value)=="")
	 	 {
     		//warn('<s:text name="adoptee.address.lbl"/>');
     		addError(populate("adoptionDetail.adopteeAddress.streetAddress1"),error);
     		isError = true;
     	 }
     		
		 if(trimAll(populate("adoptionDate").value)=="" )
		 {
	     	//warn('<s:text name="adoptiondate.required"/>');
	     	addError(populate("adoptionDate"),error);
	     	isError = true;
		 } 
	 
		 if(trimAll(populate("adoptionDetail.adoptionNumber").value)=="" )
		 {
		  	//warn('<s:text name="adoption.number.required"/>');
		  	addError(populate("adoptionDetail.adoptionNumber"),error);
		  	isError = true;
		 }
		 
		 if(trimAll(populate("adoptionDetail.affidavitNumber").value)=="" )
		 {
		  	//warn('<s:text name="adoption.affidavit.required"/>');
		  	addError(populate("adoptionDetail.affidavitNumber"),error);
		  	isError = true;
		 }
		 
	    /*if(!validateDateWithCurrentDate(populate("adoptionDate").value))
	   	{
	   		 alert('<s:text name="adoptiondate.today.validate"/>');
	   		 populate("adoption.adoptionDate").value="";
	   		 isError = true;
	    }
	 
	    if(!compareFirstDateGtThanSecondDate(populate("dateOfEvent").value,populate("adoptionDate").value ))
	    {
 	   		alert('<s:text name="adoption.date.birthdate.validate"/>');
 	   		populate("adoptionDate").value="";
 	   		//addError(populate("adoptionDetail.affidavitNumber"),error);
 	   		isError = true;
	    }*/
      }
   ...
   function disableAlltheAdoptionFields()
   {
		makeReadOnlyCitizen(ADOPTEEFATHER,"true");
		makeReadOnlyCitizen(ADOPTEEMOTHER,"true");
		populate("adoptionDetail.adopteeAddress.streetAddress1").readOnly= true; 
		populate("adoptionDate").disabled = true;
		populate("adoptionDetail.adoptionInstitute").disabled= true;
		populate("adoptionDetail.courtOrderNumber").readOnly = true;
		populate("adoptionDetail.adoptionNumber").readOnly = true; 
		populate("adoptionDetail.affidavitNumber").readOnly = true; 
		populate("adoptionDetail.remarks").readOnly = true; 
		populate("adoptionDetail").value = "-1";
		
   }
   
   function enableAlltheAdoptionFields()
   {
		makeEnableCitizen(ADOPTEEFATHER,"true");
		makeEnableCitizen(ADOPTEEMOTHER,"true");
		populate("adoptionDetail.adopteeAddress.streetAddress1").readOnly= false; 
		populate("adoptionDate").disabled = false;
		populate("adoptionDetail.adoptionInstitute").disabled= false;
		populate("adoptionDetail.courtOrderNumber").readOnly = false;
		populate("adoptionDetail.adoptionNumber").readOnly = false; 
		populate("adoptionDetail.affidavitNumber").readOnly = false; 
		populate("adoptionDetail.remarks").readOnly = false; 
	}
	
   
   function makeEmptyAdoptionDetails()
   {
        makeEmptyCitizen(ADOPTEEFATHER,"true");
        makeEmptyCitizen(ADOPTEEMOTHER,"true");
        populate("adoptionDetail.adopteeAddress.streetAddress1").value= ""; 
		populate("adoptionDate").value ="";
		populate("adoptionDetail.adoptionInstitute").value= "-1";
		populate("adoptionDetail.courtOrderNumber").value ="";
   		populate("adoptionDetail.affidavitNumber").value = ""; 
   		populate("adoptionDetail.adoptionNumber").value = ""; 
		populate("adoptionDetail.remarks").value = ""; 
		populate("adoptionDetail").value = "-1";
   }
   
   
   function populateAdoptionDetails()
   {
       populateAdoption();
	   makeEmptyAdoptionDetails();
   }
	  
  function populateAdoption()
  {
     if(populate("isChildAdopted").checked){
             dom.get("adoptionDetails").style.display='block';
	     	 enableAlltheAdoptionFields();
  ...
  /*
   *     This method is to populate Event Address
   */
  ...
  myAddressSuccessHandler = function(req,res){
	    ...
	    if(hosptype=='false'){
  		...
  		makeEmptyAddress(EVENTADDRESS);
  		makeEmptyAddress(INFORMANTADDRESS);
  		alert('For the selected hospital certificate cannot be registered here it has to be registered in the hospital where the birth occurred');
  		}
	    return true;
	}
  ...
  {
       return checkNoOfDigitBeforeAndAfterDecimal(obj,"",'<s:text name="invalid.weight"/>',1,3);
  }
  ...
  function getDropDownValue(value,dropDownObj)
  {
   ...
   function validateNameInclusion()
  { 
  
		    if(populate("receiptNoFlag").value=='true')
		    {
				   if(trimAll(populate("nameChange.receiptNo" ).value) == "" || populate("nameChange.receiptNo" ).value ==null)
				   {
				          dom.get("nameinclusion_error").style.display = '';
						   document.getElementById("nameinclusion_error").innerHTML = " Receipt Number is Required";
						    return false; 
				    }
			}   
		    if(trimAll(populate("nameChange.firstName" ).value) == "" || populate("nameChange.firstName" ).value ==null)
		   {
		          dom.get("nameinclusion_error" ).style.display = '';
				   document.getElementById("nameinclusion_error" ).innerHTML = "First Name is Required";
				    return false; 
		    }  
		 
		 if(trimAll(populate("nameChange.remarks").value) == "" || populate("nameChange.remarks" ).value ==null)
		   {
		          dom.get("nameinclusion_error" ).style.display = '';
				   document.getElementById("nameinclusion_error" ).innerHTML = "Remarks are Required";
				    return false; 
		    }  
		    
		    
  		if((populate("updateNameFlag").value) == "false")
			  {
		
				   if(populate("citizen.firstName").value != "NA")
				   {
				     alert("Child name is aleady exists .\n You cannot change this child name.");
				     return false;
				   }   
			}
...
function isvalidReceiptNumbers() 
  {
   if(populate("receiptNoFlag").value=='true')
   {
       	var receiptNum = trimAll(populate("nameChange.receiptNo").value);
   		 var id = populate("id").value;
    
	   if(receiptNum!="" && id!="")
	   {
	    dom.get("nameinclusion_error").style.display='none';
	    populatenameInclusionReceiptCheck({id:id,receiptNum:receiptNum});
	   } 
   }

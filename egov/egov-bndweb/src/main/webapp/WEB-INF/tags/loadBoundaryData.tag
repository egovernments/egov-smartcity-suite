...
<%@ tag import="org.egov.landestate.web.BoundaryUtil,
...
<%
	...
	if(l_assetId!=null) {
	 	bndryTypeValuesMap = bndryUtil.getMapofBndryTypeAndValuesForAssetId(l_assetId);
	 	selectedVals = bndryUtil.getSelectedBndryTypeValueMapForAsset(l_assetId);
...
<script type="text/javascript">
  ...
  function setBoundaryInfoForAsset(assetId) {
  	//var assetId = obj.value;
  
  	var url = "<%=request.getContextPath()%>"+"/common/landEstateAjax!getBoundaryInfoForAsset.action?assetId="+assetId;
  	var boundaryJSON;
  	var req = initiateRequest();
  		req.onreadystatechange = function(){
  		      if (req.readyState == 4){
  				...
  				boundaryJSON = retString.evalJSON(true);
  				for (var index in boundaryJSON.boundaryObjects) {
  				if (boundaryJSON.boundaryObjects.hasOwnProperty(index)) {

  				    var bType = boundaryJSON.boundaryObjects[index]
  				   // alert(bType.boundaryTypeName);
  				    var bTypeSelObj = document.getElementById(bType.boundaryTypeName);
  				   // alert(bTypeSelObj);
  				    var elOptNew = document.createElement('option');
				    elOptNew.text = bType.boundaryValue;
				    elOptNew.value = bType.idboundaryId
				    elOptNew.selected = true;
				    try {
				        bTypeSelObj.add(elOptNew, null); // standards compliant; doesn't work in IE
				      }
				      catch(ex) {
				        bTypeSelObj.add(elOptNew); // IE only
				      } 
				    bTypeSelObj.disabled = true;
  				}
  				}
  				var bTypeSelectObj;
					<s:iterator var="bndryType" value="#request.bndryTypeValuesMap.keySet()" status="rowstatus">
					bTypeSelectObj = document.getElementById("${bndryType.name}");
					bTypeSelectObj.disabled = true;
					</s:iterator>
  			   }
  		       }
  		};
...
function loadLeafLevelData(obj,values,heirarchyType, crossHier)
{
	...
	if (crossHier == '1')
	{
		url = "<%=request.getContextPath()%>"+"/common/landEstateAjax!getCrossHierarchyBoundaries.action?boundaryId="+obj.value;
		values = 'Area';
	}
	else
	{
		url = "<%=request.getContextPath()%>"+"/common/landEstateAjax!getChildBoundaries.action?boundaryId="+obj.value;
	}

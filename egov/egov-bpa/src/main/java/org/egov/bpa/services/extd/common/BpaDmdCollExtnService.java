...
public class BpaDmdCollExtnService {
	...
	//	PersistenceService persistenceService = feeService.getPersistenceService();
		...
		try {
			...
			List<BpaFeeDetailExtn> bpafeeDet = feeExtnService.getFeeDetailByServiceTypeandArea(serviceTypeId, areasqmt, feeType);
			...
			if (bpafeeDet != null && !bpafeeDet.isEmpty()) {
				...
				for (BpaFeeDetailExtn feeDet : bpafeeDet) {
					if (!feeDet.getAmount().equals(BigDecimal.ZERO)) {
						isAmountExist = true;
						EgDemandDetails dmdDet = createDemandDetail(feeDet.getBpafee(),feeDet.getAmount());
	...
	protected EgDemandDetails createDemandDetail(BpaFeeExtn  feeDet,BigDecimal amount) {
		...
		if (bpafeeDet!=null && !bpafeeDet.isEmpty() && bpafeeDet.size() != 0) {
			...
			try {
				...
				for (BpaFeeDetailExtn feeDet : bpafeeDet) {
					...
					EgDemandReason dmdRsn = getEgDemandReason(feeDet.getBpafee());
					if(dmdRsn!=null)
						dmdRsnList.add(dmdRsn);
					// Checking whether demand reason already present in existing demand ? Id yes then update demand details. 
					List<EgDemandDetails> dmdDetList = demandGenericDao
							.getDemandDetailsForDemandAndReasons(demand, dmdRsnList);
					
					if (dmdDetList != null && !dmdDetList.isEmpty() && dmdDetList.size() != 0) {
					...
					else 
					{
						if (!feeDet.getAmount().equals(BigDecimal.ZERO)) {
							...
							for(EgDemandDetails dmdDtl:demand.getEgDemandDetails() ){
								// Check any demand details contain admission fee type ? if yes... update the fee..Else create new demand detail
								if(dmdRsn!=null  && dmdRsn.getEgDemandReasonMaster().getReasonMaster()!=null
										&& feeDet.getBpafee()!=null &&  dmdRsn.getEgDemandReasonMaster().getReasonMaster()!=null && feeDet.getBpafee().getFeeDescription()!=null &&  dmdRsn.getEgDemandReasonMaster().getReasonMaster().trim().equalsIgnoreCase(feeDet.getBpafee().getFeeDescription().trim()))
								{ 
									// using the same fee detail for admission fee. We are not deleting if user changed service type.
							...
							if(createNewDemandDetailFlag){
								...
								EgDemandDetails dmdDet = createDemandDetail(feeDet.getBpafee(),feeDet.getAmount());
								totaldmdAmt = totaldmdAmt.add(feeDet.getAmount());
								demand.setBaseDemand(demand.getBaseDemand().add(feeDet.getAmount()));
			...
			try {
				for (BpaFeeDetail feeDet : bpafeeDet) {
					...
					List<EgDemandDetails> dmdDetList = demandGenericDao
							.getDemandDetailsForDemandAndReasons(demand, dmdRsnList);
				   check if EgDemandDetails exist for this EgDemandReason (ie. BpaFeeDetail), then update existing EgDemandDetails
					else create new EgDemandDetails and add to existing demand
					if (dmdDetList != null && !dmdDetList.isEmpty() && dmdDetList.size() != 0) {
						EgDemandDetails dmdDet = dmdDetList.get(0);
						
						if the BpaFeeDetail amount is ZERO and the EgDemandDetails exists, then remove the specific 
							EgDemandDetails	EgDemand. Else update the existing EgDemandDetails
						if (feeDet.getAmount().equals(BigDecimal.ZERO)) {
							removeDmdDetSet.add(dmdDet);
						} else {
						...
						if (!feeDet.getAmount().equals(BigDecimal.ZERO)) {
							EgDemandDetails dmdDet = createDemandDetail(feeDet);
	...
	public EgDemandReason getEgDemandReason(BpaFeeExtn bpaFee) {
		...
		try {
			...
			egDemandReason = (EgDemandReason) demandGenericDao
					.getDmdReasonByDmdReasonMsterInstallAndMod(egDemandReasonMaster, getCurrentInstallment(), module);	
			
			 if (egDemandReason == null) {
				 egDemandReason = createEgDemandReason(bpaFee,egDemandReasonMaster);
			}
			
		} catch(Exception ex) {
			throw new EGOVRuntimeException("Exception in getEgDemandReason method." +ex);	
		}
	...
	public EgDemandReasonMaster createEgDemandReasonMaster(BpaFeeExtn bpaFee) {
		...
		egDmdRsnMstr = (EgDemandReasonMaster) demandGenericDao.getDemandReasonMasterByCode(bpaFee.getFeeCode(), module);
	...
	*/	
	...
	public RegistrationExtn generateDemandUsingSanctionFeeList(
		...
		EgDemand dmd = registrationObj.getEgDemand()!=null?registrationObj.getEgDemand():buildDemandObject(new HashSet<EgDemandDetails>(), BigDecimal.ZERO,
				installment);
			...
			//if(bpaFee.getFeeGroup()!=null && bpaFee.getFeeGroup().equals(BpaConstants.COCFEE)){
				if (bpaFee.getFeeAmount()!=null&&!bpaFee.getFeeAmount().equals(BigDecimal.ZERO)) {
					...
					if(bpaFee.getDemandDetailId()==null){
						EgDemandDetails dmdDet =createDemandDetail(bpaFee,bpaFee.getFeeAmount());
						if(dmdDet!=null){
						...
						{
							if(dmdDtl.getId()!=null && dmdDtl.getId().equals(bpaFee.getDemandDetailId())){
					...
					if(bpaFee.getDemandDetailId()!=null&&!bpaFee.getDemandDetailId().equals("")){
						delDmdDetailList.add(bpaFee.getDemandDetailId()); // Delete from the existing list.
					}
				}
			//}
		}
	...
	public RegistrationExtn updateDemandByUsingSanctionFeeList(
		...
		for(BpaFeeExtn bpaFee:santionFeeList){
			...
			// if for demand reason , if amount present, then we are updating/adding the same in demand details.
			if (bpaFee.getFeeAmount()!=null&& bpaFee.getFeeAmount().compareTo(BigDecimal.ZERO)>0) {
				
					  
					 for(EgDemandDetails dmdDtl : dmd.getEgDemandDetails())
						{
							if(dmdReason!=null && dmdDtl.getEgDemandReason()!=null && dmdDtl.getEgDemandReason().getId().equals(dmdReason.getId())){
								dmdDtl.setAmount(bpaFee.getFeeAmount());

...
public class LetterToPartyExtnService extends PersistenceService<LetterToPartyExtn, Long>{
	...
	public CMDALetterToPartyExtn getLatestCMDALetterToPartyForRegObj(RegistrationExtn registrationObj) {
		CMDALetterToPartyExtn lettertoParty = null;
		
		Criteria lpCriteria = getSession().createCriteria(CMDALetterToPartyExtn.class,"letterToParty");
		lpCriteria.createAlias("letterToParty.registration", "registration");
		lpCriteria.add(Restrictions.eq("registration", registrationObj));
		lpCriteria.addOrder(Order.desc("letterToParty.id"));
		lpCriteria.setMaxResults(1);
	...
	public List<CMDALetterToPartyExtn> getListofCMDALetterToPartyForRegObj(RegistrationExtn registrationObj,CMDALetterToPartyExtn letterToParty) {

 ...
 */
package org.egov.bpa.web.actions.common;
...
@Results( { @Result(name = AjaxCommonAction.AJAX_RESULT, type = StreamResult.class, value = "returnStream", params = { "contentType","text/plain" }) })
@ParentPackage("egov")
public class AjaxCommonAction extends BaseFormAction {
	...
	public String getCrossHierarchyBoundaries() {
		List<Boundary> bndryList = bpaCommonService.populateArea(boundaryId);
	...
	public String ajaxLoadActionsForSearch(){
		...
		String message=bpaCommonService.getValidActionDropdowns(arguments,registration);
	...
	//	BpaFee fee=(BpaFee)persistenceService.find("from BpaFee where serviceType.id=? ", serviceTypeId);
		//if(fee!=null && fee.getFeeType()!=null){
			admissionfeeAmount=feeService.getTotalFeeAmountByPassingServiceTypeandArea(serviceTypeId, sitalAreaInSqmt, BpaConstants.ADMISSIONFEE);
		//}
	...
	public String ajaxGetMandatoryFieldsForServiceType(){
		...
		ServiceType serviceTypeObj = (ServiceType) persistenceService.find("from ServiceType where id=?",serviceTypeId);
		...
		returnStream =isCMDA.toString()+"-"+isAutoDcrReq.toString()+"-"+isPropertyMandatory.toString();
		return AJAX_RESULT;

...
public class InspectionExtnAction extends BaseFormAction{
	...
	private final static  Logger LOGGER=Logger.getLogger(InspectionExtnAction.class);
	...
	public void setBpaDmdCollExtnService(BpaDmdCollExtnService bpaDmdCollExtnService) {
	...
	public void prepare()
	{
		...
		if(getRegistrationId()!=null){
			...
			if(registrationObj!=null &&  registrationObj.getServiceType()!=null && registrationObj.getServiceType().getCode()!=null &&
				...
				if(!isUserMappedToSurveyorRole()){
					existingSiteInspectionDetails=inspectionExtnService.getSiteInspectionListforRegistrationObject(registrationObj);
					if(existingSiteInspectionDetails.isEmpty()){
						List<InspectionExtn> surveyorIns=  new ArrayList<InspectionExtn>();
						surveyorIns=inspectionExtnService.getSiteInspectionListforRegBySurveyorAndOfficial(registrationObj);
						InspectionExtn insObj=surveyorIns.get(0);
						inspection=new InspectionExtn(insObj.getLandZoning(),insObj.getLayoutType(),insObj.getLndMinPlotExtent(),insObj.getLndProposedPlotExtent(),
								insObj.getLndOsrLandExtent(),insObj.getLndGuideLineValue(),insObj.getLandUsage(),insObj.getLndRegularizationArea(),
								insObj.getLndPenaltyPeriod(),insObj.getLndIsRegularisationCharges(),insObj.getBuildingType(),insObj.getBldngBuildUpArea(),
								insObj.getBldngProposedPlotFrntage(),insObj.getBldngRoadWidth(),insObj.getBldngProposedBldngArea(),
								insObj.getBldngGFloor_TiledFloor(),insObj.getBldngGFloor_OtherTypes(),insObj.getBldngFrstFloor_TotalArea(),
								insObj.getBldngStormWaterDrain(),insObj.getBldngCompoundWall(),insObj.getBldngWellOht_SumpTankArea(),insObj.getBldngCommercial(),
								insObj.getBldngResidential(),insObj.getBldngIsRegularisationCharges(),insObj.getBldngIsImprovementCharges(),insObj.getDwellingUnit()); 
					} 
				}
			}
		...
		addDropdownData("surroundedBuildingList", inspectionExtnService.getAllSurroundedBuildingDetails());
		addDropdownData("layoutTypeList", inspectionExtnService.getAllLayoutTypes()); 
		addDropdownData("landUsageList", inspectionExtnService.getAllLandBuildingTypes(LAND_TYPE)); 
		addDropdownData("buildingTypeList", inspectionExtnService.getAllLandBuildingTypes(BUILDING_TYPE)); 
		addDropdownData("penaltyPeriodList", getPenaltyPeriod());
		addDropdownData("stormWaterDrainList",bpaCommonExtnService.getAllStormWaterDrain());
	...
	public Map<Integer,String> getFloorNoMap()
	{
		...
		if(inspection.getDocket()==null || inspection.getDocket().getFloorCount()==null)
			return  Collections.emptyMap();
		else
		{
			floorNoMap=bpaCommonExtnService.getFloorNumberMap(inspection.getDocket().getFloorCount());
			return floorNoMap;
		}
	}
...
{
	if(inspection!=null && inspection.getRegistration()!=null && inspection.getDocket()==null){
			List<CheckListDetailsExtn> docketDocChkListDetails=inspectionExtnService.getCheckListByServiceAndType(inspection.getRegistration().getServiceType().getId(),"DOCKETSHEET-DOCUMENTENCLOSED");
			List<CheckListDetailsExtn> constStageChkListDetails=inspectionExtnService.getCheckListByServiceAndType(inspection.getRegistration().getServiceType().getId(),"DOCKETSHEET-CONSTRUCTIONSTAGE");

			 List <CheckListDetailsExtn> 	devContrlChkListDetails = inspectionExtnService.getCheckListByServiceAndType(inspection.getRegistration().getServiceType().getId(),"DOCKETSHEET-VIOLATION-DEVCONTROLRULE");
			 List <CheckListDetailsExtn> 	setBackChkListDetails = inspectionExtnService.getCheckListByServiceAndType(inspection.getRegistration().getServiceType().getId(),"DOCKETSHEET-VIOLATION-SETBACK");
			 List <CheckListDetailsExtn> 	parkingChkListDetails = inspectionExtnService.getCheckListByServiceAndType(inspection.getRegistration().getServiceType().getId(),"DOCKETSHEET-VIOLATION-PARKING");
			 List <CheckListDetailsExtn> 	generalChkListDetails = inspectionExtnService.getCheckListByServiceAndType(inspection.getRegistration().getServiceType().getId(),"DOCKETSHEET-VIOLATION-GENERAL");
			 List <CheckListDetailsExtn> 	minDistancePowerLineChkListDetails = inspectionExtnService.getCheckListByServiceAndType(inspection.getRegistration().getServiceType().getId(),"DOCKETSHEET-VIOLATION-MINDISTANCEPOWERLINE");
			...
			{
				docketFloorDetail.add(new DocketFloorDetails());// commenting cos as of now they dont need docket sheet for servicetype 2
			}*/
		...
		{	
			if( !inspection.getDocket().getDocumentEnclosedSet().isEmpty())
				docketDocumentDtls= new ArrayList<DocketDocumentDetails> (inspection.getDocket().getDocumentEnclosedSet());
			if( !inspection.getDocket().getConstructionStagesSet().isEmpty())
				constructionStages=new ArrayList<DocketConstructionStage> (inspection.getDocket().getConstructionStagesSet());
			if( !inspection.getDocket().getDocketFlrDtlsSet().isEmpty()){
				docketFloorDetail.clear();
				docketFloorDetail=new ArrayList<DocketFloorDetails> (inspection.getDocket().getDocketFlrDtlsSet());
			}
			if( !inspection.getDocket().getViolationSet().isEmpty()){
					for( DocketViolations docViolations: inspection.getDocket().getViolationSet())
					{
...
public String printDocketSheet()
{
	LOGGER.debug("Entered into print docket sheet method");
 	RegistrationExtn registration=registerBpaExtnService.findById(registrationId,false);
	...
	private void getLandBldngZoning() {
		...
		landZoningMap.put(LandBldngZoneingExtn.CONTINOUS.toString(), LandBldngZoneingExtn.CONTINOUS.toString());
		landZoningMap.put(LandBldngZoneingExtn.DETACHED.toString(), LandBldngZoneingExtn.DETACHED.toString());
		landZoningMap.put(LandBldngZoneingExtn.EWS.toString(), LandBldngZoneingExtn.EWS.toString());
		
		bldngZoningMap = new LinkedHashMap<String, String>();
		bldngZoningMap.put(LandBldngZoneingExtn.CONTINOUS.toString(), LandBldngZoneingExtn.CONTINOUS.toString());
		bldngZoningMap.put(LandBldngZoneingExtn.DETACHED.toString(), LandBldngZoneingExtn.DETACHED.toString());
		bldngZoningMap.put(LandBldngZoneingExtn.EWS.toString(), LandBldngZoneingExtn.EWS.toString());
	}	
	...
	public String newForm(){
		...
		autoGenInspectionDates=bpaCommonExtnService.getAppconfigValueResult(BpaConstants.BPAMODULENAME,BpaConstants.AUTO_GENERATION_INSPCTIONDATES,null);
		if(null!=autoGenInspectionDates && !"".equals(autoGenInspectionDates) && autoGenInspectionDates.equalsIgnoreCase("YES")){
			...
			if(isUserMappedToSurveyorRole()){
				siteInspectionScheduledateList=bpaCommonExtnService.autoGenerateSiteInspectionDatesForSurveyor(registrationObj);
			}
			else{
			  siteInspectionScheduledateList=bpaCommonExtnService.autoGenerateSiteInspectionDates(registrationObj);
			}
			 if(!siteInspectionScheduledateList.isEmpty()){
				 for(int i=0;i<siteInspectionScheduledateList.size();i++){
					 dateStringBuffer.append(siteInspectionScheduledateList.get(i));
	...
	public String inspectionForm(){
		...
		if(!isUserMappedToSurveyorRole() && inspection.getRegistration()!=null  && inspection.getRegistration().getServiceType()!=null && inspection.getRegistration().getServiceType().getCode()!=null &&
				(inspection.getRegistration().getServiceType().getCode().equals(BpaConstants.NEWBUILDINGONVACANTPLOTCODE) ||
						inspection.getRegistration().getServiceType().getCode().equals(BpaConstants.DEMOLITIONRECONSTRUCTIONCODE) ||
						inspection.getRegistration().getServiceType().getCode().equals(BpaConstants.ADDITIONALCONSTRUCTIONCODE) ||
						inspection.getRegistration().getServiceType().getCode().equals(BpaConstants.APPLICATIONFORDEMOLITIONCODE) ||
						inspection.getRegistration().getServiceType().getCode().equals(BpaConstants.CMDACODE)) ){
				buildDocketObjectDetails();
		}
		existingSiteInspectionDetails=inspectionExtnService.getSiteInspectionListforRegistrationObject(registrationObj);
		if(existingSiteInspectionDetails.size()!=0){
			setLastInspectionDate(sdf.format(existingSiteInspectionDetails.get(0).getInspectionDate())); 
	...
	private void showInspectionDetails() {
		postponedInspectionDetails=inspectionExtnService.getInspectionListforRegistrationObject(registrationObj);
	...
	public String  create(){
		if(inspection.getInspectionDate()==null||inspection.getInspectionDate().equals("")){
		...
		if(postponedInspectionDetails.size()>0){
			...
			inspection.getParent().setPostponementReason(inspection.getPostponementReason());
		} 
		oldStatus=inspection.getRegistration().getEgwStatus();
		if(isUserMappedToSurveyorRole())
			inspectionExtnService.save(inspection,loginUser,null,Boolean.FALSE,bpaCommonExtnService.getstatusbyCode(BpaConstants.SITEINSPECTIONSCHEDULEDBYLS));
		else
			inspectionExtnService.save(inspection,loginUser,null,Boolean.FALSE,bpaCommonExtnService.getstatusbyCode(BpaConstants.INSPECTIONSCHEDULED));
		String UserRole=bpaCommonExtnService.getUserRolesForLoggedInUser();
		if(UserRole!=null && !"".equals(UserRole) && !UserRole.contains(BpaConstants.PORTALUSERSURVEYORROLE))
		{
		bpaCommonExtnService.buildEmail(registrationObj,BpaConstants.SMSEMAILINSPECTION,null); 
		bpaCommonExtnService.buildSMS(registrationObj,BpaConstants.SMSEMAILINSPECTION);  
		} 
		showInspectionDetails();
	...
	public String  createSiteInspection(){
		...
		Set<InspectMeasurementDtlsExtn> inspectionmeasuredtls=new HashSet<InspectMeasurementDtlsExtn>(getInspectMeasurementDtls());
		Set<InspectMeasurementDtlsExtn> constructionmeasuredtls=new HashSet<InspectMeasurementDtlsExtn>(getConstructionMesDtls());
		Set<InspectionChecklistExtn> checkListDtls=new HashSet<InspectionChecklistExtn>(getChkListDet());
		
		if(getConstructionstage()!=null)		
		inspection.getInspectionDetails().setConstStages(inspectionExtnService.getConstructionStage(getConstructionstage()));
		
		inspection.getInspectionDetails().setInspectMeasurementDtlsSet(inspectionmeasuredtls);
		inspection.getInspectionDetails().getInspectMeasurementDtlsSet().addAll(constructionmeasuredtls);
		// To set BuildingZone and LandProposedPlotExtent with LandZoning and LndMinPlotExtent value
		...
		for(InspectMeasurementDtlsExtn inspectionMeasuremtDtls:inspection.getInspectionDetails().getInspectMeasurementDtlsSet()){
			inspectionMeasuremtDtls.setInspectionDetails(inspection.getInspectionDetails());
			if (inspection.getRegistration().getServiceType().getCode() != null
						&& inspection.getRegistration().getServiceType().getCode().equals(BpaConstants.RECLASSIFICATION)) {
				if(inspectionMeasuremtDtls.getSurroundedByEast()!=null && inspectionMeasuremtDtls.getSurroundedByEast().getId()!=null
						&& inspectionMeasuremtDtls.getSurroundedByEast().getId()==-1)   
				{
					inspectionMeasuremtDtls.setSurroundedByEast(null); 
				}
				if(inspectionMeasuremtDtls.getSurroundedByWest()!=null && inspectionMeasuremtDtls.getSurroundedByWest().getId()!=null
					&& inspectionMeasuremtDtls.getSurroundedByWest().getId()==-1)
				{
					inspectionMeasuremtDtls.setSurroundedByWest(null);
				}
			}
		} 
		inspection.getInspectionDetails().setInspectionChecklistSet(checkListDtls);
		for(InspectionChecklistExtn inspectionchklist:inspection.getInspectionDetails().getInspectionChecklistSet()){
			inspectionchklist.setInspectionDetails(inspection.getInspectionDetails());
		}
						...
						inspection.getRegistration().getServiceType().getCode().equals(BpaConstants.CMDACODE)) ){
						 if(inspection.getDocket()!=null ){
							...
							//SEt docket object reference in docket document detail.
							 Set<DocketDocumentDetails> docDetails=new HashSet<DocketDocumentDetails>(getDocketDocumentDtls());
							
							 Set<DocketConstructionStage> consTDetails=new HashSet<DocketConstructionStage>(getConstructionStages());
							 
							 Set<DocketViolations> violDetails=new HashSet<DocketViolations>(getGeneralList());
							 Set<DocketFloorDetails> floorDetails=new HashSet<DocketFloorDetails>(getDocketFloorDetail());
							 if(minDistancePowerLineList!=null && minDistancePowerLineList.size()>0)
						    ...
						    for(DocketDocumentDetails docketDocument :inspection.getDocket().getDocumentEnclosedSet())
						    {
						    	docketDocument.setDocket(inspection.getDocket());
						    }
						    for(DocketConstructionStage constStage :inspection.getDocket().getConstructionStagesSet())
						    {
						    	constStage.setDocket(inspection.getDocket());
						    }
						    for(DocketViolations violation :inspection.getDocket().getViolationSet())
						    {
		...
		if((inspection.getInspectionNum()==null)||(inspection.getInspectionNum()!=null&&inspection.getInspectionNum().equals("")))
		inspection.setInspectionNum(bpaNumberGenerationExtnService.generateSiteInspectionNumber());
		oldStatus=inspection.getRegistration().getEgwStatus();
		
		
		if(isUserMappedToSurveyorRole()){
			inspectionExtnService.save(inspection, null, null, Boolean.TRUE,bpaCommonExtnService.getstatusbyCode(BpaConstants.INSPECTEDBYLS));	
		}else{
			inspectionExtnService.save(inspection, null, null, Boolean.TRUE,bpaCommonExtnService.getstatusbyCode(BpaConstants.INSPECTED));
		}
		...
		/*
		 * Auto Generate Fee's and update demand on creation or modification of inspection. Each time new fee details will be generated. 
		 */
		if(!isUserMappedToSurveyorRole() && inspection.getRegistration()!=null  && inspection.getRegistration().getServiceType()!=null && inspection.getRegistration().getServiceType().getCode()!=null &&
				(inspection.getRegistration().getServiceType().getCode().equals(BpaConstants.NEWBUILDINGONVACANTPLOTCODE) ||
						inspection.getRegistration().getServiceType().getCode().equals(BpaConstants.DEMOLITIONRECONSTRUCTIONCODE) ||
			...
			//Based on registration, calculate fee and save fee and demand details.
			 List<BpaFeeExtn> santionFeeList=new ArrayList();
			 
			 //HibernateUtil.getCurrentSession().flush();
			 if(inspection.getRegistration()!=null){
					santionFeeList=	feeExtnService.getAllSanctionedFeesbyServiceType(inspection.getRegistration().getServiceType().getId());
			 }
			 for (BpaFeeExtn feeObject : santionFeeList) {

					// Check is required to calculate for current service type ?
					if (feeDetailsExtnService.isFeeCalculationRequiredForServiceType(inspection, inspection.getRegistration(),feeObject.getFeeCode(),
							inspection.getRegistration().getServiceType().getCode())) {
						//feeDetailsExtnService.setFeeExtnService(feeExtnService);
						feeDetailsExtnService.calculateFeeByServiceType(feeObject, inspection.getRegistration(), inspection);
					}
				}
			 if(!santionFeeList.isEmpty()) {
				  
				 registrationObj=bpaDmdCollExtnService.updateDemandByUsingSanctionFeeList(santionFeeList,inspection.getRegistration());
				 oldStatus=inspection.getRegistration().getEgwStatus();
				 ...
				 registrationObj=registrationFeeExtnService.saveFeesinRegistrationFee(registrationObj,santionFeeList);
		...
		if (inspection.getRegistration().getServiceType().getCode() != null
				&& inspection.getRegistration().getServiceType().getCode().equals(BpaConstants.RECLASSIFICATION)) {
			setServiceTypeCode(inspection.getRegistration().getServiceType().getCode());
		}
		...
		if(isUserMappedToSurveyorRole()){
			...
			if(inspection.getRegistration()!=null){
				...
				setServiceRegId(inspection.getRegistration().getServiceRegistryId().toString());
			}
		}
		return SUCCESS;
	}
	...
	private void buildDocumentFloorDetails()
	{
		...
		docketFloorDetail.removeAll(temp);
		 if(docketFloorDetail!=null && docketFloorDetail.size()>0){
	    	...
	    	{
				if(unitdetail.getFloorNum()!=null && !"".equals(unitdetail.getFloorNum()))
	...
	private void buildPlanDetails() {
		...
		ServiceTypeExtn serviceType=inspectionExtnService.getServiceTypeById(getServiceTypeId());
		...
		planDetails.setInspectionSource(inspectionExtnService.getInspectionSource(BpaConstants.INSPECTIONSOURCEFOREXISTINGPLAN));		
		siteDetails.setInspectionSource(inspectionExtnService.getInspectionSource(BpaConstants.INSPECTIONSOURCEFOREXISTINGSITE));	
		...
		if(getInspectionId()==null||getInspectionId().equals("")){		
			...
			if(serviceType.getCode()!=null&&serviceType.getCode().equals(BpaConstants.APPLICATIONFORDEMOLITIONCODE)||
					serviceType.getCode()!=null&&serviceType.getCode().equals(BpaConstants.DEMOLITIONRECONSTRUCTIONCODE)||
					serviceType.getCode()!=null&&serviceType.getCode().equals(BpaConstants.CMDACODE)
					||serviceType.getCode().equals(BpaConstants.ADDITIONALCONSTRUCTIONCODE)){
			...
			if(serviceType.getCode()!=null&&serviceType.getCode().equals(BpaConstants.APPLICATIONFORDEMOLITIONCODE)||
					...
					serviceType.getCode()!=null&&serviceType.getCode().equals(BpaConstants.CMDACODE)
					||serviceType.getCode().equals(BpaConstants.ADDITIONALCONSTRUCTIONCODE)){
				for(InspectMeasurementDtlsExtn measuredtls:inspectionExtnService.getInspectionMeasureMentDtls(getInspectionId())){
					if(measuredtls.getInspectionSource().getCode().equals(BpaConstants.INSPECTIONSOURCEFOREXISTINGPLAN)){
						planDetails=measuredtls;
						planDetails.setHeader("Existing Building details as per Plan");


						inspectMeasurementDtls.add(planDetails);
					}else if(measuredtls.getInspectionSource().getCode().equals(BpaConstants.INSPECTIONSOURCEFOREXISTINGSITE)){
	...
	private void buildConstructionDetails() {
		...
		InspectMeasurementDtlsExtn planDetails=new InspectMeasurementDtlsExtn();
		InspectMeasurementDtlsExtn siteDetails=new InspectMeasurementDtlsExtn();
		planDetails.setInspectionSource(inspectionExtnService.getInspectionSource(BpaConstants.INSPECTIONSOURCEFORCONSTRUCTIONPLAN));			
		siteDetails.setInspectionSource(inspectionExtnService.getInspectionSource(BpaConstants.INSPECTIONSOURCEFORCONSTRUCTIONSITE));

		if(getInspectionId()==null||getInspectionId().equals("")){		

			if (serviceType.getCode() != null
					&& serviceType.getCode().equals(BpaConstants.RECLASSIFICATION)) {
			...
			else if(serviceType.getCode()!=null&&!serviceType.getCode().equals(BpaConstants.SUBDIVISIONOFLANDCODE)&&
					serviceType.getCode()!=null&&!serviceType.getCode().equals(BpaConstants.LAYOUTAPPPROVALCODE)&&
					serviceType.getCode()!=null&&!serviceType.getCode().equals(BpaConstants.APPLICATIONFORDEMOLITIONCODE)){
			...
			for(InspectMeasurementDtlsExtn measuredtls:inspectionExtnService.getInspectionMeasureMentDtls(getInspectionId())){
				...
				if(measuredtls.getInspectionSource().getCode().equals(BpaConstants.INSPECTIONSOURCEFORCONSTRUCTIONPLAN)){
					planDetails=measuredtls;
				}else if(measuredtls.getInspectionSource().getCode().equals(BpaConstants.INSPECTIONSOURCEFORCONSTRUCTIONSITE)){
					siteDetails=measuredtls;
				}
			}			
	...
	public void buildChecklist(){
		...
		ServiceTypeExtn serviceType=inspectionExtnService.getServiceTypeById(getServiceTypeId());
		if(getInspectionId()==null||getInspectionId().equals("")){
			if(serviceType!=null && serviceType.getCode()!=null&&!serviceType.getCode().equals(BpaConstants.SUBDIVISIONOFLANDCODE)&&
					serviceType.getCode()!=null&&!serviceType.getCode().equals(BpaConstants.LAYOUTAPPPROVALCODE)){
				List<CheckListDetailsExtn> chkListDetails=inspectionExtnService.getCheckListforService(serviceType.getId());
			...
			if(serviceType.getCode()!=null&&!serviceType.getCode().equals(BpaConstants.SUBDIVISIONOFLANDCODE)&&
					serviceType.getCode()!=null&&!serviceType.getCode().equals(BpaConstants.LAYOUTAPPPROVALCODE)){
				chkListDet=inspectionExtnService.getInspectionCheckDtls(getInspectionId());
			}
		}
...
public String showSurveyorMeasurementDetails(){
	...
	public String showExistingInspectionMeasurementDetails(){
		...
		if(registrationObj!=null){
			existingSiteInspectionDetails=inspectionExtnService.getSiteInspectionListforRegBySurveyorAndOfficial(registrationObj);
			setServiceTypeCode(registrationObj.getServiceType().getCode());
			for(InspectionExtn inspectiondetails:existingSiteInspectionDetails){
				if(inspectiondetails!=null && inspectiondetails.getIsInspected().equals(Boolean.TRUE) ){
					isInspectedByAEorAEE=bpaCommonExtnService.ShowUserROles(inspectiondetails.getCreatedBy().getRoles(),BpaConstants.BPAAEROLE,BpaConstants.BPAAEEROLE);
		...
		if(registrationObj!=null && registrationObj.getEgwStatus()!=null)
			{ 
			if(registrationObj.getEgwStatus().getCode().equalsIgnoreCase(BpaConstants.INSPECTEDBYLS) && isUserMappedToSurveyorRole())
				allowModifyInspection=true;
			if(registrationObj.getEgwStatus().getCode().equalsIgnoreCase(BpaConstants.FILECONSIDERATIONCHECKED) && (isUserMappedToSelectedRole(BpaConstants.BPAAEEROLE) || isUserMappedToSelectedRole(BpaConstants.BPAAEROLE)))
				allowModifyInspection=true;
			}
	...
	public void setSiteInspectionScheduledates(String siteInspectionScheduledates) {
	...
	public void setFeeDetailsExtnService(FeeDetailsExtnService feeDetailsExtnService) {

...
@Results({ 
	...
	@Result(name = CitizenRegisterBpaExtnActioncopy.FEE_PAYMENT_PDF, type = StreamResult.class, value = "feePaymentReportPDF", params = { "contentType", "application/pdf"})
})
@ParentPackage("egov")

public class CitizenRegisterBpaExtnActioncopy extends RegisterBpaExtnAction{
...
//	private List<InspectionExtn> postponedInspectionDetails=new ArrayList<InspectionExtn>();	
	...
	public void validate(){
		...
		if(isUserMappedToSurveyorRole() && getAutoDcrNum()!=null && !"".equals(getAutoDcrNum()) && validateAutodcr()){
		...
		if(isUserMappedToSurveyorRole())
		{
			...
			if(isDocUploadForSurveyorFlag==Boolean.TRUE && (getDocumentNum()==null || "".equals(getDocumentNum())))
			{
				addFieldError("registration.DocumentUplaod", getMessage("registration.Document.required"));	
			}
 ...
 */
	...
	public String newCitizenForm() {	
		...
		siteAddressObj.setCityTown("Chennai");
	...
	public String autoCompleteapprovedAutoDcrNUmber() {
		approvedAutoDcrList=persistenceService.findAllBy("from AutoDcrExtn auto where auto.id is not null and not exists (select 1 from RegnAutoDcrExtn regnautodcr, RegistrationExtn reg" +
				" where auto.autoDcrNum=regnautodcr.autoDcrNum and reg.id=regnautodcr.registration.id and reg.egwStatus.moduletype=? and reg.egwStatus.code not in(?,?)) and auto.autoDcrNum like ?", BpaConstants.NEWBPAREGISTRATIONMODULE,BpaConstants.NEWBPACANCELLEDSTATUS,BpaConstants.REJECTIONAPPROVED,"%"+autoDcrNumberAutocomplete.toUpperCase()+"%");
			return "autoDcr";

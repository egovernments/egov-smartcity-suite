...
@Results({ //Phionix TODO
	...
	@Result(name = RegisterBpaExtnAction.FEE_PAYMENT_PDF, type = "stream", location = "feePaymentReportPDF", params = { "contentType", "application/pdf"})	
})
@ParentPackage("egov")
...
/*	protected Map<String,BillReceiptInfo> billReceiptInfoMap = new HashMap<String,BillReceiptInfo>();
	...
	*/private List<LpChecklistExtn> lpReplyChkListDet = new ArrayList<LpChecklistExtn>();
	...
	private String regObjectHistoryRemarks;
		...
		this.addRelatedEntity("state", org.egov.infra.workflow.entity.State.class);
		this.addRelatedEntity("regnDetails", RegnDetailsExtn.class);
		this.addRelatedEntity("regnDetails.existingBldgCatg", BuildingCategoryExtn.class);
		this.addRelatedEntity("regnDetails.proposedBldgCatg", BuildingCategoryExtn.class);
		...
		this.addRelatedEntity("letterToParty.letterToPartyReason", LpReasonExtn.class);
	...
	public void prepare() {
		...
		addDropdownData("proposedBuildingCategoryList", bpaCommonExtnService.getAllActiveBuildingCategoryList());
		addDropdownData("existingBuildingCategoryList", bpaCommonExtnService.getAllActiveBuildingCategoryList());
		//addDropdownData("surveyorNameList", bpaCommonExtnService.getAllActiveSurveyorNameList());
		//Phionix TODO
		addDropdownData("bndryStateList", bpaCommonExtnService.getAllStatesOfIndia());
		addDropdownData("letterToPartyReasonList", bpaCommonExtnService.getAllLpReasonList());
		addDropdownData("bankList",bpaCommonExtnService.getAllBanks());
		addDropdownData("usageOfConstructionList",Arrays.asList(UsageOfConstruction.values())); 
		applicantStatus=  new LinkedHashMap<String, String>();
		applicantStatus.put(ApplicantStatus.OWNER.toString(), ApplicantStatus.OWNER.toString());
		applicantStatus.put(ApplicantStatus.LASSEE.toString(), ApplicantStatus.LASSEE.toString());
		applicantStatus.put(ApplicantStatus.POWER_OF_ATTORNEY.toString(), ApplicantStatus.POWER_OF_ATTORNEY.toString());
		
		addDropdownData("applicationModeList",Arrays.asList(ApplicationMode.values())); 
		if(registration!=null && registration.getPlanSubmissionDate()!=null) 
		...
		if(workFlowAction==null && user!=null ){//&& user.getIsPortalUser()==Boolean.TRUE))
			...
			if(!(showActions!=null && showActions.size()!=0))
				showSave_ActionField=Boolean.FALSE; // If no actions to perform, Do not show save and action buttons 
		}
	}
	...
	/*public Integer getZonalLevelAssistantForLoggedInUser() {
		...
		if(employeeList!=null&&employeeList.size()!=0){
			Integer posId= employeeList.get(0).getPosition().getId();
			return 	posId;
		}else{
			return null;
		}
	...
	/**
	 * @Description Returns position of a logged in user based on userdepartment, designation and boundary
	 * @return
	 *///Phionix TODO
	public Long getZonalLevelAssistantForLoggedInUser() {
		Position pos = bpaPimsExtnFactory.getEmployeeInfoList(registration.getAdminboundaryid());
		if (pos != null) {
	...
	public String closeRegistration() {
	    ...
	    registration=registerBpaExtnService.getRegistrationById(registrationId);
	 	registerBpaExtnService.closeRegistration(registration,workFlowAction,approverComments);
	...
	public Long autoSelectApproverPosition(List<EmployeeView> employeeViewList,List<Long> designationId){
		...
		if (employeeViewList != null && employeeViewList.size() != 0) {
			...
			if(!posId.isEmpty()){
				List<Object[]> result=utilsExtnService.getLessLoadedApproversForBpa(designationId,posId); 
	...
	public String save() {
		...
		try{
			...
			if(workFlowAction!=null && !"".equals(workFlowAction) && !(BpaConstants.SCRIPT_SAVE.equalsIgnoreCase(workFlowAction) || BpaConstants.SCRIPT_REJECT.equalsIgnoreCase(workFlowAction))){
				...
				if(workFlowAction.equalsIgnoreCase("Approve Registration") && registration!=null && registration.getState()!=null && registration.getState().getValue().equalsIgnoreCase("Forwarded to Approval")){
					List<EmployeeView> employeeList=	bpaPimsExtnFactory.getEmployeeInfoList();//Phionix TODO
				...
				{	registration.setApproverId(registration.getApproverPositionId());
				//Phionix TODO   
				}
			}
			if(!enableDocketSheetForView && !mode.equals("reject") && additionalRule!=null && !additionalRule.equals(BpaConstants.LETTERTOPARTYDETAILS)
					&&  !additionalRule.equals(BpaConstants.LETTERTOCMDA)){
				// For surviceType where code 1,3,6 populate values from Surveyor Entries and Save with New Record For Official Users.
				String regServType = registration!=null?registration.getServiceType().getCode():null;
				List<DocumentHistoryExtnDetails>documentHistodetailTempList=new ArrayList<DocumentHistoryExtnDetails>();
				if(regServType!=null && regServType.equals(BpaConstants.NEWBUILDINGONVACANTPLOTCODE) 
						|| regServType.equals(BpaConstants.DEMOLITIONRECONSTRUCTIONCODE) 
						|| regServType.equals(BpaConstants.ADDITIONALCONSTRUCTIONCODE))
				{
				List<DocumentHistoryExtn> docHistoryFromOfficial=bpaCommonExtnService.getRegnDocumentHistoryObjectForAEorAEE(registration);
				//docHistoryFromOfficial this will give DocumentHistory entered by AE/AEE .. if docHistoryFromOfficial Empty den Populate VAlues from Surveyor and set Id for DocumentHistory is NULL and Save New DocumentHistory and details.
				if(docHistoryFromOfficial!=null && docHistoryFromOfficial.isEmpty()){
				...
				//other than Servicetypes 1,3,6 assigning DocumenthistoryList From UI
				else{
					documentHistodetailTempList=documentHistoryDetailList;
					}
				builddocumentHistoryExtraDeatils(documentHistory,documentHistodetailTempList);
			}
			
			registration=registerBpaExtnService.createBpa(registration,applicantrAddress,autoDcrNum,siteAddress,boundaryStateId,workFlowAction,approverComments,existingbuildingCategoryId,proposedbuildingCategoryId,true,null);
			...
			if(((workFlowAction!=null && !workFlowAction.equalsIgnoreCase(BpaConstants.SCRIPT_SAVE) && !workFlowAction.equalsIgnoreCase(BpaConstants.SCRIPT_REJECT))) && (admissionfeeAmount!=null && admissionfeeAmount >0)){
				...
				if (registration != null && registration.getId() != null /*	&& registration.getEgwStatus()!=null && registration.getEgwStatus().getDescription().equalsIgnoreCase(BpaConstants.STATUS_APPLICATIONREGISTERED)*/
						&& registration.getState()!=null && registration.getState().getValue().equalsIgnoreCase(BpaConstants.WF_NEW_STATE)
			...
			if(null!=getAdditionalMode() && getAdditionalMode().equals("editApprovedRecord")){
	...
	protected void builddocumentHistoryExtraDeatils(DocumentHistoryExtn docHistory ,List<DocumentHistoryExtnDetails> documentHistodetailTempList)
	{
		if(docHistory!=null){
			if(docHistory.getWheatherdocumentEnclosed()!=null && docHistory.getWheatherdocumentEnclosed().equals(true))
				{
				...
				else if(docHistory.getWheatherpartOfLayout()!=null && docHistory.getWheatherpartOfLayout().equals(true))
				{
					docHistory.setWheatherFmsOrSketchCopyOfReg(null);
					//docHistory.setWheatherplotDevelopedBy("");
				}
				else if(docHistory.getPlotDevelopedBy()!=null && !docHistory.getPlotDevelopedBy().equals("Individual"))	{
					docHistory.setWheatherplotDevelopedBy("");
				}
	
		Set<DocumentHistoryExtnDetails> unitSet=new HashSet<DocumentHistoryExtnDetails>();
		if(documentHistodetailTempList!=null && !documentHistodetailTempList.isEmpty()){
			for(DocumentHistoryExtnDetails unitdetail:documentHistodetailTempList)
	    	{
				if(unitdetail!=null && unitdetail.getSurveyNumber()!=null && !"".equals(unitdetail.getSurveyNumber())){
		...
		documentHistodetailTempList=new ArrayList<DocumentHistoryExtnDetails>(docHistory.getDocumentHistoryDetailSet());
	...
	private void createObjectHistoryForRegistration() {
		...
		//	objHistory.setModifiedBy(registration.getModifiedBy());
			...
			objHistory.setObjectId(Integer.valueOf(registration.getId().toString()));
	...
	protected void confirmationAlertMessages() {
		...
		if(registration!=null && registration.getState()!=null){					
			if(registration.getState().getValue()!=null &&  !registration.getState().getValue().equals(BpaConstants.WF_END_STATE) && 
					!registration.getState().getValue().equals(BpaConstants.WF_CANCELLED) && 
					!registration.getState().getValue().equals(BpaConstants.WF_NEW_STATE) )
				userNameWithDesignation=registerBpaExtnService.getEmpNameDesignation(registration.getState().getOwnerPosition(), new Date());
		}//todo
		
		if("".equals(mode) || mode==null) 
			addActionMessage("Building Plan Application "+registration.getPlanSubmissionNum() +" is created successfully");	
		else
			addActionMessage("Building Plan Application "+registration.getPlanSubmissionNum() +" is updated successfully");
		   ...
		   else  if(workFlowAction!=null && !"".equals(workFlowAction) && (workFlowAction.equalsIgnoreCase(BpaConstants.CLOSEREGISTRATION)))
					   addActionMessage("The File has been Closed.");
		   ...
		   else
		   {
			  if(!workFlowAction.equalsIgnoreCase(BpaConstants.SCRIPT_SAVE))
				   if( !"@".equals(userNameWithDesignation) )
						addActionMessage("The File has been forwarded to  "+userNameWithDesignation );
	...
	protected void setCheckListForRegistration() {
		...
		Set <RegistrationChecklistExtn> checkListDtls=new HashSet<RegistrationChecklistExtn>(getChkListDet());			
		...
		for(RegistrationChecklistExtn checklistObj:registration.getRegistrationChecklistSet()){
			...
			{
				checklistObj.setCheckListRemarks(checklistObj.getCheckListRemarks());
			}
			
			 if( checklistObj.getUploadFile()!=null){/*
					File mainDir=new File("C:/pdf/");
					if(!mainDir.exists()){
					...
					if(mainDir.exists() && mainDir.canRead() && mainDir.canWrite() && mainDir.canExecute()){
						File file = new File("C:/pdf/",checklistObj.getUploadFile().getName());
						
						try {
								//FileUtils.writeByteArrayToFile(file, checklistObj.getUploadFile().getFileData());
							 FileUtils.copyFile(checklistObj.getUploadFile(), file);
						...
						RegDocumentUpload docUploadObj= new RegDocumentUpload();
						
						docUploadObj.setObjectType("RegistrationCheckList");
						docUploadObj.setReferenceId(checklistObj.getId());
						
						RegDocumentUploadDtls docUploadDtlObj= new RegDocumentUploadDtls();
						docUploadDtlObj.setContentType(new MimetypesFileTypeMap().getContentType(checklistObj.getUploadFile()));
						docUploadDtlObj.setFileName(checklistObj.getUploadFile().getName());
	...
	private void setRejectionCheckListForRegistration() {
		if(getRejectChkListDet()!=null && getRejectChkListDet().size()!=0){
			Set<RejectionChecklistExtn> rejectcheckListDtls=new HashSet<RejectionChecklistExtn>(getRejectChkListDet());
			...
			registration.getRejection().setRejectionChecklistSet(rejectcheckListDtls);
...
/*
 * For reclassification, we are not collecting any permit fee. We are generating Building and planning permit number on approval of record.
 * Here if record submitted from citizen, then generate new number else use same plan submit number as plan permit number.
 */
	private void setBAandRejectNumberForRegistration(BigDecimal totalAmount) {
		if(totalAmount!=null && totalAmount.equals(BigDecimal.ZERO)){  
			if(null!=registration.getEgwStatus() && null!=registration.getEgwStatus().getCode() && registration.getEgwStatus().getCode().equalsIgnoreCase(BpaConstants.STATUSAPPROVED)){
				if(registration.getBaNum()==null || "".equals(registration.getBaNum()))
					registration.setBaNum(bpaNumberGenerationExtnService.generateOrderNumber(registration));
				if(registration.getPlanPermitApprovalNum()==null || "".equals(registration.getPlanPermitApprovalNum())){
					if(registration.getServiceType()!=null && !registration.getServiceType().getIsCmdaType() && registration.getRequest_number()!=null){ //Mean record registered from citizen.
						registration.setPlanPermitApprovalNum(bpaNumberGenerationExtnService.generatePlanSubmissionNumber(registration.getServiceType(), bpaCommonExtnService.getZoneNameFromAdminboundaryid(registration.getAdminboundaryid())));
					}else
		...
		if(null!=registration.getEgwStatus() && null!=registration.getEgwStatus().getCode() && registration.getEgwStatus().getCode().equalsIgnoreCase(BpaConstants.REJECTIONAPPROVED)){
			if(registration.getRejection()!=null&&registration.getRejection().getRejectionNumber()==null || "".equals(registration.getRejection().getRejectionNumber()))
				registration.getRejection().setRejectionNumber((bpaNumberGenerationExtnService.generateRejectionOrderNumber()));
		}
	}
	
	protected void smsAndEmailonSaveMethodCitizen(List<Map<String, Object>> finalAttachmentList,BigDecimal totalAmount) {
		/*
		 * send sms and email to citizen on registration i.e., when record is forwarded for the 1st time 
		 *  and on approve.
		 *  For Both Citizen through registered and COC Operator through registered Bpa records using this method to send SMS and Mail on forward...
		 */
		if (workFlowAction != null && workFlowAction.equalsIgnoreCase(BpaConstants.FORWARDWORKFLOWSTATUS)) {
			if (("".equals(mode) || mode == null)
					|| (registration.getState() != null && registration	.getState().getValue().equalsIgnoreCase(BpaConstants.WF_NEW_STATE))
					|| (!"".equals(registration.getRequest_number())
							&& registration.getRequest_number() != null
							&& registration.getServiceRegistryId() != null && registration.getState() != null 
							&& registration.getState().getValue().equalsIgnoreCase(BpaConstants.CITIZENAPPLICATIONREGISTERED)))
			{
				if (totalAmount != null && totalAmount.doubleValue() == 0)
					{
						bpaCommonExtnService.buildEmail(registration,BpaConstants.SMSEMAILSAVE,finalAttachmentList);
						bpaCommonExtnService.buildSMS(registration,BpaConstants.SMSEMAILSAVE);
					}
			}
		}
		/*
		 * Added workFlowAction !='Save' because in Assistant inbox for approved records On Save its sending SMS and Mail so Now sending SMS  and Mail only on workflowaction!='Save' and status='Approved' and NextAction='Dispatch record'
		 */
		else if(null!=workFlowAction && !workFlowAction.equalsIgnoreCase(BpaConstants.SCRIPT_SAVE) && null!=registration.getEgwStatus() && null!=registration.getEgwStatus().getCode() && 
			registration.getEgwStatus().getCode().equalsIgnoreCase(BpaConstants.STATUSAPPROVED) && 
			registration.getState()!=null && registration.getState().getNextAction().equalsIgnoreCase(BpaConstants.DISPATCHRECORD)){
			
			
			bpaCommonExtnService.buildEmail(registration,BpaConstants.SMSEMAILAPPROVE,null);				
			bpaCommonExtnService.buildSMS(registration,BpaConstants.SMSEMAILAPPROVE);
		 ...
		 */
		if(workFlowAction!=null && workFlowAction.equalsIgnoreCase(BpaConstants.SMSEMAILAPPROVELETTERTOPARTY) && null!=registration.getEgwStatus() && null!=registration.getEgwStatus().getCode() && (registration.getEgwStatus().getCode().equalsIgnoreCase(BpaConstants.LETTERTOPARTYSENT))) {
			
			List <LetterToPartyExtn> letterToParty = registerBpaExtnService.getLetterToPartyForRegistrationObject(registration);
			...
			attachmentFileNames.put("letterToPartyFileName", "letterToPartyPDF.pdf"); 
			finalAttachmentList.add(attachmentList);
			finalAttachmentList.add(attachmentFileNames); 
			bpaCommonExtnService.buildEmail(registration,BpaConstants.SMSEMAILLP,finalAttachmentList);			
			bpaCommonExtnService.buildSMS(registration,BpaConstants.SMSEMAILLP);  
		}
		if(workFlowAction!=null && workFlowAction.equalsIgnoreCase(BpaConstants.CMDASMSEMAILAPPROVELETTERTOPARTY) && null!=registration.getEgwStatus() && null!=registration.getEgwStatus().getCode() && (registration.getEgwStatus().getCode().equalsIgnoreCase(BpaConstants.CMDALETTERTOPARTYSENT))) {
			
			List <CMDALetterToPartyExtn> letterToParty = registerBpaExtnService.getcmdaLetterToPartyForRegistrationObject(registration);
			if(letterToParty!=null && !letterToParty.isEmpty()){
			setLetterToPartyId(letterToParty.get(0).getId());
			}
			HashMap<String, Object> attachmentList = new HashMap<String, Object>(); 
			HashMap<String, Object> attachmentFileNames = new HashMap<String, Object>(); 
		    attachmentList.put("letterToPartycmda", generateLetterToCMDAPDF(registration));
			attachmentFileNames.put("letterToPartyFileNamecmda", "letterToPartyPDF1.pdf"); 
			finalAttachmentList.add(attachmentList);
			finalAttachmentList.add(attachmentFileNames); 
			bpaCommonExtnService.buildEmail(registration,BpaConstants.SMSEMAILCMDALP,finalAttachmentList);			
			bpaCommonExtnService.buildSMS(registration,BpaConstants.SMSEMAILCMDALP);  
		}
		
	}
	
	/*
	 * Send SMS to AEE when record is forwarded and letter to reply is collected. 
	 * (designation's of the employees for which SMS has to be sent is picked from Appconfig )
	 * approverDesignation
	 */
	private void smsOnSaveMethodOfficials() {
	
		if(null!=registration.getEgwStatus() && null!=registration.getEgwStatus().getCode() && 
				registration.getEgwStatus().getCode().equalsIgnoreCase(BpaConstants.CITIZENAPPLICATIONREGISTERED) && 
				workFlowAction!=null && workFlowAction.equalsIgnoreCase(BpaConstants.FORWARDWORKFLOWSTATUS) &&
				...
				for(AppConfigValues appConfigValueObj:appConfigDesignationList){
					...
					for(EmployeeView ev:empViewList){
						...
						if(null!=appConfigValueObj && null!=appConfigValueObj.getValue() 
								&& null!=ev.getDesigId() && null!=ev.getDesigId().getDesignationName()){
							
							if(appConfigValueObj.getValue().equalsIgnoreCase(ev.getDesigId().getDesignationName())){
								flag=Boolean.TRUE;
							}
						}
					}
			...
			if(flag){
				...
				EgwStatus oldstatus=bpaCommonExtnService.getoldStatus(registration);
				
				if(null!=oldstatus && oldstatus.getCode().equalsIgnoreCase(BpaConstants.LPREPLYRECEIVED)){				
					 
					bpaCommonExtnService.buildSMS(registration,BpaConstants.SMSEMAILASSISTANTTOAEEONLP);
							
				}else{
					
					bpaCommonExtnService.buildSMS(registration,BpaConstants.SMSEMAILASSISTANTTOAEEONSAVE);
	...
	protected void setAdmissionFeeAmountForRegistration() {
		...
		if(registration.getServiceType()!=null && registration.getServiceType().getId()!=null && registration.getRegnDetails()!=null && registration.getRegnDetails().getSitalAreasqmt()!=null ){
			BigDecimal admissionfeeAmount=feeExtnService.getTotalFeeAmountByPassingServiceTypeandArea(registration.getServiceType().getId(), registration.getRegnDetails().getSitalAreasqmt(), BpaConstants.ADMISSIONFEE);		
	...
	{	
		...
		buildRegistrationForViewModify();
		buildOrderDetails();
		if(registration.getEgwStatus()!=null && (registration.getCurrentState().getValue().equals(BpaConstants.FORWARDEDTORDCSTATE)
				|| registration.getCurrentState().getValue().equals(BpaConstants.FORWADREDTOSESTATE))){
			buildOfficialViewActions(); 
		}
		
		if(registration.getEgwStatus().getCode().equalsIgnoreCase(BpaConstants.UNDERREJECTION)
				||registration.getEgwStatus().getCode().equalsIgnoreCase(BpaConstants.REJECTIONAPPROVED)
				||registration.getEgwStatus().getCode().equalsIgnoreCase(BpaConstants.REJECTORDERPREPARED)
		...
		/*if(registration.getCurrentState()!=null && registration.getCurrentState().getValue().equalsIgnoreCase(BpaConstants.APPLICATIONREGISTERED)){
			...
			if (registration.getState() != null && firstInboxItem!=null
				...
				addActionMessage(getText("registrationExtn.EE.fifo.validation",new String[] {firstInboxItem.getPlanSubmissionNum()}));
				forwardNotApplicable = true; 
			}
		}*/
	
		// To remove approval information is SE inbox for Create LetterToParty  workflow
		if(registration.getCurrentState()!=null && (registration.getCurrentState().getValue().equalsIgnoreCase("LpForworded")
				|| registration.getCurrentState().getValue().equalsIgnoreCase("LettertoCMDAForworded")))
		{
		...
		}else if(registration.getEgwStatus()!=null && (registration.getEgwStatus().getCode().equalsIgnoreCase(BpaConstants.CMDACREATEDLETTERTOPARTY))){
			   registration.setAdditionalRule(BpaConstants.LETTERTOCMDA); 
			   setAdditionalRule(BpaConstants.LETTERTOCMDA);		 
			   registration.setAdditionalState(registration.getState().getValue());
		}else if(registration.getEgwStatus()!=null && registration.getEgwStatus().getCode().equalsIgnoreCase(BpaConstants.LETTERTOPARTYSENT)){
		...
		}else if(registration.getEgwStatus()!=null && registration.getEgwStatus().getCode().equalsIgnoreCase(BpaConstants.CMDALETTERTOPARTYSENT)){
			   registration.setAdditionalRule(BpaConstants.BPAAPPLICATIONREGISTRATION);
			   setAdditionalRule(BpaConstants.BPAAPPLICATIONREGISTRATION);
		}else{
			   registration.setAdditionalRule(BpaConstants.BPAAPPLICATIONREGISTRATION);
		...
		BigDecimal amountPending=bpaCommonExtnService.isFeeCollectionPending(registration);
		if(null!=amountPending && amountPending.equals(BigDecimal.ZERO)){
			setIsPlotAreaEditable("false");
		}else{
			setIsPlotAreaEditable("true");
			if(registration.getEgwStatus()!=null && registration.getEgwStatus().getCode().equalsIgnoreCase(BpaConstants.CHALLANNOTICESENT) )
				addActionMessage("Collect Fees to get further actions");
		}
		/*
		 * Validate Only if Record is entered By citizen setting noEdit MOde disable all the fields to be edit for Assistant and next level user..
		 */
		if(registration!=null && registration.getRequest_number()!=null && registration.getServiceRegistryId()!=null && !"".equals(registration.getRequest_number()) ){
		...
		if(null!=registration.getState() && null!=registration.getState().getNextAction() && registration.getState().getNextAction().equalsIgnoreCase(BpaConstants.DISPATCHRECORD)){
			setMode(BpaConstants.MODENOEDIT);
		}
		...
		for(Integer statusId:statusIdList){
			if(statusMap.get(statusId).equals(BpaConstants.CREATEDLETTERTOPARTY) || statusMap.get(statusId).equals(BpaConstants.CMDACREATEDLETTERTOPARTY)){
				setMode(BpaConstants.MODENOEDIT);
				break;
			}
		}
		
		if(registration.getEgwStatus()!=null&&(registration.getEgwStatus().getCode().equals(BpaConstants.UNDERREJECTION))){
			setRejectview(BpaConstants.MODEREJECT);	
		}else {
			String status = bpaCommonExtnService.getPrimaryStatusforRegistration(registration);
			if(status!=null && status.equals(BpaConstants.PRIMARYSTATUSREJECT)){
				setRejectview(BpaConstants.MODEREJECT);	
			}
		}
		if(registration.getEgwStatus()!=null && registration.getEgwStatus().getCode().equalsIgnoreCase(BpaConstants.CHALLANAMOUNTCOLLECTED) )
		{
			setAdditionalMode("editApprovedRecord");
		}
		
		/*if(registration.getEgwStatus()!=null && registration.getEgwStatus().getCode()!=null&&registration.getEgwStatus().getCode().equalsIgnoreCase(BpaConstants.CREATEDLETTERTOPARTY)&&registration.getState()!=null&&registration.getState().getNextAction()!=null&&registration.getState().getNextAction().equals("LP Raised-Send LettertoParty"))
			addActionMessage("Enter Letter to Party Sent Date in the Letter to Party section");*/
		LOGGER.info(" RegisterBpaAction || Save || End");
		return NEW;
	}
	...
	protected void buildRegistrationObject() {
		if(registration!=null){
		RegnAutoDcrExtn  autoDcrObj=autoDcrExtnService.getAutoDcrByRegId(registration.getId());
		 if(autoDcrObj!=null)
		...
		// In Approval Cycle Build Docket sheet and Document History For UI show
		if(!bpaCommonExtnService.isUserMappedToSurveyorRole() 
				&& registration.getEgwStatus()!=null && registration.getEgwStatus().getCode().equals(BpaConstants.FILECONSIDERATIONCHECKED) 
				&& (registration.getCurrentState().getValue().equals(BpaConstants.FORWARDEDTORDCSTATE) 
						|| registration.getCurrentState().getValue().equals(BpaConstants.FORWARDEDTOEESTATE)
						|| registration.getCurrentState().getValue().equals(BpaConstants.FORWADREDTOSESTATE))){
			
			 enableDocketSheetForView=Boolean.TRUE;
		}
		if(registration.getCurrentState()!=null && registration.getCurrentState().getValue().equalsIgnoreCase(BpaConstants.WORKFLOWSTATUSFORWARDEDTOAEORAEE ))
		{
		...
		if(registration.getCurrentState()!=null && registration.getCurrentState().getValue().equalsIgnoreCase(BpaConstants.APPROVEDWORKFLOWSTATUS))
		{
			approvedWorkflowState=registration.getCurrentState().getValue();
		}
		// For Official User and ServiceTypes only 01,03,06 
		if(!bpaCommonExtnService.isUserMappedToSurveyorRole() && registration.getServiceType()!=null && registration.getServiceType().getCode()!=null && (registration.getServiceType().getCode().equals(BpaConstants.NEWBUILDINGONVACANTPLOTCODE)
		...
		/*if(registration.getSurveyorName()!=null)
		{
			...
			SurveyorDetail surveyourDetailObj=bpaCommonExtnService.getSurveyourDetail(surveyorCode,null);
			if(surveyourDetailObj!=null){
			surveyorClass=surveyourDetailObj.getSurveyorClass();
			setSurveyor(surveyourDetailObj.getSurveyor().getId());
			}
			else{
				surveyorClass="II";
		...
		List<DocumentHistoryExtn>docHistoryList=bpaCommonExtnService.getRegnDocumentHistoryObjectBySurveyorOrOtherOfficials(registration);
		for(DocumentHistoryExtn docHistObj:docHistoryList){
			if(docHistObj!=null && docHistObj.getCreatedUser()!=null )
			isDocHistoryByAEorAEE=bpaCommonExtnService.ShowUserROles(docHistObj.getCreatedUser().getRoles(),BpaConstants.BPAAEROLE,BpaConstants.BPAAEEROLE);
		 ...
		 */
		previousApproverComments=bpaCommonExtnService.getPreviousUserCommentsFromWorkfow(registration);
		
		if(registration!=null && registration.getDocumentid()!=null)
			this.setDocumentNum(registration.getDocumentid());
		if(null!=registration.getRegnDetails()){
			if(null!=registration.getRegnDetails().getExistingBldgCatg() && null!=registration.getRegnDetails().getExistingBldgCatg().getId()){
				setExistingbuildingCategoryId(registration.getRegnDetails().getExistingBldgCatg().getId());
			}
			if(null!=registration.getRegnDetails().getProposedBldgCatg() && null!=registration.getRegnDetails().getProposedBldgCatg().getId()){
				setProposedbuildingCategoryId(registration.getRegnDetails().getProposedBldgCatg().getId());
			}
		}
		...
		//docHistory if documentHistory By AE/AEE is not entered and other than servicetypes 02,04,05,07,08 will add new Document Object
		if( docHistoryFromOfficial.isEmpty() && (regServType.equals(BpaConstants.APPLICATIONFORDEMOLITIONCODE) 
				|| regServType.equals(BpaConstants.SUBDIVISIONOFLANDCODE) 
				|| regServType.equals(BpaConstants.LAYOUTAPPPROVALCODE)
				|| regServType.equals(BpaConstants.CMDACODE) 
				|| regServType.equals(BpaConstants.RECLASSIFICATIONCODE)))
		{
			setDocumentHistory(new DocumentHistoryExtn());
			documentHistoryDetailList.add(new DocumentHistoryExtnDetails());
		}
		else{
			// for Servicetypes 01,03,06 get documentHistory of Surveyor and populate into AE/AEE Document History..  If AE/AEE is not entered 
			if(docHistoryFromOfficial==null || docHistoryFromOfficial.isEmpty()){
			List<DocumentHistoryExtn> docHistoryFromSurveyor=bpaCommonExtnService.getRegnDocumentHistoryObjectBySurveyorOrOtherOfficials(registration);
			if(docHistoryFromSurveyor!=null && !docHistoryFromSurveyor.isEmpty()){
			setDocumentHistory(docHistoryFromSurveyor.get(0));
			documentHistoryDetailList=new ArrayList<DocumentHistoryExtnDetails>(docHistoryFromSurveyor.get(0).getDocumentHistoryDetailSet());
			}
			...
			documentHistoryDetailList=new ArrayList<DocumentHistoryExtnDetails>(getDocumentHistory().getDocumentHistoryDetailSet());
			if(documentHistoryDetailList.isEmpty())
			{
				documentHistoryDetailList.add(new DocumentHistoryExtnDetails());
			}
		...
		//Build DocumentHistory sFor Surveyor and Other Than AE/AEE Official Users....
		else {
		...
		if(UserRole!=null && !UserRole.contains(BpaConstants.PORTALUSERCITIZENROLE) && !bpaCommonExtnService.isUserMappedToSurveyorRole()){
			for(RegistrationChecklistExtn checkListDtlsObj:registration.getRegistrationChecklistSet())
			{
				if(checkListDtlsObj!=null && checkListDtlsObj.getIsChecked()!=null && checkListDtlsObj.getIsChecked().equals(Boolean.TRUE))
				{
		 ...
		 checkListDtlsList=new ArrayList<RegistrationChecklistExtn>(registration.getRegistrationChecklistSet());
		...
		File mainDir = new File(fileLocationName+"/"+BpaConstants.BPAMODULENAME+ (registration.getInitialPlanSubmissionNum()!=null? "/"+registration.getInitialPlanSubmissionNum().replace("/", ""):(registration.getPlanSubmissionNum()!=null ? "/"+registration.getPlanSubmissionNum().replace("/", ""):"")));
		for(RegistrationChecklistExtn chkList:checkListDtlsList){
			/*if(chkList.getDocUpload()!=null)
				{
					RegDocumentUpload docUploadObj = regDocUploadService.findById(chkList.getDocUpload(), false);
					if (docUploadObj != null) {
						if (mainDir != null) {
							File docObj = new File(mainDir, docUploadObj.getDocumentList().get(0).getFileName());
							if (docObj != null) {
								chkList.setUploadFile(docObj);
								chkList.setFileName(docUploadObj.getDocumentList().get(0).getFileName());
								chkList.setContentType(docUploadObj.getDocumentList().get(0).getContentType());
							}
						}
					} 
				}*/
		...
		existingSiteInspectionDetails=inspectionExtnService.getSiteInspectionListforRegistrationObject(registration);
	...
	private void getDocumentHistDetailsWithDateFormat(List<DocumentHistoryExtnDetails>documentHistoryDetailList) {
		//for Format Date To Show in UI
				if(documentHistoryDetailList !=null && !documentHistoryDetailList.isEmpty()){
	...
	public String existPPAnumberforRegistration(){
		 ppanum= registerBpaExtnService.checkPPANumberIsValid(existPpaNumber,registration.getId());
		return "ppanum";
	}
	/*
	...
	public String existBanumberCheckForAjax(){
		ppanum = registerBpaExtnService.checkBuildingApprovalNumberIsValid(existBaNumber,registration.getId());
		LOGGER.info(ppanum);
		return "ppanum";
	}
	...
	{
		return   registerBpaExtnService.checkAutodcr(getAutoDcrNum(),registration.getId());
	}
	
	public Boolean validateSericeTypeOnSubmit()
	{
		ServiceTypeExtn servicetypeObj= registerBpaExtnService.getServiceTypeByRegistrationId(registration.getId());
		   
		if(servicetypeObj!=null && registration!=null && registration.getEgwStatus()!=null &&  registration.getEgwStatus().getCode()!=null && 
				(registration.getEgwStatus().getCode().equals(BpaConstants.APPLICANTSIGNUPDATED) 
				|| registration.getEgwStatus().getCode().equals(BpaConstants.CHALLANAMOUNTCOLLECTED) 
				|| registration.getEgwStatus().getCode().equals(BpaConstants.CHALLANNOTICESENT) 
				||registration.getEgwStatus().getCode().equals(BpaConstants.ORDERPREPARED) 
				||registration.getEgwStatus().getCode().equals(BpaConstants.STATUSAPPROVED)))
		{		
		if( servicetypeObj.getId().equals(registration.getServiceType().getId()))
		{
			return false;
		}
		return true;
	...
	{
		return	registerBpaExtnService.checkExistingBuildingApprovalNumberIsUnique(registration.getExistingBANum(),registration.getId());
	}
	/*
	 * validation for Unique ExistBaNumber in Table....
	 */
	...
	{
		return	registerBpaExtnService.checkExistingPpaNumberIsUnique(registration.getExistingPPANum(),registration.getId());
	}
	/*
	 * It will allow records to save only Applications have status as cancelled, Rejection order issued,order issued to applicant ... 
	 */
	public Boolean existPPAnumberValidationforRegistration()
	{
		return	registerBpaExtnService.checkPPANumberIsValid(registration.getExistingPPANum(),registration.getId());
	...
	{
		return	registerBpaExtnService.checkBuildingApprovalNumberIsValid(registration.getExistingBANum(),registration.getId());
	...
	public void validate(){
	   ...
	   if(null==registration.getPlanSubmissionDate() ||"".equals(registration.getPlanSubmissionDate())){
		...
		{
			addFieldError("registration.serviceType.id", getMessage("servicetype.allready.selected"));
		}
		...
		if(registration.getId()==null || (registration.getState()!=null && registration.getState().getValue().equalsIgnoreCase(BpaConstants.WF_NEW_STATE))){
			...
			if (registration.getAdminboundaryid() != null
					&& userPosition == null) {
				addFieldError("registration.assistantJurisdictionMap.required", getMessage("registration.assistantJurisdictionMap.required"));
			}	
		...
		/*if(registration.getLocboundaryid()==null){
			addFieldError("registration.locboundaryid", getMessage("locboundaryid.required"));
		}*/
		if(registration.getOwner()==null){
		...
		/*if(applicantrAddress.getStreetAddress2()==null || "".equals(applicantrAddress.getStreetAddress2())){
			addFieldError("registration.applicantrAddress.StreetAddress2", getMessage("applicantrAddress.StreetAddress2.required"));
		}*/
		}
		...
		/*if(siteAddress.getVillage()==null || "".equals(siteAddress.getVillageName().getName()==-1)){
		...
		if(registration.getExistingPPANum()!=null && !"".equals(registration.getExistingPPANum()))
		{
			/*
			 * Checking PPA NUMBER IS VALID NUMBER ? AND CHECKING STATUS OF PPA NUMBER.. 
			 */
			
			if(!existPPAnumberValidationforRegistration()){
				addFieldError("registration.ExistBaNum", getMessage("registration.ExistPpaNum.present"));
			}
			else{
				// Checking uniqueness in existing ppa number
				if(checkUniqueforExistPPaNumber()){
					addFieldError("registration.ExistBaNum", getMessage("registration.ExistPpaNumColumn.exist"));
				}
				
			}
		}
		if(registration.getExistingBANum()!=null && !"".equals(registration.getExistingBANum()))
		{
			/*
			 * Checking Building Approval NUMBER IS VALID NUMBER ? AND CHECKING STATUS OF BA NUMBER..
			 */
			if(!existBAnumberValidationforRegistration()){
				addFieldError("registration.ExistBaNum", getMessage("registration.ExistBaNum.present"));
			}
			else{
			// Checking uniqueness in existing ba number
			if(checkUniqueforExistBaNumber()){
				addFieldError("registration.ExistBaNum", getMessage("registration.ExistBaNumColumn.exist"));
			}
			/*
			 * To Validate Both Exist PPA Number and Exist BA Number Belongs to Same Application...
			 */
			else if(registration.getExistingPPANum()!=null && !"".equals(registration.getExistingPPANum()))
				{
				RegistrationExtn regObj=bpaCommonExtnService.validateExistBaAndExistPpaOfSameApplication(registration.getExistingPPANum(), registration.getExistingBANum());
				//LOGGER.info(regObj.getId());
				if(regObj==null)
				{
					addFieldError("registration.ExistBaNum", getMessage("registration.ExistBaPpanum.SameApplication"));
				}
		...
		/*else if(registration.getSurveyorName().getName()==null){
			addFieldError("registration.surveyorName", getMessage("surveyorName.required"));
		}*/
			...
			{
				addFieldError("registration.surveyorClass", getMessage("surveyorClass.required"));
			}
		...
		// To validate Lp reply is mendatory for created Letter To Party except reject Unconsidered action..
		if(!getMode().equals(BpaConstants.MODEREJECT)){
			if(( workFlowAction!=null && (workFlowAction.equalsIgnoreCase(BpaConstants.FORWARDWORKFLOWSTATUS) || workFlowAction.equalsIgnoreCase(BpaConstants.REJECTWORKFLOWSTATUS)) && (registration.getEgwStatus()!=null && registration.getEgwStatus().getCode().equals(BpaConstants.LETTERTOPARTYSENT))))
			{
				LetterToPartyExtn letterToParty=(LetterToPartyExtn) letterToPartyExtnService.getLatestLetterToPartyForRegObj(registration);
				if(letterToParty.getSentDate()!=null)
				{
				List<LetterToPartyExtn>	 lpParty=letterToPartyExtnService.getLetterToPartyForRegnByComparingLPSentDateWithSysDateToTenDays(registration, letterToParty.getSentDate());
			// validate for less than or equal to 10 days .. not allowing for any action till receiving Letter To party Reply 
			if(lpParty==null || lpParty.isEmpty())
			{
				addActionError(getMessage("registration.lessThantendays.validate"));
			}
			// validate For greater Than 10 days .. Then allowing only reject Unconsideration to proceed
			...
			if(( workFlowAction!=null && (workFlowAction.equalsIgnoreCase(BpaConstants.FORWARDWORKFLOWSTATUS) || workFlowAction.equalsIgnoreCase(BpaConstants.REJECTWORKFLOWSTATUS)) && (registration.getEgwStatus()!=null && registration.getEgwStatus().getCode().equals(BpaConstants.CMDALETTERTOPARTYSENT))))
				{
		...
		/*
		 * Inspection details are  mandatory for all the services. Before forwarding to EE/SE/RDC, AE must enter these details in the system.
		 */ 
		if(null!=workFlowAction && workFlowAction.equalsIgnoreCase(BpaConstants.FORWARDWORKFLOWSTATUS) && (getAdditionalRule()!=null && (getAdditionalRule().equals(BpaConstants.BPAAPPLICATIONREGISTRATION)))
				&& registration!=null && registration.getCurrentState()!=null && registration.getCurrentState().getValue()!=null 
				&& registration.getCurrentState().getValue().equalsIgnoreCase(BpaConstants.WORKFLOWSTATUSFORWARDEDTOAEORAEE )
				)
		{
			...
			List<DocumentHistoryExtn> docHistory=bpaCommonExtnService.getRegnDocumentHistoryObjectForAEorAEE(registration);
			...
			//TODO:For service type reclassification, check if approver information entered or not. If no approval information entered, then show error message.
			if(null!=registration && registration.getServiceType()!=null && registration.getServiceType().getCode()!=null &&
					registration.getServiceType().getCode().equals(BpaConstants.RECLASSIFICATIONCODE) ){
			List<RegnApprovalInformationExtn> ApproveInfoList=bpaCommonExtnService.getRegnApprovalInfobyRegistrationId(registration);
			...
			/*
			 * Mandatory Fee details should be entered by AEE before forwarding the record to EE.	
			 */
			if(null!=registration && null!=registration.getEgwStatus() && null!=registration.getEgwStatus().getCode() && null!=registration.getServiceType() && null!=registration.getServiceType().getId()){
			//if(registration.getEgwStatus().getCode().equals(BpaConstants.INSPECTED) ||registration.getEgwStatus().getCode().equals(BpaConstants.APPLICATIONREGISTERED))
				...
				if(!registration.getEgwStatus().getCode().equals(BpaConstants.FEESCREATED) )					
				{	
				 List statusCrit=bpaCommonExtnService.getStatusChangeByPassingRegistrationAndStatusCode(registration,BpaConstants.FEESCREATED);
				 if(statusCrit!=null && statusCrit.isEmpty()){	
					List<BpaFeeExtn> santionFeeList=feeExtnService.getAllMandatorySanctionedFeesbyServiceType(registration.getServiceType().getId());
					if(santionFeeList!=null && !santionFeeList.isEmpty()){
						...
						/*if(santionFeeList==null && documentHistoryDetailList.isEmpty())
							documentHistoryDetailList.add(new DocumentHistoryExtn());
						}*/
					}
				 }	
				}
		...
		//if(null!=workFlowAction && workFlowAction.equals(BpaConstants.CLOSEREGISTRATION)){
			String status = bpaCommonExtnService.getPrimaryStatusforRegistration(registration);
			if(status!=null && status.equals(BpaConstants.PRIMARYSTATUSREJECT)){
				if(!registration.getEgwStatus().getCode().equals(BpaConstants.REJECTORDERISSUED))				
				...
				rejectChkListDet=new ArrayList(registration.getRejection().getRejectionChecklistSet());
			}	else if(status!=null && status.equals(BpaConstants.PRIMARYSTATUSAPPROVE)){
				if(!registration.getEgwStatus().getCode().equals(BpaConstants.ORDERISSUEDTOAPPLICANT))	{	
					if(null!=workFlowAction && !workFlowAction.equalsIgnoreCase(BpaConstants.SCRIPT_SAVE)){
	...
	public String showCheckList(){
		...
		chkListDet.clear();
		registration.getRegistrationChecklistSet().clear();
		List<CheckListDetailsExtn> chkListDetails=registerBpaExtnService.getRegistrationCheckListforService(getServiceTypeIdTemp());
	...
	if(registration!=null){
		...
		if(checkFeeforApprovedRegistration()){
			...
			String status = bpaCommonExtnService.getPrimaryStatusforRegistration(registration);
			if(status==null||(!status.equals(BpaConstants.PRIMARYSTATUSREJECT)&&!status.equals(BpaConstants.PRIMARYSTATUSAPPROVE))){
			 previousStatusbeforeLp=bpaCommonExtnService.getAllOlderStatusforRegistration(registration);
			}
			...
			if(previousStatusbeforeLp==null){
					...
					if((ServType.equals(BpaConstants.APPLICATIONFORDEMOLITIONCODE) 
							|| ServType.equals(BpaConstants.SUBDIVISIONOFLANDCODE)
							|| ServType.equals(BpaConstants.RECLASSIFICATIONCODE) 
							|| ServType.equals(BpaConstants.LAYOUTAPPPROVALCODE)) && showActions.contains(BpaConstants.BUILDINGMEASUREMENT))
					{
						showActions.remove(BpaConstants.BUILDINGMEASUREMENT);
					}
					/*
					 * To get Capture DD details alone in Actions : To make it  capture_DD_Details As mendatory Except ServiceType 2..
					 */
					
					//IF CITIZEN PAID ALL THE AMOUNT THROUGH ONLINE THEN NO NEED TO CAPTURE DD INFORMATION. IF REQUIRED, UNCOMMENT BELOW LINES.
					/*else if((!ServType.equals(BpaConstants.APPLICATIONFORDEMOLITIONCODE))
							&& registration.getFeeDDSet().size() ==0 && null!=registration.getEgwStatus() &&
							registration.getEgwStatus().getCode().equals(BpaConstants.CHALLANAMOUNTCOLLECTED) && 
					...
					showActions=  BpaExtnRuleBook.getInstance().getActionsByRoles(roleList,previousStatusbeforeLp);
				}
			
			if(showActions!=null && !ServType.equals(BpaConstants.RECLASSIFICATIONCODE)){
			...
			if(ServType.equals(BpaConstants.RECLASSIFICATIONCODE)){
					...
					if(null!=registration.getEgwStatus() && registration.getEgwStatus().getCode().equals(BpaConstants.STATUSAPPROVED)){
					...
					if(null!=registration.getEgwStatus() && registration.getEgwStatus().getCode().equals(BpaConstants.ORDERPREPARED)){
						showActions.remove(BpaConstants.ORDERPREPARED);
					}		
					}
			}
			...
			//For service type reclassification, We are not capturing fees. No need to send challan to citizen.
			if(ServType.equals(BpaConstants.RECLASSIFICATIONCODE)){
				showActions.remove(BpaConstants.ADDCHALLANSENTDATE);
				showActions.remove(BpaConstants.BUILDINGMEASUREMENT);
				showActions.remove(BpaConstants.CAPTURE_DD_DETAILS);
		...
		if(null!=registration.getEgwStatus() && registration.getEgwStatus().getCode().equals(BpaConstants.APPLICATIONREGISTERED) && showActions.contains(BpaConstants.CREATELETTERTOPARTY))
		{
			showActions.remove(BpaConstants.CREATELETTERTOPARTY);
		}
		if(!showActions.isEmpty() && (!showActions.contains(BpaConstants.CREATELETTERTOPARTY)) && null!=registration.getEgwStatus() && (registration.getEgwStatus().getCode().equals(BpaConstants.FILECONSIDERATIONCHECKED)|| registration.getEgwStatus().getCode().equals(BpaConstants.INSPECTED)))
		{
			showActions.add(BpaConstants.CREATELETTERTOPARTY);
		}
		/*
		 * Capture DD Details is applicable only after Status Challen Amount is collected. and  checking wheather fee details captured or wat..If its collected then removing this action to get Update Signature Action
		 */

		if(registration.getFeeDDSet().size() !=0  && showActions.contains(BpaConstants.CAPTURE_DD_DETAILS)) {
			showActions.remove(BpaConstants.CAPTURE_DD_DETAILS);
		}
		/*
		 * removing create letterToParty action if allready Letter To Party is present for registraion
		 */
		LetterToPartyExtn lpObject=letterToPartyExtnService.getLatestLetterToPartyForRegnTOUniqueValidate(registration);
		if(lpObject==null && registration.getLetterToPartySet().size() !=0  && showActions.contains(BpaConstants.CREATELETTERTOPARTY)) {
			showActions.remove(BpaConstants.CREATELETTERTOPARTY);
		}
		if(registration.getServiceType()!=null && registration.getServiceType().getCode()!=null && registration.getServiceType().getCode().equals(BpaConstants.CMDACODE)){
			
		if((registration.getEgwStatus()!=null && registration.getEgwStatus().getCode().equals(BpaConstants.LETTERTOPARTYSENT)))
		{
			LetterToPartyExtn letterToParty=(LetterToPartyExtn) letterToPartyExtnService.getLatestLetterToPartyForRegObj(registration);
			if(letterToParty.getSentDate()!=null)
			{
		...
		if( registration.getEgwStatus().getCode().equals(BpaConstants.CMDALETTERTOPARTYSENT)  ) {
			if(showActions.contains(BpaConstants.CREATELETTERTOCMDA)){
			showActions.remove(BpaConstants.CREATELETTERTOCMDA);
			}
			if(showActions.contains(BpaConstants.ADDNEWSITEINSPECTIONDETAILS))
					{showActions.remove(BpaConstants.ADDNEWSITEINSPECTIONDETAILS);}
			if(showActions.contains(BpaConstants.MODIFYFEEDETAILS)){
			showActions.remove(BpaConstants.MODIFYFEEDETAILS);
			}
			if(showActions.contains(BpaConstants.COLLECTFEEDETAILS)){
				showActions.remove(BpaConstants.COLLECTFEEDETAILS);
				}
			if(showActions.contains(BpaConstants.ADDAPPROVALINFORMATION)){
				showActions.remove(BpaConstants.ADDAPPROVALINFORMATION);
				}
		}
		if(null!=registration.getEgwStatus() && (registration.getEgwStatus().getCode().equals(BpaConstants.CMDALPREPLYRECEIVED))) {
			showActions.add(BpaConstants.CREATELETTERTOCMDA);
		}
		}
		if(registration.getServiceType()!=null && registration.getServiceType().getCode()!=null && !registration.getServiceType().getCode().equals(BpaConstants.CMDACODE)
				&& showActions.contains(BpaConstants.CREATELETTERTOCMDA)){
			showActions.remove(BpaConstants.CREATELETTERTOCMDA);
		}
		/*
		 * Removing FeeDatils and add second time siteinspection deatils if registration status is LETTERTOPARTYSENT.
		 */
		if(registration.getEgwStatus()!=null && registration.getEgwStatus().getCode().equals(BpaConstants.LETTERTOPARTYSENT))
		{
			showActions.remove(BpaConstants.COLLECTFEEDETAILS);
		...
		//Building Measurement update feature is applicable when the status is Approved. There is no status update so in this part
		//of code, removing it manually from the actions list
		if(registration.getApprdBuildingDetailsSet().size() !=0 && showActions.contains(BpaConstants.BUILDINGMEASUREMENT)) {
			showActions.remove(BpaConstants.BUILDINGMEASUREMENT);
		}
	  //Building Measurement is mandatory in case of service types(1,3,6,7). So Order Prepared action is not allowed
		//without Building Measurement update
		String regServType = registration.getServiceType().getCode();
		if(regServType.equals(BpaConstants.NEWBUILDINGONVACANTPLOTCODE) 
				|| regServType.equals(BpaConstants.DEMOLITIONRECONSTRUCTIONCODE) 
				|| regServType.equals(BpaConstants.ADDITIONALCONSTRUCTIONCODE) 
				|| regServType.equals(BpaConstants.CMDACODE)) {
			
			//If Building Measurement has not yet been created, then Order Prepared feature should be disabled
			if(registration.getApprdBuildingDetailsSet().size()==0 && showActions.contains(BpaConstants.ORDERPREPARED)) {
				showActions.remove(BpaConstants.ORDERPREPARED);
			}
		}
	...
	public Boolean checkFeeforApprovedRegistration() {
		if(registration.getEgwStatus()!=null&&registration.getEgwStatus().getCode().equals(BpaConstants.STATUSAPPROVED)){
			BigDecimal finalFeeTobePaid=bpaCommonExtnService.isFeeCollectionPending(registration);
	...
	/**
	 * checks if registration status is Approved and fee amount to be collected is zero and based on  
	 * service type enables/disables Print Building Permit and Print Plan Permit actions
	 */
	private void checkForPermitPrintAction(RegistrationExtn registration) {
		...
		if(showActions.contains(BpaConstants.PRINTBUILDINGPERMIT)){
			if(!bpaCommonExtnService.isPlanOrBuildingPermitAllowed(registration, finalFeeTobePaid,BpaConstants.BUILDINGPERMIT)){
			showActions.remove(BpaConstants.PRINTBUILDINGPERMIT);
			}
		}
		if(showActions.contains(BpaConstants.PRINTPLANPERMIT)){
			if(!bpaCommonExtnService.isPlanOrBuildingPermitAllowed(registration,finalFeeTobePaid, BpaConstants.PLANPERMIT)){
			showActions.remove(BpaConstants.PRINTPLANPERMIT);
			}
		}
		}
	...
	//	LOGGER.debug("Plan Submission Number in print : " + registration.getPlanSubmissionNum());
		registration=registerBpaExtnService.findById(registration.getId(),false);
		
		if(registration!=null)
			reportId=registerBpaExtnService.printRegistrationForm(registration,getSession());
	...
	public String printInspectionDetails()
	{
		if(getRegistrationId()!=null){
			registration=registerBpaExtnService.findById(getRegistrationId(),false);
		}
		else
		{
			registration=registerBpaExtnService.findById(registration.getId(),false);
		...
		try{
			if(registration!=null){
				if(!bpaCommonExtnService.isUserMappedToSurveyorRole() 
						&& registration.getEgwStatus()!=null && registration.getEgwStatus().getCode().equals(BpaConstants.FILECONSIDERATIONCHECKED) 
						&& (registration.getCurrentState()!=null && registration.getCurrentState().getValue().equals(BpaConstants.FORWARDEDTORDCSTATE) 
								|| registration.getCurrentState().getValue().equals(BpaConstants.FORWARDEDTOEESTATE)
								|| registration.getCurrentState().getValue().equals(BpaConstants.FORWADREDTOSESTATE))){
					// buildDocketObjectDetails();
					 enableDocketSheetForView=Boolean.TRUE;
				}
				reportId=registerBpaExtnService.printInspectionForm(registration,getSession(),printMode,enableDocketSheetForView);
				}
			}catch (Exception e) { 
	...
	public String printDocketSheet()
	{
		...
		if(registration!=null){
			...
			reportId=registerBpaExtnService.printDocketSheet(registration,getSession(),enableDocketSheetForView);
		}
		LOGGER.debug("Exit from print method");
		return "report";
	...
	else
	{
		...
		if(registration!=null){
			...
			reportId=registerBpaExtnService.printDocumentHistoryForm(registration,getSession(),printMode,enableDocketSheetForView);
		}
		}catch (Exception e) { 
			throw new EGOVRuntimeException("Exception : " + e);
		}
		...
		{
			String  userNameWithDesignation="@";
			RegistrationExtn newregistration=registerBpaExtnService.getRegistrationById(registrationId);
			if(registration.getApproverPositionId()!=null && registration.getApproverPositionId()!=-1){
				newregistration.setApproverPositionId(registration.getApproverPositionId());
			}
			...
			List<LetterToPartyExtn> previouslpList= letterToPartyExtnService.getListofLetterToPartyForRegObj(registration,letterToParty);
			if(previouslpList.size()>=1){
				for(LetterToPartyExtn lpdtl:previouslpList){
					lpdtl.setIsHistory('Y');
				}
			}
			// enetr all mendatory fields and press Enter keyword its taking workFlowAction as save....to avaid this test case 
			...
			registration=registerBpaExtnService.createLetterToParty(letterToParty,lpChkListDet,newregistration,workFlowAction,approverComments);
			List <LetterToPartyExtn> letterToParty = registerBpaExtnService.getLetterToPartyForRegistrationObject(registration);
			registration.setLetterToParty(letterToParty.get(0));
			setLetterToPartyId(letterToParty.get(0).getId());
			
			if(null!=registration.getEgwStatus() && null!=registration.getEgwStatus().getCode() && (registration.getEgwStatus().getCode().equalsIgnoreCase(BpaConstants.LETTERTOPARTYRAISED) || registration.getEgwStatus().getCode().equalsIgnoreCase(BpaConstants.LETTERTOPARTYSENT))) {
			...
			if("".equals(mode) || mode==null) {
				addActionMessage("Letter To Party "+letterToParty.get(0).getLetterToPartyNumber() +" is created successfully");	
				}
				else if(mode.equals("modify")) 
				{
					addActionMessage("Letter To Party "+letterToParty.get(0).getLetterToPartyNumber() +" is updated successfully");
				}
				 
				else if(mode.equals("enterSentDate")) 
				{
					addActionMessage("Letter To Party "+letterToParty.get(0).getLetterToPartyNumber() +" has been sent to Applicant");	
				} 
				if(workFlowAction!=null && workFlowAction.equalsIgnoreCase(BpaConstants.FORWARDWORKFLOWSTATUS))
				{
				userNameWithDesignation=registerBpaExtnService.getEmpNameDesignationByApproveDesgIdAndApproveId( approverDesignation,approverPositionId,new Date());
				if( !"@".equals(userNameWithDesignation) )
				addActionMessage("The File has been forwarded to  "+userNameWithDesignation );
				}
			setMode(BpaConstants.MODEVIEW);
		...
		public String saveLetterToCMDA()
		{
			...
			newregistration.setAdditionalRule(registration.getAdditionalRule());
			newregistration.setAdditionalState(registration.getAdditionalState());
			List<CMDALetterToPartyExtn> previouslpList= letterToPartyExtnService.getListofCMDALetterToPartyForRegObj(registration,letterToCMDA);
			if(previouslpList.size()>=1){ 
			...
			registration=registerBpaExtnService.createCMDALetterToParty(letterToCMDA,newregistration,workFlowAction,approverComments);
		
			List <LetterToPartyExtn> letterToParty = registerBpaExtnService.getLetterToPartyForRegistrationObject(registration);
			registration.setLetterToParty(letterToParty.get(0));
			setLetterToPartyId(letterToParty.get(0).getId());
			
			if(null!=registration.getEgwStatus() && null!=registration.getEgwStatus().getCode() && (registration.getEgwStatus().getCode().equalsIgnoreCase(BpaConstants.CMDALETTERTOPARTYSENT))) {
				
				/*List<Map<String, Object>> finalAttachmentList = new ArrayList<Map<String,Object>>();
				HashMap<String, Object> attachmentList = new HashMap<String, Object>(); 
				HashMap<String, Object> attachmentFileNames = new HashMap<String, Object>(); 
			    attachmentList.put("letterToCMDA", generateLetterToPartyPDF());
				attachmentFileNames.put("letterToCMDAFileName", "letterToCMDAPDF.pdf"); 
				finalAttachmentList.add(attachmentList);
				finalAttachmentList.add(attachmentFileNames); */
				bpaCommonExtnService.buildEmail(registration,BpaConstants.SMSEMAILCMDALP,null);			
				bpaCommonExtnService.buildSMS(registration,BpaConstants.SMSEMAILCMDALP);  
			}
			
			if("".equals(mode) || mode==null) {
				addActionMessage("Letter To CMDA "+letterToCMDA.getLetterToPartyNumber() +" is created successfully");	
				}
				else if(mode.equals("modify")) 
				{
					addActionMessage("Letter To CMDA "+letterToCMDA.getLetterToPartyNumber() +" is updated successfully");
				}
		...
		public String  showLetterToPartyCheckList(){
			ServiceTypeExtn serviceType=registerBpaExtnService.getServiceTypeById(serviceTypeIdTemp);
			if(letterToPartyId==null || letterToPartyId.equals("")){
				if(serviceType.getCode()!=null){
					List<CheckListDetailsExtn> chkListDetails=registerBpaExtnService.getLPCheckListforService(serviceTypeIdTemp);
					for(CheckListDetailsExtn chkList:chkListDetails){
				...
				if(serviceType.getCode()!=null){
					lpChkListDet = registerBpaExtnService.getLetterToPartyCheckDtlsForType(letterToPartyId,
		...
		public String showExistingLetterToPartyCMDADetails(){
			...
			if(letterToPartyId!=null){
				LetterToPartyExtn lpObj=registerBpaExtnService.getLetterToPartyById(letterToPartyId);
				if(lpObj!=null){
				existingCmdaLetterToPartyDetails.addAll(lpObj.getCmdaLetterToPartySet());
				}
		...
		public String viewLetterToPartyForm(){
			registration=registerBpaExtnService.getRegistrationById(registrationId);
			List <LetterToPartyExtn> letterToPartyList = registerBpaExtnService.getLetterToPartyForRegistrationObject(registration);
			if(!letterToPartyList.isEmpty())
			letterToParty=registerBpaExtnService.getLetterToPartyById(letterToPartyList.get(0).getId());
		...
		public String modifyLetterToPartyForm(){
			letterToParty=registerBpaExtnService.getLetterToPartyById(letterToPartyId);
			setRegistration(letterToParty.getRegistration());
			registration.setLetterToParty(letterToParty);
			List<LpChecklistExtn> checkListDtlsList=new ArrayList<LpChecklistExtn>(letterToParty.getLpChecklistSet());
		...
		/**
		 * RegnStatusDetails (Signature,Order Preparation, Order Issue Details,Reject Order Preparation Details,
		 *	Reject Order Issue Details)
		 */
		@SkipValidation
				...
				if(registration != null) {
					if(registration.getRegnStatusDetailsSet()!=null) {
						for(RegnStatusDetailsExtn regnStatDet : registration.getRegnStatusDetailsSet()) {
							...
							if(regnStatDet.getStatus().getCode().equals(BpaConstants.ORDERPREPARED)) {
								registration.setOrderDetails(regnStatDet);
								setValidateDate(regnStatDet.getStatusdate()); 
							}
							if(regnStatDet.getStatus().getCode().equals(BpaConstants.ORDERISSUEDTOAPPLICANT)) {
								registration.setOrderIssueDet(regnStatDet);
								setValidateDate(regnStatDet.getStatusdate()); 
							}
							if(regnStatDet.getStatus().getCode().equals(BpaConstants.REJECTORDERPREPARED)) {
								registration.setRejectOrdPrepDet(regnStatDet);
							//	setValidateDate(regnStatDet.getStatusdate());  
							}
							if(regnStatDet.getStatus().getCode().equals(BpaConstants.REJECTORDERISSUED)) {
		...
		public void buildOfficialViewActions(){
			...
			regnOffcialActionsList=registerBpaExtnService.getOfficialActionsByRegId(registration.getId());
		...
		public void createRegOfficialViewObj(List<String> typeList,List<RegnOfficialActionsExtn> regnOffcialActionsList){
					...
					RegnOfficialActionsExtnView regnOffcialActionsExtnViewObj=new RegnOfficialActionsExtnView();
						for(RegnOfficialActionsExtn record : regnOffcialActionsList){
							...
							if(viewType.equalsIgnoreCase(BpaConstants.VIEWED_SURVEYOR_INSPECTION)) 
								...
								officialActionObj.setActionsValue(record.getViewedAE_AEEInsp());
							if(viewType.equalsIgnoreCase(BpaConstants.VIEWED_SURVEYOR_DOCDETAILS))
								officialActionObj.setActionsValue(record.getViewedSurveyorDocDtls());
							if(viewType.equalsIgnoreCase(BpaConstants.VIEWED_AE_AEE_DOCDETAILS))
								officialActionObj.setActionsValue(record.getViewedAE_AEEDocDtls());
							if(viewType.equalsIgnoreCase(BpaConstants.VIEWED_AUTODCR_DETAILS))
								officialActionObj.setActionsValue(record.getViewedAutoDcrDtls());
							if(viewType.equalsIgnoreCase(BpaConstants.VIEWED_DOCKETSHEET))
								officialActionObj.setActionsValue(record.getViewedDocketSheet());
							
							regnOffcialActionsExtnViewObj.addOfficialActions(officialActionObj);
						}
		...
		//	existReg = (Registration)persistenceService.find("from org.egov.bpa.models.Registration where id=?" ,registrationId);
			
			if(registrationId!=null)
				existReg=registerBpaExtnService.getRegistrationById(registrationId);
		 //Registration existReg = registerBpaService.findById(registrationId,false);
			LOGGER.info("Within updateOrders method-After existReg"+existReg);
		
			if(reqdAction!=null && !"".equals(reqdAction) && existReg!=null) {	
					if (reqdAction!=null && reqdAction.equals(BpaConstants.ADDCHALLANSENTDATE)) {
						existReg.setChallanDetails(registration.getChallanDetails());
						registration = registerBpaExtnService.updateChallanSentDate(existReg);
						List<Map<String, Object>> finalAttachmentList = new ArrayList<Map<String,Object>>();
						BigDecimal totalAmount = bpaCommonExtnService.isFeeCollectionPending(registration);			
						if(!totalAmount.equals(BigDecimal.ZERO)){
							...
							attachmentList.put("feePayment",bpaCommonExtnService.generateFeePaymentReportPDF(registration,reportFeeDetailsList));
							attachmentFileNames.put("feePaymentFileName", "feePaymentReportPDF.pdf");
							finalAttachmentList.add(attachmentList);
							finalAttachmentList.add(attachmentFileNames);
						}	
						bpaCommonExtnService.buildEmail(registration,BpaConstants.SMSCHALLANNOTICESENTDATE,finalAttachmentList);
						bpaCommonExtnService.buildSMS(registration,BpaConstants.SMSCHALLANNOTICESENTDATE);
						...
						if(registration!=null && registration.getServiceType()!=null && registration.getServiceType().getCode()!=null){
							if(!registration.getServiceType().getCode().equals(BpaConstants.CMDACODE)){
								attachmentList.put("planPermitProvisional",bpaReportExtnService.getPlanningpermitReportDetails(registration));
								attachmentFileNames.put("planPermitProvisionalName", "PlanPermitProvisional.pdf");
							}
							...
							attachmentList.put("buildingPermitProvisional",bpaReportExtnService.getBuildingPermitReportdetails(registration));
							attachmentFileNames.put("buildingPermitProvisionalName", "buildingPermitProvisional.pdf");
							}
							
							finalAttachmentList.add(attachmentList);
							finalAttachmentList.add(attachmentFileNames);
							bpaCommonExtnService.buildEmail(registration,BpaConstants.UPDATESIGNATURE,finalAttachmentList);
						...
						registration = registerBpaExtnService.updateOrderPrepStatus(existReg);
						...
						registration = registerBpaExtnService.updateOrderIssueStatus(existReg);
						closeRegistration();
					}
					else if(reqdAction.equals(BpaConstants.REJECTORDPREP)) {
						existReg.setRejectOrdPrepDet(registration.getRejectOrdPrepDet());
						registration = registerBpaExtnService.rejectOrderPrepStatus(existReg);
						...
						registration = registerBpaExtnService.rejectOrderIssueStatus(existReg);
		...
		public String rejectForm(){
			...
			buildRegistrationForViewModify();
			if(registration.getRejection()==null){
				List<CheckListDetailsExtn> chkListDetails=registerBpaExtnService.getUnconsideredCheckListforService(registration.getServiceType().getId());
				for(CheckListDetailsExtn chkList:chkListDetails){
		...
		public String letterToPartyForm(){  
			...
			{
			 List<InspectionExtn> inspectionList=inspectionExtnService.getSiteInspectionListforRegistrationObjectWhereInsDetailsEnetered(registration,LoggedInuser);
				if(inspectionList==null||inspectionList.size()==0){
		...
		public String letterToCMDAForm(){  
			...
			registration.setAdditionalRule(BpaConstants.LETTERTOCMDA);
			setAdditionalRule(BpaConstants.LETTERTOCMDA);
			registration.setAdditionalState(BpaConstants.LETTERTOCMDA_WF_INITIALSTATUS);
		...
		public List<String> getValidActions() {
			try{ 
			if(getAdditionalRule()!=null && (getAdditionalRule().equals(BpaConstants.ADDITONALRULEREJECTBPA) ||
					getAdditionalRule()!=null && (getAdditionalRule().equals(BpaConstants.LETTERTOPARTYDETAILS) || getAdditionalRule().equals(BpaConstants.LETTERTOCMDA)))){
				List<String> validActions = Collections.emptyList();				
				validActions = this.customizedWorkFlowService.getNextValidActions(this.getModel().getStateType(),
						this.getWorkFlowDepartment(), this.getAmountRule(), this.getAdditionalRule(),this.getModel().getAdditionalState(), this.getPendingActions());		
				return validActions;
		...
		public String getNextAction() { 
			try{
				if((getAdditionalRule()!=null&&getAdditionalRule().equals(BpaConstants.ADDITONALRULEREJECTBPA)
						||getAdditionalRule()!=null&& (getAdditionalRule().equals(BpaConstants.LETTERTOPARTYDETAILS) || getAdditionalRule().equals(BpaConstants.LETTERTOCMDA)))
						&&(this.getModel().getAdditionalState().equals(BpaConstants.ADDITONALRULEREJECTBPA)
								||this.getModel().getAdditionalState().equals("LetterToParty saved")
								||this.getModel().getAdditionalState().equals("rejection initiated")
								||this.getModel().getAdditionalState().equals("UnconsiderdBPA")
								||this.getModel().getAdditionalState().equals("LP Initiated")
								||this.getModel().getAdditionalState().equals("LetterToCMDA saved")
								||this.getModel().getAdditionalState().equals("LetterToCMDA Initiated"))){ 
			WorkFlowMatrix wfMatrix = null;		
			String tempcurrentstate=this.getModel().getAdditionalState();
					wfMatrix = this.customizedWorkFlowService.getWfMatrix(this.getModel().getStateType(),
							this.getWorkFlowDepartment(), this.getAmountRule(), this.getAdditionalRule(),tempcurrentstate,
		...
		private InputStream generateLetterToPartyPDF()
		{
			...
			LetterToPartyExtn letterToParty = generateLetterToPartyPrint(reportParams);	
				reportRequest=new ReportRequest(BpaConstants.LETTERTOPARTYNOTICEEXTN, letterToParty, reportParams);
		...
		private InputStream generateLetterToCMDAPDF(RegistrationExtn registration)
		{
			...
			ReportRequest reportRequest = null;
			Map <String,Object>reportParams = new HashMap<String,Object>();
			Map<String,Object> reportData = bpaCommonExtnService.generateCmdaLetterToPartyPrint(registration,null);	
			...
			reportParams.put("replyDate", reportData.get("replyDate"));
				reportRequest=new ReportRequest(BpaConstants.CMDALPREPLYACKREPORTEXTN, reportData, reportParams);
		...
		public String captureDDForm(){
			 ...
			 */
		List<RegistrationFeeExtn> registrationFee=registrationFeeExtnService.getpreviousFeeDetails(registrationId);
		if(null!=registration && registrationFee.size()>0  )
			{
			/*
			 * To get Total Amount of CMDA Deatils and MWGWF details  FROM RegistrationFeeDetails table. to confirm values are entered for CMDA and MWGwF..
			 */
			Set<RegistrationFeeDetailExtn> captureAmountList=	bpaCommonExtnService.getLatestApprovedRegistrationFee(registration).getRegistrationFeeDetailsSet();
			//List<RegistrationFeeDetail> captureAmountList=registerBpaService.getCaptureDdAmountTotal(registrationId,registration);
			for(RegistrationFeeDetailExtn chkList:captureAmountList){
				if(chkList!=null)
				{
					if(chkList.getBpaFee()!=null && chkList.getBpaFee().getFeeGroup()!=null && chkList.getBpaFee().getFeeGroup().equals(BpaConstants.FEEGROUPCMDA)){
					...
					else if(chkList.getBpaFee()!=null && chkList.getBpaFee().getFeeGroup()!=null && chkList.getBpaFee().getFeeGroup().equals(BpaConstants.FEEGROUPMWGWF))
					{
						if(chkList.getAmount()!=null){
	...
	//	Date tempDate=	registration.getExternalfeecollectedDate();
			//registration=registerBpaService.find("from Registration where id=?",getRegistrationId());
	     	...
	     	/*
			 * To check FeeDetails Entry is there For current records. AND if not there den  it wil not do equal validation for fee datils for CMDA and MWGWF
			 * and Total DD Amount  of both while capturing DD Deatils. getpreviousFeeDetails from registrationFeeService
			 */
			List<RegistrationFeeExtn> registrationFee=registrationFeeExtnService.getpreviousFeeDetails(registrationId);
			if(registrationFee.size()>0  )
			{
			if(!(mwgwfAmounttotal.equals(totalmwgwf)) || !(cmdaAmounttotal.equals(totalcmda)))
			{
			...
			for(RegistrationDDDetailsExtn dddtls:getLabourWelfareddList()){
				if(dddtls!=null ){
					if(dddtls.getLwddAmount()!=null&&!dddtls.getLwddAmount().equals("")){
						dddtls.setDdAmount(dddtls.getLwddAmount());
						}else continue;
						if(dddtls.getLwddbankName()!=null&&!dddtls.getLwddbankName().equals(""))
						dddtls.setDdBank(bpaCommonExtnService.getBankbyId(dddtls.getLwddbankName()));
						else continue;
						if(dddtls.getLwddDate()!=null&&!dddtls.getLwddDate().equals(""))
						dddtls.setDdDate(dddtls.getLwddDate());
						else continue;
						if(dddtls.getLwddNumber()!=null&&!dddtls.getLwddNumber().equals(""))
					...
					dddtls.setRegistration(registration);
					dddtls.setDdAmount(dddtls.getLwddAmount());
					dddtls.setDdBank(bpaCommonService.getBankbyId(dddtls.getLwddbankName()));
					dddtls.setDdDate(dddtls.getLwddDate());
					dddtls.setDdNumber(dddtls.getLwddNumber());*/
		...
		for (RegistrationDDDetailsExtn dddtls : getCmdaddList()) {
			...
			registration.setExternalfeecollectedDate(registration.getExternalfeecollectedDate());
	...
	public String printExternalFeeDetails() {
		...
		if (registration != null && registration.getFeeDDSet() != null
			...
			reportId = registerBpaExtnService.printExternalFeeDetails(registration,
					getSession());
	...
	private RegistrationDDDetailsExtn prepareDDDetails(RegistrationDDDetailsExtn dddtls) {
		
		if(dddtls.getDdType().equals("LabourWelfare")){
		dddtls.setLwddAmount(dddtls.getDdAmount());
		dddtls.setLwddbankName(dddtls.getDdBank()!=null?dddtls.getDdBank().getId():-1);
		dddtls.setLwddNumber(dddtls.getDdNumber());
		dddtls.setLwddDate(dddtls.getDdDate());
		}
		else if(dddtls.getDdType().equals("CMDA")){
			dddtls.setMsddAmount(dddtls.getDdAmount());
			dddtls.setMsddbankName(dddtls.getDdBank()!=null?dddtls.getDdBank().getId():-1);
	...
	public void setDocumentationchecklist(List<ChecklistExtn> documentationchecklist) {
	...
	public void setRejectChkListDet(List<RejectionChecklistExtn> rejectChkListDet) {
...
/*	public FileManagementService getFileManagementService() {
	...
	public void setDocketDocumentDtls(List<DocketDocumentDetails> docketDocumentDtls) {

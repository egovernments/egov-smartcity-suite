...
public class DDDetailsForFeeGroupReportAction extends BaseFormAction{
		...
		private List<FeesReportResultExtn> regResultList = new ArrayList<FeesReportResultExtn>(0);
		private Map<String, LinkedHashMap<String,String>> FeeDetailmap= new LinkedHashMap<String,LinkedHashMap<String,String>>();
		
	public void prepare()
	{
		List<ServiceTypeExtn>servList=persistenceService.findAllBy("from ServiceTypeExtn where code NOT IN (?) order by code",BpaConstants.RECLASSIFICATION);
		addDropdownData("serviceTypeDropDownList", servList);
		addDropdownData("feeGroupList",persistenceService.findAllBy("from BpaFeeExtn order by feeCode"));
	...
	/*
	    * To get Application records whose Order Issued To Applicant DAte Update is Done....
	    */
	   ...
	   public void search() {
		   ...
		   if(getServiceTypeList()!=null && getServiceTypeList().size()>0 ){
			   regList= persistenceService.findAllBy("from RegistrationExtn  where egwStatus.code='Order Issued to Applicant' and  serviceType.id in "  + getServiceTypeList().toString().replace('[', '(').replace(']', ')' ));
			   
		  }
		   else{
			  regList= persistenceService.findAllBy("from RegistrationExtn where egwStatus.code in(?) and serviceType.id NOT IN (select id from ServiceTypeExtn where code IN (?)) order by id",BpaConstants.ORDERISSUEDTOAPPLICANT,BpaConstants.RECLASSIFICATION);	
			     
		   }
		   List<BpaFeeExtn> regFeelist=persistenceService.findAllBy("SELECT DISTINCT feeDescription FROM BpaFeeExtn where serviceType.code NOT IN (?) order by feeDescription",BpaConstants.RECLASSIFICATION);
		  Object[] objListToStringArray=regFeelist.toArray();
		   for(RegistrationExtn regobj:regList)
		   {
			   ...
			   FeeDetailmap.put(regobj.getPlanSubmissionNum(), regObjwithZeroValue);
		   ...
		   /*
		    * joining RegnStatusDetails table only if user selects permit order issued FromDate and Todate Or Anyone of these.
		    */
		   ...
		   if ((getOrderissuedFromDate() != null && !"".equals(getOrderissuedFromDate()))||(getOrderissuedToDate() != null && !"".equals(getOrderissuedToDate())) ) {
			   ...
			   if (getOrderissuedFromDate() != null && !"".equals(getOrderissuedFromDate()) ) {
				   dateqry=dateqry+" and regstatus.statusdate>= :issuedfromdate ";
				}
				if (getOrderissuedToDate() != null && !"".equals(getOrderissuedToDate()) ) {
					dateqry=dateqry+" and regstatus.statusdate<= :issuedtodate ";
				}
		   }
		  qryStr.append(" where reg.id=fees.registration.id  and fees.id=feeDet.registrationFee.id and feeDet.bpaFee.id=bpafee.id");
		  /*if(getPlanSubmissionNumber()!=null && !getPlanSubmissionNumber().equals(""))
		  {
			...
			query.setResultTransformer(Transformers	.aliasToBean(FeesReportResultExtn.class));
			...
			LinkedHashMap<String,String> tempserviceMap= FeeDetailmap.get(psnNumber);
			if(tempserviceMap!=null && !tempserviceMap.isEmpty()){
				
				
				if((tempserviceMap.get("DD Number").isEmpty() || tempserviceMap.get("DD Number")==null) && tempserviceMap.get("DD Amount").isEmpty() && tempserviceMap.get("Site Address").isEmpty()){
				ReceiptInstrumentInfo instrumentObj=	getReceiptinstrumentInfo(fedetTempObj.getRegistration());
				if(instrumentObj!=null && instrumentObj.getInstrumentType().equals("dd")){
					tempserviceMap.put("DD Number",instrumentObj.getInstrumentNumber() );
					tempserviceMap.put("DD Date",getIssuedDate(instrumentObj.getInstrumentDate()));
					tempserviceMap.put("DD Amount", instrumentObj.getInstrumentAmount().toString());
					tempserviceMap.put("Bank Name",instrumentObj.getBankName() );
				}
	   ...
	   public ReceiptInstrumentInfo getReceiptinstrumentInfo(RegistrationExtn registration)
		{
			...
			Map<String, BillReceiptInfo> billReceiptInfoMap = bpaReportExtnService.getReceiptInfoMap(registration);
			List<BpaFeeExtn> santionFeeList  =feeExtnService.getAllSanctionedFeesbyServiceTypeSortByOrderNumber(registration.getServiceType().getId());
			...
			for( BillReceiptInfo billeciept:billRecptInfoList)
			{
				for(ReceiptAccountInfo recAccInfo : billeciept.getAccountDetails())
				{
					if (recAccInfo!=null && recAccInfo.getCrAmount()!=null && recAccInfo.getCrAmount().compareTo(BigDecimal.ZERO) > 0) {
						...
						{
						if(null!=reportFeesDtls.getFeeDescription() && !"".equals(reportFeesDtls.getFeeDescription()) && reportFeesDtls.getFeeDescription().equalsIgnoreCase(demandMasterReasonDesc)){
	   ...
	   private LinkedHashMap<String, String> getEmptyFeeDetailCodeWithZeroValue(Object[] getRegistrationFeeDetailList) {

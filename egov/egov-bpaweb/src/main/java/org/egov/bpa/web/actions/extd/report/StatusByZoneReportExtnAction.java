...
public StatusByZoneReportExtnAction(){
   ...
   public void prepare()
   {
	...
	public String  showdetail()
	{
		...
		qryStr.append(" order by reg.id");
			...
			Query query = HibernateUtil.getCurrentSession().createQuery(qryStr.toString());
			 if(getWardcode()!=null && "total".equals(getWardcode())){
				 query.setString("zonecode",getZonecode()!=null?getZonecode():getZonecode());
						
			 }
			 else if(getWardcode()!=null && !"total".equals(getWardcode())){
				 query.setString("wardcode", getWardcode()!=null?getWardcode():getWardcode());
			 }
	...
	if(getStatecode()!=null){
		   		 ...
		   		 statusList= persistenceService.findAllBy("from EgwStatus where moduletype=? and code=? order by code",BpaConstants.NEWBPAREGISTRATIONMODULE,getStatecode());	
	}  
	 else
		...
		if(getStatecode()!=null){
	     ...
	     query.setResultTransformer(Transformers.aliasToBean(ZoneReportResultExtn.class));                                                                                                                                                                                          
		 ...
		 for(ZoneReportResultExtn result:wardpositionList)
	       ...
	       status=result.getStatusName();
	    	 ...
	    	 tempstatusmap.put(result.getWardName().toUpperCase(), result.getCount());	    	
	    	tempstatusmap.put("total", tempstatusmap.get("total")+result.getCount());
	...
	public void getappendquery()
	{
		 ...
		 if(getEgwStatus()!=null && getEgwStatus()!=-1 &&(getStatecode()==null||"".equals(getStatecode()) ))
		    {
...
@SuppressWarnings("unchecked")
	...
	{	
        if(getEgwStatus()!=-1){
        	statusList= persistenceService.findAllBy("from EgwStatus where moduletype=? and id=? order by orderId",BpaConstants.NEWBPAREGISTRATIONMODULE,getEgwStatus());	
         }else
        	 statusList= persistenceService.findAllBy("from EgwStatus where moduletype=? order by orderId ",BpaConstants.NEWBPAREGISTRATIONMODULE);
       ...
       //qryStr.append("SELECT DISTINCT bndry.parent.name AS zoneName, COUNT(reg.id) AS count , reg.egwStatus.code AS statusName FROM Registration reg, BoundaryImpl bndry WHERE  reg.adminboundaryid=bndry.id AND bndry.boundaryType.id IN (2,3) ");
    	...
    	query.setResultTransformer(Transformers.aliasToBean(ZoneReportResultExtn.class));
    	List<ZoneReportResultExtn> statusResult = (List<ZoneReportResultExtn>)query.list();
   
   
	   for(ZoneReportResultExtn result:statusResult)
	   {    	 
			status=result.getStatusName();
			TreeMap<String,Long> tempstatusmap= appstatusmap.get(status);  // new  TreeMap<String,Long>();        
			tempstatusmap.put(result.getZoneName().toUpperCase(), result.getCount());
			tempstatusmap.put("total", tempstatusmap.get("total")+result.getCount());
		    appstatusmap.put(status,tempstatusmap);
	    }

...
function bodyOnLoad(){
	...
	var regServiceTypeCode=document.getElementById('regServiceTypeCode').value;
	
	documentHistoryFormodifyAndView();
	var  doctabEnabled=document.getElementById('enableDocketSheetForView').value;
	...
	if(stateForValidate=='' )
		...
		if(regServiceTypeCode=='02' || regServiceTypeCode=='07' || regServiceTypeCode=='08')
		{
			jQuery('#docEnclosedDiv').hide();
		}
		}
		if(mode=='noEdit' && stateForValidate=='Forward to AEORAEE'){
	...
	if(document.getElementById('workflowDiv')!=null){
		...
		if(dept!=null){
				...
				//Do not Show Save and Action Button in higher level users login
				document.getElementById('approverDepartment').value=dept; 
				loadDesignationFromMatrixOnLoad();
		}
	}
				 if(doctabEnabled=="true"){
					 
					 //jQuery('#regDocenabledId').show();
						jQuery('#header_1').hide();
						jQuery('#header_5').hide();
						jQuery('#header_2').hide();
						jQuery('#header_3').hide();
						jQuery('#header_4').hide();
						jQuery('#header_6').hide();
						jQuery('#regForm').show();
						jQuery('#header_7').show(); 
						//jQuery('#regDocketForm').show();
						jQuery('#regForm').find('input,select,textarea,textfield').attr('disabled',true);
						
						} 
				 else
					 {
					 jQuery('#regDocenabledId').hide();
					 	jQuery('#header_1').show();
						jQuery('#header_5').show();
						jQuery('#header_2').show();
						jQuery('#header_3').show();
						jQuery('#header_4').show();
						jQuery('#header_6').show();
						//jQuery('regDocketForm').hide();
					 	jQuery('#header_7').hide();
					 
					 }
					<s:if test="%{enableDocketSheetForView}">
					 document.getElementById('surveyorDocView').disabled=false;
					document.getElementById('officialDocHistoryView').disabled=false;
					 document.getElementById('surveyorInspectionView').disabled=false;
					 document.getElementById('officialInspectionView').disabled=false;
				
					document.getElementById('officialDocketView').disabled=false;
				</s:if>
}
...
function enabelRegistrationDetailsOnFeecollection(){
	...
	var zoneData=document.getElementById("Zone").value;
	var wardData=document.getElementById("Ward").value;
	var areaData=document.getElementById("Area").value;
	var locationData=document.getElementById("Locality").value;
	var streetData=document.getElementById("Street").value;
	...
	jQuery('#surveyorDiv').find(':input').prop('disabled',true);	
...
function printDocumentHistorySheet(inspectionid){
	window.open("${pageContext.request.contextPath}/extd/register/registerBpaExtn!printDocumentHistory.action?registrationId="+inspectionid+"&printMode=OfficialPrint","simple","height=650,width=900,scrollbars=yes,left=20,top=20,status=yes");
	}
function printDocumentHistorySheetForSurveyor(inspectionid){
	window.open("${pageContext.request.contextPath}/extd/register/registerBpaExtn!printDocumentHistory.action?registrationId="+inspectionid+"&printMode=surveyorPrint","simple","height=650,width=900,scrollbars=yes,left=20,top=20,status=yes");
	}

	function printInspectionForm(inspectionid){
			window.open("${pageContext.request.contextPath}/extd/register/registerBpaExtn!printInspectionDetails.action?registrationId="+inspectionid+"&printMode=OfficialPrint","simple","height=650,width=900,scrollbars=yes,left=20,top=20,status=yes");
	}

	function printSurveyorInspectionForm(inspectionid){
		window.open("${pageContext.request.contextPath}/extd/register/registerBpaExtn!printInspectionDetails.action?registrationId="+inspectionid+"&printMode=surveyorPrint","simple","height=650,width=900,scrollbars=yes,left=20,top=20,status=yes");
}
...
function validateForm(obj){
 ...
 var rejectionDates=document.getElementById('rejectionDate').value;	
	...
	if(mode!='reject' && stateForValidate!="" && stateForValidate=='Forward to AEORAEE' && (obj=='Forward' || obj=='save' ))
	{	
			if(!validateDocHistoryDetail())
			{ 
			return false;
			}
			if(document.getElementById('regServiceTypeCode').value=='01' || document.getElementById('regServiceTypeCode').value=='03' ||
					 document.getElementById('regServiceTypeCode').value=='04'|| document.getElementById('regServiceTypeCode').value=='05' || document.getElementById('regServiceTypeCode').value=='06')
			{
...
{   
    ...
    if( regid =="" && egwStatus=="" || regid !="" && egwStatus=='CitizenRegisteredApplication' ){
...
<s:form name="RegisterBpaForm" action="registerBpaExtn" theme="simple" onsubmit="enableFields();" >
  <s:push value="model">
  <s:token/> 
  <div class="formmainbox">
  	...
  	<s:hidden id="request_number" name="request_number" value="%{request_number}"/>
			...
			<s:hidden id="userRole" name="userRole" value="%{userRole}"/>
     		...
     		<s:hidden id="createdBy" name="createdBy" value="%{createdBy.id}"/>
	   		...
	   		<s:hidden id="currentstate" name="currentstate" value="%{currentstate}"/> 
		    ...
		    <s:hidden id="enableDocketSheetForView" name="enableDocketSheetForView" value="%{enableDocketSheetForView}"/>
		    ...
		    <s:hidden id="egwStatus" name="egwStatus" value="%{egwStatus.id}"/>
		   	...
		   	<s:hidden id="regServiceTypeCode" name="regServiceTypeCode" value="%{serviceType.code}"/>
			...
			<s:hidden id="serviceTypeCode" name="serviceTypeCode" value="%{serviceTypeCode}"/>
     		...
     		<s:hidden id="forwardNotApplicable" name="forwardNotApplicable" value="%{forwardNotApplicable}"/> 
			<s:hidden id="stateForValidate" name="stateForValidate" value="%{stateForValidate}"/>
			<s:hidden id="showSave_ActionField" name="showSave_ActionField" value="%{showSave_ActionField}"/>
  	...
  	<div class="insidecontent">
  		<div class="rbroundbox2">
  			<div class="rbcontent2">
  			  <table width="100%" border="0" cellspacing="0" cellpadding="0">
  				<tr>
	            	<td><div id="tabs2">
						...
						<s:if test="%{enableDocketSheetForView}">
						<ul id="Tabs">
						<li id="regTab" class="First Active"><a id="header_7" href="#" onclick="showRegDocketTab();"><s:text name="RegisterBpaHeader"/></a></li>
							<li id="regTab" class="Befor"><a id="header_1" href="#" onclick="showRegTab();"><s:text name="RegisterBpaHeader"/></a></li>
	           ...
	           <tr>
		            <td>
		            <s:if test="%{additionalMode=='editApprovedRecord'}">
			            <div id="regForm">
			            ...
			            <s:else>
			             <div id="regForm">
			               <%@ include file="registerBpaExtn-viewForm.jsp"%>   
			            </div>
			            </s:else>
	          ...
	          <tr>
		            <td>
			            <div id="letterToPartyForm" style="display:none;">
							 <s:url id="lettertoparty" value="/extd/register/registerBpaExtn!showExistingLetterToPartyDetails.action"  escapeAmp="false">	 
								   <s:param name="registrationId" value="%{id}"></s:param>	  
	          ...
	          <tr>
		            <td>
			            ...
			            <div id="feeDetForm" style="display:none;">
	                         ...
	                         <s:url id="existingfeedetllink" value="/extd/fee/revisedFeeExtn!showExistingFeeDetails.action" escapeAmp="false" >
	                         <s:param name="registrationId" value="%{id}"></s:param>	  
	                        </s:url>
							 ...
							 <s:url id="feelink" value="/extd/fee/feeDetailsExtn!showFeeDetailsinRegistration.action" escapeAmp="false">
									      <s:param name="registrationId" value="%{id}"></s:param>	  
									   </s:url>
									   <div style="display:none">
	          ...
	          <s:if test="%{!enableDocketSheetForView}">
	   		...
	   		<s:else>
	   		<div id="docHistoryForm" style="display:none;"> </div>
	   		</s:else>
	...
	<s:if test="%{!forwardNotApplicable}">
	<s:if test="%{mode!='view'}"> 
		<div id="approverInfo">
	        ...
	        <!-- Autoselect approver during Approval Process -->
	        <s:if test="%{registration.id!=null && registration.state.value!='Forwarded to Approval'}">
	         <div id="workflowDiv">
	...
	<div class="buttonbottom" align="center">
		...
		<table>
			<s:if test="%{mode=='view'}">
  				<td>
  				   <!-- Do not show print for Approval Users -->
  				   <s:if test="%{registration.id!=null && registration.state.value!='Forwarded to Approval'}">
	  		...
	  		<s:else> 
	  		<s:if test="%{mode!='reject' && rejectview!='reject'}"> 
	  			<s:if test="%{showSave_ActionField}">	
	  	...
	  	<s:if test="%{!forwardNotApplicable}">	
	  		...
	  		<td>
				<div class="dropdown">
					...
					<s:url id="actionsajax" value="/extd/register/registerBpaExtn!getActionsByLoginUserId.action"  escapeAmp="false">	 
					<s:param name="registrationId" value="%{id}"></s:param>	
  ...
  // Used onload to default department(primary dept) and designation
  function loadDesignationFromMatrixOnLoad() 
  {
        var currentState=document.getElementById('currentState').value;
       if(document.getElementById('additionalState').value!=""){
       currentState=document.getElementById('additionalState').value;
        ...
        var pendingAction=document.getElementById('pendingActions').value;
        var dept="";
        makeJSONCall(['Text','Value'],'/egi/workflow/ajaxWorkFlow!getDesignationsByObjectType.action',{departmentRule:dept,type:'RegistrationExtn',amountRule:amountRule,additionalRule:additionalRule,
				currentState:currentState,pendingAction:pendingAction},designationLoadSuccessHandler,designationLoadFailureHandler) ;
  }

  designationLoadSuccessHandler=function(req,res){
	   
	  approverDesignationDropdown=dom.get("approverDesignation");
	  var resLength =res.results.length+1;
	  var dropDownLength = approverDesignationDropdown.length;

		for(i=0;i<res.results.length;i++){
			approverDesignationDropdown.options[i+1]=new Option(res.results[i].Text,res.results[i].Value);
		if(res.results[i].Value=='null') approverDesignation.Dropdown.selectedIndex = i;
		}
		while(dropDownLength>resLength)
		{
			approverDesignationDropdown.options[res.results.length+1] = null;
			dropDownLength=dropDownLength-1;
		}
		document.getElementById("approverDesignation").selectedIndex = "1";
		populateApprover(); 
		}
  designationLoadFailureHandler=function(){
	  alert('Unable to load Approver Designation.');
	}
  ...
  function populateApprover()
	{
	   ...
	   // Commented out as EE is removed from Workflow. Hence No Autoselection of approvers
		/*if(document.getElementById('currentState').value=='Registered'){
			var roleslist=document.getElementById('bparoles').value; 
			makeJSONCall(['Text','Value'],'${pageContext.request.contextPath}/extd/register/ajaxRegisterBpaExtn!getUsersByDesgAndDept.action',{approverDepartmentId:document.getElementById('approverDepartment').value,designationId:document.getElementById('approverDesignation').value,roleList: roleslist},userLoadSuccessHandler,userLoadFailureHandler) ;
		}
		else{ 
			getUsersByDesignationAndDept(); 
		}*/
  ...
  userLoadSuccessHandler=function(req,res){
	  approverPositionIdDropdown=dom.get("approverPositionId");
		  var resLength =res.results.length+1;
		  var dropDownLength =approverPositionIdDropdown.length; 
		  for(i=0;i<res.results.length;i++){
			  approverPositionIdDropdown.options[i+1]=new Option(res.results[i].Text,res.results[i].Value);
		    if(res.results[i].Value=='null')   approverPositionId.Dropdown.selectedIndex = i;
		  }
		 while(dropDownLength>resLength)
		 {
			 approverPositionIdDropdown.options[res.results.length+1] = null;
		     dropDownLength=dropDownLength-1;
		 }
		 if(res.results.length!=0){
			 approverPositionId.selectedIndex=1; 
		 }
	  document.getElementById("approverPositionId").disabled=true;
	}
  userLoadFailureHandler=function(){
	  alert('Unable to load Approver'); 
	}	

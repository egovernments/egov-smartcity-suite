...
function onSearchSubmit()
{
	...
	if((psnnum==""  ) && (servicetype==-1) && (owner=="" ) && ( phonenum=="") )
	{
...
function resetValues(){
	...
	function setAllValues(obj){
		 ...
		 ownerName= getControlInBranch(currRow,'ownerFirstName').value; 
		bpaData_DW=bpano + '`~`' + ownerName;
	//	alert(bpaData_DW);
		
	}

	 function viewRegisterBpa(registrationId){
			window.open("${pageContext.request.contextPath}/register/registerBpa!viewForm.action?registrationId="+registrationId,"",'height=650,width=980,scrollbars=yes,left=0,top=0,status=yes');		
	}
	...
	<s:form action="search" theme="simple" name="searchForm">
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="2">
			...
			<tr>
				...
				<td class="bluebox" ><s:select id="statusSearch" name="egwStatus.id"  headerKey="-1" headerValue="---choose----"
		 ...
		 <s:if test="%{searchMode=='result'}">
          		 ...
          		 <span class="pagebanner"></span>
							...
							<display:column class="hidden" headerClass="hidden"  media="html">
 						 		...
 						 		<s:hidden id="ownerName" name="ownerName" value="%{getUsertName(#attr.currentRowObject.state.owner.id)}" />
 						 		<s:hidden id="ownerFirstName" name="ownerFirstName" value="%{#attr.currentRowObject.owner.firstName}" />
							</display:column>
							...
							<display:column title="Next Action " style="text-align:center;" property="state.nextAction" />	 
								
						</div>
						</display:table>
						<div class="buttonbottom" align="center">
						<s:if test="%{mode=='AEJEDiary' && searchResult.list.size>0}">			
						<table>
							<tr>

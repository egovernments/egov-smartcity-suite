...
function getUsersByDesignationAndDept()
{
	...
	//var tempvalues=roleslist.split(",");
	//populateapproverPositionId({approverDepartmentId:document.getElementById('approverDepartment').value,designationId:document.getElementById('approverDesignation').value,roleList: tempvalues[0],roleList:tempvalues[3],roleList:tempvalues[2],roleList:tempvalues[1]});
	populateapproverPositionId({approverDepartmentId:document.getElementById('approverDepartment').value,designationId:document.getElementById('approverDesignation').value,roleList: roleslist});
...
<s:hidden id="bparoles" name="bparoles" value="%{bparoles}"/>
<s:if test="%{#request.approverOddTextCss==null}">
     <c:set var="approverOddTextCss" value="${approverOddCSS}" scope="request"/>
</s:if>

<s:if test="%{#request.approverEvenTextCSS==null}">
   <c:set var="approverEvenTextCSS" value="${approverEvenCSS}" scope="request"/>
</s:if>

 <table width="100%" border="0" cellspacing="0" cellpadding="0">
 
        <tr>
            <td colspan="9" class="${approverHeadTDCSS}">
               	<s:if test="%{#request.headerImgCss!=null}"><div class="${headerImgCss}"></s:if>
               	<s:if test="%{#request.headerImgUrl!=null}"><img src="${headerImgUrl}"/></div></s:if>
                <div class="${approverHeaderCss}"><s:text name="approval.authority.lbl"/></div>
            </td>
        </tr>
        
	  	<tr>   
	  	 	 <td class="${approverOddCSS}" width="16%">&nbsp;</td>
			 <td class="${approverOddCSS}" id="deptLabel"><s:text name="workflow.approver.department"/>:</td>
			...
			<td class="${approverOddCSS}" width="14%"><s:text name="approval.designation.lbl"/>:</td>
			<td class="${approverOddTextCss}" width="33%">
				...
				<egov:ajaxdropdown id="approverPositionId" fields="['Text','Value']" dropdownId="approverPositionId" 
					url="workflow/ajaxWorkFlow!getPositionByPassingDesigAndRoles.action" contextToBeUsed="/egi" />
					
			</td>
			<td class="${approverOddCSS}" width="10%"><s:text name="approval.lbl"/>:</td>
 ...
 <div id="workflowCommentsDiv" align="center">
         ...
         <tr>
           ...
           <td class="${approverEvenCSS}" width="13%"><s:text name="approver.comments.lbl"/>: </td>
           <td class="${approverEvenTextCSS}"> 
           	<s:textarea id="approverComments" name="approverComments" value="%{approverComments}" rows="2" cols="35" />  
           </td>
           <td class="${approverEvenCSS}">&nbsp;</td>
            <td class="${approverEvenCSS}" width="13%"><s:text name="Previous Approver Comments"/>: </td>
           <td class="${approverEvenTextCSS}"> 
           	<s:textarea id="previousApproverComments" name="previousApproverComments" value="%{previousApproverComments}"  disabled="true"   rows="4" cols="50" />  
           </td>
           <td width="10%" class="${approverEvenCSS}">&nbsp;</td>
           <td  class="${approverEvenCSS}">&nbsp;</td>
           </tr>

...
public class ReceiptHeaderService extends PersistenceService<ReceiptHeader, Long> {
	...
	public void startWorkflow(ReceiptHeader receiptHeader, Boolean receiptBulkUpload) throws EGOVRuntimeException {
	    ...
	    if (receiptHeader.getState() == null) {
	        ...
	        receiptHeader.transition().start().withSenderName(receiptHeader.getCreatedBy().getName()).withComments(CollectionConstants.WF_STATE_RECEIPT_CREATED)
	        .withStateValue(CollectionConstants.WF_STATE_RECEIPT_CREATED).withOwner(position).withDateInfo(new Date()).withNextAction(CollectionConstants.WF_ACTION_SUBMIT);
	    }
	    ...
	    if (receiptBulkUpload) {
	        ...
	        receiptHeader.transition().withSenderName(receiptHeader.getCreatedBy().getName()).withComments("Approval of Data Migration Receipt Complete")
		...
		for (int i = 0; i < serviceNameArr.length; i++) {
			...
			if (serviceName != null && serviceName.length() > 0) {
				...
				if (totalCashAmount[i].trim() != null && totalCashAmount[i].trim().length() > 0 && cashInHandGLCode != null) {
					...
					List<InstrumentHeader> instrumentHeaderListCash = persistenceService.findAllBy(cashQueryBuilder.toString(), arguments);
				...
				if (totalChequeAmount[i].trim() != null && totalChequeAmount[i].trim().length() > 0 && chequeInHandGlcode != null) {
					...
					List<InstrumentHeader> instrumentHeaderListCheque = persistenceService.findAllBy(chequeQueryBuilder.toString(), arguments);
				...
				if (totalCardAmount[i].trim() != null && totalCardAmount[i].trim().length() > 0 && cardPaymentGlCode != null) {
					...
					List<InstrumentHeader> instrumentHeaderListOnline = persistenceService.findAllBy(onlineQueryBuilder.toString(), arguments);
	...
	public void performWorkflow(String actionName, ReceiptHeader receiptHeader, String remarks) {
	    try {
	        ...
	        Boolean isEmployee = collectionsUtil.isEmployee(receiptHeader.getCreatedBy());
	        if(!isEmployee) {
	            ...
	            operatorPosition = collectionsUtil.getPositionByDeptDesgAndBoundary(department,desgn, receiptHeader.getReceiptMisc().getBoundary());
	        }
	        else {
	        ...
	        Designation designation=designationService.getDesignationByName("Section manager");
	        ...
	        if (actionName.equals(CollectionConstants.WF_ACTION_SUBMIT)) 
	            perform(receiptHeader, CollectionConstants.WF_ACTION_APPROVE, CollectionConstants.RECEIPT_STATUS_CODE_SUBMITTED, CollectionConstants.WF_ACTION_APPROVE, approverPosition,remarks);  
	...
	public void perform(ReceiptHeader receiptHeader, String wfState, String newStatusCode,String  nextAction,Position ownerPosition, String remarks) {
	    ...
	    if (receiptHeader.getStatus().getCode().equals(
	        ...
	        .withStateValue(CollectionConstants.WF_STATE_END).withOwner(ownerPosition).withDateInfo(new Date());
	    }
	    else {

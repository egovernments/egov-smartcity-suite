...
import org.egov.erpcollection.integration.pgi.DefaultPaymentResponse;
...
public class CollectionObjectFactory {
	...
	public ReceiptHeader createUnsavedReceiptHeader(String receiptnumber,
			char receiptType, String refNum, String statusCode, User user,EgwStatus status) {
	...
	public ReceiptHeader createReceiptHeaderWithInstrument(String receiptNum,
			...
			Double instrumentAmount, Date instrumentDate,
			String instrumentStatusCode, String glCode, String functionName,
			String userName, String counterName) {

		UserImpl user = createUser(userName);
	...
	public ReceiptHeader createReceiptHeaderWithInstrument(String receiptNum,
			...
			Double instrumentAmount, Date instrumentDate,
			String instrumentStatusCode, String glCode, String functionName,
			UserImpl user, Location counter) {
	...
	public ReceiptHeader createUnsavedReceiptHeader() {
		ReceiptHeader receiptHeader = new ReceiptHeader();
	...
	public ReceiptHeader createUnsavedPendingReceiptHeader() {
		ReceiptHeader receiptHeader = new ReceiptHeader();
		receiptHeader.setReceipttype('A');
		receiptHeader.setService(createServiceDetails());
		receiptHeader.setCreatedDate(new Date());
	...
	public UserCounterMap createUserCounterMap(UserImpl user, Location counter) {
		Date currentDate = new Date();
		UserCounterMap ucm = new UserCounterMap();
		ucm.setUserId(user);
		ucm.setCounterId(counter);
		ucm.setFromDate(currentDate);
		ucm.setModifiedBy(user.getId());
	...
	public UserImpl createUser(String userName, Department dept) {
		UserImpl user = new UserImpl();
		user.setUserName(userName + getRandomNumber());
		user.setFirstName(userName);
		user.setPwd("testpassword");
	 ...
	 * @return
	 */
	public InstrumentHeader createBankInstrumentHeader() {
		InstrumentHeader instrHdr = new InstrumentHeader();
		instrHdr.setInstrumentAmount(BigDecimal.valueOf(1000));
	...
	public ReceiptVoucher createReceiptVoucher() {
		ReceiptVoucher receiptVoucher = new ReceiptVoucher();
	...
	public ReceiptVoucher createReceiptVoucher(CVoucherHeader voucherHeader) {
		ReceiptVoucher receiptVoucher = new ReceiptVoucher();
		...
		ReceiptHeader savedReceiptHeader1 = payee.getReceiptHeaders()
				.iterator().next();
		receiptVoucher.setReceiptHeader(savedReceiptHeader1);
		receiptVoucher.setVoucherheader(voucherHeader);
		receiptVoucher.setInternalrefno("123456/2009-10");
		session.saveOrUpdate(receiptVoucher);
		return receiptVoucher;
	...
	public ReceiptDetail createReceiptDetail() {
	...
	public ReceiptMisc createReceiptMis() {
		ReceiptMisc receiptMisc = new ReceiptMisc();
	...
	public DepartmentImpl createDept(String deptName, String deptCode) {
		DepartmentImpl empDept = new DepartmentImpl();
		empDept.setDeptName(deptName + getRandomNumber());
		empDept.setUpdateTime(new Date());
		empDept.setBillingLocation("0");
		empDept.setDeptCode(deptCode + getRandomNumber());
		session.saveOrUpdate(empDept);
		return empDept;
	}

	public DepartmentImpl createDeptForCode(String deptCode) {
		DepartmentImpl empDept = new DepartmentImpl();
		empDept.setDeptName(deptCode);
		empDept.setDeptCode(deptCode);
		empDept.setUpdateTime(new Date());
		empDept.setBillingLocation("0");
		session.saveOrUpdate(empDept);
		return empDept;
	...
	public Assignment createAssignment(AssignmentPrd assignmentPrd,
	    ...
	    DepartmentImpl dimpl = (DepartmentImpl) department;
	...
	public Position createPosition(DesignationMaster desig) {
		...
		position.setDesigId(desig);
		session.saveOrUpdate(position);
		return position;
	}

	public DepartmentImpl createDept(String name) {
		DepartmentImpl d = new DepartmentImpl();
		d.setDeptName(name + getRandomNumber());
		d.setDeptCode(name + getRandomNumber());
		session.save(d);
		return d;
	}
	...
	public PersonalInformation createPersonalInformation(UserImpl user,
			Department dept) {
		PersonalInformation personalInformation = new PersonalInformation();
		personalInformation.setEmployeeFirstName(user.getFirstName());
		Random ran = new Random();
	...
	public ReceiptMisc createReceiptMisForMiscReceipt() {
		ReceiptMisc receiptMisc = new ReceiptMisc();
		Fund fund = createFund("001");
		Scheme scheme = createScheme("234", "testscheme1", fund);
		receiptMisc.setBoundary(createBoundary());
	...
	public OnlinePayment createOnlinePayment(ReceiptHeader receiptHeader,
			String transNo, BigDecimal transAmt,EgwStatus status) {
		OnlinePayment onlinePayment = new OnlinePayment();
		User user = createUser("testUser");
	...
	public ReceiptHeader createReceiptHeaderWithChallan() {
		ReceiptHeader receiptHeader = createReceiptHeader("testReceiptNo");
		...
		challan.setModifiedDate(date);
		
		challan.setCreatedBy(user);
		challan.setModifiedBy(user);
	...
	public Challan createUnsavedChallan() {
		Challan challan = new Challan();
		User user = createUser("testUser");
		Date date = new Date();
	...
	public ReceiptMisc createReceiptMisForChallan() {
		ReceiptMisc receiptMisc = new ReceiptMisc();
		Fund fund = createFund("001");
		receiptMisc.setBoundary(createBoundary());
		receiptMisc.setDepartment(createDept("testDeptName", "testDeptCode"));
	...
	public ReceiptHeader createReceiptHeaderForChallan(){
		ReceiptPayeeDetails payee = createReceiptPayeeDetails();
	...
	public ReceiptDetail createReceiptDetailForChallan(){
	...
	public ServiceDetails createChallanServiceDetails() {
		ServiceDetails service = new ServiceDetails();
		String serviceName = "@testChallanSrvc$" + getRandomNumber(9999);
		service.setServiceName(serviceName);
		service.setServiceUrl("testServiceURL");

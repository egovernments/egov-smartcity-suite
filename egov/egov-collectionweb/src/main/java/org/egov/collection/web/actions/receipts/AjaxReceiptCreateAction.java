...
public class AjaxReceiptCreateAction extends BaseFormAction {
    ...
    /*
     * public String getMISdetailsForService() { value = ""; String serviceId = parameters.get("serviceId")[0]; String queryString
     * = "select nvl(fund.id,'-1'),nvl(department.id,'-1') from ServiceDetails where id='" + serviceId + "'"; List<Object[]> list
     * = getPersistenceService().findAllBy(queryString); if (list != null && !list.isEmpty()) { for (int i = 0; i < list.size();
     * i++) { Object[] arrayObjectInitialIndex = list.get(i); value = arrayObjectInitialIndex[0].toString() + "~" +
    ...
    public String getMISdetailsForService() {
        ...
        final ServiceDetails service = (ServiceDetails) getPersistenceService().find(" from ServiceDetails where id=? ",
   ...
   @Action(value = "/receipts/ajaxReceiptCreate-getDetailCode")
        ...
        for (final String element : arr) {
            final CChartOfAccountDetail chartOfAccountDetail = (CChartOfAccountDetail) getPersistenceService().find(
                    " from CChartOfAccountDetail" +
                            " where glCodeId=(select id from CChartOfAccounts where glcode=?)", element);
        ...
        if (entityList == null || entityList.isEmpty())
            ...
            for (final EntityType entity : entityList)
                if (entity == null) {
                    value = index + "~" + ERROR + "#";
                    break;
                } else if (codeorname.equalsIgnoreCase("both")) {// To Check if both name and code has to be compared
                    if (entity.getName().equals(code) || entity.getCode().equals(code)) {
                        value = index + "~" + entity.getEntityId() + "~" + entity.getName() + "~" + entity.getCode();
                        break;
                    } else
                        value = index + "~" + ERROR + "#";
                } else if (entity.getCode().equals(code)) {// In case of view mode, to get the details from the code
                    value = index + "~" + entity.getEntityId() + "~" + entity.getName() + "~" + entity.getCode();
                    break;
                } else
                    value = index + "~" + ERROR + "#";
    ...
    public String ajaxValidateDetailCode()
    {
        ...
        try {
            ...
            final Accountdetailtype adt = (Accountdetailtype) getPersistenceService().find(
            ...
            final List<EntityType> entityList = getPersistenceService().findAllBy(" from " + adt.getFullQualifiedName() + "" +

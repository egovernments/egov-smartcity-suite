...
@Results({
        ...
        @Result(name = CollectionsWorkflowAction.SUBMISSION_REPORT_CASH, location = "collectionsWorkflow-submissionReportCash.jsp"),
        @Result(name = CollectionsWorkflowAction.SUBMISSION_REPORT_CHEQUE, location = "collectionsWorkflow-submissionReportCheque.jsp")
})
public class CollectionsWorkflowAction extends BaseFormAction {
    ...
    private void fetchReceipts(String statusCode, String workflowAction) {
        ...
        //Hack for current AP implementation for the role of CSC Operator
        receiptHeaders = getAllReceiptsCreated(statusCode, workflowAction);
    ...
    //This method returns all the Receipts create by current logged in users as well as all the receipts created by CSC Operators
    private List<ReceiptHeader> getAllReceiptsCreated(String statusCode, String workflowAction) {
        List<ReceiptHeader> receiptHeaders = receiptHeaderService
                .findAllByStatusUserCounterService(statusCode, userName,
                        counterId, serviceCode);
        String roleName = collectionsUtil.getThirdPartyUserRoles().get(0).getValue();
        Set<User> users = userservice.getUsersByRoleName(roleName);
        for(User user:users)
        receiptHeaders.addAll(receiptHeaderService
                .findAllByStatusUserCounterService(statusCode, user.getUsername(),
                        counterId, serviceCode));
        return receiptHeaders;
    }

    /**
     * Retrieves all receipt headers created by currently logged in user from current counter and in status "To be submitted"
     * 
    ...
    /**
     * Retrieves all receipt headers created by set user from set counter and in status "submitted". Also populates the dropdown
     * data for counter list and user list.
     * 
     * @return Next page to be displayed (index)
     */

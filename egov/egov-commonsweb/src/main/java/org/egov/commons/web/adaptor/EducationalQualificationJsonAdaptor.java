...
public class EducationalQualificationJsonAdaptor implements JsonSerializer<EducationalQualification> {
    @Override
    public JsonElement serialize(final EducationalQualification qualification, final Type type,
            final JsonSerializationContext jsc) {
        final JsonObject jsonObject = new JsonObject();
        if (qualification != null) {
            if (qualification.getName() != null)
                jsonObject.addProperty("name", qualification.getName());
            else
                jsonObject.addProperty("name", StringUtils.EMPTY);
            if (qualification.getDescription() != null)
                jsonObject.addProperty("description", qualification.getDescription());
            else
                jsonObject.addProperty("description", StringUtils.EMPTY);
            if (qualification.getIsActive() != null)
                jsonObject.addProperty("isActive", qualification.getIsActive());
            else
                jsonObject.addProperty("isActive", StringUtils.EMPTY);
            jsonObject.addProperty("id", qualification.getId());
        }
        return jsonObject;
    }
}
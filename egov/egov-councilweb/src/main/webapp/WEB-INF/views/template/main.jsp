...
<head>
	<c:if test="${empty maxFileSize}">
        <spring:eval expression="@environment.getProperty('council.max.file.size')" scope="application" var="maxFileSize"/>
        <spring:eval expression="@environment.getProperty('council.allowed.file.ext')" scope="application" var="allowedFileExt"/>
    </c:if>

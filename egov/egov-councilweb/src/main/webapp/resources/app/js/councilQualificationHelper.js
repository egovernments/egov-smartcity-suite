...
function callAjaxSearch() {
	...
	jQuery('.report-section').removeClass('display-hide');
		reportdatatable = drillDowntableContainer
			.dataTable({
				ajax : {
					url : "/council/councilqualification/ajaxsearch/"+$('#mode').val(),      
					type: "POST",
				...
				console.log(data.id);
				window.open('/council/councilqualification/'+ $('#mode').val() +'/'+data.id,'','width=800, height=600');
			});
				 },*/
				"bDestroy" : true,
...
$("#resultTable").on('click','tbody tr td  .view',function(event) {
	...
	window.open('/council/councilqualification/'+ $('#mode').val() +'/'+ id,'','width=800, height=600,scrollbars=yes');

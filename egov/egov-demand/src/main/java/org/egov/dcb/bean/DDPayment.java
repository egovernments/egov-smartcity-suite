...
public class DDPayment extends Payment {
	...
	private static final Logger LOGGER = Logger.getLogger(DDPayment.class);
	private final SimpleDateFormat DD_DATE_FORMAT = new SimpleDateFormat("dd-MM-yyyy");

	public static DDPayment create(Map<String, String> paymentInfo) {
		return new DDPayment(paymentInfo);
	}

	private DDPayment(Map<String, String> paymentInfo) {
		validate(paymentInfo);
		LOGGER.debug("-DD Payment -paymentInfo " + paymentInfo);
		this.setPaymentMode(PAYMENTMODE.dd);
		...
		LOGGER.debug("-DD Payment is created " + this);
	}

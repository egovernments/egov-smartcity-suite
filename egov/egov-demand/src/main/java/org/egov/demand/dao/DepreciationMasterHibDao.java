		...
		DepreciationMasterDao
{
	...
	{
		Query qry = getCurrentSession().createQuery("from DepreciationMaster DM where DM.module=:mod and DM.year=:year ");//and DM.IsHistory='N' ");
	...
	{
		Query qry = getCurrentSession().createQuery("from DepreciationMaster DM where DM.module=:mod and DM.year=:year and DM.IsHistory='N' and DM.startInstallment=:insYear ");
		qry.setEntity("mod", mod);
		qry.setInteger("year",year);
	...
	public List getDepreciationsForModulebyHistory(Module mod) {
		...
		Query qry = getCurrentSession().createQuery("from DepreciationMaster DM where DM.module=:module and DM.IsHistory='N'");
		qry.setEntity("module", mod);
		return qry.list();
	}
	...
	{
		Query qry = getCurrentSession().createQuery("select DM.depreciationPct from DepreciationMaster DM where DM.year=:year");
		qry.setInteger("year",year);
    ...
    {
        Query qry = getCurrentSession().createQuery("from DepreciationMaster DM where  and DM.IsHistory='N'");
        return qry.list();
    }
    ...
    public Float getLeastDepreciationPercent(Integer year)
	{
		...
		{
			Query qry = getCurrentSession().createQuery("select min(DM.year) from DepreciationMaster DM ");

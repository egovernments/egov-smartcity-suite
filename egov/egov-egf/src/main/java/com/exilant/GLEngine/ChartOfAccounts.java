...
public class ChartOfAccounts {
    ...
    private void checkfuctreqd(final String glcode, final String fuctid,
        ...
        final String sql = "select FUNCTIONREQD from chartofaccounts where glcode = :glcode";
        final List<Object[]> rs = persistenceService.getSession().createNativeQuery(sql)
    ...
    private boolean updateInGL(final Transaxtion txnList[],
        ...
        final String query = "select id from generalledger where voucherheaderid = :voucherHeaderId order by id";
            ...
            try {
                ...
                if (LOGGER.isInfoEnabled())
                    LOGGER.info("delete from generalledgerdetail where generalledgerid='"
                            + glHeaderId.get(k).toString() + "'");
                final String delGenLedDet = "delete from generalledgerdetail where generalledgerid = :generalledgerid";
                pst = persistenceService.getSession().createNativeQuery(delGenLedDet)
                        .setParameter("generalledgerid", glHeaderId.get(k).toString(), StringType.INSTANCE);
                ...
                LOGGER.error("Exp in reading from generalledgerdetail: ", e);
                throw new TaskFailedException(e.getMessage());
            }
        ...
        if (count > 0)
            try {
                ...
                final String genLed = "DELETE FROM generalledger WHERE voucherheaderid = :voucherheaderid";
        ...
        for (final Transaxtion txn : txnList) {
            ...
            CVoucherHeader cVoucherHeader = (CVoucherHeader) persistenceService
                    .find("from CVoucherHeader where id = ?1",
                    ...
                    { // Post the details sent apart from defaults
                        for (int z = 0; z < txnPrm.size(); z++) {
                            ...
                            if (tParam.getDetailName().equalsIgnoreCase(
                                ...
                                gLedgerDet.setGeneralLedgerId(gLedger);
                                Accountdetailtype acctype = (Accountdetailtype) persistenceService
                                        .find("from Accountdetailtype where id = ?1",
                                                glPrm.getDetailId());
                                gLedgerDet.setDetailTypeId(acctype);
                                gLedgerDet.setDetailKeyId(Integer
                                        .parseInt(detKeyId));
    ...
    public String getGLCode(final String detailName, final String detailKey,
        ...
        String code = "";
        try {
            final String str = "select glcode as \"code\" from chartofaccounts, bankaccount where bankaccount.glcodeid = chartofaccounts.id and bankaccount.id = :bankAccId";
            final Query pst = persistenceService.getSession().createNativeQuery(str)
                    .setParameter("bankAccId", detailKey, StringType.INSTANCE);
            final List<String> list = pst.list();
    ...
    public String getFiscalYearID(final String voucherDate,
        ...
        String fiscalyearid = "";
        final String sql = "select ID as \"fiscalperiodID\" from fiscalperiod where to_date(:vcDate,'dd-mon-yyyy') between startingdate and endingdate";
        try {
    ...
    private boolean validPeriod(final String vDate) throws TaskFailedException {
        try {
            ...
            LOGGER.error("Inside validPeriod ", e);
            throw new TaskFailedException();
        }
        return true;
    }

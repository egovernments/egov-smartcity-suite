...
public class ChartOfAccDetail extends AbstractTask {
    ...
    public String setQuotes(final String str) {
    ...
    public void execute(final String taskName, final String gridName, final DataCollection dc,
        ...
        final ChartOfAccts chart = new ChartOfAccts();
        connection = conn;
        final String glcode = dc.getValue(CHARTOFACCGLCODE);
        hs.add(glcode);
        try {
        ...
        final String budgetCheck = dc.getValue("budgetCheckReqd");
        // Check glcode is of detail code then check for budget check required
        // or not
        final boolean detailAccountCode = isDetailAccountCode(glcode, dc); // if(LOGGER.isDebugEnabled())
        // LOGGER.debug("detailAccountCode:"+detailAccountCode);
        ...
        if (dc.getValue(CHARTOFACCID).equalsIgnoreCase("")) {
            ...
            try {
                ...
                if (!str1.equals("0") && str1.length() != 0 && str1 != null) {
                    final ChartOfAccountDetail chartDetail = new ChartOfAccountDetail();
    ...
    public void setIeList(final Set ieList) {

    }

    public boolean isUniqueGL(final String glcode, final DataCollection dc, final Connection conn) {
        boolean isUnique = false;
        try {
            final String sql = "SELECT id FROM CHARTOFACCOUNTS WHERE glcode =:glcode";
            if (LOGGER.isDebugEnabled())
                LOGGER.debug("SELECT id FROM CHARTOFACCOUNTS WHERE glcode = '"
                        + glcode + "'");
            final List<Object[]> rs = HibernateUtil.getCurrentSession().createSQLQuery(sql).setString("glcode", glcode).list();
    ...
    public boolean isMaxLength(final String glcode, final DataCollection dc) {
        boolean isMax = false;
        try {
            final String glcodelength = EGovConfig.getProperty("egf_config.xml",
                    "glcodeMaxLength", "", "AccountCode");
            final int gl = glcode.length();
            Integer.parseInt(glcodelength);
            if (LOGGER.isDebugEnabled())
                LOGGER.debug("glcodelength ===" + gl + "glcodelength from xml=="
                        + Integer.parseInt(glcodelength));
    ...
    public boolean isDetailAccountCode(final String glcode, final DataCollection dc) {
        boolean isDetailCode = false;
        try {
            if (LOGGER.isDebugEnabled())
                LOGGER.debug("glcodelength === " + glcode);
            final String glcodelength = EGovConfig.getProperty("egf_config.xml",
                    "glcodeMaxLength", "", "AccountCode");
            final int gl = glcode.length();
            if (LOGGER.isDebugEnabled())

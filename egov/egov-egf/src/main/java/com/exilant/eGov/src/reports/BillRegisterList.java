...
public class BillRegisterList
{
    ...
    public String getQueryString(final BillRegisterReportBean reportBean) {
        String dateStart = "";
        String dateEnd = "";
        ...
        final String lastAppend = "  ORDER BY PO";
        final String unionAppend = " UNION ";
        final String intersectAppend = " INTERSECT ";
        String creditorAppend = "";

        String returnQueryStringOne = "SELECT DISTINCT egb.PO, TO_CHAR(egb.BILLDATE, 'dd-Mon-yyyy')  AS " + " BILLDATE" +
                ", egb.BILLNUMBER, egb.BILLAMOUNT, egb.BILLAPPROVALSTATUS, TO_CHAR(egb.PAYREQDATE, 'dd-Mon-yyyy')  AS "
                + " PAYREQDATE" +
                ",cbd.PASSEDAMOUNT, cbd.PAIDAMOUNT, vh.VOUCHERNUMBER, TO_CHAR(vh.VOUCHERDATE, 'dd-Mon-yyyy') AS "
                + " VOUCHERDATE" + " FROM EG_BILLREGISTER egb , VOUCHERHEADER vh," +
        ...
        String returnQueryStringTwo = " SELECT DISTINCT egb.PO, TO_CHAR(egb.BILLDATE, 'dd-Mon-yyyy')  AS " + " BILLDATE" +
                ", egb.BILLNUMBER,egb.BILLAMOUNT, egb.BILLAPPROVALSTATUS, TO_CHAR(egb.PAYREQDATE, 'dd-Mon-yyyy')  AS "
                + " PAYREQDATE" +
                ",cbd.PASSEDAMOUNT, cbd.PAIDAMOUNT, vh.VOUCHERNUMBER,TO_CHAR(vh.VOUCHERDATE, 'dd-Mon-yyyy') AS " + " VOUCHERDATE"
                + "  FROM EG_BILLREGISTER egb , VOUCHERHEADER vh," +
        ...
        // "SELECT PO, TO_CHAR(BILLDATE, 'dd-Mon-yyyy')  AS "+" BILLDATE"+", BILLNUMBER, BILLAMOUNT, BILLAPPROVALSTATUS, TO_CHAR(PAYREQDATE, 'dd-Mon-yyyy')  AS "+" PAYREQDATE"+" FROM EG_BILLREGISTER  WHERE BILLDATE between '"+dateStart+"' AND '"+dateEnd+"'"
        // +
        // "ORDER BY PO";
        /*
         ...
         * " UNION "+ "SELECT DISTINCT egb.PO, TO_CHAR(egb.BILLDATE, 'dd-Mon-yyyy')  AS" + "BILLDATE"+
         * ", egb.BILLNUMBER,egb.BILLAMOUNT, egb.BILLAPPROVALSTATUS, TO_CHAR(egb.PAYREQDATE, 'dd-Mon-yyyy')  AS "+"PAYREQDATE"+
        ...
        return returnQueryStringOne + intersectAppend + returnQueryStringTwo + unionAppend + returnQueryStringOneU + unionAppend
                + returnQueryStringTwoU + lastAppend;

    }

    public LinkedList getBillRegisterList(final BillRegisterReportBean reportBean)
    {
        List<Object[]> rs = null;
        ...
        // //if(LOGGER.isDebugEnabled()) LOGGER.debug("2***");
        /*
         * try { dateStart=covertDate(reportBean.getStartDate()); // //if(LOGGER.isDebugEnabled()) LOGGER.debug("3***");
         * dateEnd=covertDate(reportBean.getEndDate()); //if(LOGGER.isDebugEnabled()) LOGGER.debug("startDate :"+dateStart);
         * //if(LOGGER.isDebugEnabled()) LOGGER.debug("endDate :"+dateEnd); //if(LOGGER.isDebugEnabled()) LOGGER.debug("4***"); }
         * catch(Exception error){}
         */
        /*
         * String queryString = "SELECT TO_CHAR(voucherdate, 'dd-Mon-yyyy')  AS "+" voucherdate"+", vouchernumber , gd.glcode AS "
         * +" glcode"+ ",  ca.name AS "+ "particulars" + ",vh.name ||' - '|| vh.TYPE AS " + "type" +", vh.description AS "+
         * "narration" +", status , debitamount  ," +
         * "creditamount,vh.CGN  FROM voucherheader vh, generalledger gd, chartofaccounts ca WHERE vh.ID=gd.VOUCHERHEADERID" +
         * " AND ca.GLCODE=gd.GLCODE AND voucherdate between '"+dateStart+"' AND '"+dateEnd+"'" +
         * "ORDER BY voucherdate, vh.TYPE,vouchernumber,status";
         */
        // if(LOGGER.isDebugEnabled()) LOGGER.debug("5***");

        final String queryString = getQueryString(reportBean);
        ...
        try
        {
            for (final Object[] element : rs) {
                 ...
                 * totalBillAmount=totalBillAmount+Double.parseDouble(rs.getString("BILLAMOUNT")); totalBill=totalBill+1;
                 * if(rs.getString("BILLAPPROVALSTATUS").equals("PASSED")) { totalPassed=totalPassed+1; } }
                 * BillRegister.setTotalBillAmount(""+totalBillAmount); BillRegister.setTotalBill(""+totalBill);
            ...
            /*
             * BillRegisterList.setTotalBillAmount(""+totalBillAmount); BillRegisterList.setTotalBill(""+totalBill);
             * BillRegisterList.setTotalPassed(""+totalPassed);
             */
            /*
             * //if(LOGGER.isDebugEnabled()) LOGGER.debug("dbTotal:"+dbTotal); //if(LOGGER.isDebugEnabled())
             * LOGGER.debug("crTotal:"+crTotal); this.setCAmount(crTotal); this.setDAmount(dbTotal);
             */
            ...
            if (LOGGER.isDebugEnabled())
                LOGGER.debug("Exception in while loop:" + e);
        }
     ...
     */
    public void setTotalBill(final String totalBill) {
     ...
     */
    public void setTotalPassed(final String totalPassed) {
     ...
     */
    public void setTotalPending(final String totalPending) {
     ...
     */
    public void setTotalRejected(final String totalRejected) {

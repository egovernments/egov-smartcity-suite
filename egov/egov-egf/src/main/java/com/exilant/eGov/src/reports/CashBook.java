...
public class CashBook {
	...
	public LinkedList getGeneralLedgerList(GeneralLedgerReportBean reportBean)
		...
		try {
			...
			String cashPId = EGovConfig.getProperty("egf_config.xml",
			...
			String maxlength = EGovConfig.getProperty("egf_config.xml",
			...
			/*
			 * left side of the report is receipts so we are assigning (opbal.dr
			 * - opbal.cr) to creditTotal of cash and cheque respectively and
				...
				try {
					...
					if (vhId != VhidPrevious) {
						...
						if (!vcDate.equalsIgnoreCase(currVhDate)
							...
							GeneralLedgerReportBean glbean1 = new GeneralLedgerReportBean(
									"<hr>&nbsp;</hr>");
							...
							if (LOGGER.isInfoEnabled())
								LOGGER.info(cashcreditTotal + ":crDr: "
										+ cashdebitTotal);
							GeneralLedgerReportBean glbeanOb = new GeneralLedgerReportBean(
									"<hr>&nbsp;</hr>");
					...
					if (vhId == VhidPrevious && !code.equalsIgnoreCase(accCode)) {
						...
						for (int i = 0; i < name.length; i++) {
							...
							if ((formatedName.length()
								...
								formatedName = formatedName + " " + name[i];
							else {
	...
	private OpBalance getOpeningBalance(String glCode, String fundId,
			String fundSourceId, String fyId, String tillDate)
	...
	private void setDates(String startDate, String endDate)
		...
		if ((startDate == null || startDate.equalsIgnoreCase(""))
			...
			} catch (Exception ex) {// dc.addMessage("eGovFailure","setDates");
	...
	public void isCurDate(String VDate) throws TaskFailedException {
		...
		try {
			...
			int ret = (Integer.parseInt(dt2[2]) > Integer.parseInt(dt1[2])) ? 1

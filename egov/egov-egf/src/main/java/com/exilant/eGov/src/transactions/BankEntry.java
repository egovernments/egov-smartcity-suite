...
public class BankEntry extends AbstractTask {
    ...
    public void execute(final String taskName,
        ...
        // if(LOGGER.isDebugEnabled()) LOGGER.debug("**************************************************************");
        final String gridBankEntry[][] = dc.getGrid("gridBankEntry");
        for (final String[] element : gridBankEntry) {
            // //if(LOGGER.isDebugEnabled()) LOGGER.debug(gridBankEntry[i][0] + "  " + gridBankEntry[i][1] + "  " +
            // gridBankEntry[i][2] + "  " + gridBankEntry[i][3] + "  " + gridBankEntry[i][4]);
        }
        // if(LOGGER.isDebugEnabled()) LOGGER.debug("**************************************************************");
        ...
        /*
         * if(!validate(dc)) { dc.addMessage("exilRPError","Server Validation Failed..Check The Data Sent"); throw taskExc; }
         */
        ...
        try {
            ...
            final String fId = dc.getValue("fund_id").toString();
            ...
            /*
             * if( validation by chartofaccountengine returns true) post the data in voucher header
             */
    ...
    private void postInVoucherHeader(final DataCollection dc, final Connection conn) throws SQLException, TaskFailedException {
    ...
    /*
     * private void postInBankAccount (DataCollection dc) throws TaskFailedException { Bankaccount ba = new Bankaccount(); try{
     * String sql = "select currentBalance from bankAccount where id = ?  for update"; pstmt = connection.prepareStatement(sql);
     * pstmt.setString(1, dc.getValue("bankAccount_id")); resultset = pstmt.executeQuery(); resultset.next(); double balAvailable
     * = resultset.getDouble("currentBalance"); resultset = null; String gridBankEntry[][] = dc.getGrid("gridBankEntry"); //String
     * bankGLCode = dc.getValue("bankGLCode"); accountBalance = new double[gridBankEntry.length]; //if(LOGGER.isDebugEnabled())
     * LOGGER.debug("********************** BANK ACCOUNT ****************************"); //if(LOGGER.isDebugEnabled())
     * LOGGER.debug("balAvailable: "+balAvailable); for(int i=0; i<gridBankEntry.length; i++){
     * if(gridBankEntry[i][0].equalsIgnoreCase("")) continue; gridBankEntry[i][2] =
     * gridBankEntry[i][2].equalsIgnoreCase("")?"0":gridBankEntry[i][2]; gridBankEntry[i][3] =
     * gridBankEntry[i][3].equalsIgnoreCase("")?"0":gridBankEntry[i][3]; if(!gridBankEntry[i][2].equalsIgnoreCase("0"))
     * balAvailable = ExilPrecision.convertToDouble(balAvailable - Double.parseDouble(gridBankEntry[i][2]),2);
     * if(!gridBankEntry[i][3].equalsIgnoreCase("0")) balAvailable = ExilPrecision.convertToDouble(balAvailable +
     * Double.parseDouble(gridBankEntry[i][3]),2); if(LOGGER.isDebugEnabled()) LOGGER.debug("bal at " + i + ": " + balAvailable +
     * "    " + "dr: " + gridBankEntry[i][2]+"   cr: " + gridBankEntry[i][3] ); accountBalance[i] = balAvailable; }
     * //if(LOGGER.isDebugEnabled()) LOGGER.debug("********************** BANK ACCOUNT ****************************"); //if
     * insufficient balance, throw the TaskFailedException if(balAvailable < 0){ dc.addMessage("exilError","Insufficient Balace: "
     * + "balance available is " + balAvailable); throw taskExc; } ba.setId((Long)(dc.getValue("bankAccount_id").toString()));
     * //ba.setCurrentBalance(balAvailable + ""); ba.update(); }catch(SQLException sqlEx){ LOGGER.error(sqlEx.getMessage(),
     * sqlEx); dc.addMessage("exilError","no record for account id " + dc.getValue("bankAccount_id") + " in Bankaccount"); throw
     * taskExc; } }
     */

    private void postInBankReconciliation(final DataCollection dc) throws TaskFailedException
    {
        final BankReconciliation rec = new BankReconciliation();
        ...
        for (int i = 0; i < gridBankEntry.length; i++) {
            if (gridBankEntry[i][0].equalsIgnoreCase(""))
                continue;
    ...
    private void postInVoucherDetail(final DataCollection dc,
            final ArrayList transactions) throws TaskFailedException, SQLException {
        ...
        for (int i = 0; i < gridBankEntry.length; i++) {
            ...
            /******************* account code entry ********************/
            /*
             * vd.setGLCode(gridBankEntry[i][0]); vd.setAccountName(gridBankEntry[i][1]); vd.setDebitAmount(gridBankEntry[i][2]);
             * vd.setCreditAmount(gridBankEntry[i][3]); vd.setNarration(gridBankEntry[i][4]);
             * pstmt=connection.prepareStatement("select max(LineId)+1 as LineId from VoucherDetail"); resultset =
             * pstmt.executeQuery(); resultset.next(); vd.setLineID(resultset.getInt("LineId") + ""); resultset = null;
             * vd.insert();
             */
            ...
            /******************* bank account entry ********************/
            /*
             * vd.setGLCode(dc.getValue("bankGLCode")); vd.setAccountName(cm.getCodeName(dc.getValue("bankGLCode")));
             * vd.setDebitAmount(gridBankEntry[i][3]); //debit to bank account vd.setCreditAmount(gridBankEntry[i][2]); //credit
             * to bank account vd.setNarration(gridBankEntry[i][4]);
             * pstmt=connection.prepareStatement("select max(LineId)+1 as LineId from VoucherDetail"); resultset =
             * pstmt.executeQuery(); resultset.next(); vd.setLineID(resultset.getInt("LineId") + ""); resultset = null;
             * vd.insert();
             */

            final Transaxtion transaction1 = new Transaxtion();
    ...
    public boolean validate(final DataCollection dc) {
        final DataValidator valid = new DataValidator();
    ...
    public ArrayList addRequiredDataToList(final ArrayList transactions, final DataCollection dc) throws TaskFailedException {
        for (int i = 0; i < transactions.size(); i++) {
            final ArrayList reqData = getRequiredData(dc, i + 1);
            final Transaxtion trnDetail = (Transaxtion) transactions.get(i);
            trnDetail.setTransaxtionParam(reqData);
        }
        return transactions;
    }

    public ArrayList getRequiredData(final DataCollection dc, final int rowIdx) throws TaskFailedException {
        final DataExtractor de = DataExtractor.getExtractor();
        final String sql = "select ID as \"ID\",name as  \"name\",tableName as \"tableName\"," +
        ...
        final String[][] entityGrid = dc.getGrid("entities_grid");
        for (int i = 1; i < entityGrid.length; i++)
            if (entityGrid[i][3].equals(String.valueOf(rowIdx))) {
                final AccountDetailType accType = (AccountDetailType) detailTypes.get(entityGrid[i][0]);
                if (accType != null) {

...
public class JbReport
{
            ...
            throws TaskFailedException
    {
        ...
        final String fyId = getFYID(startDate);
        ...
        if (fundId == null || fundId.equalsIgnoreCase(""))
            try
        {
                final String query = "SELECT max(id) as \"max\"  FROM fund";
        ...
        if (fundSourceId == null || fundSourceId.trim().equalsIgnoreCase(""))
            try
        {
                final String query1 = "SELECT max(id) as \"max\"  FROM fundsource";
        ...
        try
        {
            ...
            /**
             * When using List<Object[]>.TYPE_SCROLL_INSENSITIVE in createStatement if no records are there, rs.next() will return
             * true but when trying to access (rs.getXXX()), it will throw an error
             **/
            // if(LOGGER.isDebugEnabled()) LOGGER.debug("RS: " + resultset.isBeforeFirst() + " " + resultset.isAfterLast());

            final ArrayList data = new ArrayList();
            String voucherNo = "", vcDate = "";
            String cgn = "", vhId = "", glcode = "", accName = "", desc = "", curVou = "", isconfirmed = "";
            String vname = "";
            Double dr, cr;
            ...
            final String gridData[][] = new String[data.size() + 1][10];
            ...
            for (int i = 1; i <= data.size(); i++)
            {
                final GeneralLedgerBean jbReportBean = new GeneralLedgerBean();
    ...
    private String getQuery() {
        ...
        "  where a.id=b.voucherheaderid "
        + fundSourceCond
        + voucherNameCond
        + deptNameCondition
        +
        " and a.fundid=c.id and c.id >= "
        + fromFund
        + " and c.id <= "
        + toFund
        + " "
        ...
        + " "
        +
        " and a.fundid=c.id  and voucherdate >='"
        + startDate
        + "'  "
        +
        " and voucherdate<='"
        ...
        /*
         * }else{ return
         * "SELECT voucherdate as \"vDate1\", to_char(voucherdate,'dd-Mon-yyyy') as \"vDate\",vouchernumber as \"vNumber\",a.cgn as \"cgn\",c.name as \"fund\",glcode as \"glcode\", "
         * +
         ...
         * " and  d.id >= "+fromFundSource+" and d.id <= "+toFundSource+" "+ " and voucherdate>='"+startDate+"' "+
         * " and voucherdate<='"+endDate+"' and debitamount>0  "+ " AND a.status<>4 "+
         * " and( cgn like 'JVG%' or  cgn like 'CJV%' or  cgn like 'SJV%' or cgn like 'SAL%' or cgn like 'OJV%') "+ " union "+
         * " SELECT voucherdate as \"vDate1\", to_char(voucherdate,'dd-Mon-yyyy') as \"vDate\",vouchernumber as \"vNumber\",a.cgn as \"cgn\",c.name as \"fund\", "
         * +
         ...
         * + " , 2 as \"dbOrCr\" "+ " from voucherheader a,voucherdetail b,fund c,fundsource d" + addTableToQuery +
         * " where b.voucherheaderid=a.id and a.status = 0 "+ fundSourceCond + " and c.id >="+fromFund+" and c.id <= "+toFund+" "+
         * " and d.id >="+fromFundSource+" and d.id <= "+toFundSource+" "+
         * " and a.fundid=c.id  and voucherdate >='"+startDate+"'  "+ " and voucherdate<='"+endDate+"' and creditamount>0 "+
         * " AND a.status<>4 "+
    ...
    public String getULBName()
    {
        String ulbName = "";
        try {
            final List<Object[]> rset = HibernateUtil.getCurrentSession().createSQLQuery("select name from companydetail").list();

	...
	public void addGrid(DataCollection dc, HttpServletRequest req){
		...
		for (int i=0; i<ncols; i++){ //note that we are going by column first...
			...
			if (col == null || col.length != nrows){ //some error in form design
				if(LOGGER.isDebugEnabled())     LOGGER.debug("Error in form Design " + name +" is not defined or has different number of values than the rest in the form");
			}else{
...
/*
	public static void main(String[] args) {
		...
		if(LOGGER.isDebugEnabled())     LOGGER.debug(gm.toJavaScript(dc, "ddcc"));	
	}
*/
}

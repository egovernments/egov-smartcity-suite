...
public class PageMaps {
	...
	public static PageMap getPageMap(String pageName){
		...
		if (obj == null){
			...
			if(LOGGER.isDebugEnabled())     LOGGER.debug("file name in pagemaps === " + fileName);			
			if (file.isFile()){
				xl.load(fileName, pm);
				pageMaps.put(pageName, pm);
				if(LOGGER.isDebugEnabled())     LOGGER.debug("added toi pagemaps === ");				
			}
		}else pm = (PageMap)obj;
	...
	public static void main(String[] args) {
		...
		if(LOGGER.isDebugEnabled())     LOGGER.debug(pm.toJavaScript(dc));

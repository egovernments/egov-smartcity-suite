        ...
        try {
            ...
            try {
                ...
                if (billMis.getVoucherHeader() != null) {
                    result = (CVoucherHeader) voucherService
                            .find("select vh from CVoucherHeader vh where vh.id = ?1 and vh.status != ?2",
                                    billMis.getVoucherHeader().getId(),
                                    FinancialConstants.CANCELLEDVOUCHERSTATUS);
                    if (result != null)
                        throw new ApplicationRuntimeException("Voucher ".concat(result.getVoucherNumber()).concat(" already exists for this bill "));
                }
            ...
            for (final EgBilldetails egBilldetails : billDetailslist) {
                ...
                final String glcode = persistenceService
                        .getSession()
                        .createQuery(
                                "select glcode from CChartOfAccounts where id = :glCodeId")
                        .setParameter("glCodeId", egBilldetails.getGlcodeid().longValue())
                        .list().get(0)
                        .toString();
...
*/
    ...
    private Position getNextPosition(final CVoucherHeader voucherheader,
        ...
        final Designation next_desig = (Designation) persistenceService
                .find("from Designation  where upper(name) = upper(?1)",
                        designationName);
        final Functionary functionary = (Functionary) persistenceService.find(
                "from Functionary where upper(name) = upper(?1)", functionaryName);
        ...
        try {
            ...
            if (null != headerdetails.get(VoucherConstant.ORIGIONALVOUCHER)) {
                ...
                query = persistenceService.getSession().createQuery(
                        ...
                        .setParameter("id", origionalVId, LongType.INSTANCE);
    ...
    public void validateTransaction(
        ...
        for (final HashMap<String, Object> subdetailDetailMap : subledgerdetails) {
            ...
            final Query querytds = persistenceService.getSession().createQuery(
                    "select t.id from Recovery t where t.chartofaccounts.glcode = :glcode")
            ...
            final Query query = persistenceService.getSession().createQuery(
                    "from CChartOfAccountDetail cd,CChartOfAccounts c where cd.glCodeId = c.id and c.glcode = :glcode")
                    .setParameter(VoucherConstant.GLCODE, glcode, StringType.INSTANCE);
            ...
            if (null != subdetailDetailMap.get(VoucherConstant.DETAILTYPEID)) {
                ...
                final Query qry = session
                        .createQuery(new StringBuilder("from CChartOfAccountDetail cd,CChartOfAccounts c")
                                .append(" where cd.glCodeId = c.id and c.glcode = :glcode and cd.detailTypeId.id = :detailTypeId").toString())
                        .setParameter(VoucherConstant.GLCODE, glcode, StringType.INSTANCE)
                        .setParameter("detailTypeId", Integer.valueOf(detailtypeid), IntegerType.INSTANCE)
                        .setCacheable(true);
            ...
            if (null != subdetailDetailMap.get(VoucherConstant.DETAILKEYID)) {
                ...
                final Query qry = session
                        ...
                        .setParameter(VoucherConstant.DETAILTYPEID, Integer.valueOf(detailtypeid), IntegerType.INSTANCE)
                        .setParameter("detailkey", Integer.valueOf(detailKeyId), IntegerType.INSTANCE)
                        .setCacheable(true);
                if (null == qry.list() || qry.list().size() == 0)
                    throw new ApplicationRuntimeException(
    ...
    public Functionary getFunctionaryByCode(final BigDecimal code) {
        ...
        final Functionary functionary = (Functionary) persistenceService.find(
                "from Functionary where code = ?1", code);
        return functionary;
    ...
    public void deleteVoucherdetailAndGL(final CVoucherHeader vh) throws ApplicationRuntimeException {
        try {

            final String glQry = "select id from generalledger where voucherheaderid = :vhId";
            final String glidQry = "select id from generalledgerdetail where generalledgerid = :glId";
            final String delQry = "delete from EG_REMITTANCE_GLDTL where gldtlid = :gldtlId";
            final String delQrr = "delete from generalledgerdetail where generalledgerid = :glId";
            final String delgl = " delete from generalledger where voucherheaderid = :vhId";
            final String delvh = " delete from voucherdetail where voucherheaderid = :vhId";
            final List<Object[]> rs = persistenceService.getSession().createNativeQuery(glQry)
                    .setParameter("vhId", vh.getId(), LongType.INSTANCE)
                    .list();
            ...
            while (rs != null && rs.size() > 0) {
                ...
                while (rs1 != null && rs1.size() > 0) {
                    delete = true;
                    persistenceService.getSession().createNativeQuery(delQry)
                            .setParameter("gldtlId", Long.parseLong(rs1.get(1).toString()), LongType.INSTANCE)
                            .executeUpdate();
                }
                if (delete) {
                    persistenceService.getSession().createNativeQuery(delQrr)
                            .setParameter("glId", Long.parseLong(rs1.get(1).toString()))
                            .executeUpdate();
                }
            }
            persistenceService.getSession().createNativeQuery(delgl)
                    .setParameter("vhId", vh.getId(), LongType.INSTANCE)
                    .executeUpdate();

            persistenceService.getSession().createNativeQuery(delvh)
                    .setParameter("vhId", vh.getId(), LongType.INSTANCE)
                    .executeUpdate();
    ...
    public CVoucherHeader reverseVoucher(
        ...
        for (final HashMap<String, Object> paramMap : paramList) {
            ...
            if (paramMap.get(VOUCHER_HEADER_ID) == null)
                ...
                try {
                    originalVocher = voucherService.find("from CVoucherHeader where id = ?1", (Long) paramMap.get(VOUCHER_HEADER_ID));
        ...
        originalVocher = (CVoucherHeader) persistenceService.find(
                "from CVoucherHeader where id = ?1",
        ...
        List<CGeneralLedger> orginalLedgerEntries = generalLedgerService
                .findAllBy("from CGeneralLedger where voucherHeaderId = ?1",
                        originalVocher);
        HashMap<String, Object> detailMap = null;
        HashMap<String, Object> subledgerMap = null;
        final List<HashMap<String, Object>> accountdetails = new ArrayList<HashMap<String, Object>>();
        ...
        for (final CGeneralLedger ledger : orginalLedgerEntries) {
            ...
            List<CGeneralLedgerDetail> ledgerDetailSet = generalLedgerDetailService
                    .findAllBy(
                            "from CGeneralLedgerDetail where generalLedgerId.id = ?1",
                            ledger.getId());

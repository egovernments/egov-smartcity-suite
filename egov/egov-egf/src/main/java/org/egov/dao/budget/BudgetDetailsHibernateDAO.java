...
public class BudgetDetailsHibernateDAO implements BudgetDetailsDAO {
    ...
    public List<BudgetDetail> findAll() {
        return getCurrentSession().createQuery("from BudgetDetail").list();
    }
     ...
     *
     * @param detailsMap is the map containig following fields financialyearid This is
        ...
        try {
            ...
            if (amtavailable != null && amtavailable.compareTo(BigDecimal.ZERO) >= 0) {
                ...
                final Query q = getCurrentSession().createQuery(
                        " from BudgetDetail bd where  bd.budget.financialYear.id = :finYearId and bd.budget.isbere = :type and bd.budgetGroup.id in (:bgId)".concat(query));
                ...
                q.setParameter("finYearId", financialyearid, LongType.INSTANCE);
                q.setParameterList("bgId", budgetheadid, LongType.INSTANCE);
    ...
    @Deprecated
    @Transactional
    public BudgetUsage getBudgetUsageDetails(final Long financialyearid, final Integer moduleid,
                                              final String referencenumber, final Integer departmentid, final Long functionid,
                                              final Integer functionaryid, final Integer schemeid, final Integer subschemeid, final Integer boundaryid,
    ...
    private String prepareAgregateQuery(final Integer departmentid, final Long functionid, final Integer functionaryid,
        ...
        if (departmentid != null && departmentid != 0)
            query.append(getQuery(Department.class, departmentid, " and bd.executingDepartment = "));
        ...
        if (boundaryid != null && boundaryid != 0)
            query.append(getQuery(Boundary.class, boundaryid, " and bd.boundary = "));

        return " and bd.budget.status.code = 'Approved' ".concat(query.toString());
    }
    ...
    private String prepareQuery(final Integer departmentid, final Long functionid, final Integer functionaryid,
        ...
        return " and bd.budget.status.description = 'Approved' and bd.status.description = 'Approved'  ".concat(query.toString());
    }
    ...
    public List<BudgetDetail> getBudgetAvailableDetail(final Long financialyearid, final Integer departmentid,
        ...
        try {
            ...
            final String finalquery = "select bd from BudgetDetail bd where bd.budget.isbere = :type and bd.budget.financialYear.id = :financialyearid".concat(query);
    ...
    public BigDecimal getActualBudgetUtilized(final Map<String, Object> paramMap) {
        ...
        StringBuilder query = new StringBuilder(EMPTY_STRING);
        StringBuilder select = new StringBuilder(EMPTY_STRING);
        ...
        try {
            ...
            query.append(getQuery(CFunction.class, functionid, " and gl.functionId = "))
                    .append(getQuery(Department.class, deptid, " and vmis.departmentid = "))
                    .append(getQuery(Functionary.class, functionaryid, " and vmis.functionary = "))
                    .append(getQuery(Scheme.class, schemeid, " and vmis.schemeid = "))
                    .append(getQuery(SubScheme.class, subschemeid, " and vmis.subschemeid = "))
            ...
            if (budgetgroup.getMinCode() != null) {
                query.append(" and substr(gl.glcode, 1, ").append(budgetgroup.getMinCode().getGlcode().length()).append(")<='")
                        .append(budgetgroup.getMinCode().getGlcode()).append("' ");
                if (budgetgroup.getMaxCode() == null)
                    query.append(" and substr(gl.glcode,1,").append(budgetgroup.getMinCode().getGlcode().length()).append(")>='")
                            .append(budgetgroup.getMinCode().getGlcode()).append("' ");
                else
                    query.append(" and substr(gl.glcode,1,").append(budgetgroup.getMinCode().getGlcode().length()).append(")>='")
                            .append(budgetgroup.getMaxCode().getGlcode()).append("' ");
            } else if (budgetgroup.getMajorCode() != null)
                query.append(" and substr(gl.glcode,1,").append(majorcodelength).append(")='")
                        .append(budgetgroup.getMajorCode().getGlcode()).append("'");
            ...
            if (BudgetAccountType.REVENUE_RECEIPTS.equals(budgetgroup.getAccountType())
                ...
                select.append(" SELECT SUM(gl.creditAmount) - SUM(gl.debitAmount) ");
                ...
                select.append(" SELECT SUM(gl.debitAmount) - SUM(gl.creditAmount) ");
            else
                select.append(" SELECT SUM(gl.debitAmount) - SUM(gl.creditAmount) ");
    ...
    public BigDecimal getActualBudgetUtilizedForBudgetaryCheck(final Map<String, Object> paramMap) {
        ...
        try {
            ...
            if (budgetGrouplist.isEmpty())
                ...
                for (final String value : values)
                    if (value.equals("department"))
                        query.append(getQuery(Department.class, deptid, " and vmis.departmentid = "));
                    else if (value.equals("function"))
                        query.append(getQuery(CFunction.class, functionid, " and gl.functionId = "));
                    else if (value.equals("functionary"))
                        query.append(getQuery(Functionary.class, functionaryid, " and vmis.functionary = "));
                    else if (value.equals("fund"))
                        query.append(getQuery(Fund.class, fundid, " and vh.fundId = "));
                    else if (value.equals("scheme"))
                        query.append(getQuery(Scheme.class, schemeid, " and vmis.schemeid = "));
                    else if (value.equals("subscheme"))
                        query.append(getQuery(SubScheme.class, subschemeid, " and vmis.subschemeid = "));
                    else if (value.equals("boundary"))
                        query.append(getQuery(Boundary.class, boundaryid, " and vmis.divisionid = "));
                    else
                        throw new ValidationException(EMPTY_STRING,
                                "budgetaryCheck_groupby_values is not matching=" + value);
            }
    ...
    public BigDecimal getBudgetedAmtForYear(final Map<String, Object> paramMap) {
        ...
        try {
            ...
            if (budgetService.hasApprovedReForYear(financialyearid))
                finalquery.append(" from BudgetDetail bd where bd.budget.isbere = 'RE' ").append(query);
            else
                finalquery.append(" from BudgetDetail bd where bd.budget.isbere = 'BE' ").append(query);
    ...
    public BigDecimal getBudgetedAmtForYearAsOnDate(final Map<String, Object> paramMap, final Date asOnDate) {
        ...
        try {
            ...
            query.append(prepareQuery(deptid, functionid, functionaryid, schemeid, subschemeid, boundaryid, fundid));
        ...
        try {
            ...
            if (LOGGER.isDebugEnabled())
                LOGGER.debug("Inside getAggregateBudgetedAmtForYear---> deptid " + deptid + ",functionid " + functionid
                        ...
                        + financialyearid);
            query.append(prepareAgregateQuery(deptid, functionid, functionaryid, schemeid, subschemeid, boundaryid, fundid));
            ...
            StringBuilder finalquery = new StringBuilder();
            if (budgetService.hasApprovedReForYear(financialyearid))
                finalquery.append(" from BudgetDetail bd where bd.budget.isbere = 'RE' ").append(query).append(" order by bd.executingDepartment ");
            else
                finalquery.append(" from BudgetDetail bd where bd.budget.isbere = 'BE' ").append(query).append(" order by bd.executingDepartment ");
     ...
     *
     * @param paramMap paramMap contains 1. debitAmt (mandatory) 2. creditAmt
    ...
    public List<BudgetGroup> getBudgetHeadByGlcode(final CChartOfAccounts coa) {
        try {
            ...
            if (bgList.isEmpty()) {
                ...
                bgList = persistenceService.findAllBy(query.toString(), coa.getGlcode().substring(0, minorcodelength), coa.getGlcode().substring(0, minorcodelength));
                if (bgList.isEmpty()) {
                    query = new StringBuilder(" from BudgetGroup bg where bg.majorCode.glcode = ?1 and bg in (select budgetGroup from BudgetDetail) and bg.isActive = true ");
    ...
    public List<BudgetGroup> getBudgetHeadForGlcodeList(final List<CChartOfAccounts> coa) {
        try {
            ...
            final String query = new StringBuilder(" from BudgetGroup bg where ")
                    .append(coaQry)
                    .append(" and bg in (select budgetGroup from BudgetDetail) and bg.isActive = true order by bg.name").toString();
            if (LOGGER.isDebugEnabled())
    ...
    public BigDecimal getBillAmountForBudgetCheck(final Map<String, Object> paramMap) {
        ...
        StringBuilder query = new StringBuilder(EMPTY_STRING);
        StringBuilder query1 = new StringBuilder(EMPTY_STRING);
        try {
            if (paramMap.get(Constants.DEPTID) != null)
                deptid = (Long) paramMap.get(Constants.DEPTID);
            if (paramMap.get(Constants.FUNCTIONID) != null)
                ...
                for (final String value : values)
                    if (value.equals("department")) {
                        if (deptid == null || deptid == 0)
                            ...
                            query.append(getQuery(Department.class, deptid, " and bmis.egDepartment.id = "));
                    } else if (value.equals("function")) {
                        if (functionid == null || functionid == 0)
                            throw new ValidationException(EMPTY_STRING, "Function is required");
                        else
                            query.append(getQuery(CFunction.class, functionid, " and bd.functionid = "));
                    } else if (value.equals("functionary")) {
                        if (functionaryid == null || functionaryid == 0)
                            throw new ValidationException(EMPTY_STRING, "Functionary is required");
                        else
                            query.append(getQuery(Functionary.class, functionaryid, " and bmis.functionaryid.id = "));
                    } else if (value.equals("fund")) {
                        if (fundid == null || fundid == 0)
                            throw new ValidationException(EMPTY_STRING, "Fund is required");
                        else
                            query.append(getQuery(Fund.class, fundid, " and bmis.fund.id = "));
                    } else if (value.equals("scheme")) {
                        if (schemeid == null || schemeid == 0)
                            throw new ValidationException(EMPTY_STRING, "Scheme is required");
                        else
                            query.append(getQuery(Scheme.class, schemeid, " and bmis.scheme.id = "));
                    } else if (value.equals("subscheme")) {
                        if (subschemeid == null || subschemeid == 0)
                            throw new ValidationException(EMPTY_STRING, "Sub scheme is required");
                        else
                            query.append(getQuery(SubScheme.class, subschemeid, " and bmis.subScheme.id = "));
                    } else if (value.equals("boundary")) {
                        if (boundaryid == null || boundaryid == 0)
                            throw new ValidationException(EMPTY_STRING, "Boundary is required");
                        else
                            query.append(getQuery(Boundary.class, boundaryid, " and bmis.fieldid.id = "));
                    } else
                        throw new ValidationException(EMPTY_STRING,
                                "budgetaryCheck_groupby_values is not matching=" + value);
            }
            ...
            query1 = new StringBuilder("select sum(case when bd.debitamount is null then 0 ELSE bd.debitamount end -case when bd.creditamount is null then 0 else bd.creditamount end) ")
    ...
    private BigDecimal getBillAmountWhereCancelledVouchers(final String query, final Date fromdate,
        ...
        final StringBuilder newQuery = new StringBuilder("select sum(case when bd.debitamount is null then 0 else bd.debitamount end - case when bd.creditamount is null then 0")
    ...
    public BigDecimal getBudgetedAmtForYearRegardingBEorRE(final Map<String, Object> paramMap, final String typeBeRe) {
        ...
        try {
            ...
            if (budgetheadid == null)
                throw new ValidationException(EMPTY_STRING, "Budget head id is null or empty");
            query.append(getQuery(BudgetGroup.class, budgetheadid, " and bd.budgetGroup = "));

            if (financialyearid == null)
                throw new ValidationException(EMPTY_STRING, "Financial Year id is null");
            query.append(getQuery(CFinancialYear.class, financialyearid, " and bd.budget.financialYear = "));

            final String finalquery = " from BudgetDetail bd where bd.budget.isbere = '".concat(typeBeRe).concat("' ").concat(query.toString());
    ...
    /**
     * @param paramMap Map contains deptid,functionid, functionaryid,schemeid,
    ...
    /**
     * @param queryParamMap - HashMap<String, Object> queryParamMap will have data
    ...
    public List<BudgetUsage> getListBudgetUsage(final Map<String, Object> queryParamMap) {
        ...
        if (!isNull(grpByVls.get("department")))
            if (isNull(queryParamMap.get("ExecutionDepartmentId")))
                ...
                params.put("execDeptId", Integer.valueOf(queryParamMap.get("ExecutionDepartmentId").toString()));
            }
        ...
        if (!isNull(grpByVls.get("scheme")))
            if (isNull(queryParamMap.get("schemeId")))
                ...
                query.append(" and bd.scheme.id = :schemeId");
                params.put("schemeId", Integer.valueOf(queryParamMap.get("schemeId").toString()));
            }
        if (!isNull(grpByVls.get("subscheme")))
            if (isNull(queryParamMap.get("subschemeId")))
                throw new ValidationException(EMPTY_STRING, "SubScheme is required");
            else {
                query.append(" and bd.subScheme.id = :subSchemeId");
                params.put("subSchemeId", Integer.valueOf(queryParamMap.get("subschemeId").toString()));
            }
        if (!isNull(grpByVls.get(Constants.FUNCTIONARY)))
            if (isNull(queryParamMap.get("functionaryId")))
                throw new ValidationException(EMPTY_STRING, "Functionary is required");
            else {
                query.append(" and bd.functionary.id = :functionaryId");
                params.put("functionaryId", Integer.valueOf(queryParamMap.get("functionaryId").toString()));
            }
        if (!isNull(grpByVls.get("boundary")))
            if (isNull(queryParamMap.get("boundaryId")))
                throw new ValidationException(EMPTY_STRING, "Boundary is required");
            else {
                query.append(" and bd.boundary.id = :bndryId");
                params.put("bndryId", Integer.valueOf(queryParamMap.get("boundaryId").toString()));
            }
        ...
        if (!isNull(queryParamMap.get("financialYearId"))) {
            ...
            params.put("fyId", Integer.valueOf(queryParamMap.get("financialYearId").toString()));
        }
        ...
        params.keySet().forEach(key -> query1.setParameter(key, params.get(key)));
        if (!isNull(queryParamMap.get("fromDate")))
            query1.setParameter("from", (Date) queryParamMap.get("fromDate"), TimestampType.INSTANCE);
        if (!isNull(queryParamMap.get("toDate"))) {
            Calendar date = Calendar.getInstance();
            date.setTime((Date) queryParamMap.get("toDate"));
            date.set(Calendar.MINUTE, 59);
            date.set(Calendar.HOUR_OF_DAY, 23);
            date.set(Calendar.SECOND, 59);
            query1.setParameter("to", date, TimestampType.INSTANCE);
        }
    ...
    public BigDecimal getPlannigBudgetBy(final Integer fundId, final Integer deptId, Date asOnDate1) {
        ...
        BigDecimal reappAmount = (BigDecimal) persistenceService.find(new StringBuilder("select sum(additionAmount-deductionAmount)")
                        ...
                        .append(" and br.budgetDetail.budget.isbere = ?4 and br.status.description='Approved' and to_date(br.modifiedDate) <= ?5 ").toString(),

...
public class RecoverySetupDelegate {
	...
	public void createRecovery(RecoverySetupForm rsf, Integer userId) throws Exception {
		...
		if (rsf.getRecovAccCodeId() != null && rsf.getRecovAccCodeId().length() > 0) {
			tds1.setChartofaccounts((CChartOfAccounts) cmnMngr.getCChartOfAccountsById(Long.parseLong(rsf.getRecovAccCodeId())));
		}
		...
		/*
		 * if(rsf.getRecMode().equalsIgnoreCase("automatic") || (rsf.getRecMode().equalsIgnoreCase("manual") && rsf.getBankLoan().equalsIgnoreCase("off"))) { if(rsf.getRecovRemitTo()!=null &&
		 * rsf.getRecovRemitTo().length()>0) tds1.setRemitted(rsf.getRecovRemitTo());
		 * 
		 * if(rsf.getRecovBSRCode()!=null && rsf.getRecovBSRCode().length()>0) tds1.setBsrcode(rsf.getRecovBSRCode()); } else
		 */
		if (rsf.getRecMode().equalsIgnoreCase("manual") && rsf.getBankLoan().equalsIgnoreCase("on")) {
			if (!rsf.getBank().equals("0") && rsf.getBank().length() > 0) {
				Bank bank = cmnMngr.getBankById(Integer.parseInt(rsf.getBank()));
				tds1.setBank(bank);
		...
		if (rsf.getRecovAppliedTo() != null && rsf.getRecovAppliedTo().length() > 0)
			tds1.setEgPartytype((EgPartytype) cmnMngr.getPartytypeById(Integer.parseInt(rsf.getRecovAppliedTo())));
		...
		if(LOGGER.isInfoEnabled())     LOGGER.info(" BEFORE INSERT INTO TDS TABLE");
		recoveryService.createTds(tds1);
		if(LOGGER.isInfoEnabled())     LOGGER.info("AFTER INSERT INTO TDS TABLE");
		
		// For eg_deduction_details table insert
		Long tdsid = tds1.getId();
		if (rsf.getRecMode().equalsIgnoreCase("automatic")) {
			for (int i = 0; i < rsf.getLowAmount().length; i++) {
				...
				if (tdsid != null) {
					dedDetail.setRecovery((Recovery) recoveryService.getTdsById(tdsid));
				}
				
				if (rsf.getPartyType() != null) {
					if (rsf.getPartyType()[i] != null && !rsf.getPartyType()[i].equals("0") && rsf.getPartyType()[i].length() > 0)
						dedDetail.setEgpartytype((EgPartytype) cmnMngr.getPartytypeById(Integer.parseInt((rsf.getPartyType()[i]))));
				}
				
				if (rsf.getDocType() != null) {
					if (rsf.getDocType()[i] != null && !rsf.getDocType()[i].equals("0") && rsf.getDocType()[i].length() > 0)
						dedDetail.setWorkDocType((EgwTypeOfWork) cmnMngr.getTypeOfWorkById(Long.parseLong(rsf.getDocType()[i])));
				}
				/*if (rsf.getDocType() != null) {
					if (rsf.getDocType()[i] != null && !rsf.getDocType()[i].equals("0") && rsf.getDocType()[i].length() > 0) {
						if (rsf.getSubType()[i] != null && !rsf.getSubType()[i].equals("0") && rsf.getSubType()[i].length() > 0)
							dedDetail.setWorkDocSubType((EgwTypeOfWork) cmnMngr.getTypeOfWorkById(Long.parseLong(rsf.getSubType()[i])));
					}
				}*/
				...
				if(LOGGER.isInfoEnabled())     LOGGER.info(" BEFORE INSERT INTO eg_deduction_details table");
				recoveryService.createEgDeductionDetails(dedDetail);
				if(LOGGER.isInfoEnabled())     LOGGER.info(" AFTER INSERT INTO eg_deduction_details table");
			}
		}
	...
	public void modifyRecovery(RecoverySetupForm rsf, Integer userId) throws Exception {
		...
		Recovery tds1 = (Recovery) recoveryService.findById(Long.parseLong(rsf.getTdsTypeId()));
		...
		if (rsf.getRecMode().equalsIgnoreCase("automatic") || (rsf.getRecMode().equalsIgnoreCase("manual") && rsf.getBankLoan().equalsIgnoreCase("off"))) {
			tds1.setBank(null);
		...
		if(LOGGER.isInfoEnabled())     LOGGER.info(" BEFORE UPDATE INTO TDS TABLE");
		recoveryService.updateTds(tds1);
		if(LOGGER.isInfoEnabled())     LOGGER.info("AFTER UPDATE INTO TDS TABLE");
		...
		if (rsf.getRecMode().equalsIgnoreCase("automatic")) {
			...
			if (recoveryDetailList.size() == rsf.getId().length) {
				...
				int i = 0, j = 0, k = 0;
				Integer[] id = new Integer[recoveryDetailList.size()];
				for (; i < recoveryDetailList.size(); i++) {
				
					EgDeductionDetails ded =  recoveryDetailList.get(i);
					id[i] = (ded.getId()).intValue();
					if (id[i] == Integer.parseInt(rsf.getId()[i])) {
											
						EgDeductionDetails dedDetail1 = (EgDeductionDetails) recoveryService.getEgDeductionDetailsById(Integer.parseInt(rsf.getId()[i]));
						dedDetail1.setRecovery(tds1);
						
						
						if (rsf.getPartyType() != null) {
							if (rsf.getPartyType()[i] != null && !rsf.getPartyType()[i].equals("0") && rsf.getPartyType()[i].length() > 0) {
								dedDetail1.setEgpartytype((EgPartytype) cmnMngr.getPartytypeById(Integer.parseInt(rsf.getPartyType()[i])));
							} else {
						...
						if (rsf.getDocType() != null) {
							if (rsf.getDocType()[i] != null && !rsf.getDocType()[i].equals("0") && rsf.getDocType()[i].length() > 0)
								dedDetail1.setWorkDocType((EgwTypeOfWork) cmnMngr.getTypeOfWorkById(Long.parseLong(rsf.getDocType()[i])));
							else {
						...
						/*if (rsf.getDocType() != null) {
							if (rsf.getDocType()[i] != null && !rsf.getDocType()[i].equals("0") && rsf.getDocType()[i].length() > 0) {
								if (rsf.getSubType()[i] != null && !rsf.getSubType()[i].equals("0") && rsf.getSubType()[i].length() > 0)
									dedDetail1.setWorkDocSubType((EgwTypeOfWork) cmnMngr.getTypeOfWorkById(Long.parseLong(rsf.getSubType()[i])));
								else {
						...
						for (; j < recoveryDetailList.size(); j++) {
							...
							EgDeductionDetails dedDetail2 = (EgDeductionDetails) recoveryDetailList.get(j);
						...
						for (; k < recoveryDetailList.size(); k++) {
							...
							if (rsf.getPartyType() != null) {
								if (rsf.getPartyType()[k] != null && !rsf.getPartyType()[k].equals("0") && rsf.getPartyType()[k].length() > 0)
									dedDetail3.setEgpartytype((EgPartytype) cmnMngr.getPartytypeById(Integer.parseInt(rsf.getPartyType()[k])));
								else
							...
							if (rsf.getDocType() != null) {
								if (rsf.getDocType()[k] != null && !rsf.getDocType()[k].equals("0") && rsf.getDocType()[k].length() > 0)
									dedDetail3.setWorkDocType((EgwTypeOfWork) cmnMngr.getTypeOfWorkById(Long.parseLong(rsf.getDocType()[k])));
								else
							...
							/*if (rsf.getDocType() != null) {
								if (rsf.getDocType()[k] != null && !rsf.getDocType()[k].equals("0") && rsf.getDocType()[k].length() > 0) {
									if (rsf.getSubType()[k] != null && !rsf.getSubType()[k].equals("0") && rsf.getSubType()[k].length() > 0)
										dedDetail3.setWorkDocSubType((EgwTypeOfWork) cmnMngr.getTypeOfWorkById(Long.parseLong(rsf.getSubType()[k])));
									else
			...
			/* NOT EQUAL (MODEL SIZE > FORM BEAN SIZE)--- UPDATE,DELETE,INSERT START */
				...
				for (; i < rsf.getId().length; i++) {
					EgDeductionDetails ded = (EgDeductionDetails) recoveryDetailList.get(i);
					id[i] = (ded.getId()).intValue();
					
					if (id[i] == Integer.parseInt(rsf.getId()[i])) {
						EgDeductionDetails dedDetail4 = (EgDeductionDetails) recoveryService.getEgDeductionDetailsById(Integer.parseInt(rsf.getId()[i]));
						dedDetail4.setRecovery(tds1);
						
						if (rsf.getPartyType() != null) {
							if (rsf.getPartyType()[i] != null && !rsf.getPartyType()[i].equals("0") && rsf.getPartyType()[i].length() > 0)
								dedDetail4.setEgpartytype((EgPartytype) cmnMngr.getPartytypeById(Integer.parseInt(rsf.getPartyType()[i])));
							else
						...
						if (rsf.getDocType() != null) {
							if (rsf.getDocType()[i] != null && !rsf.getDocType()[i].equals("0") && rsf.getDocType()[i].length() > 0)
								dedDetail4.setWorkDocType((EgwTypeOfWork) cmnMngr.getTypeOfWorkById(Long.parseLong(rsf.getDocType()[i])));
							else
						...
						/*if (rsf.getDocType() != null) {
							if (rsf.getDocType()[i] != null && !rsf.getDocType()[i].equals("0") && rsf.getDocType()[i].length() > 0) {
								if (rsf.getSubType()[i] != null && !rsf.getSubType()[i].equals("0") && rsf.getSubType()[i].length() > 0)
									dedDetail4.setWorkDocSubType((EgwTypeOfWork) cmnMngr.getTypeOfWorkById(Long.parseLong(rsf.getSubType()[i])));
								else
				...
				for (; k < rsf.getId().length; k++) {
					...
					if (rsf.getDocType() != null) {
						if (rsf.getDocType()[k] != null && !rsf.getDocType()[k].equals("0") && rsf.getDocType()[k].length() > 0)
							dedDetail.setWorkDocType((EgwTypeOfWork) cmnMngr.getTypeOfWorkById(Long.parseLong(rsf.getDocType()[k])));
						else
				...
				if (recoveryDetailList.size() < rsf.getId().length) {
					int i = 0, j = 0, k = 0;
					Integer[] id = new Integer[rsf.getId().length];
					...
					for (; i < recoveryDetailList.size(); i++) {
						...
						if (id[i] == Integer.parseInt(rsf.getId()[i])) {
							...
							EgDeductionDetails dedDetail5 = (EgDeductionDetails) recoveryService.getEgDeductionDetailsById(Integer.parseInt(rsf.getId()[i]));
							dedDetail5.setRecovery(tds1);
							
							if (rsf.getPartyType() != null) {
								if (rsf.getPartyType()[i] != null && !rsf.getPartyType()[i].equals("0") && rsf.getPartyType()[i].length() > 0)
									dedDetail5.setEgpartytype((EgPartytype) cmnMngr.getPartytypeById(Integer.parseInt(rsf.getPartyType()[i])));
								else
							...
							if (rsf.getDocType() != null) {
								if (rsf.getDocType()[i] != null && !rsf.getDocType()[i].equals("0") && rsf.getDocType()[i].length() > 0)
									dedDetail5.setWorkDocType((EgwTypeOfWork) cmnMngr.getTypeOfWorkById(Long.parseLong(rsf.getDocType()[i])));
								else
							...
							/*if (rsf.getDocType() != null) {
								if (rsf.getDocType()[i] != null && !rsf.getDocType()[i].equals("0") && rsf.getDocType()[i].length() > 0) {
									if (rsf.getSubType()[i] != null && !rsf.getSubType()[i].equals("0") && rsf.getSubType()[i].length() > 0)
										dedDetail5.setWorkDocSubType((EgwTypeOfWork) cmnMngr.getTypeOfWorkById(Long.parseLong(rsf.getSubType()[i])));
									else
					...
					for (; k < rsf.getId().length; k++) {
						...
						if (rsf.getPartyType() != null) {
							if (rsf.getPartyType()[k] != null && !rsf.getPartyType()[k].equals("0") && rsf.getPartyType()[k].length() > 0)
								dedDetail6.setEgpartytype((EgPartytype) cmnMngr.getPartytypeById(Integer.parseInt(rsf.getPartyType()[k])));
							else
						...
						if (rsf.getDocType() != null) {
							if (rsf.getDocType()[k] != null && !rsf.getDocType()[k].equals("0") && rsf.getDocType()[k].length() > 0)
								dedDetail6.setWorkDocType((EgwTypeOfWork) cmnMngr.getTypeOfWorkById(Long.parseLong(rsf.getDocType()[k])));
							else
						...
						/*if (rsf.getDocType() != null) {
							if (rsf.getDocType()[k] != null && !rsf.getDocType()[k].equals("0") && rsf.getDocType()[k].length() > 0) {
								if (rsf.getSubType()[k] != null && !rsf.getSubType()[k].equals("0") && rsf.getSubType()[k].length() > 0)
									dedDetail6.setWorkDocSubType((EgwTypeOfWork) cmnMngr.getTypeOfWorkById(Long.parseLong(rsf.getSubType()[k])));
								else
									dedDetail6.setWorkDocSubType(null);
							} else
								dedDetail6.setWorkDocSubType(null);
						} else
							dedDetail6.setWorkDocSubType(null);*/
	...
	public void getRecoveryAndPopulateBean(RecoverySetupForm rsf) throws Exception {
		if(LOGGER.isInfoEnabled())     LOGGER.info("inside getRecoveryAndPopulateBean");
		
		Recovery tds = (Recovery) recoveryService.findById(Long.parseLong(rsf.getTdsTypeId()));
		...
		List<EgDeductionDetails> recoveryDetailList = recoveryService.findByTds(tds);
		if (!recoveryDetailList.isEmpty()) {
			int listSize = recoveryDetailList.size();
			...
			String[] appliedToHiddenId = new String[listSize];
			String[] partyType = new String[listSize];
			String[] docType = new String[listSize];
			String[] subType = new String[listSize];
			String[] recovDateFrom = new String[listSize];
			String[] recovDateTo = new String[listSize];
			
			String[] lowAmount = new String[listSize];
			String[] highAmount = new String[listSize];
			String[] ITPercentage = new String[listSize];
			String[] surPercentage = new String[listSize];
			String[] eduCessPercentage = new String[listSize];
			String[] flatAmount = new String[listSize];
			String[] cumulativeAmtHigh = new String[listSize];
			String[] cumulativeAmtLow = new String[listSize];
			
			for (int n = 0; n < listSize; n++) {
				EgDeductionDetails ded =  recoveryDetailList.get(n);
				...
				if (ded.getDatefrom() != null) {
					String fromDate = formatter2.format(ded.getDatefrom());
					recovDateFrom[n] = fromDate;
				} else
					recovDateFrom[n] = "";
				
				if (ded.getDateto() != null) {
					String toDate = formatter2.format(ded.getDateto());

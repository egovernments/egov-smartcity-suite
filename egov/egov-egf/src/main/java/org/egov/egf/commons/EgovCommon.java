...
public class EgovCommon {
        ...
        public void setFinDao(FinancialYearHibernateDAO finDao) {
                this.finDao = finDao;
        }
        ...
        public Boundary getBoundaryForUser(User user) {
                /*Set<JurisdictionValues> s = userManager.getJurisdictionsForUser(user.getId(), new Date());
                ...
                try {
                        ...
                        if (!employeeViewList.isEmpty()) {
                                if (employeeViewList.size() == 1) {
                                        return (Department) HibernateUtil.getCurrentSession().load(Department.class, Integer.valueOf((employeeViewList.get(0)[1].toString())));
                                } else {
        ...
        public BigDecimal getCashBalance(final Date VoucherDate,
                ...
                try {
                        ...
                        if(LOGGER.isDebugEnabled())     LOGGER.debug("exception occuered while geeting cash balance", e);
                        throw new HibernateException(e);
                }
        ...
        public BigDecimal getBankBalanceAvailableforPayment(final Date VoucherDate,
                ...
                if(LOGGER.isDebugEnabled())     LOGGER.debug("Total amount available for payment :" + TotalbankBalance);
        ...
        public BigDecimal getAmountApprovedForPaymentAndVoucherNotCreated(
                ...
                try {
                        ...
                        if(LOGGER.isDebugEnabled())     LOGGER.debug("exception occuered while geeting cash balance"
                                        + e.getMessage(), e);
                        throw new HibernateException(e);
                }
                return bankBalance;
        ...
        public BigDecimal getAmountForApprovedPaymentAndChequeNotAssigned(
                ...
                try {
                        ...
                        List<Object> list = (List<Object>) 
                                        HibernateUtil.getCurrentSession().createSQLQuery(paymentQuery.toString())
                        ...
                        if(LOGGER.isDebugEnabled())     LOGGER.debug("exception occuered while getting cash balance"
                                        + e.getMessage(), e);
                        throw new HibernateException(e);
                }
        ...
        /**
         *  This method will return sum of bill amount for the given combination of AccountdetailTypeid and AccountdetailKeyid
         *  for which the AccountdetailTypeid amount is on the DEBIT SIDE .
         *
        ...
        public BigDecimal getSumOfBillAmount(String glcode,String subledgerType ,Long accountdetailkeyId,Date toBillDate)
                ...
                if(toBillDate==null)
                 ...
                 List<Object> actRes=actQry.list();
                 if(null ==actRes || actRes.size() ==0){
                         throw new ValidationException(Arrays.asList(new ValidationError("Accountdetailkey","The accountdetailkey  "+accountdetailkeyId+" for the accountdetailType : " +
                                        subledgerType+" does not exist ")));
                 }
                         
                         query.append("select sum(epayee.debitAmount) FROM EgBillPayeedetails epayee,  EgwStatus estatus, CChartOfAccounts coa,Accountdetailtype act  ")
                        ...
                        //      if(LOGGER.isInfoEnabled())     LOGGER.info("----------------:"+amountQry.list());
                                ...
                                if(LOGGER.isDebugEnabled())     LOGGER.debug(" Total bill amount generated for the "+subledgerType+"is :"+result);
        ...
        public BigDecimal getAccountBalanceFromLedger(final Date VoucherDate,
                ...
                try {
                        ...
                        if(LOGGER.isDebugEnabled())     LOGGER.debug("bankBalance :" + bankBalance);
                } catch (Exception e) {
                        if(LOGGER.isDebugEnabled())     LOGGER.debug("exception occuered while geeting cash balance"
                                        + e.getMessage(), e);
                        throw new HibernateException(e);
        ...
        public EntityType getEntityType(Accountdetailtype accountdetailtype,
                ...
                try {
                        ...
                        if(LOGGER.isDebugEnabled())     LOGGER.debug("data Type = " + dataType);
        ...
        public Map<String, Object> getCashChequeInfoForBoundary()
                ...
                final List<AppConfigValues> appList = appConfigValuesService.getConfigValuesByModuleAndKey(Constants.EGF, "boundaryforaccounts");
                ...
                List<Boundary> listBndryLvl = (List<Boundary>) persistenceService.findAllBy("from Boundary where boundaryType.id=?",
                                                boundaryTypeId);
                ...
                if (null != listBndryLvl && !listBndryLvl.isEmpty()) {
                        ...
                        try {
                                ...
                                String bndQry = "SELECT glcode AS chequeinhand,id FROM CHARTOFACCOUNTS where id = (SELECT chequeinhand FROM CODEMAPPING WHERE EG_BOUNDARYID=?)";
                                PreparedStatement pstmt = connection.prepareStatement(bndQry);
                                pstmt.setLong(1, boundaryId);
                                if(LOGGER.isDebugEnabled())     LOGGER.debug("Cheque In hand account code query =" + bndQry);
                                ResultSet resultSet = pstmt.executeQuery();
                                if (resultSet.next()) {
                                        chequeInHand = resultSet.getString("chequeinhand");
                                        chequeInHandId = resultSet.getLong("id");
                                }
                                if(LOGGER.isDebugEnabled())     LOGGER.debug("chequeInHand is " + chequeInHand
                                                + " chequeInHandId is " + chequeInHandId);
                                String sqlQuery2 = "SELECT glcode AS cashinhand,id FROM CHARTOFACCOUNTS where id = (SELECT cashinhand FROM CODEMAPPING WHERE EG_BOUNDARYID=?)";
                                PreparedStatement pstmt1 = connection
                                                .prepareStatement(sqlQuery2);
                                pstmt1.setLong(1, boundaryId);
                                if(LOGGER.isDebugEnabled())     LOGGER.debug("Cheque In hand account code query =" + sqlQuery2);
                                resultSet = pstmt1.executeQuery();
                                if (resultSet.next()) {
                                        cashInHand = resultSet.getString("cashinhand");
                                        cashInHandId = resultSet.getLong("id");
                                }
                                if(LOGGER.isDebugEnabled())     LOGGER.debug("cashInHand is " + cashInHand
                        ...
                        if(LOGGER.isDebugEnabled())     LOGGER.debug("listBndryLvl is either null or blank");
                        throw new ValidationException(Arrays.asList(new ValidationError(
        ...
        public boolean isShowChequeNumber() {
                String value = appConfigValuesService.getConfigValuesByModuleAndKey(Constants.EGF,Constants.CHEQUE_NO_GENERATION_APPCONFIG_KEY).get(0)
                                .getValue();
         ...
         *             -
         */
        public BigDecimal getAccountBalanceforDate(Date asondate, String glcode,String fundcode, Integer accountdetailType, Integer accountdetailkey, Integer deptId)
                        throws ValidationException {

                if(LOGGER.isDebugEnabled())     LOGGER.debug("EgovCommon | getAccountBalanceforDate | Start");
                validateParameterData(asondate, glcode, fundcode, accountdetailType,accountdetailkey);
                if(LOGGER.isDebugEnabled())     LOGGER.debug("validation of data is sucessfull");
                BigDecimal opBalAsonDate = getOpeningBalAsonDate(asondate, glcode,fundcode, accountdetailType, accountdetailkey, deptId);
                BigDecimal glBalAsonDate = getGlcodeBalBeforeDate(asondate, glcode,fundcode, accountdetailType, accountdetailkey, deptId);
         ...
         *             -
         */
        public BigDecimal getAccountBalanceTillDate(Date asondate, String glcode,String fundcode, Integer accountdetailType, Integer accountdetailkey, Integer deptId)
                        throws ValidationException {

                if(LOGGER.isDebugEnabled())     LOGGER.debug("EgovCommon | getAccountBalanceTillDate | Start");
                validateParameterData(asondate, glcode, fundcode, accountdetailType,accountdetailkey);
                if(LOGGER.isDebugEnabled())     LOGGER.debug("validation of data is sucessfull");
                BigDecimal opBalAsonDate = getOpeningBalAsonDate(asondate, glcode,fundcode, accountdetailType, accountdetailkey, deptId);
                BigDecimal glBalAsonDate = getGlcodeBalTillDate(asondate, glcode,fundcode, accountdetailType, accountdetailkey, deptId);

                if(LOGGER.isDebugEnabled())     LOGGER.debug("EgovCommon | getAccountBalanceTillDate | Opening Balance :"+opBalAsonDate+" Txn Balance  :"+glBalAsonDate);
                return opBalAsonDate.add(glBalAsonDate);
        }
        ...
        private void validateParameterData(Date asondate, String glcode,
                ...
                if (null == fundcode || StringUtils.isEmpty(fundcode)) {
                        ...
                        if (null == fundDAO.fundByCode(fundcode)) {
        ...
        public BigDecimal getOpeningBalAsonDate(Date asondate, String glcode,
                        String fundCode, Integer accountdetailType, Integer accountdetailkey, Integer deptId)
                        throws ValidationException {
                ...
                List<Object> tsummarylist = (List<Object>) getPersistenceService().findAllBy(opBalncQuery.toString(), glcode);
                opBalAsonDate = BigDecimal.valueOf((Double) tsummarylist.get(0));

                if(LOGGER.isDebugEnabled())     LOGGER.debug("Opening balance :" + opBalAsonDate);
        ...
        private BigDecimal getCreditOpeningBalAsonDate(Date asondate,
                ...
                List<Object> list = (List<Object>) HibernateUtil.getCurrentSession()
        ...
        protected BigDecimal getGlcodeBalBeforeDate(Date asondate, String glcode,
                        String fundcode, Integer accountdetailType, Integer accountdetailkey, Integer deptId)
                        throws ValidationException {
                ...
                if (null == accountdetailType && null == accountdetailkey) {
                        glCodeBalQry
                                        ...
                                        .append(Constants.DDMMYYYYFORMAT1.format(asondate)).append("' AND endingDate >='").append(
                                                        ...
                                                        Constants.DDMMYYYYFORMAT1.format(asondate)).append("'and vh.status not in (").append(statusExclude).append(")");

                        List<Object> list = (List<Object>) getPersistenceService().findAllBy(glCodeBalQry.toString(), glcode);
                        ...
                        glCodeDbtBalQry
                                        ...
                                        .append(misTab)
                                        .append(" WHERE gl.voucherHeaderId.id=vh.id and gl.id = gld.generalLedgerId and gl.glcodeId.glcode=? ").append(fundCond)
                                        .append(deptCond)
                                        .append(" and vh.voucherDate >= (select startingDate from CFinancialYear where  startingDate <= '")
                                        .append(Constants.DDMMYYYYFORMAT1.format(asondate)).append("' AND endingDate >='").append(
                                                        Constants.DDMMYYYYFORMAT1.format(asondate)).append("') and vh.voucherDate <'").append(
                        ...
                        List<Object> listDbt = (List<Object>) getPersistenceService().findAllBy(glCodeDbtBalQry.toString(), glcode);
                        glCodeDbtBalance = (BigDecimal) listDbt.get(0) == null ? BigDecimal.ZERO
                                        : (BigDecimal) listDbt.get(0);
                        if(LOGGER.isDebugEnabled())     LOGGER.debug(" total debit amount :  " + glCodeDbtBalance);
                        ...
                        List<Object> listCrd = (List<Object>) getPersistenceService().findAllBy(glCodeCrdBalQry.toString(), glcode);
                        glCodeCrdBalance = (BigDecimal) listCrd.get(0) == null ? BigDecimal.ZERO: (BigDecimal) listCrd.get(0);
                        if(LOGGER.isDebugEnabled())     LOGGER.debug(" total credit amount :  " + glCodeCrdBalance);
                        glCodeBalance = glCodeDbtBalance.subtract(glCodeCrdBalance);
                        if(LOGGER.isDebugEnabled())     LOGGER.debug(" total balance amount :  " + glCodeBalance);
        ...
        protected BigDecimal getGlcodeBalTillDate(Date asondate, String glcode,
                        String fundcode, Integer accountdetailType, Integer accountdetailkey, Integer deptId)
                        throws ValidationException {
                StringBuffer glCodeBalQry = new StringBuffer(400);
                StringBuffer glCodeDbtBalQry = new StringBuffer(400);
 ...
 */
        public List<CChartOfAccounts> getAllAccountCodesForAccountDetailType(
                ...
                if(LOGGER.isDebugEnabled())     LOGGER.debug("finished getAllAccountCodesForAccountDetailType for detailtypeId "
                                                + accountDetailTypeId
                                                + ".size:"
                                                + subledgerCodes.size() + ".");
        ...
        public BigDecimal getBillAccountBalanceforDate(Date asondate,
                ...
                if(LOGGER.isDebugEnabled())     LOGGER.debug("EgovCommon | getBillAccountBalanceforDate | Start");
                LOGGER
                ...
                if(LOGGER.isDebugEnabled())     LOGGER.debug("EgovCommon | getBillAccountBalanceforDate | End");
        ...
        private BigDecimal getBillAccBalAsonDate(Date asondate, String glcode,
                ...
                if(LOGGER.isDebugEnabled())     LOGGER.debug("getBillAccBalAsonDate query = " + query.toString());
                ...
                if(LOGGER.isDebugEnabled())     LOGGER.debug("getBillAccBalAsonDate | Bill Account Balance = "
                                + billAccCodeBalance);
        ...
        public BigDecimal getCreditBalanceforDate(Date asondate, String glcode,
                ...
                if(LOGGER.isDebugEnabled())     LOGGER.debug("EgovCommon | getCreditBalanceforDate | Start");
                LOGGER
                                .debug("Data Received asondate = " + asondate + " glcode = "
                                                + glcode + " fundcode = " + fundcode
                                                + " accountdetailType = " + accountdetailType
                ...
                if(LOGGER.isDebugEnabled())     LOGGER.debug("EgovCommon | getCreditBalanceforDate | End");
         ...
         *
         */
        public BigDecimal getDepositAmountForDepositCode(Date asondate,
                        String glcode, String fundcode, Integer accountdetailType,
                        Integer accountdetailkey) throws ValidationException {
                ...
                queryString.append("SELECT MIN(vh.voucherDate) as vhDate from CVoucherHeader vh , CGeneralLedger gl,CGeneralLedgerDetail gld")
                                .append(" WHERE gl.voucherHeaderId.id=vh.id and gl.id = gld.generalLedgerId and gl.glcodeId.glcode=? and vh.fundId.code=? ")
                                .append(" and vh.voucherDate <= '").append(Constants.DDMMYYYYFORMAT1.format(asondate)).append("' and vh.status = 0")
                ...
                BigDecimal creditOpeningBalance = getFirstCreditOpeningBalForDepositCodeAsonDate(minVouDate, glcode, fundcode,accountdetailkey);
                BigDecimal creditBalance = null;
                StringBuffer query = new StringBuffer(400);
        ...
        /**
         * This API will return the sum total of credit opening balances for a given
         * account code and deposit code details.
         *
         * @param asondate
         * @param glcode
         * @param fundCode
    ...
    StringBuffer opBalncQuery = new StringBuffer(300);
     ...
     System.out.println("Opening balance query :"+ opBalncQuery);
            ...
            List<Object> list = (List<Object>)HibernateUtil.getCurrentSession().createSQLQuery(opBalncQuery.toString()).list();
            if(list!=null && list.size()>0)
                    opBalAsonDate = (BigDecimal) list.get(0);
                    opBalAsonDate = opBalAsonDate==null?BigDecimal.ZERO:opBalAsonDate;
            System.out.println("Opening balance  :"+ opBalAsonDate);
        ...
        public BigDecimal getAccCodeBalanceForIndirectExpense(Date asondate,
                ...
                if (null == accountdetailType && null == accountdetailkey) {
                        ...
                        subledgerDbtBalance = (BigDecimal) listDbt.get(0) == null ? BigDecimal.ZERO
                                        : (BigDecimal) listDbt.get(0);
                        if(LOGGER.isDebugEnabled())     LOGGER.debug(" total debit amount :  " + subledgerDbtBalance);
                        ...
                        subledgerCrdBalance = (BigDecimal) listCrd.get(0) == null ? BigDecimal.ZERO
                                        : (BigDecimal) listCrd.get(0);
                        if(LOGGER.isDebugEnabled())     LOGGER.debug(" total credit amount :  " + subledgerCrdBalance);
                        glCodeBalance = subledgerDbtBalance.subtract(subledgerCrdBalance);
                        if(LOGGER.isDebugEnabled())     LOGGER.debug(" total balance amount :  " + glCodeBalance);

                }
                if(LOGGER.isDebugEnabled())     LOGGER.debug("EgovCommon | getAccCodeBalanceForIndirectExpense | End");
                glCodeBalance = glCodeBalance.setScale(2);
                return glCodeBalance;
        }

        private void validateParameterData(Date asondate, String glcode,
                        Integer accountdetailType, String accountdetailkey) {
        ...
        /**
         * @description -This method returns the number of payments, the total
         *              payment amount made as on a particular date for a list of
         *              ProjectCode ids that is passed. NOTE - ASSUMPTION IS EJVs
         *              don't have partial payments and CJVs have only 1 project
         *              code on debit side.
         * @param entityList
        ...
        public Map<String, BigDecimal> getPaymentInfoforProjectCode(List<Long> projectCodeIdList, Date asOnDate) throws ApplicationException {
                ...
                List<List<Long>> limitedEntityList = new ArrayList<List<Long>>();
                ...
                String validationQuery = "SELECT detailkey FROM accountdetailkey WHERE detailtypeid= (SELECT id FROM accountdetailtype "
                                + "WHERE name ='PROJECTCODE' AND description='PROJECTCODE' ) and detailkey in (";
                List<BigDecimal> dbEntIdList = new ArrayList<BigDecimal>();
                boolean isPresent;
                List<Long> incorrectEntityIds = new ArrayList<Long>();
                String dbEntIdQuery;
                if(LOGGER.isDebugEnabled())     LOGGER.debug(" Validation Starts ");
                for (int i = 0; i < commaSeperatedEntitiesList.size(); i++) {
                        ...
                        dbEntIdList = (List<BigDecimal>) HibernateUtil.getCurrentSession().createSQLQuery(dbEntIdQuery).list();
                        if (dbEntIdList != null         && dbEntIdList.size() != limitedEntityList.get(i).size()) {
                                for (Long entId : limitedEntityList.get(i)) {
                                        isPresent = false;
                ...
                for (int i = 0; i < commaSeperatedEntitiesList.size(); i++) {
                        ...
                        if(LOGGER.isDebugEnabled())     LOGGER.debug(i + ": qryForExpense- " + qryForExpense);
                        if(LOGGER.isDebugEnabled())     LOGGER.debug(i + ": qryForNonExpense- " + qryForNonExpense);
        ...
        /**
         * @description -This method returns  the total payment amount and payment count made till date for a list of Project codes that is passed.
         * @param entityList
         *            - Integer list containing ProjectCode ids.
        ...
        public  Map<String, BigDecimal> getTotalPaymentforProjectCode(List<Long> projectCodeIdList) throws ApplicationException {
                ...
                for (int i = 0; i < commaSeperatedEntitiesList.size(); i++) {
                                String stringIdsList=commaSeperatedEntitiesList.get(i);
                                if(i!=0)
                ...
                String payQuery="SELECT nvl(sum(mb.paidamount),0),count(vh1.id) FROM    miscbilldetail mb,voucherheader vh1 "
                                ...
                                +"WHERE name ='PROJECTCODE' AND description='PROJECTCODE' ) and (bp.ACCOUNTDETAILKEYID in("+projectCodeListCondition+")))";
                
                if(LOGGER.isDebugEnabled())     LOGGER.debug("Final payQuery - " + payQuery);
        ...
        /**
         * @description -This method returns the number of payments, the total
         *              payment amount made, department wise as on a particular date for a list of
         *              ProjectCode ids that is passed. NOTE - ASSUMPTION IS EJVs
         *              don't have partial payments and CJVs have only 1 project
         *              code on debit side.
         * @param entityList
         *            - Integer list containing ProjectCode ids.
         * @param asOnDate
         *            - The payments are considered from the beginning to asOnDate. Only fully approved payments are considered.
         *            (including asOnDate)
        ...
        public Map<String, String> getPaymentInfoforProjectCodeByDepartment(List<Long> projectCodeIdList, Date asOnDate) throws ApplicationException {
                ...
                for (int i = 0; i < commaSeperatedEntitiesList.size(); i++) {
                        isPresent = false;
                        dbEntIdQuery = validationQuery + commaSeperatedEntitiesList.get(i);
                        if(i!=0)
                                dbEntIdQuery=dbEntIdQuery+ ") or detailkey in(";
                        if(LOGGER.isDebugEnabled())     LOGGER.debug(i + ":dbEntIdQuery- " + dbEntIdQuery);
                        if(i==commaSeperatedEntitiesList.size()-1)
                                dbEntIdQuery=dbEntIdQuery+ ")) order by detailkey ";
                }
                if(LOGGER.isDebugEnabled())     LOGGER.debug("Final Query- " + dbEntIdQuery);
                        dbEntIdList = (List<BigDecimal>) HibernateUtil.getCurrentSession().createSQLQuery(dbEntIdQuery).list();
                        if (dbEntIdList != null         && dbEntIdList.size() != limitedEntityList.size()) {
                                for (int i = 0; i < commaSeperatedEntitiesList.size(); i++) {
                                for (Long entId : limitedEntityList.get(i)) {
                                        isPresent = false;
                                        for (BigDecimal dbEntId : dbEntIdList) {
        ...
        /**
         * This method returns the total payment amount of all uncancelled payments
         * for a particular billId.
         *
         * @param billId
        ...
        public BigDecimal getPaymentAmount(Long billId) throws ApplicationException {
                ...
                List<BigDecimal> paymentAmount = (List<BigDecimal>) 
                                HibernateUtil.getCurrentSession().createSQLQuery(sqlQuery).list();
                return (paymentAmount.get(0) == null ? BigDecimal.ZERO : paymentAmount
                                .get(0));
        }

        private String getPaymentInfoQuery(String stringIdsList, String strDate,
                        boolean isExpenseType) {
                String qryForExpense = "select nvl(sum(amt),0), count(*) from ( select sum("
        ...
        private String getPaymentAmountByDept(String stringIdsList, String strDate,
                        boolean isExpenseType) {
                String qryForExpense = "select sum("
                                + (isExpenseType ? "gd.amount" : "m.paidamount")
                                + ") as amt,"
        ...
        /**
         * This API returns List of Map, Map containing Amount, VoucherNumber,
         * VoucherDate, BillNumber & BillId of passed projectcode and till passed
         * date. Vouchers selected accross funds and financial year.
         *
         * @author chetan
         * @param projectCodeId
         * @param asOnDate
        ...
        public List<Map<String, String>> getExpenditureDetailsforProject(
                ...
                String queryForGLList = "SELECT gld.amount, vh.id, vh.voucherNumber, vh.voucherDate, egmis.billid, egbill.billnumber "
                ...
                if(LOGGER.isDebugEnabled())     LOGGER.debug("queryForGLList >> " + queryForGLList);
                List<Object[]> generalLedgerList = HibernateUtil.getCurrentSession()
                                .createSQLQuery(queryForGLList).list();
                for (Object[] objects : generalLedgerList) {
                        if(LOGGER.isInfoEnabled())     LOGGER.info("Project code has vouchers.");
                ...
                if(LOGGER.isDebugEnabled())     LOGGER.debug("Completed getExpenditureDetailsforProject.");
                return result;
        }

        /**
         * This API returns List of Map, Map containing Amount, VoucherNumber,
         * VoucherDate, BillNumber & BillId of passed Depositcode and till passed
         * date. Vouchers selected accross funds and financial year.
         *
         * @author chetan
         * @param depositCodeId
        ...
        public List<Map<String, String>> getExpenditureDetailsforDepositCode(
                ...
                List<Object[]> generalLedgerList = HibernateUtil.getCurrentSession()
                                .createSQLQuery(queryForGLList).list();
                for (Object[] objects : generalLedgerList) {
                        if(LOGGER.isInfoEnabled())     LOGGER.info("Deposit code has vouchers.");
                        Map<String, String> mp = new HashMap<String, String>();
                        if (objects[0].toString() != null)
                                mp.put("Amount", objects[0].toString());
                        if (objects[2] != null && objects[3] != null) {
                ...
                if(LOGGER.isDebugEnabled())     LOGGER.debug("Completed getExpenditureDetailsforDepositCode.");
                return result;
        }

        /**
         * This API returns List of Map, Map containing Amount, VoucherNumber,
         * VoucherDate, BillNumber & BillId of passed projectcode and. Vouchers
         * selected accross funds. Vouchers selected within asOnDate Financial Year.
         *
         * @author chetan
         * @param projectCodeId
         * @param asOnDate
        ...
        public List<Map<String, String>> getExpenditureDetailsforProjectforFinYear(
                ...
                if(LOGGER.isDebugEnabled())     LOGGER.debug("Completed getExpenditureDetailsforProjectforFinYear.");
                return result;
        }
        
        
        /**
         * finds the sum  of debit amount of all active Journal vouchers ie is VoucherType is 'Journal Voucher'
         * for the provided list of detailkeyIds.
         * Vouchers selected across funds and financial year.
         *  
         * @author mani
         * @param detailTypeId
        ...
        public BigDecimal getVoucherExpenditureByEntities(Integer detailTypeId,List<Integer> entityIdList)
        {
                ...
                if(detailTypeId == null || entityIdList == null || entityIdList.size() == 0){
                        throw new  ValidationException("DetailTypeId or EntityIdList not provided","DetailTypeId or EntityIdList not provided");
                }
                String query = "select sum(gld.amount) from CGeneralLedger gl, CGeneralLedgerDetail gld, CVoucherHeader vh "
                ...
                if(LOGGER.isDebugEnabled())     LOGGER.debug("query For getVoucherExpenditureByEntities >> " + query);
                
                Query expenditureQuery = HibernateUtil.getCurrentSession().createQuery(query);
                
                expenditureQuery.setInteger("detailTypeId",detailTypeId);
                expenditureQuery.setParameterList("entityIdList",entityIdList);
                List<Object> result = expenditureQuery.list();
                if(result!=null){
        ...
        /**
         * Finds the sum  of debit amount of all approved Direct Bank Payments vouchers ie is VoucherType is 'Payment'
         * and voucher name is 'Direct Bank Payment'
         * for the provided list of detailkeyIds.
         * Vouchers selected across funds and financial year.
         *  
         * @author mani
        ...
        public BigDecimal       getDirectBankPaymentExpenditureByEntities(Integer detailTypeId,List<Integer> entityIdList)
        {
                ...
                if(LOGGER.isDebugEnabled())     LOGGER.debug("query For getDirectBankPaymentExpenditureByEntities >> " + query);

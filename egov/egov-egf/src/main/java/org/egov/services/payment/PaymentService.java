...
public class PaymentService extends PersistenceService<Paymentheader, Long> {
	...
	public Paymentheader createPayment(final Map<String, String[]> parameters,
		...
		try {
			...
			/*
			 * if (contractorids != null) conBillIdlength =
	...
	/**
	 * Partial payment is not allowed for netpayable subledger practically all
	 * bills will have single entity in netpayable Cbill is not allowed to make
	 * partial payment
	 * 
	 * @param ids
	 * @param parameters
	...
	private void updateVoucherHeader(final Map<String, String[]> parameters,
		 ...
		 * VoucherHelper.getGeneratedVoucherNumber(existingVH
			...
			for (final ChequeAssignment bean : billList) {
				...
				if (type.equals("Contractor"))
					...
					obj = (Object[]) persistenceService.findByNamedQuery(
							...
							purchaseBillGlcodeList);
				/*
				 * else if(type.equals(FinancialConstants.
	 ...
	 * @throws ApplicationException
	 *             this api check if the voucher contains subledger on credit
	 *             side and
	 */
	...
	public List<ChequeAssignment> getPaymentVoucherForRTGSInstrument(
		...
		/*
		 * EgBillSubType egSubType = new EgBillSubType(); egSubType =
		 * (EgBillSubType)
		 * persistenceService.find(" from EgBillSubType where name = ?",
		 * FinancialConstants.BILLSUBTYPE_TNEBBILL); if (egSubType.getId() !=
		 * null)
		 * sql.append(" and (bmis.billsubtype is null or bmis.billsubtype not in ("
		 * + egSubType.getId() + "))");
	 ...
	 * @return updates the generalledger remittancedate to null of bill voucher
	 *         when remittance payment is cancelled
	 */

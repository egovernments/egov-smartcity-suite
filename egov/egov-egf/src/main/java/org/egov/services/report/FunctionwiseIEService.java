...
public class FunctionwiseIEService  	
{
	...
	private String getFiltersExcludingDate(ReportSearch reportSearch,
			String excludeStatus, String appendQry) {
	...
	public void getMajorCodeList(final FunctionwiseIE functionwiseIE,ReportSearch reportSearch) throws EGOVException,ParseException
	{
		...
		String sql="select distinct SUBSTR(gl.glcode,1,"+reportSearch.getMinorCodeLen()+"),coa.name from CHARTOFACCOUNTS coa,GENERALLEDGER gl WHERE gl.functionid is not null and gl.voucherheaderid IN (SELECT vh.id FROM VOUCHERHEADER vh,vouchermis vmis WHERE vh.id=vmis.voucherheaderid "+filterQuery+" AND coa.TYPE='"+reportSearch.getIncExp()+"' AND SUBSTR(gl.glcode,1,"+reportSearch.getMajorCodeLen()+")=coa.glcode) "+getFilterQueryGL(reportSearch)+" ORDER BY 1";
	...
	public List<String> getMinorCodeList(ReportSearch reportSearch) throws EGOVException,ParseException
	{
		...
		String filterQuery=getFilterQueryVoucher(reportSearch);
		String sql="select distinct SUBSTR(gl.glcode,1,"+reportSearch.getMinorCodeLen()+"),coa.name from CHARTOFACCOUNTS coa,GENERALLEDGER gl WHERE gl.functionid is not null and gl.voucherheaderid IN (SELECT vh.id FROM VOUCHERHEADER vh,vouchermis vmis WHERE vh.id=vmis.voucherheaderid "+filterQuery+" AND coa.TYPE='"+reportSearch.getIncExp()+"' AND SUBSTR(gl.glcode,1,"+reportSearch.getMinorCodeLen()+")=coa.glcode) "+getFilterQueryGL(reportSearch)+" ORDER BY 1";
		if(LOGGER.isDebugEnabled())     LOGGER.debug("sql===================="+sql);
		Query query =HibernateUtil.getCurrentSession().createSQLQuery(sql);
		List<Object[]> list = query.list();
		for(Object[] obj : list)
	...
	public List<CommonReportBean> getMinorAndMajorCodeListForCapitalExp(ReportSearch reportSearch) throws EGOVException,ParseException
	{
		...
		}else
		{
			...
			" Union "+
			" select coa.glcode as accCode,coa.name as name,null as schedule,null as FIEscheduleId ,1 as isMajor from Chartofaccounts coa"+
			" where  coa.type=:type and length(coa.glcode)="+reportSearch.getMajorCodeLen()+ "and coa.glcode in ("+capExpCodesWithQuotesCond+")"+
			" order by 1";
		}
		Query query = HibernateUtil.getCurrentSession().createSQLQuery(sql)
												.addScalar("accCode",StringType.INSTANCE )
	...
	public void getAmountList(final FunctionwiseIE functionwiseIE,ReportSearch reportSearch) throws EGOVException,ParseException
	{
		...
		String sql="SELECT fn.code,fn.name,CONCAT(CONCAT(coa.majorcode,'-'),coa.name),DECODE('"+reportSearch.getIncExp()+"','I',(SUM(gl.creditamount)-SUM(gl.debitamount)),'E', (SUM(gl.debitamount)-SUM(gl.creditamount))) AS amt " +
				...
				" WHERE vh.id=vmis.voucherheaderid and vh.ID=gl.voucherheaderid AND SUBSTR(gl.glcode,1,"+reportSearch.getMajorCodeLen()+")=coa.glcode AND coa.TYPE='"+reportSearch.getIncExp()+"' " +
				" AND fn.id = gl.functionid "+getFilterQueryVoucher(reportSearch)+getFilterQueryGL(reportSearch)+" GROUP BY fn.code,fn.name,CONCAT(CONCAT(coa.majorcode,'-'),coa.name) order by 1,3";
		if(LOGGER.isDebugEnabled())     LOGGER.debug("sql==="+sql);
		Query query =HibernateUtil.getCurrentSession().createSQLQuery(sql);
		List<Object[]> list = query.list();
	...
	public List<CommonReportBean>  getAmountListForMinorCode(final FunctionwiseIE functionwiseIE,ReportSearch reportSearch) throws EGOVException,ParseException
	{
		...
		Query query=null;
			...
			if(reportSearch.getIncExp().equalsIgnoreCase("E"))
				sql="SELECT SUBSTR(coa.glcode,1,"+reportSearch.getMinorCodeLen()+") as accCode,d.dept_name as deptName ,(SUM(gl.debitamount)-SUM(gl.creditamount)) AS amount,0 as isMajor " ;
			else
				sql="SELECT SUBSTR(coa.glcode,1,"+reportSearch.getMinorCodeLen()+") as accCode,d.dept_name as deptName ,(SUM(gl.creditamount)-SUM(gl.debitamount)) AS amount,0 as isMajor " ;
			
			if(reportSearch.getIncExp().equalsIgnoreCase("E")){
				sql=sql+	" FROM GENERALLEDGER gl,FUNCTION fn,VOUCHERHEADER vh, CHARTOFACCOUNTS coa,vouchermis vmis,eg_department d  " +
				" WHERE vh.id=vmis.voucherheaderid and vh.ID=gl.voucherheaderid AND SUBSTR(gl.glcode,1,"+reportSearch.getMinorCodeLen()+")=coa.glcode AND (coa.TYPE='"+reportSearch.getIncExp()+"' "+capExpCodeCond+")" +
				" and d.dept_name=:deptName and coa.glcode like :glcode AND fn.id = gl.functionid  and vmis.departmentid=d.id_dept "+getFilterQueryVoucherAsOnDate(reportSearch)+getFilterQueryGL(reportSearch)+" GROUP BY  SUBSTR(coa.glcode,1,"+reportSearch.getMinorCodeLen()+"),d.dept_name ";				
			}else{
				sql=sql+	" FROM GENERALLEDGER gl,FUNCTION fn,VOUCHERHEADER vh, CHARTOFACCOUNTS coa,vouchermis vmis,eg_department d  " +
				" WHERE vh.id=vmis.voucherheaderid and vh.ID=gl.voucherheaderid AND SUBSTR(gl.glcode,1,"+reportSearch.getMinorCodeLen()+")=coa.glcode AND coa.TYPE='"+reportSearch.getIncExp()+"' " +
				" and d.dept_name=:deptName and coa.glcode like :glcode AND fn.id = gl.functionid  and vmis.departmentid=d.id_dept "+getFilterQueryVoucherAsOnDate(reportSearch)+getFilterQueryGL(reportSearch)+" GROUP BY  SUBSTR(coa.glcode,1,"+reportSearch.getMinorCodeLen()+"),d.dept_name ";
			}
				sql=sql+"order by 2,1 ";
				 query =HibernateUtil.getCurrentSession().createSQLQuery(sql).addScalar("accCode",StringType.INSTANCE).
			...
			}else if(reportSearch.getByDepartment())
			{
				...
				if(reportSearch.getIncExp().equalsIgnoreCase("E")){
					sql=sql+" FROM GENERALLEDGER gl,FUNCTION fn,VOUCHERHEADER vh, CHARTOFACCOUNTS coa,vouchermis vmis,eg_department d  " +
					" WHERE vh.id=vmis.voucherheaderid and vh.ID=gl.voucherheaderid AND SUBSTR(gl.glcode,1,"+reportSearch.getMinorCodeLen()+")=coa.glcode AND (coa.TYPE='"+reportSearch.getIncExp()+"'"+capExpCodeCond+")" +
					" AND fn.id = gl.functionid  and vmis.departmentid=d.id_dept "+getFilterQueryVoucherAsOnDate(reportSearch)+getFilterQueryGL(reportSearch)+" GROUP BY  SUBSTR(coa.glcode,1,"+reportSearch.getMinorCodeLen()+"),d.dept_name ";
				}else{
					sql=sql+" FROM GENERALLEDGER gl,FUNCTION fn,VOUCHERHEADER vh, CHARTOFACCOUNTS coa,vouchermis vmis,eg_department d  " +
					" WHERE vh.id=vmis.voucherheaderid and vh.ID=gl.voucherheaderid AND SUBSTR(gl.glcode,1,"+reportSearch.getMinorCodeLen()+")=coa.glcode AND coa.TYPE='"+reportSearch.getIncExp()+"' " +
					" AND fn.id = gl.functionid  and vmis.departmentid=d.id_dept "+getFilterQueryVoucherAsOnDate(reportSearch)+getFilterQueryGL(reportSearch)+" GROUP BY  SUBSTR(coa.glcode,1,"+reportSearch.getMinorCodeLen()+"),d.dept_name ";
				...
				if(reportSearch.getIncExp().equalsIgnoreCase("E")){
					...
					" WHERE vh.id=vmis.voucherheaderid and vh.ID=gl.voucherheaderid AND SUBSTR(gl.glcode,1,"+reportSearch.getMajorCodeLen()+")=coa.glcode AND (coa.TYPE='"+reportSearch.getIncExp()+"'"+capExpCodeCond+")" +
					" AND fn.id = gl.functionid "+getFilterQueryVoucherAsOnDate(reportSearch)+getFilterQueryGL(reportSearch)+" GROUP BY coa.majorcode,coa.name ";
				}else{
					sql=sql+" FROM GENERALLEDGER gl,FUNCTION fn,VOUCHERHEADER vh, CHARTOFACCOUNTS coa,vouchermis vmis  " +
					" WHERE vh.id=vmis.voucherheaderid and vh.ID=gl.voucherheaderid AND SUBSTR(gl.glcode,1,"+reportSearch.getMajorCodeLen()+")=coa.glcode AND coa.TYPE='"+reportSearch.getIncExp()+"' " +
					" AND fn.id = gl.functionid "+getFilterQueryVoucherAsOnDate(reportSearch)+getFilterQueryGL(reportSearch)+" GROUP BY coa.majorcode,coa.name ";
				...
				if(reportSearch.getIncExp().equalsIgnoreCase("E"))
					sql=sql+" Union SELECT SUBSTR(coa.glcode,1,"+reportSearch.getMinorCodeLen()+") as accCode, coa.name  as accName,(SUM(gl.debitamount)-SUM(gl.creditamount)) AS amount ,0 as isMajor " ;
				else
					sql=sql+" Union SELECT SUBSTR(coa.glcode,1,"+reportSearch.getMinorCodeLen()+") as accCode, coa.name  as accName,(SUM(gl.creditamount)-SUM(gl.debitamount)) AS amount ,0 as isMajor " ;
					
				
				if(reportSearch.getIncExp().equalsIgnoreCase("E")){
					sql=sql+	" FROM GENERALLEDGER gl,FUNCTION fn,VOUCHERHEADER vh, CHARTOFACCOUNTS coa,vouchermis vmis " +
					" WHERE vh.id=vmis.voucherheaderid and vh.ID=gl.voucherheaderid AND SUBSTR(gl.glcode,1,"+reportSearch.getMinorCodeLen()+")=coa.glcode AND (coa.TYPE='"+reportSearch.getIncExp()+"'"+capExpCodeCond+")" +
					" AND fn.id = gl.functionid "+getFilterQueryVoucherAsOnDate(reportSearch)+getFilterQueryGL(reportSearch)+" GROUP BY SUBSTR(coa.glcode,1,"+reportSearch.getMinorCodeLen()+"),coa.name order by 1,2 ";
				}else{
					sql=sql+	" FROM GENERALLEDGER gl,FUNCTION fn,VOUCHERHEADER vh, CHARTOFACCOUNTS coa,vouchermis vmis " +
					" WHERE vh.id=vmis.voucherheaderid and vh.ID=gl.voucherheaderid AND SUBSTR(gl.glcode,1,"+reportSearch.getMinorCodeLen()+")=coa.glcode AND coa.TYPE='"+reportSearch.getIncExp()+"' " +
					" AND fn.id = gl.functionid "+getFilterQueryVoucherAsOnDate(reportSearch)+getFilterQueryGL(reportSearch)+" GROUP BY SUBSTR(coa.glcode,1,"+reportSearch.getMinorCodeLen()+"),coa.name order by 1,2 ";
		...
		if(LOGGER.isDebugEnabled())     LOGGER.debug("sql==="+sql);
		List<CommonReportBean> list =(List<CommonReportBean>) query.list();
		return list;
	...
	public List<CommonReportBean>  getPreviousYearAmountListForMinorCode(final FunctionwiseIE functionwiseIE,ReportSearch reportSearch) throws EGOVException,ParseException
	{
		...
		sql=sql+	" FROM GENERALLEDGER gl,FUNCTION fn,VOUCHERHEADER vh, CHARTOFACCOUNTS coa,vouchermis vmis,eg_department d  " +
			" WHERE vh.id=vmis.voucherheaderid and vh.ID=gl.voucherheaderid AND SUBSTR(gl.glcode,1,"+reportSearch.getMinorCodeLen()+")=coa.glcode AND coa.TYPE='"+reportSearch.getIncExp()+"' " +
			" and d.dept_name=:deptName and coa.glcode like :glcode AND fn.id = gl.functionid  and vmis.departmentid=d.id_dept "+getFilterQueryVoucherAsOnPreviousYearDate(reportSearch)+getFilterQueryGL(reportSearch)+" GROUP BY  SUBSTR(coa.glcode,1,"+reportSearch.getMinorCodeLen()+"),d.dept_name ";
			sql=sql+"order by 2,1 ";
			 query =HibernateUtil.getCurrentSession().createSQLQuery(sql).addScalar("accCode",StringType.INSTANCE).
			addScalar("amount",BigDecimalType.INSTANCE)
			.addScalar("isMajor", BooleanType.INSTANCE)
		...
		}else if(reportSearch.getByDepartment())
		{
			...
			sql=sql+" FROM GENERALLEDGER gl,FUNCTION fn,VOUCHERHEADER vh, CHARTOFACCOUNTS coa,vouchermis vmis,eg_department d  " +
			" WHERE vh.id=vmis.voucherheaderid and vh.ID=gl.voucherheaderid AND SUBSTR(gl.glcode,1,"+reportSearch.getMinorCodeLen()+")=coa.glcode AND coa.TYPE='"+reportSearch.getIncExp()+"' " +
			" AND fn.id = gl.functionid  and vmis.departmentid=d.id_dept "+getFilterQueryVoucherAsOnPreviousYearDate(reportSearch)+getFilterQueryGL(reportSearch)+" GROUP BY  SUBSTR(coa.glcode,1,"+reportSearch.getMinorCodeLen()+"),d.dept_name ";
			sql=sql+"order by 2,1 ";
			 query =HibernateUtil.getCurrentSession().createSQLQuery(sql).addScalar("accCode",StringType.INSTANCE).
			addScalar("amount",BigDecimalType.INSTANCE)
			.addScalar("isMajor", BooleanType.INSTANCE)
			...
			sql=sql+" FROM GENERALLEDGER gl,FUNCTION fn,VOUCHERHEADER vh, CHARTOFACCOUNTS coa,vouchermis vmis  " +
			" WHERE vh.id=vmis.voucherheaderid and vh.ID=gl.voucherheaderid AND SUBSTR(gl.glcode,1,"+reportSearch.getMajorCodeLen()+")=coa.glcode AND coa.TYPE='"+reportSearch.getIncExp()+"' " +
			" AND fn.id = gl.functionid "+getFilterQueryVoucherAsOnPreviousYearDate(reportSearch)+getFilterQueryGL(reportSearch)+" GROUP BY coa.majorcode,coa.name ";
			if(reportSearch.getIncExp().equalsIgnoreCase("E"))
				sql=sql+" Union SELECT SUBSTR(coa.glcode,1,"+reportSearch.getMinorCodeLen()+") as accCode, coa.name  as accName,(SUM(gl.debitamount)-SUM(gl.creditamount)) AS amount ,0 as isMajor " ;
			else
				sql=sql+" Union SELECT SUBSTR(coa.glcode,1,"+reportSearch.getMinorCodeLen()+") as accCode, coa.name  as accName,(SUM(gl.creditamount)-SUM(gl.debitamount)) AS amount ,0 as isMajor " ;
		...
		sql=sql+	" FROM GENERALLEDGER gl,FUNCTION fn,VOUCHERHEADER vh, CHARTOFACCOUNTS coa,vouchermis vmis " +
			" WHERE vh.id=vmis.voucherheaderid and vh.ID=gl.voucherheaderid AND SUBSTR(gl.glcode,1,"+reportSearch.getMinorCodeLen()+")=coa.glcode AND coa.TYPE='"+reportSearch.getIncExp()+"' " +
			" AND fn.id = gl.functionid "+getFilterQueryVoucherAsOnPreviousYearDate(reportSearch)+getFilterQueryGL(reportSearch)+" GROUP BY SUBSTR(coa.glcode,1,"+reportSearch.getMinorCodeLen()+"),coa.name order by 1,2 ";
			query =HibernateUtil.getCurrentSession().createSQLQuery(sql).addScalar("accCode",StringType.INSTANCE).
			addScalar("amount",BigDecimalType.INSTANCE)
			.addScalar("isMajor", BooleanType.INSTANCE)
			.setResultTransformer(Transformers.aliasToBean(CommonReportBean.class));
	...
	public  List<CommonReportBean> populateDataWithBudget(final FunctionwiseIE functionwiseIE,ReportSearch reportSearch)throws EGOVException,ParseException
	{
		...
		for (int i = 0; i < temp.length; i++) {
			capExpCodes = capExpCodes+" or coa.glcode like '"+temp[i]+"%'";
		}
		...
		if(reportSearch.getIncExp().equalsIgnoreCase("E")){
			List<CommonReportBean> minorAndMajorCodeListForCapitalExp = getMinorAndMajorCodeListForCapitalExp(reportSearch);
			minorAndMajorCodeList.addAll(minorAndMajorCodeListForCapitalExp);
		}
		if(LOGGER.isDebugEnabled())     LOGGER.debug("deptName\t Acccode\t Name\t Amount");
		List<CommonReportBean> amountListForMinorCode = getAmountListForMinorCode(functionwiseIE,reportSearch);
		List<CommonReportBean> amountPreviousyearListForMinorCode = getPreviousYearAmountListForMinorCode(functionwiseIE,reportSearch);
	//	if(LOGGER.isDebugEnabled())     LOGGER.debug("amountListForMinorCode---------------------------------------------------------------------------------------");
	    print(amountListForMinorCode);
		StringBuffer queryStr = getBudgetQueryForMinorCodes(reportSearch);
		List<CommonReportBean> beAmountListForMinorCode = getBudgetAmountListForMinorCode(reportSearch,"BE",queryStr.toString());
	//	if(LOGGER.isDebugEnabled())     LOGGER.debug("beAmountListForMinorCode---------------------------------------------------------------------------------------");
		print(beAmountListForMinorCode);
		List<CommonReportBean> reAmountListForMinorCode = getBudgetAmountListForMinorCode(reportSearch,"RE",queryStr.toString());
	//	if(LOGGER.isDebugEnabled())     LOGGER.debug("reAmountListForMinorCode---------------------------------------------------------------------------------------");
		print(reAmountListForMinorCode);
		StringBuffer reappQueryStr = getBudgetReappQueryForMinorCodes(reportSearch);
		List<CommonReportBean> beappAmountListForMinorCode = getBudgetApprAmountListForMinorCode(reportSearch,"BE",reappQueryStr.toString());
		List<CommonReportBean> reappAmountListForMinorCode = getBudgetApprAmountListForMinorCode(reportSearch,"RE",reappQueryStr.toString());
		if(LOGGER.isDebugEnabled())     LOGGER.debug("beappAmountListForMinorCode---------------------------------------------------------------------------------------");
		print(beappAmountListForMinorCode);
		//two logic here 1. put to linked hashmap and get 
		//2. merge by comparing
		Map<String, CommonReportBean> minorCodeAmountMap = loadIntoMap(amountListForMinorCode,reportSearch);
		Map<String, CommonReportBean> beAmountMap = loadIntoMap(beAmountListForMinorCode,reportSearch);
		Map<String, CommonReportBean> reAmountMap = loadIntoMap(reAmountListForMinorCode,reportSearch);
		Map<String, CommonReportBean> reAppAmountMap = loadIntoMap(reappAmountListForMinorCode,reportSearch);
		Map<String, CommonReportBean> beAppAmountMap = loadIntoMap(beappAmountListForMinorCode,reportSearch);
		Map<String, CommonReportBean> prevousAmountMap = loadIntoMap(amountPreviousyearListForMinorCode,reportSearch);
		
		List<CommonReportBean>  resultList=new ArrayList<CommonReportBean>();
		...
		if(reportSearch.getByDepartment()&&!reportSearch.getByDetailCode())
		{
			List<CommonReportBean> deptWiseWithBudgetList=new ArrayList<CommonReportBean>();
			  ...
			  if(pyglCRB!=null)
				 {
					 ...
					 if(pyglCRB.getIsMajor()==false)
					  pyAmountSum=pyglCRB.getAmount()==null?pyAmountSum.add(BigDecimal.ZERO):pyAmountSum.add(pyglCRB.getAmount());
				 }
				 if(beCRB!=null)
				 {
				 ...
				 if(reAppCRB!=null)
				 {
				  ...
				  if(reAppCRB.getIsMajor()==false)
					  reAppSum=reAppCRB.getAmount()==null?reAppSum.add(BigDecimal.ZERO):reAppSum.add(reAppCRB.getAmount());
				 }
				 if(beAppCRB!=null)
				 {
			...
			for(Department dept:reportSearch.getDeptList())
			{
				for(CommonReportBean bean:minorAndMajorCodeList)
		        {
				 ...
				 if(pyglCRB!=null)
				 {
				   crb.setPyAmount(pyglCRB.getAmount());
				   if(pyglCRB.getIsMajor()==false)
					   previousSum=pyglCRB.getAmount()==null?previousSum.add(BigDecimal.ZERO):previousSum.add(pyglCRB.getAmount());
				 }
				 if(objBE!=null)
				 {
	...
	private Map<String,CommonReportBean> loadIntoMap(List<CommonReportBean> amountList,ReportSearch reportSearch) {
		Map<String,CommonReportBean> amountMap=new LinkedHashMap<String, CommonReportBean>();
	...
	private void print(List<CommonReportBean> crbList) {
		...
		for(CommonReportBean crb:crbList)
		   {
			  if(!crb.isZero())
			   if(LOGGER.isDebugEnabled())     LOGGER.debug(crb.toString());
		   }
		}
	}
	...
	private List<CommonReportBean> getBudgetApprAmountListForMinorCode(ReportSearch reportSearch,String isBeRe,String queryStr)
	{
		...
		if(reportSearch.getAsOnDate().getMonth()==2 && reportSearch.getAsOnDate().getDate()==31){
			...
			{
				query = HibernateUtil.getCurrentSession().createSQLQuery(queryStr)
				.addScalar("accCode",StringType.INSTANCE )
				.addScalar("amount",BigDecimalType.INSTANCE)
				.addScalar("isMajor",BooleanType.INSTANCE)
				.addScalar("deptName",StringType.INSTANCE)
	...
	private StringBuffer getBudgetQueryForMinorCodes(ReportSearch reportSearch) {
		...
		queryStr.append(
				" select SUBSTR(coa.glcode,1,"+reportSearch.getMinorCodeLen()+") as accCode, sum(bd.approvedamount) as amount ,0 as isMajor ");
		if(reportSearch.getByDepartment())
		{
		...
		if(reportSearch.getIncExp().equals("E"))
				queryStr.append(	" and (coa.type='E'"+capExpCodeCond+") group by SUBSTR(coa.glcode,1,"+reportSearch.getMinorCodeLen()+")");
		else 
		...
		if(reportSearch.getIncExp().equals("E"))
			queryStr.append(" and (coa.type='E'"+capExpCodeCond+") and coa.majorcode is not null  group by coa.majorCode ");
		else 
	...
	private StringBuffer getBudgetReappQueryForMinorCodes(ReportSearch reportSearch) {
		StringBuffer queryStr=new StringBuffer(1024);
		queryStr.append(
				" select SUBSTR(coa.glcode,1,"+reportSearch.getMinorCodeLen()+") as accCode, sum(bdr.addition_amount- bdr.deduction_amount) as amount ,0 as isMajor ");
		if(reportSearch.getByDepartment())
		{
			queryStr.append(",d.dept_name  as deptName ");
		}
		...
		queryStr.append(" and (coa.type='E'"+capExpCodeCond+") group by SUBSTR(coa.glcode,1,"+reportSearch.getMajorCodeLen()+")");
		if(reportSearch.getByDepartment())  
		{

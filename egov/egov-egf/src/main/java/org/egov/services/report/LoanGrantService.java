...
public class LoanGrantService extends PersistenceService {
	...
	public List<Object> schemeUtilizationBy(Integer schemeId, Integer subSchemeId, Date fromDate, Date toDate, List<Integer> projectCodeIdList, Integer fundId) {
		Accountdetailtype detailType = (Accountdetailtype) find("from Accountdetailtype where upper(name)=?", "PROJECTCODE");
		...
		if (subSchemeId != null) {
			schemeUtilSql.append( "select ss.name as subScheme, pc.code as code, vh.voucherNumber as vouchernumber, vh.voucherDate as voucherdate, gld.amount as amount, gld.detailkeyid as id "
					+ "from voucherheader vh, generalledger gl, generalledgerdetail gld , egf_subscheme_project ssp, egw_projectcode pc"
					+ ",sub_scheme ss "
					+ "	where " + " vh.id= gl.voucherheaderid  and vh.status not in (1,2,4) " + " and gl.id= gld.generalledgerid ");
			if (fromDate != null)
			...
			schemeUtilSql.append(" and gld.detailtypeid=" + detailType.getId() + " and gld.detailkeyid= ssp.projectcodeid "
					+ " and ssp.subschemeid=" + subSchemeId + " and ss.id=" + subSchemeId + " and ss.id=ssp.subschemeid" + " and pc.id= gld.detailkeyid "
			...
			schemeUtilSql.append("select ss.name as subScheme, pc.code as code, vh.voucherNumber as vouchernumber, vh.voucherDate as voucherdate, gld.amount as amount, gld.detailkeyid as id "
					+ "from voucherheader vh, generalledger gl, generalledgerdetail gld , egf_subscheme_project ssp, egw_projectcode pc"
					+ ",sub_scheme ss,scheme s " + "	where " + " vh.id= gl.voucherheaderid  and vh.status not in (1,2,4) " + " and gl.id= gld.generalledgerid ");
			if (fromDate != null)
				schemeUtilSql.append( " and vh.voucherdate>='" + Constants.DD_MON_YYYYFORMAT.format(fromDate) + "'");
			if (toDate != null)  
				schemeUtilSql.append(" and vh.voucherdate<='" + Constants.DD_MON_YYYYFORMAT.format(toDate) + "'");
			schemeUtilSql .append(" and gld.detailtypeid=" + detailType.getId() + " and gld.detailkeyid= ssp.projectcodeid "
					+ " and ssp.subschemeid=ss.id" + " and ss.schemeid=s.id" + " and s.id=" + schemeId + " and ss.id=ssp.subschemeid"
		...
		List<Object> projecCodeResultList = schemeUtilQry.list();
	...
	public List<LoanGrantBean> fundingPatternBy(Integer subSchemeId, Integer schemeId) {
		...
		SQLQuery patternSql = getSearchSession().createSQLQuery(sql.toString());
		patternSql.addScalar("subScheme", StringType.INSTANCE).addScalar("name", StringType.INSTANCE).addScalar("amount", BigDecimalType.INSTANCE)
	 ...
	 * @return List<LoanGrantBean>
	 * SCHEME     SUBSCHEME     FUNDINGAGENCY    RESULT
	 *  YES          YES              YES        Get the GJVs having both FA And PC as subledgers and PC ids are mapped for the SUBSCHEME
	 *                                           and for the selected FA
	 *                                            
	 *  YES           NO              YES        Get the GJVs having both FA And PC as subledgers and PC ids are mapped for the   the
	 *  										 SUBSCHEMES under selected SCHEME and for the selected FA
	 *  
	 *  YES           YES             NO         Get the GJVs having both FA And PC as subledgers and PC ids are mapped for the  
	 *                                           SUBSCHEME    and for all FA under the selected SUBSCHEME
	 *                                               
	 *  YES           NO              NO         Get the GJVs having both FA And PC as subledgers and PC ids are mapped for the  
	 *  										 SUBSCHEME  under selected Scheme  and for all FA under the selected SCHEME
	 *  NOTE: 1.If  more than one FA is there in the JV the AgencyAmount will not match the projectwiseAmount since report is by Agency
	 *        2.If a GJV uses  project codes of different SUBSCHEMES report by scheme only will give the correct result
	 *        3.If a GJV uses project codes of Different schemes itself then Total amount will never match since widest search is by Scheme
	 *              
	 *          
	 */
	@SuppressWarnings("unchecked")
	public List<Object> searchGC(Integer schemeId, Integer subSchemeId, Date fromDate, Date toDate, Long agencyId, Integer pcTypeId, Integer faTypeId, Integer fundId) {
		if(LOGGER.isDebugEnabled())     LOGGER.debug("Starting searchGC");
		...
		if (agencyId != null & agencyId != -1) {
			...
			if (subSchemeId != null) {
				fundingAgencyList = findAllBy("select distinct fundingAgency.id from LoanGrantDetail lgd  where lgd.header.subScheme.id=? ", subSchemeId);
			}
			else {
			...
			for (Long faId : fundingAgencyList) {
				...
				if (subSchemeId != null) {
					grantContribListByAgency = getDataByAgency(schemeId, subSchemeId, fromDate, toDate, faId, pcTypeId, faTypeId, fundId);
				}
				else {
		...
		if(LOGGER.isDebugEnabled())     LOGGER.debug("exiting from searchGC  and result size"+grantContribList.size());
	 ...
	 * @return
	 */
	private List<Object> getDataByAgency(Integer schemeId, Integer subSchemeId, Date fromDate, Date toDate, Long agencyId, 
			Integer pcTypeId, Integer faTypeId, Integer fundId) {
		if(LOGGER.isDebugEnabled())     LOGGER.debug("Starting getDataByAgency for agencyId:"+agencyId );
		StringBuffer sql = new StringBuffer();
		sql
...
// check here if u have errors
		...
		sql.append(" where vh.id= gl1.voucherheaderid and gl1.id= gld1.generalledgerid "
				+ "and gl2.id=gld2.generalledgerid and gl2.voucherheaderid=vh.id and gl1.creditamount>0 "
				+ "and gl2.debitamount>0 and gld1.detailtypeid="+pcTypeId+" and gld2.detailtypeid="+faTypeId
				+ "and vh.type='Journal Voucher' and vh.name='JVGeneral' and lg.id=lgd.headerid and ssp.projectcodeid=gld1.detailkeyid and vh.fundid="+fundId+" ");
		if (subSchemeId != null)
		...
		List<Object> gcList = gcSql.list();
		if (gcList.size() > 0) {
			List<LoanGrantBean> grantAmountList = getGrantAmountBy(schemeId, subSchemeId, agencyId);
			...
			List<LoanGrantBean> grantAmountList = getGrantAmountBy(schemeId, subSchemeId, agencyId);
			if (grantAmountList != null && grantAmountList.size() > 0) {
				if(grantAmountList.get(0).getGrantAmount()!=null && grantAmountList.get(0).getGrantAmount().compareTo(BigDecimal.ZERO)!=0)
				{
		...
		if(LOGGER.isDebugEnabled())     LOGGER.debug("exiting getDataByAgency " );
		return gcList;
	}
	...
	private List<LoanGrantBean> getGrantAmountBy(Integer schemeId, Integer subSchemeId, Long agencyId) {
		if(LOGGER.isDebugEnabled())     LOGGER.debug("Starting getGrantAmountBy for"+agencyId);
		...
		if(LOGGER.isInfoEnabled())     LOGGER.info("GrantAmoountSql for Schemeid" + schemeId + " SubSchemeId " + subSchemeId + "  agencyId" + agencyId + ":" + gaSql.toString());
		SQLQuery gaSQLQuery = getSearchSession().createSQLQuery(gaSql.toString());
		gaSQLQuery.addScalar("agencyName").addScalar("grantAmount", BigDecimalType.INSTANCE).setResultTransformer(Transformers.aliasToBean(LoanGrantBean.class));
		List<LoanGrantBean> galist = gaSQLQuery.list();
		if(LOGGER.isDebugEnabled())     LOGGER.debug("Exiting from  getGrantAmountBy for"+agencyId);
		return galist;
	...
	public List<Object> getLoanBy(Integer schemeId, Long agencyId,Integer faTypeId, Integer fundId) {
		...
		}else
		{
			...
			List<Long> agencyList  = findAllBy(ss.toString(),schemeId);
	...
	public List<Object> getLoanByAgency(Integer schemeId, Long agencyId,Integer faTypeId, Integer fundId) {
		...
		sql.append("SELECT DISTINCT vh.vouchernumber AS voucherNumber,  gld.amount    AS amount,  "+
				   " gld.detailkeyid               AS detailKey,   gld.detailtypeid detailType ,vh.voucherdate  "+
					...
					SQLQuery loanSql=getSearchSession().createSQLQuery(sql.toString());
					if(LOGGER.isDebugEnabled())     LOGGER.debug("getLoanByAgency sql:"+sql.toString());
					loanSql.addScalar("voucherNumber")
					       ...
					       .setResultTransformer(Transformers.aliasToBean(LoanGrantBean.class));
			List<Object> repayedList = loanSql.list();
				
				List<LoanGrantBean> loanAmountList = getLoanAmountBy(schemeId,agencyId);
				if (loanAmountList != null && loanAmountList.size() > 0 ) {
					if(loanAmountList.get(0).getLoanAmount()!=null && loanAmountList.get(0).getLoanAmount().compareTo(BigDecimal.ZERO)!=0)
					{
					...
					BigDecimal loanPaidSoFar = getLoanPaidSoFar(schemeId,agencyId);
	...
	private List<LoanGrantBean> getLoanAmountBy(Integer schemeId, Long agencyId) {
			...
			if(LOGGER.isDebugEnabled())     LOGGER.debug("Starting getLoanAmountBy for"+agencyId);
			...
			if(LOGGER.isInfoEnabled())     LOGGER.info("GrantAmoountSql for Schemeid" + schemeId + "  agencyId" + agencyId + ":" + loanSql.toString());
			SQLQuery gaSQLQuery = getSearchSession().createSQLQuery(loanSql.toString());
			gaSQLQuery.addScalar("agencyName")
					 .addScalar("loanAmount", BigDecimalType.INSTANCE)
					 .setResultTransformer(Transformers.aliasToBean(LoanGrantBean.class));
			List<LoanGrantBean> galist = gaSQLQuery.list();
			if(LOGGER.isDebugEnabled())     LOGGER.debug("Exiting from  getLoanAmountBy for"+agencyId);
			return galist;

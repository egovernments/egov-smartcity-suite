...
public class VoucherService extends PersistenceService<CVoucherHeader, Long>
{
    ...
    public Department getCurrentDepartment()
    {
        ...
        final Assignment assignment = eisCommonService.getLatestAssignmentForEmployeeByDate(ApplicationThreadLocals.getUserId(),
                new Date());
    ...
    public boolean budgetaryCheck(final EgBillregister billregister) throws ValidationException
    {
    ...
    public Map<String, Object> getDesgByDeptAndTypeAndVoucherDate(final String type, final String scriptName, final Date vouDate,
        ...
        final List<String> list = (List<String>) scriptService.executeScript(
                ...
                ScriptService.createContext("eisCommonServiceBean", eisCommonService, "grossAmount", grossAmount, "userId",
                        ApplicationThreadLocals.getUserId().intValue(), "DATE", new Date(), "type", type, "vouDate",
                        vouDate.getTime(),
    ...
    public List<Map<String, Object>> getJournalVouchers(final CVoucherHeader voucherHeader,
        ...
        final List<Map<String, Object>> voucherList = new ArrayList<Map<String, Object>>();
        if (LOGGER.isDebugEnabled())
    ...
    public Map<String, Object> getVoucherInfo(final Long voucherId) {
        ...
        final List<VoucherDetails> billDetailslist = new ArrayList<VoucherDetails>();
        final List<VoucherDetails> subLedgerlist = new ArrayList<VoucherDetails>();
        ...
        try {
            for (final CGeneralLedger generalLedger : glList) {
                ...
                if (null != generalLedger.getFunctionId()) {
                    ...
                    voucherDetail.setFunctionDetail(functionDAO.getFunctionById(Long.valueOf(generalLedger.getFunctionId()
                            .toString())).getName());
                }
                ...
                voucherDetail.setAccounthead(((CChartOfAccounts) coaDAO.findById(generalLedger.getGlcodeId().getId(), false))
                        .getName());
    ...
    public CVoucherHeader updateVoucherHeader(final CVoucherHeader voucherHeader, final String voucherNumType) {
        ...
        try {
            ...
            // existingVH.setModifiedBy(userMngr.getUserById(Integer.valueOf(ApplicationThreadLocals.getUserId())));
    ...
    public CVoucherHeader getUpdatedVNumCGVN(final CVoucherHeader existingVH, final CVoucherHeader voucherHeader,
        ...
        try {
            final String fiscalPeriodIdStr = eGovernCommon.getFiscalPeriod(sdf.format(voucherHeader.getVoucherDate()));
    ...
    public List<Transaxtion> postInTransaction(final List<VoucherDetails> billDetailslist,
        ...
        try {
            for (final VoucherDetails accountDetails : billDetailslist) {
                 ...
                 * transaction.setFunctionId(voucherHeader.getVouchermis().getFunction().getId().toString()); } }else{
                ...
                for (final VoucherDetails subledgerDetails : subLedgerlist) {
                    ...
                    if (glcodeId.equals(detailGlCode)
                        ...
                        final Accountdetailtype adt = (Accountdetailtype) accountdetailtypeHibernateDAO.findById(
                                Integer.valueOf(detailtypeid), false);
    ...
    public CVoucherHeader postIntoVoucherHeader(final CVoucherHeader voucherHeader, final VoucherTypeBean voucherTypeBean)
            throws Exception {
        if (LOGGER.isDebugEnabled())
         ...
         * { Query query=getSession().createQuery("select f.identifier from Fund f where id=:fundId"); query.setInteger("fundId",
         * voucherHeader.getFundId().getId()); String fundIdentifier = query.uniqueResult().toString(); //String vn2 =
         * getFormattedManualVoucherNumber(fundIdentifier, autoVoucherType, voucherHeader.getVoucherNumber());
        ...
        try {
            ...
            // vh.setCreatedBy(userMngr.getUserById(Integer.valueOf(ApplicationThreadLocals.getUserId())));
    ...
    public Map<String, Object> getDesgByDeptAndType(final String type, final String scriptName) {
         ...
         * desgFuncryMap = new HashMap<String, Object>(); if(LOGGER.isDebugEnabled())
         * LOGGER.debug("Designation and Functionary  Name  = "+ desgFuncryName); try { designation =new
    ...
    @SuppressWarnings({ "unchecked", "deprecation" })
    public Map<String, Object> getDesgByDeptAndTypeAndvouDate(final String type, final String scriptName, final Date vouDate) {
        if (LOGGER.isDebugEnabled())
            LOGGER.debug("Voucher Service | getDesgUserByDeptAndDesgName | Start");
        final Map<String, Object> map = new HashMap<String, Object>();
        Designation designation = null;
        ...
        final List<String> list = null;/*
                                        ...
                                        * eisCommonService,"userId",
                                        * Integer.valueOf(ApplicationThreadLocals.getUserId().trim()),"DATE",new
                                        * Date(),"type",type,"vouDate",vouDate.getTime()));
                                        */
        Map<String, Object> desgFuncryMap;
        final List<Map<String, Object>> designationList = new ArrayList<Map<String, Object>>();
        for (final String desgFuncryName : list)
    ...
    @SuppressWarnings({ "unchecked", "deprecation" })
    public Map<String, Object> getDesgBYPassingWfItem(final String scriptName, final Object wfitem, final Integer deptId) {
        if (LOGGER.isDebugEnabled())
            LOGGER.debug("Voucher Service | getDesgUserByDeptAndDesgName | Start");
        final Map<String, Object> map = new HashMap<String, Object>();
        Designation designation = null;
        ...
        final List<String> list = (List<String>) scriptService.executeScript(validScript, ScriptService.createContext(
                ...
                ApplicationThreadLocals.getUserId().intValue(), "DATE", new Date(), "type", type, "wfitem", wfitem, "deptId",
                deptId,
    ...
    public List<Map<String, Object>> getAllDesgByAndDept(final Integer deptId, final String desgfuncry) {
        ...
        final List<Map<String, Object>> desgList = new ArrayList<Map<String, Object>>();
        if (LOGGER.isDebugEnabled())
    ...
    public List<EmployeeView> getUserByDeptAndDesgName(final String departmentId, final String designationId,
        ...
        final List<EmployeeView> empInfoList = (List<EmployeeView>) eisService.getEmployeeInfoList(paramMap);
        return empInfoList;
    }
    ...
    public EgBillregister createBillForVoucherSubType(final List<VoucherDetails> billDetailslist,
            final List<VoucherDetails> subLedgerlist,
            final CVoucherHeader voucherHeader,
            final VoucherTypeBean voucherTypeBean, final BigDecimal totalBillAmount) throws Exception {
        if (LOGGER.isDebugEnabled())
        ...
        try {
            ...
            if ("Contractor Journal".equalsIgnoreCase(voucherTypeBean.getVoucherName())) {
                egwstatus = (EgwStatus) persistenceService.find(
                        "from EgwStatus where upper(moduletype)=? and upper(description)=?", "CONTRACTORBILL", "APPROVED");
            ...
            if (voucherTypeBean.getVoucherNumType().equalsIgnoreCase("fixedassetjv")) {
                final EgBillSubType egBillSubType = (EgBillSubType) persistenceService.find(
                        "from EgBillSubType where name=? and expenditureType=?", "Fixed Asset", "Purchase");
            ...
            if (null != voucherTypeBean.getBillNum() && StringUtils.isNotEmpty(voucherTypeBean.getBillNum()))
                ...
                if (LOGGER.isDebugEnabled())
                    LOGGER.debug("VoucherService | createBillForVoucherSubType | Bill number generated :=" + billNumber);
            }
    ...
    public EgBillregister updateBillForVSubType(final List<VoucherDetails> billDetailslist,
            final List<VoucherDetails> subLedgerlist,
            final CVoucherHeader voucherHeader,
            final VoucherTypeBean voucherTypeBean, final BigDecimal totalBillAmount) throws ValidationException {
        if (LOGGER.isDebugEnabled())
    ...
    public Integer getDefaultDepartment() {
        ...
        final String defaultDepartmentName = null;/*
                                                   ...
                                                   * eisCommonService,
                                                   * "userId",Integer.valueOf(ApplicationThreadLocals.getUserId().trim())));
                                                   */

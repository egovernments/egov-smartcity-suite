...
public class PayInSlipAction extends BaseVoucherAction {
	...
	public String search() {
		...
		query.append("select distinct iHeader.id ,iHeader.instrumentNumber,iHeader.instrumentDate,iHeader.instrumentAmount,vh.voucherNumber,vh.voucherDate")
				...
				append(" and iHeader.statusId.id = ").append(comm.getStatusByModuleAndCode("Instrument",FinancialConstants.INSTRUMENT_CREATED_STATUS).getId());
		...
		try{
			if(mode.equalsIgnoreCase("create")){
			 ...
			 for (String instHeaderId : iHeaderId) {
				...
				Map valuesMap = contraService.prepareForUpdateInstrumentDeposit(comm.getBankaccountById(Integer.valueOf(contraBean.getAccountNumberId())).getChartofaccounts().getGlcode());
				 contraService.updateCheque_DD_Card_Deposit(voucherHeader.getId(), 
				 comm.getBankaccountById(Integer.valueOf(contraBean.getAccountNumberId())).getChartofaccounts().getGlcode(), instrumentHeader,valuesMap);

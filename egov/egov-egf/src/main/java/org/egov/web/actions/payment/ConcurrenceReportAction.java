...
public class ConcurrenceReportAction extends BaseFormAction {
	...
	private String jasperpath = "/reports/templates/ConcurrenceReport.jasper";
...
@Action(value="/payment/concurrenceReport-ajaxLoadPaymentHeader")
	public String ajaxLoadPaymentHeader() {
		...
		if (parameters.containsKey("bankAccount.id")
			...
			String bnk = bankAccount.getBankbranch().getBank().getName();
		}
	...
	public void setPaymentHeaderList(
			List<ConcurrenceReportData> paymentHeaderList) {
	...
	private StringBuffer getQueryString() {
		...
		if(StringUtils.isNotBlank(chequeOrRTGS)) {
			...
			if(Constants.CHEQUE.equals(chequeOrRTGS)) {
				...
				chqOrRtgsQry = "ih.INSTRUMENTNUMBER is not null and ih.INSTRUMENTTYPE = (select id from egf_instrumenttype where type = '"+ FinancialConstants.INSTRUMENT_TYPE_CHEQUE +"') and iv.instrumentheaderId = ih.id and ";
			} else if (Constants.RTGS.equals(chequeOrRTGS)) {
				chqOrRtgsQry = "ih.TRANSACTIONNUMBER is not null and ih.INSTRUMENTTYPE = (select id from egf_instrumenttype where type = '"+ FinancialConstants.INSTRUMENT_TYPE_ADVICE +"') and iv.instrumentheaderId = ih.id and ";
			}
			queryString.append("select bk.name   As bankName,ba.accountnumber As bankAccountNumber, vh.fundid As fundId,d.dept_name as departmentName,ms.billnumber as billNumber, ")
			...
			.append(chqOrRtgsQry).append(dateQry).append(bankQry)  						
			...
			.append(" egusr.first_name, ms.billdate,gl.glcode,vh.vouchernumber,bk.name,ba.accountnumber, vh.voucherdate, ms.paidamount ").append(" union ")
			...
			.append(insturmentQry).append(" group by bankid,bankaccountid,")
	...
	public void setPaymentHeaderListFnd(
			List<ConcurrenceReportData> paymentHeaderListFnd) {

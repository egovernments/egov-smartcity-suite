...
public class BudgetAppropriationReportAction extends BaseFormAction {
    ...
    private void prepareFormattedList() {
        for (int index = 0, slNo = 1; index < budgetDisplayList.size(); index++) {
            ...
            budgetAppropriationList.get(index).setAppDate(
                    Constants.DDMMYYYYFORMAT2.format(budgetDisplayList.get(index).getAppropriationDate()));
        }
    }
    ...
    public String ajaxGenerateReportXls() throws Exception {
        ...
        final JasperPrint jasper = reportHelper.generateBudgetAppropriationJasperPrint(budgetAppropriationList, title, subtitle,
                budgetName, getIsFundSelected(), getIsFunctionSelected(), getIsDepartmentSelected());

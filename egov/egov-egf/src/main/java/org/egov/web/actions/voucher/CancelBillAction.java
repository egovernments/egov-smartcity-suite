 ...
 @Autowired
    ...
    public void prepare()
    {
        ...
        addDropdownData(
                "expenditureList",
                persistenceService
                .findAllBy("select distinct bill.expendituretype from EgBillregister bill where bill.expendituretype like '"
                        + FinancialConstants.STANDARD_EXPENDITURETYPE_CONTINGENT + "'   order by bill.expendituretype"));
    }
    ...
    private boolean isSuperUser() {
        final List<Integer> superUserList = new ArrayList<Integer>();
        superUserList.addAll(persistenceService.findAllBy(
                "select user.id from UserRole usrRole where lower(usrRole.role.roleName) =?", FinancialConstants.SUPERUSER));
    ...
    public StringBuilder filterQuery()
    {
        ...
        final StringBuilder query = new StringBuilder(
                " select billmis.egBillregister.id, billmis.egBillregister.billnumber, billmis.egBillregister.billdate, billmis.egBillregister.billamount, billmis.egDepartment.deptName  from EgBillregistermis billmis left join billmis.egBillSubType  egBillSubType where ");
    ...
    public String cancelBill()
    {
        ...
        final StringBuilder cancelQuery = new StringBuilder("Update EgBillregister set ");
        for (final BillRegisterBean billRgstrBean : billListDisplay)
        ...
        if (expType == null || expType.equalsIgnoreCase(""))
        {
            ...
            cancelQuery
            .append(" billstatus='" + FinancialConstants.CONTINGENCYBILL_CANCELLED_STATUS + "' , status.id=:statusId ");
        ...
        } else if (FinancialConstants.STANDARD_EXPENDITURETYPE_PURCHASE.equalsIgnoreCase(expType))
        {
            ...
            cancelQuery.append(" billstatus='" + FinancialConstants.SUPPLIERBILL_CANCELLED_STATUS
                    + "' , status.id=:statusId ");
        } else if (FinancialConstants.STANDARD_EXPENDITURETYPE_WORKS.equalsIgnoreCase(expType))
        {
        ...
        if (idListLength != 0)
        {
            ...
            final Query query = session.createQuery(cancelQuery.toString());

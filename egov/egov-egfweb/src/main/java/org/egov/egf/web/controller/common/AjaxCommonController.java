...
public class AjaxCommonController {
	...
	public @ResponseBody List<String> findFunctionNames(@RequestParam final String name) {
		...
		List<CFunction> functions = functionService.findByNameLikeOrCodeLike(name);
	...
	public @ResponseBody String findAccountCodesForAccountDetailType(@RequestParam final String glcode,
			@RequestParam final String accountDetailType) {
		List<CChartOfAccounts> chartOfAccounts = chartOfAccountsService
	...
	public static <T> String toJSON(Collection<T> objects, Class<? extends T> objectClazz,
			Class<? extends JsonSerializer<T>> adptorClazz) {
		try {

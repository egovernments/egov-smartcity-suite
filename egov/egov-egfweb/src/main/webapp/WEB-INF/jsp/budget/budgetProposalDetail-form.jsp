...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	<tr>
	    ...
	    <td class="bluebox">
	    	<div id="budgets">
	    	<s:select list="dropdownData.budgetList"  listKey="id" listValue="name" name="budget"  value="model.budget.id" id="budgetDetail_budget" disabled="%{headerDisabled}" headerKey="0" headerValue="--- Select ---"></s:select>
	    	</div>
	    </td>
	</tr>
	<tr>
		...
		<s:if test="%{shouldShowHeaderField('executingDepartment')}">  
		    ...
		    <td width="15%" class="bluebox"><s:select list="dropdownData.executingDepartmentList"  listKey="id" listValue="deptName" name="executingDepartment" headerKey="0" headerValue="--- Select ---" onchange="getSavedData();updateGrid('executingDepartment.id',document.getElementById('budgetDetail_executingDepartment').selectedIndex);updateApproverDepartment(this)" value="executingDepartment.id" id="budgetDetail_executingDepartment"></s:select></td>
		</s:if>
		<s:if test="%{showRe}">
	...
	<tr>
		...
		<s:if test="%{shouldShowHeaderField('subScheme')}">
				<egov:ajaxdropdown id="subScheme" fields="['Text','Value']" dropdownId="budgetDetail_subScheme" url="budget/budgetProposalDetail!ajaxLoadSubSchemes.action" afterSuccess="onHeaderSubSchemePopulation"/>
				<td class="bluebox">

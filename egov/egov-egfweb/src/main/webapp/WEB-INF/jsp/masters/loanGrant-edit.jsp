...
<script type="text/javascript"	src="/EGF/resources/javascript/tabber2.js?rnd=${app_release_no}"></script>
<script type="text/javascript" 	src="/EGF/resources/javascript/loanGrantHelper.js?rnd=${app_release_no}"></script>
<script type="text/javascript" 	src="/EGF/resources/javascript/voucherHelper.js?rnd=${app_release_no}"></script>
...
var makeSanctionedAmountTable = function() {
	...
	var index=0;
		<s:if test="%{sanctionedAmountLGDetails.size!=0}">
		sanctionedAmountDT.addRow({SlNo:sanctionedAmountDT.getRecordSet().getLength()+1,
...
var makeUnsanctionedAmountTable = function() {
	...
	var index=0;
	<s:if test="%{unsanctionedAmountLGDetails.size!=0}">
		unsanctionedAmountDT.addRow({SlNo:unsanctionedAmountDT.getRecordSet().getLength()+1,
						"id":'<s:property value="id"/>',
						"fundingBy":'<s:property value="patternType"/>',
						"fundingagency":'<s:property value="fundingAgency.id"/>',
					...
					updateUnsanctionedAmountTableIndex();
					totalunsanctionedloan=0;
					totalunsanctionedgrant=0;
	</s:if>
	<s:else>
...
var makeRevisedAmountTable = function() {
	...
	var index=0;
	<s:if test="%{revisedAmountLGDetails.size!=0}">
...
var makeReceiptTable = function() {
		...
		document.getElementById('totalreceiptamount').value=totalreceiptamount; 
			...
			document.getElementById('sanctionedAmountTable').getElementsByTagName('table')[0].width="100%";
						...
						<div align="center" class="buttonbottom">
							<s:submit method="update" value="Submit"
			...
			<script>
  	var fundChosen=document.getElementById('fundId').selectedIndex;
	...
	{
		document.getElementById('schemeId').readonly='readonly';
		document.getElementById('subScheme.scheme.name').disabled='true';
		document.getElementById('subSchemeId').readonly='readonly';
		document.getElementById('subScheme.name').disabled='true';
		document.getElementById('subScheme.scheme.name').onblur='';
		document.getElementById('subScheme.name').onblur='';
		document.getElementById('subScheme.scheme.name').onfocus='';
	...
	var fundObj=document.getElementById('fundId');	
		...
		fundObj.onchange=function(){
		}
	fundObj.className="greyed";
	}
	emptyRevisedCostIfZero(document.getElementById('revisedCost'));
	makeFieldsReadOnly();
	freezeList();
  	</script>

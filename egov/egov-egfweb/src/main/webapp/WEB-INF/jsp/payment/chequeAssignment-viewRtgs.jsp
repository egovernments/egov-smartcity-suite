...
<head>  
	...
	<body>  
		<s:form action="chequeAssignment" theme="simple" >
			...
			<div class="formmainbox"><div class="subheadnew">RTGS Ref. No Assignment View</div>
			<table align="center" width="100%" cellpadding="0" cellspacing="0">
				<tr>  
					...
					<s:iterator var="p" value="instVoucherList" status="s"> 
					 <s:set var="rtgsNumber" value="%{instrumentHeaderId.transactionNumber}"/>
					 <s:if test="%{#rtgsNumber!=#PreRtgsNumber}">
					 <tr>    
							<td style="text-align:center" class="blueborderfortdnew">  
							<strong><s:property value='%{instrumentHeaderId.transactionNumber}'/></strong>
							
							
							<a href="#" onclick="generateReport('pdf','<s:property value='%{instrumentHeaderId.id}'/>',
							'<s:property value="%{instrumentHeaderId.bankAccountId.id}" />',
							'<s:property value="%{instrumentHeaderId.bankAccountId.bankbranch.id}" />',
							'<s:property value="%{instrumentHeaderId.bankAccountId.bankbranch.bank.id}" />'); ">Generate pdf</a>
							<s:if test='%{billSubType.equalsIgnoreCase("TNEB")}'> 
							<a href="#" onclick="generateReport('text','<s:property value='%{instrumentHeaderId.id}'/>',
							'<s:property value="%{instrumentHeaderId.bankAccountId.id}" />',
							'<s:property value="%{instrumentHeaderId.bankAccountId.bankbranch.id}" />',
							'<s:property value="%{instrumentHeaderId.bankAccountId.bankbranch.bank.id}" />'); ">Download text</a>
							</s:if>
							
							<!-- <a href="#" onclick="generateReport('xls','<s:property value='%{instrumentHeaderId.id}'/>',
							'<s:property value="%{instrumentHeaderId.bankAccountId.id}" />',
							'<s:property value="%{instrumentHeaderId.bankAccountId.bankbranch.id}" />',
							'<s:property value="%{instrumentHeaderId.bankAccountId.bankbranch.bank.id}" />');">Generate Excel</a> -->
							</td> <td ></td> <td ></td> <td ></td> <td ></td> 
					</tr>
					   <s:set var="PreRtgsNumber" value="instVoucherList[0].instrumentHeaderId.transactionNumber"/>    
					</s:if> 
		        	...
		        	<tr>  
				    	...
				    	<td style="text-align:center" class="blueborderfortdnew"><s:property value="%{voucherHeaderId.voucherNumber}" /></td>
	      			  	...
	      			  	<s:set var="PreRtgsNumber" value="%{instrumentHeaderId.transactionNumber}"/>            
					</tr>

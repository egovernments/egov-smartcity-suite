...
<body><br>
	<s:form action="payment" theme="simple" >
		<jsp:include page="../budget/budgetHeader.jsp">
        	<jsp:param name="heading" value="Modify Advance Payment" />
		</jsp:include>
		...
		<div class="formmainbox"><div class="subheadnew">Modify Advance Payment</div>
          		...
          		<td> 
					...
					<span>
						...
						<tr>
							...
							<td class="greybox"  colspan="2"><s:select name="paymentheader.bankaccount.id" id="bankaccount" list="dropdownData.bankaccountList" listKey="id" listValue="chartofaccounts.glcode+'--'+accountnumber+'--'+accounttype"  headerKey="-1" headerValue="----Choose----" value="%{paymentheader.bankaccount.id}"/></td>
						</tr>
						<tr>
							<td class="bluebox">&nbsp;</td>
							<td class="bluebox"><strong><s:text name="payment.mode"/></strong></td>
							<td class="bluebox" colspan="2"><s:radio id="paymentMode" name="paymentheader.type" list="#{'cheque':'Cheque','cash':'Cash','rtgs':'RTGS'}" value="%{paymentheader.type}"/></td>
	                ...
	                <span>
						...
						<tr><td colspan="6"><div class="subheadsmallnew">Advance Details</div></td></tr>
						<tr>
						<td colspan="6">
							...
							<table id="miscBillTable" align="center" border="0" cellpadding="0" cellspacing="0" width="100%">
								...
								<tr>
									...
									<td style="text-align:right" class="blueborderfortdnew"><div align="right"><input type="text" name="grandTotal" id="grandTotal" value="<c:out value="${totalAmt}"/>" style="text-align:right" readonly/></div></td>
								</tr>
							</table>
							</div>
						</td>
	...
	<div  class="buttonbottom" id="buttondiv">
		...
		<s:iterator value="%{getValidActions()}" var="p"  status="s">
		  <s:submit type="submit" cssClass="buttonsubmit" value="%{description}" id="wfBtn%{#s.index}" name="%{name}" method="updateAdvancePayment" onclick="return validate('%{name}','%{description}')"/>
		</s:iterator>
	...
	<script>
		...
		function calcGrandTotal(obj)
		{
			...
			if(obj.value>parseFloat(document.getElementById('payableAmt'+index).value))
			{
				...
				document.getElementById('grandTotal').value = parseFloat(document.getElementById('grandTotal').value) - parseFloat(document.getElementById('hiddenText').value) + parseFloat(obj.value);
				return;
			}
			document.getElementById('grandTotal').value = parseFloat(document.getElementById('grandTotal').value) - parseFloat(document.getElementById('hiddenText').value) + parseFloat(obj.value);

...
<!--******************************************************************************************
* This JSP is used for Dynamic Search Tree where there is no ParentId exists and for 
  Foreign Key - Primary Key constraints exists. i.e., only for two level search
  Node level		--	Primary Key
  Sub Node level 	--  Foreign Key Reference Table of the Node level Tree
********************************************************************************************-->
...
<html>
    ...
    /* Calling window sends the xml config file name */

	<% String xmlConfigName = request.getParameter("xmlconfigname"); %>    	
    	
    	/* Calling window sends the category name */

	<% String categoryName = request.getParameter("categoryname"); %>
	
	/* Calling window sends the text to filter */
	...
	function loadNodeData(node, fnLoadComplete) 
	{
		...
		parameterMap=parameterMap+"parentId="+nodedata[1];
			...
			for(var b = 0; b < name.length; b++)
			{
				if(name[b] != "")
				{
					var childnode = new YAHOO.widget.TextNode(name[b] +"~"+ id[b], node, false);		
			...
			for(var c = 0; c < name.length; c++)
			{
				...
				document.getElementsByName("attributeId")[tableObj.rows.length - 2].innerHTML=id[c];
	...
	function buildTree() 
	{
		...
		{
			replaceFilterText =  filterText + "%25" ;					
		}	
		
		parameterMap="text="+replaceFilterText;
		...
		for(var b = 0; b < name.length; b++)
		{
			var parentnode = new YAHOO.widget.TextNode(name[b] +"~"+ id[b], root, false); 
	...
	{	
		...
		id = trimAll(result[0]).split("+");
		name = trimAll(result[1]).split("+");
		narration = trimAll(result[2]).split("+");
	...
	{	
		...
		var response=request.responseText;
		var result = response.split("^");
		checkid = trimAll(result[0]).split("+");
		checkname = trimAll(result[1]).split("+");
		checknarration = trimAll(result[2]).split("+");

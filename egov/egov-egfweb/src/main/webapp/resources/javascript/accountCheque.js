...
// validate invalid cheque range.
   ...
   for(var i=0 ; i<deptSelectedValue.length;i++){
	...
	for(var j=0 ; j<chequeRangeArray.length;j++){
		...
		continue;
	}else if( fromchqNum == parseInt(tokens[0]*1) && tochqNum == parseInt(tokens[1]*1) && deptSelectedValue[i] == tokens[2] && serialNo == tokens[3]){
		
		document.getElementById("lblError").innerHTML="Cheque Range is already assigned for department :"+deptSelectedText[i]+" & SerialNo :"+serialNo ;
		return false;
	...
	for(var i=0 ; i<deptSelectedValue.length;i++){
		...
		document.getElementById(CHQDETAILSLIST+'['+chqDetailsIndex+'].serialNoL').innerHTML = document.getElementById("serialNo").value;
		document.getElementById(CHQDETAILSLIST+'['+chqDetailsIndex+'].serialNo').value = document.getElementById("serialNo").value;
		...
		chequeRangeArray.push(document.getElementById("fromChqNo").value.trim()+"-"+document.getElementById("toChqNo").value.trim()+"-"+deptSelectedValue[i]+"-"+document.getElementById("serialNo").value);
	}
	clearHeaderData();
	return true;
}
...
function validateCheque(obj){
	...
	if(!obj.readOnly){
		...
		var serialNo = document.getElementById(CHQDETAILSLIST+'['+index+'].serialNo').value;
		chequeRangeArray.splice(index,1,fromchqNum+"-"+tochqNum+"-"+deptId+"-"+serialNo);
		...
		if( (fromchqNum<parseInt(tokens[0]*1) && tochqNum < parseInt(tokens[0]*1)) || (fromchqNum>parseInt(tokens[0]*1) && fromchqNum > parseInt(tokens[1]*1)) ){
		continue;
	}else if( fromchqNum == parseInt(tokens[0]*1) && tochqNum == parseInt(tokens[1]*1) && deptId == tokens[2]*1 && serialNo == tokens[3]){
		
		count = count +1;
		if(count >1){
			document.getElementById("lblErrorGrid").innerHTML="Cheque Range is already assigned for department :"+deptName+" & SerialNo :"+serialNo;
			return false;
		}
		continue;
	}else if(fromchqNum == parseInt(tokens[0]*1) && tochqNum == parseInt(tokens[1]*1)){

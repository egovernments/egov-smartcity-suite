		...
		request.onreadystatechange = function () {
			if (request.readyState == 4) {
				if (request.status == 200) {
					...
					if (response[0] == "false") {
						bootbox.alert(bootbox.alert("Entered " + columnname + " already exists. ");
		...
		request.onreadystatechange = function () {
			if (request.readyState == 4) {
				if (request.status == 200) {
					var response = request.responseText.split("^");
					if (response[0] == "false") {
						bootbox.alert(bootbox.alert("Entered " + columnname1 + " already exists for " + columnname2 + ". ");
		...
		request.onreadystatechange = function () {
			if (request.readyState == 4) {
				if (request.status == 200) {
					var response = request.responseText.split("^");
					if (response[0] == "false") {
						bootbox.alert(bootbox.alert(columnname + " already Selected. ");
						document.getElementById(fieldObj).value = "";
						document.getElementById(fieldObj).focus();
					}
				}

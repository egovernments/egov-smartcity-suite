...
public class CompositeUniqueCheckValidator implements
		ConstraintValidator<CompositeUnique, Object> {
	...
	public boolean isValid(final Object arg0,
		...
		if (unique.compositefields() == null
				|| unique.compositefields().length == 0)
			return true;
		...
		for (int i = 0; i < unique.compositefields().length; i++) {
			...
			if (columnNameStr.isEmpty()) {
				...
				columnNameStr = columnNameStr + "||''||"
						+ unique.compositecolumnName()[i];
			}
		}
		if (!isCompositeUnique(arg0, fieldStr, columnNameStr)) {
			List<String> items = Arrays.asList(fieldStr.split(","));
			...
			if (unique.enableDfltMsg())
				 ...
				 * all fields in UI.
				 */
				for (String asStringObj : items) {
					constraintValidatorContext.buildConstraintViolationWithTemplate(unique.message()).addPropertyNode(asStringObj).addConstraintViolation();
					concatenatedFieldValuesForErrorMesssages = concatenatedFieldValuesForErrorMesssages.concat(constraintValidatorContext.getDefaultConstraintMessageTemplate());
	...
	private boolean isCompositeUnique(final Object arg0, final String field,
		...
		if (id == null) {
			final Query q = currentSession.createSQLQuery("select * from "
					+ unique.tableName() + " where upper(" + columnName
					+ ")=:value");
	...
	private Object getValue(final Object target, final String field) {
		...
		try {
			...
			final BeanInfo info = java.beans.Introspector.getBeanInfo(target
					.getClass());
			final PropertyDescriptor[] props = info.getPropertyDescriptors();
			for (String strlist : items) {
				for (final PropertyDescriptor propertyDescriptor : props) {
					if (!propertyDescriptor.getName().equals("new")) {
						if (Class
							 ...
							 * requirement like Unique Combination of (key_Name
							 * and module_id From(id of Eg_module)) so here in
							 * Else part Module composite Object to getId()
							 * method and returns id value..
							 */
							if (propertyDescriptor.getName().equals(strlist)) {
								/*
								 * if this
								 * "propertyDescriptor.getReadMethod().invoke(target)"
								 * returns any composite Object which is der in
								 * Ur Object then Pass That composite Object to
								 * getId() methos and Get just id of thet
								 * Composite Object.
								 */
								final Long id = getId(propertyDescriptor
										.getReadMethod().invoke(target));
								concatenatedFieldValues = concatenatedFieldValues
										.concat(id.toString());
							}

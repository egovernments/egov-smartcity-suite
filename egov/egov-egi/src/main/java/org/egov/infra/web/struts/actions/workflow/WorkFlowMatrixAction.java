...
public class WorkFlowMatrixAction extends BaseFormAction {
    ...
    /**
     * This method checks for any existing matrix (validation for duplicates) for the enterd objecttype,date,fromqty,toqty and
     * department and returns matrixdetails table on success else displays error message
     * @return
     */
    public String buildWorkflow() {
        ...
        if (formatfromDate.before(formatCurrentDate)) {
            if (!checkExistingMatrixforLegacy()) {
                ...
                addActionMessage("A workflow is already present from " + this.sdf.format(getLegacyDate())
                        + ". The workflow you are about to add will be valid for records created between "
                        + this.sdf.format(getFromDate()) + " and "
    ...
    /**
     * This method checks for any existing matrix (validation for duplicates) for the enterd objecttype,date,fromqty,toqty and
     * department and returns true if none found else returns false
     * @return
     */
    private boolean checkEistingMatrix() {
        ...
        if (checkmatrixList == null) {
            ...
            for (final Long matrixid : (List<Long>) checkmatrixList) {
                ;
                if (!new String(departmentString)
                        .contains(this.workFlowMatrixService.getWorkFlowObjectbyId(matrixid).getDepartment())) {
    ...
    /**
     * This method checks for any existing matrix for legacy date, for the entered objecttype,date,fromqty,toqty and department
     * and returns true if none found else returns false
     * @return
     */
    ...
    private String prepareWorkFlowMatrixDetails(final List<WorkFlowMatrixDetails> matrixList) {
        ...
        if (getMode().equals(MODIFY)) {
            if (updateExistingMatrix()) {
                this.workFlowMatrixService.save(createActualWorkFlowMatrixDetails(actWorkFlowMatrixList, matrixList),
                        getDepartment());
            } else {
                deleteWorkFlowMatrixObject(getSearchParams());
                this.workFlowMatrixService.save(createActualWorkFlowMatrixDetails(actWorkFlowMatrixList, matrixList),
                        getDepartment());
                // addActionMessage("The Matrix you are trying to modify was created today ,It has been deleted and a new matrix
                // is created");
                return NEW;
            }
     ...
     * @return
     */
    private List<WorkFlowMatrix> createActualWorkFlowMatrixDetails(final List<WorkFlowMatrix> ActualwfMatrixObjList,
            final List<WorkFlowMatrixDetails> matrixList) {
    ...
    public String prepareModifyForm() {
        ...
        workFlowList = this.workFlowMatrixService.prepareWorkFlowMatrixDetailsList(matrixList.subList(0, sublistIndex + 1),
                workFlowList, Boolean.TRUE);
        rejectionList = this.workFlowMatrixService.prepareWorkFlowMatrixDetailsList(
                matrixList.subList(sublistIndex + 1, matrixList.size()), rejectionList, Boolean.FALSE);

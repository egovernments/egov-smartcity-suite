		...
		<div class="page-container">
					...
					<div class="container-fluid">
						<div class="navbar-header col-md-10 col-xs-10">
							<a class="navbar-brand" href="javascript:void(0);"> 
								<img src="/egi/resources/global/images/${sessionScope.citylogo}" height="60">
								<div>
						...
						<div class="nav-right-menu col-md-2 col-xs-2">
							<ul class="hr-menu text-right">
								...
								<a href="http://www.egovernments.org" target="_blank">
									<img src="/egi/resources/global/images/logo@2x.png"
			...
			<div class="main-content">
				<div class="login-container ">
					...
					<c:if test="${citizenActivationSuccess}">
					<div class="text-center  error-msg">
						Activation Successful, Please login using your credentials.
					</div>
					</c:if>
					<c:if test="${citizenActivationFailed}">
					<div class="text-center  error-msg">
						Activation Failed, you may entered wrong activation code or your registration got expired. 
						<input type="hidden" name="citizenActivationFailed" id="citizenActivationFailed" value="true" />
					</div>
					</c:if>
					<c:if test="${activationCodeSendingFailed}">
					<div class="text-center  error-msg">
						Neither email nor mobile activation send. 
						<input type="hidden" name="activationCodeSendingFailed" id="activationCodeSendingFailed" value="true" />
					</div>
					</c:if>
					<c:if test="${mobInvalid}">
						<input type="hidden" name="mobInvalid" id="mobInvalid" value="true" />
					</c:if>
					<c:if test="${emailInvalid}">
						<input type="hidden" name="emailInvalid" id="emailInvalid" value="true" />
					</c:if>
					<c:if test="${citizenActivation}">
						<input type="hidden" name="citizenActivation" id="citizenActivation" value="true" />
					</c:if>
					<c:if test="${passwordSendingSuccess}">
						<div class="text-center  error-msg">
						Your recovered password has been sent to registered mobile and email.
						</div>
					</c:if>
					<c:if test="${passwordSendingFailed}">
						<div class="text-center  error-msg">
						Your password recovery request failed.
						</div>
					</c:if>
						...
						<div class="login-body">
							...
							<form method="post" role="form" id="signinform"
								...
								<input type="hidden" id="ipAddress" name="ipAddress"
									value="${pageContext.request.remoteAddr}" /> <input type="hidden" id="loginType"
									name="loginType" />
								<div class="form-group">
				...
				<div class="modal-content">
					...
					<div class="modal-body">
						<p>This website is designed, developed and maintained by
							eGovernments Foundation under the supervision of ${sessionScope.cityname}, 
							India.</p>
						...
						<p>The information posted on this website could include
							...
							by non-Government/private organisations. ${sessionScope.cityname} is
							providing these links and pointers solely for your information and
							convenience. When you select a link to an outside website, you are
							leaving the ${sessionScope.cityname} website and are subject to the
							privacy and security policies of the owners/sponsors of the

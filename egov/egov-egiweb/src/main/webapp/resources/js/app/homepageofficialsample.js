...
{	
	...
	//history table
	$(document).on('click', '.notification-history', function(e){
		$('.history-inbox').modal('show');
		$('.history-header').html('Section Manager/Sreekanth Nair • Create Property • Expense Bill Approval • Checked by Accounts Officer • 13 Jan 2015 6:30 PM');
		historyTableContainer = $("#historyTable"); 
		historyTableContainer.dataTable({
			"sDom": "<'row'<'col-xs-12 hidden col-right'f>r>t<'row'<'col-md-6 col-xs-12'i><'col-md-3 col-xs-6'l><'col-md-3 col-xs-6 text-right'p>>",
			"aLengthMenu": [[10, 25, 50, -1], [10, 25, 50, "All"]],

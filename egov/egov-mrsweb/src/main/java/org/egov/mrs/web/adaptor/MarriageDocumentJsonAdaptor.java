...
public class MarriageDocumentJsonAdaptor implements JsonSerializer<MarriageDocument> {
    @Override
    public JsonElement serialize(final MarriageDocument marriageDocument, final Type type, final JsonSerializationContext jsc) {
        final JsonObject jsonObject = new JsonObject();
        if (marriageDocument != null) {
            if (marriageDocument.getName() != null)
                jsonObject.addProperty("name",
                        marriageDocument.getName());
            else
                jsonObject.addProperty("name", StringUtils.EMPTY);

            if (marriageDocument.getCode() != null)
                jsonObject.addProperty("code",
                        marriageDocument.getCode());
            else
                jsonObject.addProperty("code", StringUtils.EMPTY);

            jsonObject.addProperty("active",
                    marriageDocument.isActive());
            jsonObject.addProperty("individual",
                    marriageDocument.isIndividual());

            jsonObject.addProperty("mandatory",
                    marriageDocument.isMandatory());

            if (marriageDocument.getDocumentProofType() != null)
                jsonObject.addProperty("documentprooftype",
                        marriageDocument.getDocumentProofType().toString());
            else
                jsonObject.addProperty("documentprooftype", StringUtils.EMPTY);

            if (marriageDocument.getType() != null)
                jsonObject.addProperty("type",
                        marriageDocument.getType().name());
            else

...
public final class IVRSFeedbackReviewSearchSpec {
    ...
    public static Specification<IVRSFeedback> search(
            IVRSFeedbackSearchRequest ivrsFeedbackSearchRequest) {
        return (root, query, builder) -> {
            final Predicate predicate = builder.conjunction();
            if (ivrsFeedbackSearchRequest.getComplaintId() != null)
                predicate.getExpressions().add(builder.equal(root.get(COMPALINT).get("complaintType").get("id"),
                        ivrsFeedbackSearchRequest.getComplaintId()));
            if (isNotBlank(ivrsFeedbackSearchRequest.getCrn()))
                predicate.getExpressions().add(builder.equal(root.get(COMPALINT).get("crn"),

...
CREATE OR REPLACE VIEW egpgr_mv_drilldown_report_view AS 
 SELECT DISTINCT ON (cd.crn) cd.crn,
  ...
  WHERE cd.status = cs.id AND cd.complainttype::numeric = ctype.id AND cd.state_id = state.id AND cd.currentowner = usr.id AND complainant.id = cd.complainant and usr.type='EMPLOYEE'

CREATE OR REPLACE VIEW egpgr_mv_drilldown_report_view AS
 SELECT DISTINCT ON (cd.crn) cd.crn,
    usr.id AS employeeid,
  ...
  WHERE cd.status = cs.id AND cd.complainttype::numeric = ctype.id AND cd.state_id = state.id AND cd.currentowner = usr.id AND complainant.id = cd.complainant AND usr.type::text = 'EMPLOYEE'::text
  GROUP BY usr.id, usr.name, complainant.id, cd.crn, cd.createddate, complainant.name, cd.details, cs.name, ((bndry.name::text || ' - '::text) || childlocation.name::text), cd.citizenfeedback, ctype.id, ctype.name, bndry.name, childlocation.name, dept.name, ((usr.name::text || '~'::text) || pos.name::text);

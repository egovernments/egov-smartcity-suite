...
jQuery(document).ready(function($) {
    ...
    function getSumOfRecords() {
        $.ajax({
            ...
            url: "/pgr/functionaryWiseReport/grand-total",
            type: 'GET',
            async:false,
            data : {
            ...
            buttons: [
                {
                    ...
                    action: function (e, dt, node, config) {
                        window.open("/pgr/functionaryWiseReport/download"+jsonParameters(pramdata)+"&printFormat=PDF", '', 'scrollbars=yes,width=1300,height=700,status=yes');
                    }
                },
                {
                    text: 'XLS',
                    action: function (e, dt, node, config) {
                        window.open("/pgr/functionaryWiseReport/download"+jsonParameters(pramdata)+ "&printFormat=XLS", '_self');
                    }
                }],
            ajax : {
            ...
            buttons: [
                {
                    text: 'PDF',
                    action: function (e, dt, node, config) {
                        window.open("/pgr/functionaryWiseReport/download"+complaintParameter(prameter)+"&printFormat=PDF", '', 'scrollbars=yes,width=1300,height=700,status=yes');
                    }
                },
                {
                    text: 'XLS',
                    action: function (e, dt, node, config) {
                        window.open("/pgr/functionaryWiseReport/download"+complaintParameter(prameter)+ "&printFormat=XLS", '_self');
                    }
                }],
            ajax : {
                url : "/pgr/functionaryWiseReport/result",

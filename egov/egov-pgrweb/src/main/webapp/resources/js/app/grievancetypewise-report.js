...
jQuery(document).ready(function ($) {
    ...
    function getSumOfRecords() {
        $.ajax({
            ...
            url: "/pgr/report/grievancetypewise/grand-total",
            type: 'GET',
            async: false,
            data: {
    ...
    function complaintTypeWiseReport() {
        $('#compTypeReport-table_wrapper').show();
        $('#compReport-table_wrapper').hide();
        var startDate = "";
        var endDate = "";
        ...
        compTypereportContainer.on('preXhr.dt', function (e, settings, data) {
            pramdata = data;
        })
            .dataTable({
                processing: true,
                ...
                buttons: [
                    {
                        ...
                        action: function (e, dt, node, config) {
                            ...
                            window.open("/pgr/report/grievancetypewise/download" + jsonParameters(pramdata) + "&printFormat=PDF", '', 'scrollbars=yes,width=1300,height=700,status=yes');
                        }
                    },
                    {
                        text: 'XLS',
                        action: function (e, dt, node, config) {
                            window.open("/pgr/report/grievancetypewise/download" + jsonParameters(pramdata) + "&printFormat=XLS", '_self');
                        }
                    }],
                ajax: {
                    url: "/pgr/report/grievancetypewise",
                    type: "POST",
                    data: function (args) {
                        return {
...
function callAjaxByComplaintDetail() {
    ...
    compreportContainer.on('preXhr.dt', function (e, settings, data) {
        prameter = data;
    })
        .dataTable({
            processing: true,
            ...
            buttons: [
                {
                    ...
                    action: function (e, dt, node, config) {
                        console.log(pramdata);
                        window.open("/pgr/report/grievancetypewise/download" + complaintParameter(prameter) + "&printFormat=PDF", '', 'scrollbars=yes,width=1300,height=700,status=yes');
                    }
                },
                {
                    text: 'XLS',
                    action: function (e, dt, node, config) {
                        window.open("/pgr/report/grievancetypewise/download" + complaintParameter(prameter) + "&printFormat=XLS", '_self');
                    }
                }],
            ajax: {
                url: "/pgr/report/grievancetypewise",

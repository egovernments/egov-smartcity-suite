...
public class TaxCalculatorAdapter {
	...
	public TaxCalculatorAdapter(ResidentialUnitTaxCalculator residentialUnitTaxCalculator,
			...
			PersistenceService<MigratedPropertyFloor, Long> floorService,
	...
	public void calculateALV(BasicProperty basicProperty) {
		...
		for (FloorIF floorIF : property.getPropertyDetail().getFloorDetails()) {
			...
			if (floorImpl.getPropertyUsage() != null
					&& floorImpl.getStructureClassification() != null) {

				/**
				 * Getting the applicable Base Rent
				 */
				categories = propertyTaxUtil.getBoundaryCategories(floorImpl, propertyArea,
				...
				if (floorImpl.getManualAlv() != null) {
					...
					unitTaxCalculationInfo = calculateUnitTax(property, propTypeCode, installment,
							floorImpl, unitTaxCalculationInfo);

					createMigratedPropertyFloorDetails(basicProperty.getUpicNo(),
							unitTaxCalculationInfo, floorImpl);
					...
					for (BoundaryCategory category : categories) {
						...
						unitTaxCalculationInfo = calculateUnitTax(property, propTypeCode,
								installment, floorImpl, unitTaxCalculationInfo);
	 ...
	 * @return
	 */
	private UnitTaxCalculationInfo calculateUnitTax(Property property, String propTypeCode,
			Installment installment, FloorImpl floorImpl,
		...
		if (propTypeCode.equals(PropertyTaxConstants.PROPTYPE_RESD)) {
			unitTaxCalculationInfo = residentialUnitTaxCalculator.calculateUnitTax(
					unitTaxCalculationInfo, floorImpl, installment, property.getPropertyDetail()
							.getExtra_field5(), property, null);
	...
	private void createMigratedPropertyFloorDetails(String propertyId,
			UnitTaxCalculationInfo unitTax, FloorImpl floor) {
		LOG.debug(
				"Entered into createMigratedPropertyFloorDetails unitTax.ALV = {} and floor = {} ",
		...
		List<FloorwiseDemandCalculations> floorDemands = HibernateUtil
				...
				.createQuery(
						" from FloorwiseDemandCalculations where floor.id in "
								+ PropertyTaxUtil.getFloorIdsAsString(basicProperty.getProperty()))
				.list();

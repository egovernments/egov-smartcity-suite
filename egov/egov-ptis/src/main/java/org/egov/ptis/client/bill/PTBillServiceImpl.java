...
public class PTBillServiceImpl extends BillServiceInterface {
	...
	public List<EgBillDetails> getBilldetails(Billable billObj) {
		...
		for (EgDemandDetails demandDetail : ptDemand.getEgDemandDetails()) {
			...
			if (balance.compareTo(BigDecimal.ZERO) == 1) {
				...
				PropertyInstTaxBean installmentTaxBean = nmcBillable.getInstTaxBean().get(
						installment);
				...
				if (reasonMasterCode.equalsIgnoreCase(DEMANDRSN_CODE_GENERAL_TAX)) {
					...
					if (rebateAmt != null && rebateAmt.compareTo(BigDecimal.ZERO) == 1) {
						...
						billDetailBean = new BillDetailBean(installment, orderMap.get(rebateKey),
								rebateKey, rebateAmt, GLCODE_FOR_TAXREBATE, DEMANDRSN_REBATE,
								Integer.valueOf(0));
					...
					billDetailBean = new BillDetailBean(installment, orderMap.get(key), key,
							...
							reason.getGlcodeId().getGlcode(), reason.getEgDemandReasonMaster()
		...
		for (Map.Entry<Installment, PropertyInstTaxBean> instMap : instTaxBean.entrySet()) {
			...
			if (instMap.getValue().getInstPenaltyAmt() != null
				...
				insertPenDmdDetail = prepareDmdAndBillDetails(billObj, billDetails, nmcBillable,
						orderMap, instMap.getValue().getInstPenaltyAmt(), installment, cal);
			}
		...
		createAdvanceBillDetails(billDetails, currentInstallment, currentInstallmentDemand,
				orderMap, ptDemand, nmcBillable);
	...
	private void createAdvanceBillDetails(List<EgBillDetails> billDetails,
			Installment currentInstallment, BigDecimal currentInstallmentDemand,
			HashMap<String, Integer> orderMap, Ptdemand ptDemand, PropertyTaxBillable nmcBillable) {
		...
		LOGGER.debug("getBilldetails - advanceCollection = " + advanceCollection
				+ ", noOfAdvancesPaid=" + noOfAdvancesPaid);
			...
			for (int i = noOfAdvancesPaid; i < advanceInstDescription.size(); i++) {
				...
				billDetailBean = new BillDetailBean(currentInstallment, orderMap.get(advanceKey),
						...
						i == noOfAdvancesPaid ? currentInstallmentDemand
								.subtract(partiallyCollectedAmount) : currentInstallmentDemand,
	...
	private EgDemandDetails prepareDmdAndBillDetails(Billable billObj,
			List<EgBillDetails> billDetails, PropertyTaxBillable nmcBillable,
			HashMap<String, Integer> orderMap, BigDecimal penAmt, Installment installment,
			Calendar cal) {
		LOGGER.info("Entered into prepareDmdAndBillDetails");
		LOGGER.info("preapreDmdAndBillDetails- Installment : " + installment + ", Penalty Amount: "
				+ penAmt);
		...
		if (nmcBillable.getLevyPenalty()) {
			...
			if (thereIsPenalty) {
				...
				BillDetailBean billDetailBean = new BillDetailBean(installment, orderMap.get(key),
						key, penAmt, GLCODE_FOR_PENALTY,
						PropertyTaxConstants.DEMANDRSN_STR_PENALTY_FINES, Integer.valueOf(1));
				billDetails.add(createBillDet(billDetailBean));
			}
		}
	...
	private EgDemandDetails getPenaltyDmdDtls(Billable billObj, Installment inst) {
		return propertyTaxCollection.getDemandDetail(billObj.getCurrentDemand(), inst,

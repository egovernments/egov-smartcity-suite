...
public class CollectionApportioner {
	...
	public void apportion(BigDecimal amtPaid, List<ReceiptDetail> receiptDetails, Map<String, BigDecimal> instDmdMap) {
		...
		for (ReceiptDetail rd : receiptDetails) {
			...
			if (glCode.endsWith(PropertyTaxConstants.GLCODE_FOR_TAXREBATE)
		...
		if (!currInst.isEmpty() && halfRebate.compareTo(BigDecimal.ZERO) == 1) {
			for (ReceiptDetail rd : receiptDetails) {
				if (rd.getAccounthead().getGlcode().endsWith(PropertyTaxConstants.GLCODE_FOR_TAXREBATE)) {
	...
	private boolean isRebate(String glCode) {
		return glCode.equals(GLCODE_FOR_TAXREBATE) || glCode.equalsIgnoreCase(PropertyTaxConstants.GLCODE_FOR_ADVANCE_REBATE);
	}

...
import static org.egov.ptis.constants.PropertyTaxConstants.AREA_CONSTANT_FOR_LARGE_RESIDENTIAL_PREMISES;
...
import static org.egov.ptis.constants.PropertyTaxConstants.BASERENT_FROM_APRIL2008_BUILDINGS;
import static org.egov.ptis.constants.PropertyTaxConstants.BASERENT_FROM_APRIL2008_OPENPLOT;
...
import static org.egov.ptis.constants.PropertyTaxConstants.HISTORY_TAX_DETAIL;
import static org.egov.ptis.constants.PropertyTaxConstants.LARGE_RESIDENTIAL_PREMISES_ALV;
...
import static org.egov.ptis.constants.PropertyTaxConstants.PROPERTYTYPE_STR_TO_CODE;
...
import static org.egov.ptis.constants.PropertyTaxConstants.QUERY_BASERENT_BY_BOUNDARY_FOR_OPENPLOT;
import static org.egov.ptis.constants.PropertyTaxConstants.QUERY_BASERENT_BY_OCCUPANCY_AREA_STRUCTURE;
...
import static org.egov.ptis.constants.PropertyTaxConstants.QUERY_DEMANDREASONDETAILS_BY_DEMANDREASONID_DATE;
...
public class PropertyTaxUtil {
	...
	private Map<Date, Property> getPropertiesByCreatedDate(Map<Date, Property> historyProperties) {
		...
		for (Map.Entry<Date, Property> entry : historyProperties.entrySet()) {
			historyPropsByCrtdDate
					.put(entry.getValue().getCreatedDate().toDate(), entry.getValue());
		}
	...
	public UnitTaxCalculationInfo calculateApplicableTaxes(List<String> applicableTaxes,
			...
			Installment installment, String propType, String propTypeCategory, String amenities,
			Property property, BoundaryCategory category, TaxCalculationInfo currentTaxCalc) {
		...
		Boolean isCallForConsolidation = taxNameAndALV == null || taxNameAndALV.isEmpty() ? false
				: true;
		...
		if (!newOccupationDate.equals(installment.getFromDate())
				&& between(newOccupationDate, installment.getFromDate(), installment.getToDate())
				&& isPropertyModified) {

			for (int i = 0; i < propertyCreatedDates.size(); i++) {

				TaxCalculationInfo taxCalcInfo = null;
				Property historyProperty = historyPropsByCreatedDate.get((new ArrayList<Date>(
						propertyCreatedDates)).get(i));
				...
				if (taxCalcInfo == null && isUseMigratedAlv) {
					...
					isNewUnit = calculateTaxesOnMigratedALV(unitTaxCalculationInfo, installment,
							...
							historyProperty, oldApplicableTaxes, installment.getFromDate(),
					...
					if (isCallForConsolidation) {
						if (taxCalcInfo != null) {
							...
							for (UnitTaxCalculationInfo unit : taxCalcInfo
								 ...
								 * with diff floor *
								 */
								if (getIsUnitsMatch(unitTaxCalculationInfo, propType,
										isCallForConsolidation, unit)) {
									historyALV = unit.getAnnualRentAfterDeduction();
									oldUnitOccupancyDate = new Date(unit.getOccpancyDate()
											.getTime());
									historyUnit = unit;
									break;
								}
							...
							if (historyUnit != null) {
								if (taxCalcInfo.getUnitTaxCalculationInfos().get(0) instanceof List) {
									...
									for (int j = 0; j < taxCalcInfo.getUnitTaxCalculationInfos()
											.size(); j++) {
						...
						if (unitTaxCalcInfos.get(0) instanceof List) {
							for (List<UnitTaxCalculationInfo> units : unitTaxCalcInfos) {
								for (UnitTaxCalculationInfo unit : units) {
									isUnitsMatch = getIsUnitsMatch(unitTaxCalculationInfo,
									...
									if (!isCallForConsolidation
											...
											.getBaseRentEffectiveDate().equals(
							...
							for (int j = 0; j < unitTaxCalcInfos.size(); j++) {
								...
								if (!isCallForConsolidation
										...
										&& (unit.getBaseRentEffectiveDate() != null && unitTaxCalculationInfo
												.getBaseRentEffectiveDate() != null) ? unit
										.getBaseRentEffectiveDate().equals(
												unitTaxCalculationInfo.getBaseRentEffectiveDate())
										: isUnitsMatch) {
									historyALV = unit.getAnnualRentAfterDeduction();
									oldUnitOccupancyDate = new Date(unit.getOccpancyDate()
											.getTime());
					...
					if (!oldApplicableTaxes.isEmpty() && oldUnitOccupancyDate != null) {
						...
						LOGGER.debug("Calculating Arrears Taxes on Modification, UnitNo: "
								+ unitTaxCalculationInfo.getUnitNumber() + ", historyALV: "
								+ historyALV + ", oldUnitOccupancyDate : " + oldUnitOccupancyDate);

						isNewUnit = false;

						if (oldUnitOccupancyDate.equals(unitTaxCalculationInfo.getOccpancyDate())) {
							totalTaxPayable = copyHistoryTaxDetails(historyUnit,
									unitTaxCalculationInfo);
						} else {
							/**
							 ...
							 */
							if (category == null
									|| unitTaxCalculationInfo.getOccpancyDate().after(
											category.getFromDate())) {

								calculateTaxes(isNewUnit,
										new ArrayList<String>(oldApplicableTaxes),
										unitTaxCalculationInfo, installment, propType,
										propTypeCategory, amenities, historyALV,
										isPropertyModified, oldUnitOccupancyDate,
										(isCallForConsolidation ? historyTaxNameAndALV : null),
										property, category, nextPropOccupancyDate);
							}
						}
					}
				}
		...
		int noOfDays = getNumberOfDays(installment, null, unitTaxCalculationInfo.getOccpancyDate(),
				category);

		if (noOfDays > 0) {
			calculateTaxes(isNewUnit, applicableTaxes, unitTaxCalculationInfo, installment,
					propType, propTypeCategory, amenities, alv, isPropertyModified, null,
					taxNameAndALV, property, category, null);
		}
	...
	private boolean calculateTaxesOnMigratedALV(UnitTaxCalculationInfo unitTaxCalculationInfo,
			Installment installment, String amenities, Property property, Boolean isNewUnit,
			Boolean isCallForConsolidation, Boolean isPropertyModified, Property historyProperty,
			Set<String> oldApplicableTaxes, Date oldUnitOccupancyDate, Date nextPropOccupancyDate) {

		LOGGER.debug("Entered into calculateTaxesOnMigratedALV");

		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("calculateTaxesOnMigratedALV, installment=" + installment + ", amenities="
					+ amenities + ", property=" + property + ", isNewUnit=" + isNewUnit
					+ ", isCallForConsolidation=" + isCallForConsolidation
					+ ", isPropertyModified=" + isPropertyModified + ", oldApplicableTaxes="
					+ oldApplicableTaxes);
		}
		...
		String historyPropType = historyProperty.getPropertyDetail().getPropertyTypeMaster()
				.getCode();
		...
		if (!historyPropType.equalsIgnoreCase(PropertyTaxConstants.PROPTYPE_OPEN_PLOT)) {
				...
				for (FloorwiseDemandCalculations floorDemand : floorDemands) {
					...
					if (getIsUnitsMatch(unitTaxCalculationInfo, propType, isCallForConsolidation,
						...
						calculateTaxes(isNewUnit, new ArrayList<String>(oldApplicableTaxes),
								unitTaxCalculationInfo, installment, historyPropType,
								historyPropCat, amenities, historyALV, isPropertyModified,
								oldUnitOccupancyDate,
								(isCallForConsolidation ? historyTaxNameAndALV : null), property,
								null, nextPropOccupancyDate);
	 ...
	 * @param floor
	 */
	private Set<String> prepareApplTaxesOnFloor(Property historyProperty, FloorIF floor) {
	...
	public TaxCalculationInfo getTaxCalInfo(Installment installment, Property property) {
		LOGGER.debug("Entered into getTaxCalInfo, installment=" + installment + ", property="
				+ property);
	...
	public static Boolean getIsUnitsMatch(UnitTaxCalculationInfo unitTaxThis, String propType,
		...
		if (PROPTYPE_OPEN_PLOT.equalsIgnoreCase(propType)) {
			...
			isUnitsMatch = isCallForConsolidation ? (unitTaxThis.getUnitNumber().equals(
					unitTaxAgainst.getUnitNumber()) ? true : false) : (unitTaxThis.getUnitNumber()
					.equals(unitTaxAgainst.getUnitNumber())
	...
	private BigDecimal roundHistoryTax(UnitTaxCalculationInfo unitTaxCalculationInfo,
		...
		for (MiscellaneousTax miscTax : unitTaxCalculationInfo.getMiscellaneousTaxes()) {
			...
			if (hasOnlyHistoryTaxDetails) {
				...
				miscTax.setTotalActualTax(miscTax.getTotalActualTax().setScale(0,
						BigDecimal.ROUND_HALF_UP));
				miscTax.setTotalCalculatedTax(miscTax.getTotalCalculatedTax().setScale(0,
						BigDecimal.ROUND_HALF_UP));
	...
	private void calculateTaxes(Boolean isNewUnit, List<String> applicableTaxes,
			UnitTaxCalculationInfo unitTaxCalculationInfo, Installment installment,
			String propType, String propTypeCategory, String amenities, BigDecimal alv,
			Boolean isModify, Date oldUnitOccupancyDate, Map<String, BigDecimal> taxNameAndALV,
			Property property, BoundaryCategory boundaryCategory, Date nextPropOccupancyDate) {
		...
		Integer totalNoOfDays = getNumberOfDays(installment.getFromDate(), installment.getToDate())
				.intValue();
		...
		if (applicableTaxes != null) {
			for (String demandReasonCode : applicableTaxes) {
				...
				if (localALVValue == null) {
					 ...
					 * isModify) {
					 */
					miscellaneousTax = createMiscellaneousTax(isNewUnit, unitTaxCalculationInfo,
							demandReasonCode);
					// }
					...
					if (between(unitTaxCalculationInfo.getOccpancyDate(),
						...
						demandReasonDetails = this.getDemandReasonDetails(demandReasonCode,
								localALVValue, dummyInstallment);
					} else {
						demandReasonDetails = this.getDemandReasonDetails(demandReasonCode,
								localALVValue, installment);
					}
				}
				...
				for (EgDemandReasonDetails drd : demandReasonDetails) {
					...
					if (propTypeCategory != null
						...
						calculatedTaxValue = localALVValue.multiply(
								new BigDecimal(RESD_CUM_COMMERCIAL_PROP_ALV_PERCENTAGE)
						...
						calculatedTaxValue = localALVValue.multiply(taxPerc.divide(new BigDecimal(
								HUNDRED)));
					}
					...
					if (isModify && oldUnitOccupancyDate != null) {
						...
						noOfDays = getNumberOfDays(installment, drd,
								unitTaxCalculationInfo.getOccpancyDate(), boundaryCategory);
					}
					...
					if (drd.getFlatAmount().compareTo(ZERO) > 0) {
						calculatedFlatAmount = getMinOrMaxFlatAmount(totalNoOfDays, drd,
								calculatedTaxValue, noOfDays);
					}
					...
					if (calculatedFlatAmount.compareTo(BigDecimal.ZERO) == 0) {
						calculatedTaxValue = calculatedTaxValue.multiply(new BigDecimal(noOfDays))
				...
				if (!demandReasonDetails.isEmpty()) {
					...
					if (isModify && oldUnitOccupancyDate != null) {
						miscellaneousTax.setTotalActualTax(miscellaneousTax.getTotalActualTax()
								.add(totalActualTax));
						miscellaneousTax.setTotalCalculatedTax(miscellaneousTax
								.getTotalCalculatedTax().add(totalCalculatedTax));

					} else {
						miscellaneousTax.setTotalActualTax(miscellaneousTax.getTotalActualTax()
								.add(totalActualTax).setScale(0, BigDecimal.ROUND_HALF_UP));
						miscellaneousTax.setTotalCalculatedTax(miscellaneousTax
								.getTotalCalculatedTax().add(totalCalculatedTax)
	...
	private MiscellaneousTax createMiscellaneousTax(Boolean isNewUnit,
			UnitTaxCalculationInfo unitTaxCalculationInfo, String demandReasonCode) {
		MiscellaneousTax miscellaneousTax = null;
	 ...
	 */
	private BigDecimal copyHistoryTaxDetails(UnitTaxCalculationInfo historyUnit,
	...
	public Integer getNumberOfDays(Installment installment, EgDemandReasonDetails drd,
			Date unitOccupancyDate, BoundaryCategory boundaryCategory) {
		LOGGER.debug("Entered into getNumberOfDays - Installment: " + installment
				+ "DemandReasonDetails: " + drd + ", unitOccupancyDate: " + unitOccupancyDate
		...
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("getNumberOfDays: Installment - " + installment + ", days between "
					+ fromDate + " and " + toDate + " are " + noOfDays);
		}
	...
	public Integer getNumberOfDaysBeforeOccupancy(Installment installment,
			EgDemandReasonDetails drd, Date unitOccupancyDate, Date oldUnitOccupancyDate,
			BoundaryCategory boundaryCategory) {
		LOGGER.debug("Entered into getNumberOfDays");

		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("getNumberOfDays - Installment: " + installment + "DemandReasonDetails: "
					+ drd + ", unitOccupancyDate: " + unitOccupancyDate
	...
	public BigDecimal calcGovtTaxOnAmenities(String amenities, BigDecimal applicableTaxValue) {
		...
		if (amenities.equalsIgnoreCase(AMENITY_TYPE_FULL)) {
			applicableTaxValueDummy = applicableTaxValueDummy.multiply(new BigDecimal(
					AMENITY_PERCENTAGE_FULL).divide(new BigDecimal(HUNDRED)));
		} else if (amenities.equalsIgnoreCase(AMENITY_TYPE_PARTIAL)) {
			applicableTaxValueDummy = applicableTaxValueDummy.multiply(new BigDecimal(
					AMENITY_PERCENTAGE_PARTIAL).divide(new BigDecimal(HUNDRED)));
		} else if (amenities.equalsIgnoreCase(AMENITY_TYPE_NIL)) {
			applicableTaxValueDummy = applicableTaxValueDummy.multiply(new BigDecimal(
					AMENITY_PERCENTAGE_NIL).divide(new BigDecimal(HUNDRED)));
		}
		return applicableTaxValueDummy;
	}

	public String multiplicativeFactorAreaWise(String propertyType, FloorImpl floor,
			String areaFactor, String propCategory) {
		...
		if (propertyType.equals(PROPTYPE_RESD)) {
			...
			if (RESIDENTIAL_FLATS.equals(propCategory)) {
				unitAreaFactor = RESIDENTIAL_FLOOR_AREA_MAP
						.get("0" + STRING_SEPERATOR + areaFactor);
			} else {
				if (FLOORNO_WITH_DIFF_MULFACTOR_RESD.contains(floor.getFloorNo().toString())) {
					unitAreaFactor = RESIDENTIAL_FLOOR_AREA_MAP.get(floor.getFloorNo()
							+ STRING_SEPERATOR + areaFactor);
				} else {
					unitAreaFactor = RESIDENTIAL_FLOOR_AREA_MAP.get("3" + STRING_SEPERATOR
							+ areaFactor);
				}
			}
			...
			if (NONRESD_FLAT.equals(propCategory)) {
				unitAreaFactor = NON_RESIDENTIAL_FLOOR_AREA_MAP.get("0" + STRING_SEPERATOR
						+ areaFactor);
			} else {
				if (FLOORNO_WITH_DIFF_MULFACTOR_NONRESD.contains(floor.getFloorNo().toString())) {
					unitAreaFactor = NON_RESIDENTIAL_FLOOR_AREA_MAP.get(floor.getFloorNo()
							+ STRING_SEPERATOR + areaFactor);
				} else {
					unitAreaFactor = NON_RESIDENTIAL_FLOOR_AREA_MAP.get("1" + STRING_SEPERATOR
							+ areaFactor);
				}
			}
		}

		LOGGER.info("propertyType:" + propertyType + "; Floor No:" + floor.getFloorNo()
				+ "; areaFactor:" + areaFactor + "; unitAreaFactor: " + unitAreaFactor);
		return unitAreaFactor;

	}

	public List<ApplicableFactor> getApplicableFactorsForResidentialAndNonResidential(
			FloorImpl floorImpl, Boundary propertyArea, Installment installment, Long categoryId) {
		...
		Category category = (Category) persistenceService.findByNamedQuery(
				QUERY_BASERENT_BY_OCCUPANCY_AREA_STRUCTURE, propertyArea.getId(), floorImpl
						.getPropertyUsage().getId(),
				floorImpl.getStructureClassification().getId(), installment.getFromDate());
		...
		if (floorCompletionOccupation.after(dateConstant)
			...
			applicableStructuralFactor.setFactorIndex(floorImpl.getStructureClassification()
					.getTypeName());
			...
			applicableStructuralFactor.setFactorValue(new BigDecimal(Float.toString(category
					.getCategoryAmount())));
			applicableFactors.add(applicableStructuralFactor);
		}
		...
		if (installment.getToDate().after(dateConstant)) {
			...
			applicableUsageFactor.setFactorValue(new BigDecimal(Float.toString(floorImpl
					.getPropertyUsage().getUsagePercentage())));
		...
		applicableOccupancyFactor.setFactorIndex(floorImpl.getPropertyOccupation()
				.getOccupancyCode());
		applicableOccupancyFactor.setFactorValue(new BigDecimal(Float.toString(floorImpl
				.getPropertyOccupation().getOccupancyFactor())));
		...
		if (installment.getToDate().after(dateConstant)) {
			...
			if (null != floorImpl.getDepreciationMaster())
				applicableAgeFactor.setFactorIndex(floorImpl.getDepreciationMaster()
						.getDepreciationName());
			applicableAgeFactor.setFactorValue(new BigDecimal(Float.toString(floorImpl
					.getDepreciationMaster().getDepreciationPct())));
		...
		if (installment.getToDate().after(dateConstant)) {
			Category locationFactorCategory = (Category) persistenceService.find(
			...
			applicableLocationFactor.setFactorValue(new BigDecimal(locationFactorCategory
					.getCategoryAmount()).setScale(2, BigDecimal.ROUND_HALF_UP));
			applicableFactors.add(applicableLocationFactor);
		} else {
			applicableLocationFactor.setFactorName("LF");
			applicableLocationFactor.setFactorIndex(category.getCategoryName());
			applicableLocationFactor.setFactorValue(new BigDecimal(Float.toString(category
					.getCategoryAmount())));
			applicableFactors.add(applicableLocationFactor);
		}
	...
	public BigDecimal calculateBaseRentPerSqMtPerMonth(List<ApplicableFactor> applicableFactors,
			BigDecimal baseRent) {

		BigDecimal baseRentPerMonthPerSQMT = baseRent;

		for (ApplicableFactor applicableFactor : applicableFactors) {
			baseRentPerMonthPerSQMT = baseRentPerMonthPerSQMT.multiply(applicableFactor
					.getFactorValue());
		}
	...
	@SuppressWarnings("unchecked")
	public List<BoundaryCategory> getBoundaryCategories(FloorImpl floor, Boundary area,
			Installment installment, PropertyDetail propertyDetail) {
		SimpleDateFormat dateFormatter = new SimpleDateFormat("dd/MM/yyyy");
		Date dateConstant = null;
		...
		if (installment.getToDate().before(dateConstant)) {
			if (propType.equalsIgnoreCase(PROPTYPE_OPEN_PLOT)
				...
				categories = persistenceService.findAllByNamedQuery(
						QUERY_BASERENT_BY_BOUNDARY_FOR_OPENPLOT, area.getId(), usageId,
						installment.getFromDate(), installment.getToDate());
			} else {
				Date dateTogetCategory = null;
				try {
					if (floor.getExtraField3() != null
							&& installment.getFromDate().before(
									dateFormatter.parse(floor.getExtraField3()))) {
				...
				categories = persistenceService.findAllByNamedQuery(
						QUERY_BASERENT_BY_OCCUPANCY_AREA_STRUCTURE, area.getId(), floor
								.getPropertyUsage().getId(), floor.getStructureClassification()
								.getId(), dateTogetCategory, installment.getToDate());
			}

		} else {
			if (floor.getUnitType() != null
					&& floor.getUnitType().getCode().equalsIgnoreCase(UNITTYPE_OPEN_PLOT)) {
		...
		LOGGER.info("baseRentOfUnit - Installment : " + installment + " Base Rent Unit Wise: "
				+ unitWiseBaseRent);
		return categories;
	}
	...
	public List<Installment> getInstallmentListByStartDate(Date startDate) {
		return persistenceService.findAllByNamedQuery(QUERY_INSTALLMENTLISTBY_MODULE_AND_STARTYEAR,
				startDate, startDate, PTMODULENAME);
	}

	public EgDemandReason getDemandReasonByCodeAndInstallment(String demandReasonCode,
			Installment installment) {
		return (EgDemandReason) persistenceService.findByNamedQuery(
				QUERY_DEMANDREASONBY_CODE_AND_INSTALLMENTID, demandReasonCode, installment.getId());
	}

	public EgDemandReasonDetails getDemandReasonDetailsByDemandReasonId(
			EgDemandReason demandReason, BigDecimal grossAnnualRentAfterDeduction) {
		return (EgDemandReasonDetails) persistenceService.findByNamedQuery(
				QUERY_DEMANDREASONDETAILBY_DEMANDREASONID, demandReason.getId(),
	...
	public EgDemandReasonDetails getDemandReasonDetails(String demandReasonCode,
		...
		return (EgDemandReasonDetails) persistenceService.findByNamedQuery(
				QUERY_DEMANDREASONDETAILS_BY_DEMANDREASONID_DATE, demandReasonCode,
	...
	public List<EgDemandReasonDetails> getDemandReasonDetails(String demandReasonCode,
		...
		return persistenceService.findAllByNamedQuery(
				QUERY_DEMANDREASONDETAILS_BY_DEMANDREASON_AND_INSTALLMENT, demandReasonCode,
				grossAnnualRentAfterDeduction, installment.getFromDate(), installment.getToDate());
	}
	...
	public String getAppConfigValue(String key, String moduleName) {
		...
		if (key != null && moduleName != null) {
			AppConfigValues appConfigValues = appConfigValuesDAO.getAppConfigValueByDate(
					moduleName, key, new Date());
	...
	public String getAppConfigValue(String moduleName, String key, String defaultValue) {
		AppConfigValues appConfigValues = appConfigValuesDAO.getAppConfigValueByDate(moduleName,
				key, new Date());
			...
			if (Integer.valueOf(month) >= Integer.valueOf(7)
				...
				for (MiscellaneousTax miscellaneousTax : unitTaxCalculationInfo
					...
					for (MiscellaneousTaxDetail miscTaxDetail : miscellaneousTax.getTaxDetails()) {
						miscTaxDetail.setCalculatedTaxValue(miscTaxDetail.getCalculatedTaxValue()
				...
				unitTaxCalculationInfo.setTotalTaxPayable(totalTaxPayableByOccupancyDate.setScale(
	...
	private Map<String, BigDecimal> populateReasonsSum(Object[] data, Map<String, BigDecimal> taxSum) {
		...
		if ((data[0].toString()).equals(DEMANDRSN_CODE_GENERAL_TAX)) {
			...
			taxSum.put(DEMANDRSN_CODE_SEWERAGE_TAX,
					tmpVal.add(((BigDecimal) data[2]).subtract(((BigDecimal) data[3]))));
			...
			taxSum.put(DEMANDRSN_CODE_LIGHTINGTAX,
					tmpVal.add(((BigDecimal) data[2]).subtract(((BigDecimal) data[3]))));
	...
	public HashMap<String, Integer> generateOrderForDemandDetails(
			Set<EgDemandDetails> demandDetails, PropertyTaxBillable nmcBillable) {
		...
		for (EgDemandDetails demandDetail : demandDetails) {
			...
			if (balance.compareTo(BigDecimal.ZERO) == 1) {
				...
				if (reasonMasterCode.equals(DEMANDRSN_CODE_GENERAL_TAX)) {
					...
					key = String.valueOf(getOrder(cal.get(Calendar.YEAR), DEMAND_REASON_ORDER_MAP
							.get(reasonMasterCode).intValue()));
					instReasonMap.put(new Integer(key), cal.get(Calendar.YEAR) + "-"
							+ reasonMasterCode);
		...
		for (Map.Entry<Installment, PropertyInstTaxBean> mapEntry : nmcBillable.getInstTaxBean()
				.entrySet()) {
		...
		for (Integer installmentYear : installmentAndReason.keySet()) {
			...
			if (installmentAndReason.get(installmentYear).get(DEMANDRSN_CODE_PENALTY_FINES) != null) {
				orderMap.put(
						installmentAndReason.get(installmentYear).get(DEMANDRSN_CODE_PENALTY_FINES),
						order++);
			}
		}
	...
	public int getOrder(int year, int reasonOrder) {
		...
		return (reasonOrder == 0) ? (year + (year - 2005) * 20 + reasonOrder) : (year
				+ (year - 1990) * 20 + reasonOrder);
	}
	...
	public void consolidateUnitTaxCalcInfos(TaxCalculationInfo taxCalcInfo,
			Installment installment, String propertyTypeCategory, String amenities,
			Property property, List<BoundaryCategory> boundaryCategories) throws ParseException {
		...
		for (List<UnitTaxCalculationInfo> unitTaxCalcs : unitTaxes) {
			...
			if (unitTaxCalcs.size() > 1) {
				...
				if (combinedUnitTaxesByUnitNoForMultipleALV.get(unitClone.getUnitNumber()) == null) {
					...
					combinedUnitTaxesByUnitNoForMultipleALV.put(unitClone.getUnitNumber(),
							singleUnit);
				} else {
					combinedUnitTaxesByUnitNoForMultipleALV.get(unitClone.getUnitNumber()).add(
							unitClone);
				}
			...
			for (UnitTaxCalculationInfo unit : units) {
				...
				if (isNull(consolidatedUnitTaxForUnitNo.get(unitNo))) {
					...
					Boolean isUnitsMatch = PropertyTaxUtil.getIsUnitsMatch(consolidating,
							PROPERTYTYPE_STR_TO_CODE.get(taxCalcInfo.getPropertyType()),
							Boolean.FALSE, unit);
					...
					if (isNotNull(consolidating.getUnitArea()) && !isUnitsMatch) {
						consolidating.setUnitArea(isNotNull(unit.getUnitArea()) ? consolidating
								.getUnitArea().add(unit.getUnitArea()) : consolidating
								.getUnitArea().add(ZERO));
					} else {
					...
					if (isNotNull(consolidating.getBaseRent())) {
						consolidating.setBaseRent(isNotNull(unit.getBaseRent()) ? consolidating
								.getBaseRent().add(unit.getBaseRent()) : consolidating
								.getBaseRent().add(ZERO));
					} else {
						consolidating.setBaseRent(isNotNull(unit.getBaseRent()) ? ZERO.add(unit
								.getBaseRent()) : ZERO);
					}
					...
					if (isNotNull(consolidating.getBaseRentPerSqMtPerMonth())) {
						consolidating.setBaseRentPerSqMtPerMonth(isNotNull(unit
								...
								.getBaseRentPerSqMtPerMonth()
								.add(unit.getBaseRentPerSqMtPerMonth()) : consolidating
								.getBaseRentPerSqMtPerMonth().add(ZERO));
					} else {
						consolidating.setBaseRentPerSqMtPerMonth(isNotNull(unit
					...
					if (isNotNull(consolidating.getMonthlyRent())) {
						consolidating
								.setMonthlyRent(isNotNull(unit.getMonthlyRent()) ? consolidating
										.getMonthlyRent().add(unit.getMonthlyRent())
										: consolidating.getMonthlyRent().add(ZERO));
					} else {
						consolidating.setMonthlyRent(isNotNull(unit.getMonthlyRent()) ? ZERO
								.add(unit.getMonthlyRent()) : ZERO);
					}
					if (isNotNull(consolidating.getAnnualRentBeforeDeduction())) {
						consolidating.setAnnualRentBeforeDeduction(isNotNull(unit
								.getAnnualRentBeforeDeduction()) ? consolidating
								.getAnnualRentBeforeDeduction().add(
										unit.getAnnualRentBeforeDeduction()) : consolidating
								.getAnnualRentBeforeDeduction().add(ZERO));
					} else {
						consolidating.setAnnualRentBeforeDeduction(isNotNull(unit
								.getAnnualRentBeforeDeduction()) ? ZERO.add(unit
								.getAnnualRentBeforeDeduction()) : ZERO);
					}

					// if (consolidatedALV.compareTo(BigDecimal.ZERO) > 0) {
					consolidating.setAnnualRentAfterDeduction(consolidating
							.getAnnualRentAfterDeduction().add(unit.getAnnualRentAfterDeduction()));
					// }
		...
		Set<Integer> unitNos = (consolidatedUnitNos.isEmpty()) ? unitTaxesByUnitNo.keySet()
				: consolidatedUnitNos;
		...
		for (Integer unitNo : unitNos) {
			...
			calculateApplicableTaxes(new ArrayList<String>(applicableTaxes),
					unitNoWiseTaxAndALV.get(unitNo), unitTaxCalcInfo, installment, property
	...
	private UnitTaxCalculationInfo getUnitTaxCalcInfoWhenMultipleALV(
		...
		for (UnitTaxCalculationInfo u : unitTaxCalcs) {
			...
			if (i < (unitTaxCalcs.size() - 1)) {
				...
				unitClone.setTotalTaxPayable(unitClone.getTotalTaxPayable().add(
						u.getTotalTaxPayable()));
				...
				for (MiscellaneousTax miscTax : u.getMiscellaneousTaxes()) {
					...
					if (unitCloneTaxes.contains(miscTax.getTaxName())) {
						for (MiscellaneousTax mt : unitClone.getMiscellaneousTaxes()) {
							...
							if (miscTax.getTaxName().equalsIgnoreCase(mt.getTaxName())) {
								mt.setTotalCalculatedTax(mt.getTotalCalculatedTax().add(
	 ...
	 * @param taxNameAndALV
	 */
	private void putInTaxNameAndALV(Map<String, BigDecimal> taxNameAndALV, String taxName,
			BigDecimal alv) {
	...
	private void addConsolidatedUnitTaxCalReportClone(TaxCalculationInfo taxCalInfo,
	 ...
	 * @param clone
	 */
	private void addConsolidatedUnitTaxCalculationInfoClone(TaxCalculationInfo taxCalInfo,
			TaxCalculationInfo clone) {
	...
	private void setResdAndNonResdParts(Map<String, BigDecimal> taxNameAndALV,
		...
		for (MiscellaneousTax miscTax : unitTax.getMiscellaneousTaxes()) {
			if (DEMANDRSN_CODE_EDUCATIONAL_CESS_RESD.equals(miscTax.getTaxName())) {
				unitTax.setResdEduCess(unitTax.getResdEduCess()
	...
	public Boolean between(Date date, Date fromDate, Date toDate) {
		return ((date.after(fromDate) || date.equals(fromDate)) && date.before(toDate) || date
				.equals(toDate));
	}
	...
	public static int getMonthsBetweenDates(final Date fromDate, final Date toDate) {
		LOGGER.debug("Entered into getMonthsBetweenDates - fromDate: " + fromDate + ", toDate: "
				+ toDate);
	...
	public List<PropertyArrearBean> getPropertyArrears(List<PropertyArrear> arrears) {
		...
		for (PropertyArrear pa : arrears) {
			...
			value = value.add(pa.getGeneralTax()).add(pa.getSewerageTax())
					...
					.add(pa.getGeneralWaterTax()).add(pa.getEducationCess()).add(pa.getEgCess())
					.add(pa.getBigResidentailTax()).setScale(2, ROUND_HALF_UP);
	...
	private Boolean isWaterTaxImposedOnModify(Installment installment, Property newProperty,
		...
		Date oldPropertyEffectiveDate = PropertyTaxUtil
				.getEffectiveDateIfWaterTaxImposedForProperty(oldProperty, unitTax);
		...
		if (oldPropertyEffectiveDate != null) {
			newPropertyEffectiveDate = PropertyTaxUtil
					.getEffectiveDateIfWaterTaxImposedForProperty(newProperty, unitTax);
	 ...
	 * @throws ParseException
	 */
	private static Date getEffectiveDateIfWaterTaxImposedForProperty(Property property,
			UnitTaxCalculationInfo unitTax) throws ParseException {
		DateFormat dateFormat = new SimpleDateFormat(PropertyTaxConstants.DATE_FORMAT_DDMMYYY);
			...
			for (FloorIF floor : property.getPropertyDetail().getFloorDetails()) {
				if (unitTax.getUnitNumber() != null && unitTax.getFloorNumberInteger() != null) {
					if ((unitTax.getUnitNumber().equals(Integer.parseInt(floor.getExtraField1())) && unitTax
							...
							&& (floor.getWaterRate() != null && GWR_IMPOSED.equalsIgnoreCase(floor
									.getWaterRate()))) {
					...
					if (floor.getWaterRate() != null
							&& GWR_IMPOSED.equalsIgnoreCase(floor.getWaterRate())) {
						if (floor.getExtraField3() == null) {
							LOGGER.info("Floor effective date (floor.extraField3) is null");
						} else {
	...
	public UnitTaxCalculationInfo calculateAnnualTaxes(String propertyType,
			String propertyTypCategory, UnitTaxCalculationInfo unit, Installment installment,
			String amenities, Map<Installment, TaxCalculationInfo> taxCalcInfos,
		...
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("calculateAnnualTaxes - propertyType: " + propertyType
					+ ", propertyTypeCategory: " + propertyTypCategory + ", ALV: "
					+ unit.getAnnualRentAfterDeduction() + ", installment: " + installment
					+ ", amenities: " + amenities);
		}
		...
		try {
			unitTaxDate = new SimpleDateFormat(PropertyTaxConstants.DATE_FORMAT_DDMMYYY)
					.parse(unitTaxClone.getInstDate());
		} catch (ParseException e) {
		...
		if (unit.getIsHistory() == null || !unit.getIsHistory()) {
			for (MiscellaneousTax miscTax : unitTaxClone.getMiscellaneousTaxes()) {
				...
				for (MiscellaneousTaxDetail detail : miscTax.getTaxDetails()) {
					hasOnlyHistoryTaxDetails1 = detail.getIsHistory() == null ? false : detail
							.getIsHistory().equals(PropertyTaxConstants.HISTORY_TAX_DETAIL) ? true
							: false;
				}
				...
				if (isHistoryTaxEffective) {
					for (MiscellaneousTaxDetail detail : miscTax.getTaxDetails()) {
						if (detail.getIsHistory() != null
								&& detail.getIsHistory().equals(HISTORY_TAX_DETAIL)
								&& between(detail.getFromDate(), installment.getFromDate(),
						...
						if (isTaxFromDateBetweenInstallment && taxFromDate != null) {
							...
							unitTaxClone
									.setInstDate(DateUtils.getDefaultFormattedDate(taxFromDate));
		...
		for (MiscellaneousTax miscTax : unitTaxClone.getMiscellaneousTaxes()) {
			...
			if (isUseHistoryALV && taxFromDate != null) {
				...
				for (MiscellaneousTaxDetail detail : miscTax.getTaxDetails()) {
					hasOnlyHistoryTaxDetails = detail.getIsHistory() == null ? false : detail
							.getIsHistory().equals(PropertyTaxConstants.HISTORY_TAX_DETAIL) ? true
							: false;
			...
			if (hasOnlyHistoryTaxDetails && !isUseHistoryALV) {
				...
				if (!isUseHistoryALV) {
					if (DEMANDRSN_CODE_EDUCATIONAL_CESS_RESD.equalsIgnoreCase(miscTax.getTaxName())) {
						alv = unitTaxClone.getResidentialALV().compareTo(BigDecimal.ZERO) == 0 ? alv
								: unitTaxClone.getResidentialALV();
					} else if (DEMANDRSN_CODE_EDUCATIONAL_CESS_NONRESD.equalsIgnoreCase(miscTax
							.getTaxName())
							|| DEMANDRSN_CODE_EMPLOYEE_GUARANTEE_TAX.equalsIgnoreCase(miscTax
									.getTaxName())) {
						alv = unitTaxClone.getNonResidentialALV().compareTo(BigDecimal.ZERO) == 0 ? alv
								: unitTaxClone.getNonResidentialALV();
					} else if (DEMANDRSN_CODE_GENERAL_WATER_TAX.equalsIgnoreCase(miscTax
							.getTaxName()) && (taxCalcInfos.get(installment) != null)) {
						alv = getALVForDemandReason(installment, unitTaxClone,
								taxCalcInfos.get(installment), DEMANDRSN_CODE_GENERAL_WATER_TAX);
					} else if (DEMANDRSN_CODE_BIG_RESIDENTIAL_BLDG_TAX.equalsIgnoreCase(miscTax
							.getTaxName())) {
				...
				if (between(unitTaxClone.getOccpancyDate(), installment.getFromDate(),
						installment.getToDate())) {
					Installment dummyInstallment = new Installment();
					...
					demandReasonDetails = (EgDemandReasonDetails) HibernateUtil
							...
							.setDate(3, dummyInstallment.getToDate()).list().get(0);
				} else {
					...
					if (demandReasonDetails != null) {
						...
						if (ZERO.equals(demandReasonDetails.getFlatAmount())) {
							demandRsnDtlPercResult = alv.multiply(
									demandReasonDetails.getPercentage()).divide(
									new BigDecimal(HUNDRED));
	...
	public static BigDecimal getALVForDemandReason(Installment installment,
			UnitTaxCalculationInfo consolidatedUnitTax, TaxCalculationInfo taxCalcInfo,
	 ...
	 * @return
	 */
	public static Boolean isTaxExistsInUnitTaxCalcInfo(UnitTaxCalculationInfo unitTax,
			String taxName) {
	...
	public static boolean isPropertyModified(Property property) {
		...
		for (PropertyStatusValues psv : property.getBasicProperty().getPropertyStatusValuesSet()) {
			if (PROPERTY_MODIFY_REASON_MODIFY.equalsIgnoreCase(psv.getPropertyStatus()
					.getStatusCode())) {
				return true;
			}
		}
	...
	public void makeTheEgBillAsHistory(BasicProperty basicProperty) {
		EgBill egBill = (EgBill) persistenceService.find(
				...
				moduleDao.getModuleByName(PropertyTaxConstants.PTMODULENAME),
	...
	public static String buildAddress(PropertyAddress address) {
		...
		strAddress
				...
				.append("|");
		strAddress.append(isNotBlank(address.getExtraField1()) ? address.getExtraField1() : " ")
				.append("|");
		strAddress.append(isNotBlank(address.getExtraField2()) ? address.getExtraField2() : " ")
				.append("|");
		strAddress.append(isNotBlank(address.getExtraField3()) ? address.getExtraField3() : " ")
				.append("|");
	...
	public Map<Date, TaxCalculationInfo> getTaxCalInfoMap(Set<Ptdemand> ptDmdSet, Date occupancyDate) {
		...
		for (Ptdemand ptdmd : ptDmdSet) {
			...
			if (taxCalcInfo != null) {
				...
				if (between(occupancyDate, ptdmd.getEgInstallmentMaster().getFromDate(), ptdmd
	...
	public WorkflowDetails initWorkflowAction(PropertyImpl propertyModel,
		...
		if (LOGGER.isDebugEnabled()) {
			...
			LOGGER.debug("initWorkflowAction - propertyModel=" + propertyModel + ", workflowBean="
					+ workflowBean + ", loggedInUserId=" + loggedInUserId);
		}
				...
				for (MiscellaneousTax mt2 : unitTax.getMiscellaneousTaxes()) {
					if (taxNames.contains(mt2.getTaxName())) {
						...
						MiscellaneousTaxDetail mtd = mt2.getTaxDetails().size() > 1 ? mt2
								.getTaxDetails().get(1) : mt2.getTaxDetails().get(0);
			...
			for (MiscellaneousTax tax : unitTax.getMiscellaneousTaxes()) {
				...
				for (MiscellaneousTaxDetail mtd : tax.getTaxDetails()) {
					if (mtd.getIsHistory() == null
							|| NON_HISTORY_TAX_DETAIL.equals(mtd.getIsHistory())) {
	...
	public List<UnitTaxCalculationInfo> prepareUnitTaxesForChangedTaxes(Installment installment,
			UnitTaxCalculationInfo prevUnitTax, UnitTaxCalculationInfo currentUnitTax,
		...
		for (MiscellaneousTax currentMiscTax : currentMiscTaxes) {
			Boolean dmdRsnExists = prevTaxNames.contains(currentMiscTax.getTaxName()) ? true
					: false;
			...
			if (dmdRsnExists) {
				if (changedTaxesAndDates.containsKey(currentMiscTax.getTaxName())) {
					...
					LOGGER.info(currentMiscTax.getTaxName() + " has changed, "
							+ " for installment: " + installment + ", EffecDate: "
							+ taxEffectiveDate);
					...
					if (isPropertyModified && isModificationBetInstallment
						...
						currentUnitTaxClone.setInstDate(DateUtils
								.getDefaultFormattedDate(currentUnitTax.getOccpancyDate()));
					} else {
					...
					putUnitTaxInMapByDate(currentUnitTaxClone, unitTaxForChangeInTaxByDate,
							taxEffectiveDate, currentMiscTax, isPropertyModified,
				...
				for (MiscellaneousTaxDetail mtd : currentMiscTax.getTaxDetails()) {
					...
					if (between(taxEffectiveDate, installment.getFromDate(),
						...
						currentUnitTax.setInstDate(DateUtils
								.getDefaultFormattedDate(taxEffectiveDate));
						putUnitTaxInMapByDate(currentUnitTax, unitTaxForChangeInTaxByDate,
								taxEffectiveDate, currentMiscTax, isPropertyModified,
								isModificationBetInstallment);
						currentMiscTax.setHasChanged(false);
					}
	...
	private void putUnitTaxInMapByDate(UnitTaxCalculationInfo currentUnitTax,
			...
			MiscellaneousTax currentMiscTax, Boolean isPropertyModified,
			Boolean isModificationBetInstallment) {
	 ...
	 * @return
	 */
	private Map<Date, Property> getPropertiesByOccupancy(Property propertyModel,
			Boolean isConsiderMigrated) {
		...
		for (Map.Entry<Date, Property> entry : propertyByCreatedDate.entrySet()) {
			propertyByOccupancyDate.put(getPropertyOccupancyDate(entry.getValue()),
					entry.getValue());
		}
	...
	public Boolean isMatch(UnitTaxCalculationInfo unit, FloorIF floor) {
		...
		try {
			result = (floor.getFloorNo() != null && unit.getFloorNumberInteger().equals(
					floor.getFloorNo()))
					&& unit.getUnitNumber().equals(Integer.valueOf(floor.getExtraField1()))
					&& unit.getUnitArea().toString()
					...
					&& unit.getOccpancyDate().equals(
							new SimpleDateFormat(PropertyTaxConstants.DATE_FORMAT_DDMMYYY)
	...
	public Date getPropertyOccupancyDate(Property property) {
		return property.getPropertyDetail().getEffective_date() == null ? property
				.getEffectiveDate() : property.getPropertyDetail().getEffective_date();
	}
	 ...
	 */
	public boolean isUseMigratedALV(Property propertyModel, List<Date> occupancyDates,
			Installment installment) {
	 ...
	 * @return
	 */
	private Boolean isOccupancyDateLikeInstallmentFromDate(List<Date> occupancyDates,
			Installment installment) {
	...
	public void calcConsolidatedBigBldgTaxs(TaxCalculationInfo taxCalcInfo,
		...
		for (List<UnitTaxCalculationInfo> unitTaxCalcs : unitTaxes) {
			UnitTaxCalculationInfo unit = null;
			unit = unitTaxCalcs.get(0);
			if ((OWNER_OCC.equals(unit.getUnitOccupation()) || OCCUPIER_OCC.equals(unit
					.getUnitOccupation())) && USAGES_FOR_RESD.contains(unit.getUnitUsage())) {
				...
				if (totUnitAreaUnitWise.get(unit.getUnitNumber()) == null) {
					...
					BigDecimal unitArea = totUnitAreaUnitWise.get(unit.getUnitNumber()).add(
							unit.getUnitArea());
				...
				if (totALVUnitWise.get(unit.getUnitNumber()) == null) {
					...
					BigDecimal alv = totALVUnitWise.get(unit.getUnitNumber()).add(
							unit.getAnnualRentAfterDeduction());
		...
		if (consolidateUnitsByUnitNums.size() > 0) {
			for (Map.Entry<Integer, List<UnitTaxCalculationInfo>> entry1 : consolidateUnitsByUnitNums
				...
				Boolean isBigBuildingTaxExempted = taxExemptionReason == null ? false
						: PropertyTaxConstants.exemptedTaxesByReason.get(taxExemptionReason).contains(
								PropertyTaxConstants.DEMANDRSN_CODE_BIG_RESIDENTIAL_BLDG_TAX) ? true
								: false;
				...
				if (!isBigBuildingTaxExempted
					...
					if (!demandReasonDetails.isEmpty()) {
						...
						for (EgDemandReasonDetails drd : demandReasonDetails) {
							...
							bigResTax = totalALV.multiply(drd.getPercentage().divide(
									new BigDecimal("100")));
							...
							if (between(occDate, installment.getFromDate(), installment.getToDate())
								...
								bigResTax = bigResTax.multiply(new BigDecimal(noOfDays)).divide(
										new BigDecimal(totalNoOfDays), 2, ROUND_HALF_UP);
								taxDetail.setNoOfDays(noOfDays);
								LOGGER.info("Big Residential tax for " + noOfDays + " is "
										+ bigResTax);
							} else {
							...
							taxDetail.setCalculatedTaxValue(bigResTax.setScale(0,
									BigDecimal.ROUND_HALF_UP));
						 ...
						 * BigDecimal.ROUND_HALF_UP));
						 */
						miscellaneousTax.setTotalCalculatedTax(totalBigResTax.setScale(0,
								BigDecimal.ROUND_HALF_UP));
			...
			for (UnitTaxCalculationInfo conTaxInfo : taxCalcInfo
				...
				MiscellaneousTax bigBldgMiscTax = consdMiscTaxUnitWise.get(conTaxInfo
						.getUnitNumber());
				...
				if (bigBldgTax != null) {
					taxCalcInfo
							.setTotalTaxPayable(taxCalcInfo.getTotalTaxPayable().add(bigBldgTax));
	...
	public static List<String> prepareApplicableTaxes(Property property) {
		...
		if (propType.equalsIgnoreCase(PROPTYPE_OPEN_PLOT)) {
			if (PropertyTaxConstants.PROPTYPE_CAT_RESD.equalsIgnoreCase(propTypeCategory)) {
				applicableTaxes.add(DEMANDRSN_CODE_EDUCATIONAL_CESS_RESD);
			} else if (PropertyTaxConstants.PROPTYPE_CAT_NON_RESD.equalsIgnoreCase(propTypeCategory)) {
				applicableTaxes.add(DEMANDRSN_CODE_EDUCATIONAL_CESS_NONRESD);
				applicableTaxes.add(DEMANDRSN_CODE_EMPLOYEE_GUARANTEE_TAX);
			}
		}
		...
		if (propType.equalsIgnoreCase(PROPTYPE_OPEN_PLOT)) {
			...
			if (property.getIsExemptedFromTax() != null && property.getIsExemptedFromTax()) {
				exemptedTaxes = PropertyTaxConstants.exemptedTaxesByReason.get(property
						.getTaxExemptReason());
	...
	public static String getRevisedDemandYear(Property property) {
		...
		demandEffectiveYear = new SimpleDateFormat("yyyy").format(property.getPropertyDetail()
				.getEffective_date());
	...
	public static Integer getNoticeDaysForInactiveDemand(Property property) throws ParseException {
		...
		if (isNoticeGenerated(property)) {
			...
			if (result.isEmpty()) {
				...
				noticeDate = new SimpleDateFormat(PropertyTaxConstants.DATE_FORMAT_DDMMYYY)
						.parse(result.get(0).toString());
	...
	public static boolean isNoticeGenerated(Property property) {
		return (property.getExtra_field3() != null && property.getExtra_field3().equalsIgnoreCase(
				...
				|| (property.getExtra_field4() != null && property.getExtra_field4()
						.equalsIgnoreCase(PropertyTaxConstants.STR_YES)) ? true : false;
	}
	...
	public Date getEarliestModificationDate(String propertyId) {
		List result = HibernateUtil
				...
				.setString("propertyMigrationRemarks", PropertyTaxConstants.STR_MIGRATED_REMARKS)
				.list();
	...
	public static Date getWaterTaxEffectiveDateForPenalty() {
		...
		LOG.debug("getWaterTaxEffectiveDateForPenalty - waterTaxEffectiveDate = {} ",
		...
		for (EgDemandDetails dmdDet : ptDemand.getEgDemandDetails()) {
			...
			if (!demandReasonExcludeList.contains(demandReason)) {
				...
				if (installmentWiseDemand.get(installment) == null) {
					...
					installmentWiseDemand.put(installment, installmentWiseDemand.get(installment)
							.add(dmdDet.getAmount()));
				}
				...
				if (installmentWiseCollection.get(installment) == null) {
					...
					installmentWiseCollection.put(installment,
							installmentWiseCollection.get(installment)
									.add(dmdDet.getAmtCollected()));
				}
			}
		}
	...
	public Map<Date, Property> getPropertiesForPenlatyCalculation(BasicProperty basicProperty) {
		...
		String query = "select p from PropertyImpl p " + "inner join fetch p.basicProperty bp "
				...
				+ "and (p.remarks = null or p.remarks <> ?) " + "order by p.createdDate";

		List<Property> allProperties = HibernateUtil.getCurrentSession().createQuery(query)
				.setString(0, basicProperty.getUpicNo())
				.setString(1, PropertyTaxConstants.STR_MIGRATED_REMARKS).list();
		...
		for (int i = 0; i < noOfProperties; i++) {
			...
			prevMutationCode = prevProperty != null ? prevProperty.getPropertyDetail()
					.getPropertyMutationMaster().getCode() : null;
			mutationCode = property.getPropertyDetail().getPropertyMutationMaster().getCode();
			nextMutationCode = i != (noOfProperties - 1) ? allProperties.get(i + 1)
					.getPropertyDetail().getPropertyMutationMaster().getCode() : null;
	...
	private boolean isFirstDataEntry(String prevPropMutationCode, String mutationCode,
	...
	private boolean isDataEntryApproved(String prevPropMutationCode) {
		List<String> mutationsCodes = Arrays.asList("NEW", "MODIFY");
		return prevPropMutationCode == null || mutationsCodes.contains(prevPropMutationCode) ? true
				: false;
	}

...
public class PtDemandHibernateDao extends GenericHibernateDAO implements PtDemandDao {
	...
	*//**
	...
	public BigDecimal getCurrentDemandforProperty(Property property) {
		...
		Query qry = null;
		if (property != null) {
			qry = HibernateUtil.getCurrentSession()	.createQuery(
	...
	*//**
	...
	public Character whetherBillExistsForProperty(Property property, String billnum, Module module) {
		...
		if (property != null && billnum != null) {
			List<EgBill> egBillList = (List) demandGenericDao.getBillsByBillNumber(billnum, module);
	...
	*//**
	...
	public List getDmdDetailsByPropertyIdBoundary(BasicProperty basicProperty, Boundary divBoundary) {
		...
		if (basicProperty != null) {
			...
			qry
					.append(" where ptdem.isHistory='N'  and prop.status='N' and  prop.isDefaultProperty='Y' and bp.active='Y' "
							+ "and bp =:basicProperty ");
			if ("Y".equals(divStatus)) {
				qry.append(" and ppid.wardId =:divBoundary  ");
			}

			Query query = HibernateUtil.getCurrentSession().createQuery(qry.toString());
	...
	*//**
	...
	public List<BigDecimal> getCurrentAmountsFromBill(Long billId) {
		...
		sb.append("SELECT").append("    SUM(det.amount),").append("    SUM(det.amt_collected),").append(
				"    SUM(det.amt_rebate)").append(" FROM ").append("    eg_bill bill,").append(
				"    eg_demand_details det,").append("    eg_demand_reason reas,").append(
				"    eg_demand_reason_master reasm,").append("    eg_reason_category reascat,").append(
				"    eg_installment_master inst,").append("    eg_module module").append(" WHERE ").append(
				"    bill.id = :").append(BILLID_PARAM).append(" AND").append("    det.id_demand = bill.id_demand AND")
				.append("    det.id_demand_reason = reas.id AND").append(
						"    reas.id_installment = inst.id_installment AND").append(
						"    reas.id_demand_reason_master = reasm.id AND").append(
						"    reasm.id_category = reascat.id_type AND").append("    reascat.code = 'TAX' AND").append(
						"    inst.start_date <= SYSDATE AND").append("    inst.end_date   >= SYSDATE AND").append(
						"    inst.id_module = module.id_module AND").append("    module.module_name = 'Property Tax'");
    ...
    public Map<String, BigDecimal> getDemandCollMap(Property property) {
        ...
        for (Object object : dmdCollList) {
            ...
            if (currInst.equals(installment)) {
                ...
                adding rebate to collection (commenting this code because, the rebate amt is been 
                 * added to collection amt and is shown as a negative amt in 'Current Tax Due' in 
                 * search results and view property screen) 
                ...
                adding rebate to collection (commenting this code because, the rebate amt is been 
                 * added to collection amt and is shown as a negative amt in 
                 * search results and view property screen) 
                if (listObj[3] != null && !listObj[3].equals(BigDecimal.ZERO)) {

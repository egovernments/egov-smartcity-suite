...
public class UnitRateAuditService {
    ...
    public UnitRateAudit setUnitRateAuditDetails(final Category category, final String action) {
        UnitRateAudit unitRateAudit = new UnitRateAudit();
        unitRateAudit.setAction(action);
        unitRateAudit.setCategoryAmount(category.getCategoryAmount());
        unitRateAudit.setCategoryId(category.getId());
        unitRateAudit.setStructureClassification(category.getStructureClass().getId());
        unitRateAudit.setUsageId(category.getPropUsage().getId());
        unitRateAudit.setCategoryName(category.getCategoryName());
        return unitRateAudit;
    }

    @Transactional
    public void saveUnitRateDetails(final UnitRateAudit unitRateAudit) {
        unitRateAuditRepository.save(unitRateAudit);
    }

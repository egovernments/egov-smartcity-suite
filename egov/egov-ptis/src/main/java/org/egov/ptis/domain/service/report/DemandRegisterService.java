...
public class DemandRegisterService {
    ...
    public ReportOutput generateDemandRegisterReport(final String reportType, final Long wardId,
        ...
        reportInput = generateADRReport(reportParams, wardId, finYearStartDate);
    ...
    private ReportRequest generateADRReport(final Map<String, Object> reportParams, final Long wardId,
        ...
        if (!assessmentList.isEmpty()) {
            ...
            for (CFinancialYear cFinancialYear : finYearList) {
                demandRegisterInfoList.addAll(getDemandRegisterInfo(cFinancialYear.getFinYearRange(), wardId));
            }
        }
    ...
    public List<DemandRegisterInfo> getDemandRegisterInfo(final String finYear, final Long ward) {
        StringBuilder query = new StringBuilder(
                "select bp.propertyid \"assessmentNo\", at.ownersname \"ownerName\", at.doorno \"houseNo\", instm.financial_year \"financialYear\", cast(idi.demand as numeric) \"demand\", coalesce(cast(ici.collectiondate as character varying), '-') \"collectionDate\", cast(coalesce(ici.amount, 0) as numeric) \"collectedAmount\", ici.collectionmode \"collectionMode\", cast(idi.totalcollection as numeric) \"totalCollection\", cast(idi.writeoff as numeric) \"writeOff\", cast(idi.advance as numeric) \"advanceAmount\" from egpt_assessment_transactions at, egpt_basic_property bp, egpt_installment_demand_info idi left join egpt_installment_collection_info ici on idi.id=ici.installment_demand_info, eg_installment_master instm where idi.assessment_transactions=at.id and at.basicproperty=bp.id and idi.installment=instm.id and instm.financial_year=:finYear and at.ward =:ward order by at.transaction_date");

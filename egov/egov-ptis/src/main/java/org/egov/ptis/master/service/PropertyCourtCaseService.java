...
public class PropertyCourtCaseService {
    ...
    @Transactional
    public void save(final PropertyCourtCase propertyCourtCase) {
        propertyCourtCaseRepository.save(propertyCourtCase);
    }
    ...
    public void addModelAttributes(final Model model, BasicProperty basicProperty) {
        ...
        ownerName = ownerInfoList.stream().map(owner -> owner.getOwner().getName()).collect(Collectors.joining(","));
        }
        ...
        model.addAttribute("taxDues", propertyService.getTotalPropertyTaxDueIncludingPenalty(basicProperty));
        final Map<String, BigDecimal> propertyTaxDetails = ptDemandDAO
				.getDemandIncludingPenaltyCollMap(basicProperty.getProperty());
		model.addAttribute("yearlyTax", propertyTaxDetails.get(CURR_FIRSTHALF_DMD_STR));
	}

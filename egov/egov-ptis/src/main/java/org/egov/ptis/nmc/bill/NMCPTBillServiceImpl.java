...
import static org.egov.ptis.nmc.constants.NMCPTISConstants.DEMANDRSN_CODE_ADVANCE_REBATE;
...
import static org.egov.ptis.nmc.constants.NMCPTISConstants.GLCODE_FOR_ADVANCE_REBATE;
...
public class NMCPTBillServiceImpl extends BillServiceInterface {
	...
	public List<EgBillDetails> getBilldetails(Billable billObj) {
		...
		Map<Installment, BigDecimal> installmentWiseDemand = propertyTaxUtil.prepareRsnWiseDemandForProp(activeProperty);
		BigDecimal currentInstallmentDemand = installmentWiseDemand.get(currentInstallment);
		...
		for (EgDemandDetails demandDetail : ptDemand.getEgDemandDetails()) {
			...
			if (balance.compareTo(BigDecimal.ZERO) == 1) {
				...
				PropertyInstTaxBean installmentTaxBean = nmcBillable.getInstTaxBean().get(installment);
				...
				if (reasonMasterCode.equalsIgnoreCase(DEMANDRSN_CODE_GENERAL_TAX)) {
					...
					if (rebateAmt != null && rebateAmt.compareTo(BigDecimal.ZERO) == 1) {
						String rebateKey = cal.get(Calendar.YEAR) + "-" + DEMANDRSN_REBATE;
						billDetailBean = new BillDetailBean(installment, orderMap.get(rebateKey), rebateKey, rebateAmt,
								GLCODE_FOR_TAXREBATE, DEMANDRSN_REBATE, Integer.valueOf(0));
					...
					billDetailBean = new BillDetailBean(installment, orderMap.get(key), key, demandDetail.getAmount()
							.subtract(demandDetail.getAmtCollected()), reason.getGlcodeId().getGlcode(), reason
							.getEgDemandReasonMaster().getReasonMaster(), Integer.valueOf(1));
					billDetails.add(createBillDet(billDetailBean));

				} else {

					if (!reasonMasterCode.equalsIgnoreCase(DEMANDRSN_CODE_PENALTY_FINES)) {

						billDetailBean = new BillDetailBean(installment, orderMap.get(key), key, demandDetail
		...
		for (Map.Entry<Installment, PropertyInstTaxBean> instMap : instTaxBean.entrySet()) {
			...
			if (instMap.getValue().getInstPenaltyAmt() != null
					&& instMap.getValue().getInstPenaltyAmt().compareTo(BigDecimal.ZERO) >= 0) {
		 ...
		 * demand is zero.
		 */
		/*if ((advanceColl.abs()).compareTo(maxAdvCollection.add(currInstBal)) == -1) {
			billDetails = createAdvanceBillDetails(billDetails, advanceColl, currInstDem, orderNo);
		}*/
		
		createAdvanceBillDetails(billDetails, currentInstallment, currentInstallmentDemand, orderMap, ptDemand, nmcBillable);
	 ...
	 * @param installment
	 */
	private void createAdvanceBillDetails(List<EgBillDetails> billDetails, Installment currentInstallment,
			BigDecimal currentInstallmentDemand, HashMap<String, Integer> orderMap, Ptdemand ptDemand, NMCPropertyTaxBillable nmcBillable) {

		BillDetailBean billDetailBean = null;
		String advanceKey = null;
		
		List<String> advanceInstDescription = PropertyTaxUtil.getAdvanceYearsFromCurrentInstallment();
		BigDecimal advanceCollection = new DemandGenericHibDao().getBalanceByDmdMasterCode(ptDemand,
				DEMANDRSN_CODE_ADVANCE, getModule());
		...
		if (noOfAdvancesPaid < MAX_ADVANCES_ALLOWED) {
			...
			for (int i = noOfAdvancesPaid; i < advanceInstDescription.size(); i++) {
				...
				billDetailBean = new BillDetailBean(currentInstallment, orderMap.get(advanceKey),
	...
	private EgDemandDetails prepareDmdAndBillDetails(Billable billObj, List<EgBillDetails> billDetails,
		...
		boolean thereIsPenalty = penAmt != null && !penAmt.equals(BigDecimal.ZERO) ? true : false;
		...
		if (nmcBillable.getLevyPenalty()) {
			...
			if (thereIsPenalty) {
				...
				BillDetailBean billDetailBean = new BillDetailBean(installment, orderMap.get(key), key, penAmt,
						GLCODE_FOR_PENALTY, NMCPTISConstants.DEMANDRSN_STR_PENALTY_FINES, Integer.valueOf(1));
				billDetails.add(createBillDet(billDetailBean));
			}
		}

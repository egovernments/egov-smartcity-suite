...
import org.egov.ptis.nmc.bill.PenaltyBill;
...
public class PenaltyCalculationService {
	...
	public PenaltyCalculationService(BasicProperty basicProperty, 
			Map<Installment, BigDecimal> installmentWiseDemand, 
			Map<Installment, BigDecimal> installmentWiseCollection,
	...
	private Map<Installment, Date> getPenaltyEffectiveDates(BasicProperty basicProperty, List<Installment> installments) throws ValidationException {
		LOGGER.debug("Entered into getPenaltyEffectiveDates, basicProperty={}, installments={}", basicProperty, installments);
			...
			for (Map.Entry<Installment, Date> mapEntry : installmentAnd90DaysDate.entrySet()) {
				...
				if (dateOn91stDay == null) {
					if ( isRolloverInstallment(installment) ) {
						penaltyEffectiveDate = getPenaltyEffectiveForRolloverInstallment(latestCollectionDate, installment.getFromDate());
					} 
				} else {
		...
		LOGGER.debug("getPenaltyEffectiveDates - installmentWisePenaltyEffectiveDate={}", installmentWisePenaltyEffectiveDate);
	...
	public Map<Installment, Date> getAllBillGenerationDates(BasicProperty basicProperty, List<Installment> installments) throws ValidationException {
		...
		LOGGER.info("getAllBillGenerationDates - propertySystemCreatedDates=" + propertySystemCreatedDates + ", occupancyDates=" + occupancyDates);
		...
		//if there are more bills as compared to bills generated after modification
		if (!demandBills.isEmpty()) {
		...
		for (int i = 0; i < totalBills; i++) {
			...
			for (Installment installment : installments) {
				...
				if (billDetails.get(i).getIsBillGeneratedAfterRollover()) {
					...
					if (PropertyTaxUtil.afterOrEqual(createdDate, installment.getFromDate()) && !billDetails.get(i).getIsBillGeneratedAfterRollover()) {
		...
		// this is the for the installments for which bills not there, ex. after rollover, if no bill is generated
		for (Installment installment : installments) {
	...
	public Date getPenaltyEffectiveForRolloverInstallment(Date latestCollReceiptDate, Date fromDate) {
		LOGGER.info("Enter into calcPanalty - latestCollReceiptDate: " + latestCollReceiptDate + ", fromDate " + fromDate);
		Calendar fromDateCalendar = Calendar.getInstance();

...
public class PropertyTaxNumberGenerator {
	...
	public String generateManualBillNumber(PropertyID propertyID) {
		...
		zoneNo = StringUtils.leftPad(zoneNo, 2, '0');
		wardNo = StringUtils.leftPad(wardNo, 3, '0');

		String index = sequenceNumberGenerator.getNextNumberWithFormat(MANUAL_BILLGEN_SEQNAME_PREFIX + wardNo, 6, '0',
				Long.valueOf(1)).getFormattedNumber();

		billNo.append(zoneNo).append("/").append(wardNo).append("/").append(index);
		return billNo.toString();
	}

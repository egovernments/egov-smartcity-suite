...
begin
   ...
   select districtname into v_districtname from eg_city;
   select regionname into v_regionname from eg_city;
   for allheader in (select ch.id from egcl_collectionheader ch, egpt_basic_property bp, egpt_propertyid pid where ch.servicedetails = (select id from egcl_servicedetails where upper(name) = 'PROPERTY TAX') and bp.propertyid=ch.consumercode and bp.id_propertyid=pid.id and pid.ward_adm_id=in_wardid)
	loop
		v_temp := loadcollectionindex(allheader.id, v_ulbname, v_districtname, v_regionname);
	end loop;
exception 

...
begin
   ...
   for allheader in (select ch.id from egcl_collectionheader ch where ch.servicedetails = (select id from egcl_servicedetails where upper(code) in ('PT', 'VLT')) and receiptnumber is not null)
	loop
...
begin
   ...
   select districtname into v_districtname from eg_city;
   select regionname into v_regionname from eg_city;
   for allheader in (select ch.id from egcl_collectionheader ch, egpt_basic_property bp, egpt_propertyid pid where ch.servicedetails = (select id from egcl_servicedetails where upper(code) in ('PT', 'VLT'))  and ch.receiptnumber is not null and bp.propertyid=ch.consumercode and bp.id_propertyid=pid.id and pid.ward_adm_id=in_wardid)
	loop
		v_temp := loadcollectionindex(allheader.id, v_ulbname, v_districtname, v_regionname);
	end loop;
exception 
...
begin
   ...
   select districtname into v_districtname from eg_city;
   select regionname into v_regionname from eg_city;
   for incrheader in (select ch.id from egcl_collectionheader ch where ch.servicedetails in (select id from egcl_servicedetails where ch.receiptnumber is not null and upper(code) in ('PT', 'VLT')) and status in (select id from egw_status where  moduletype='ReceiptHeader' and code in ('TO_BE_SUBMITTED', 'SUBMITTED', 'APPROVED', 'REMITTED')) and ch.receiptdate not in (select receiptdate from egpt_collectionindex))
...
begin
   for header in (select ch.*, status.description from egcl_collectionheader ch, egw_status status where ch.status=status.id and ch.servicedetails = (select id from egcl_servicedetails where upper(name) = 'PROPERTY TAX') and ch.id=in_rcptheadid)
	...
	-- current tax without library cess 
      select sum(cd.cramount) into v_arrearamount from egcl_collectiondetails cd where cd.collectionheader=v_rcptheadid and cd.cramount>0 and cd.purpose='CURRENT_AMOUNT' and upper(cd.description) not like upper('Library Cess')||'%';
      --raise notice 'current amount (%)', v_currentamount;

	--current library cess
      select sum(cd.cramount) into v_arrearamount from egcl_collectiondetails cd where cd.collectionheader=v_rcptheadid and cd.cramount>0 and cd.purpose='CURRENT_AMOUNT' and upper(cd.description) like upper('Library Cess')||'%';
	...
	--ealry payment rebate
     select sum(cd.dramount) into v_rebateamount from egcl_collectiondetails cd where v_rcptheadid=cd.collectionheader and cd.dramount>0 and cd.purpose = 'ADVANCE_AMOUNT';
      --raise notice 'rebate (%)', v_rebateamount;v_advance

	--advance collection
     select sum(cd.dramount) into v_advance from egcl_collectiondetails cd where v_rcptheadid=cd.collectionheader and cd.dramount>0 and cd.purpose = 'REBATE';

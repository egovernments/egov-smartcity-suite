...
Insert into eg_appconfig (id,key_name,description,version,createdby,lastmodifiedby,createddate,lastmodifieddate,module) 
  values (nextval('SEQ_EG_APPCONFIG'),'PTIS_ENDORSEMENT_REQUIRED','Key to enable/disable endorsement button in JA/SA/RI login for ptis', 0, 
...
Insert into eg_appconfig_values (id, key_id, effective_from, createdby,lastmodifiedby,createddate,lastmodifieddate,value, version) values 
  (nextval('SEQ_EG_APPCONFIG_VALUES'), (select id from eg_appconfig where key_name = 'PTIS_ENDORSEMENT_REQUIRED' and module = (select id from eg_module where name = 'Property Tax')),

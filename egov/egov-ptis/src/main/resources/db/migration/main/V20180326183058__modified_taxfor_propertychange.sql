  ...
  loop
    begin
      ...
      --raise notice 'dmdincr_incrementalload: v_basicproperty (%)', v_basicproperty;
          for proprec in (select prop.id propid, prop.modify_reason, prop.status, prop.modified_date, prop.isexemptedfromtax from egpt_property prop where prop.status not in ('C', 'W') and prop.id_basic_property=v_basicproperty and not exists (select tpc.* from egpt_taxfor_propertychange tpc where tpc.property=prop.id) order by prop.id_basic_property, prop.id, prop.modified_date)
          loop
                begin

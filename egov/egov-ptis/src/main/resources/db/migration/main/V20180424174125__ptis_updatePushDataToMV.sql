...
begin
  ...
  insert into egpt_mv_assessments (id, assessmentno, date_modified, type) select nextval('seq_egpt_mv_assessments'), assessmentno, lastmodifieddate, 'UNDERCOURTCASE' from egpt_courtcases where lastmodifieddate::date  >= NOW()::date - noofdays;
  insert into egpt_mv_assessments (id, assessmentno, date_modified, type) select nextval('seq_egpt_mv_assessments'), bp.propertyid, oa.lastmodifieddate, 'EDITOWNER' from egpt_basic_property bp,egpt_owneraudit oa where bp.id=oa.id_basic_property and oa.lastmodifieddate >= NOW()::date - noofdays;
END;
$function$;

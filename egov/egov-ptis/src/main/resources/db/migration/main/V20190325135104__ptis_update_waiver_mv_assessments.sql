CREATE OR REPLACE FUNCTION egptwardwiseupdatematview(in_wardid bigint)
 RETURNS void
 LANGUAGE plpgsql
AS $function$
declare
    ...
    v_waivedoff_amount                      double precision default 0;
BEGIN
	--raise notice 'egptWardWiseUpdateMatView : data logged';
	...
	--raise notice 'egptWardWiseUpdateMatView : v_moduleid, v_curr1stinst, v_curr2ndinst (% % %)', v_moduleid, v_curr1stinst, v_curr2ndinst;
	for records in (select bp.propertyid from egpt_basic_property bp, egpt_propertyid pid where bp.propertyid is not null and bp.id_propertyid = pid.id and pid.ward_adm_id = in_wardid)
	loop
	begin
		v_assessmentno := records.propertyid;
		--raise notice 'egptWardWiseUpdateMatView : v_assessmentno (%)', v_assessmentno;
		DELETE from egpt_mv_propertyinfo where upicno = v_assessmentno;
		--raise notice 'egptWardWiseUpdateMatView : deleted from mv and started getting latest data for assesssment (% %)', v_assessmentno , v_basicpropertyid;
		...
		--raise notice 'egptWardWiseUpdateMatView : got basic details';
		...
		--raise notice 'egptWardWiseUpdateMatView : got address details';
		...
		--raise notice 'egptWardWiseUpdateMatView : got current demand %', v_iddemand;
		...
		--raise notice 'egptWardWiseUpdateMatView : got alv';

             	--penalty waive off amount
             	select sum(COALESCE(ddv.dramount, 0)) into v_waivedoff_amount 
            	from eg_demand_detail_variation ddv 
            	join eg_demand_details dd on dd.id=ddv.demand_detail
            	join eg_demand_reason_master drm on drm.id=ddv.demand_reason_master
            	where drm.code='PENALTY_WAIVEOFF' and dd.id_demand=v_iddemand;
            	--raise notice 'v_waivedoff_amount : - %',v_waivedoff_amount;
		...
		--raise notice 'egptWardWiseUpdateMatView got arrear taxes: v_aggregate_arrear_demand, v_arrearcollection (% %)', v_aggregate_arrear_demand, v_arrearcollection;
		...
		and dr.id_installment not in (v_curr1stinst, v_curr2ndinst)
		and dd.id_demand=v_iddemand;
		--raise notice 'egptWardWiseUpdateMatView : got arrear penalty tax, v_pen_aggr_arrear_demand, v_pen_aggr_arr_coll (% %)', v_pen_aggr_arrear_demand, v_pen_aggr_arr_coll;
		...
		and dd.id_demand=v_iddemand;
		--raise notice 'egptWardWiseUpdateMatView got Current 1st half tax details: v_aggregate_curr1st_demand, v_curr1st_collection (% %)', v_aggregate_curr1st_demand, v_curr1st_collection;
		...
		and dd.id_demand=v_iddemand;
		--raise notice 'egptWardWiseUpdateMatView got Current 2nd half tax details : v_aggregate_curr2nd_demand, v_curr2nd_collection (% %)', v_aggregate_curr2nd_demand, v_curr2nd_collection;
		...
		and dr.id_installment=v_curr1stinst
		and dd.id_demand=v_iddemand;
		--raise notice 'egptWardWiseUpdateMatView got Current 1st penalty details : v_pen_aggr_curr1st_demand, v_pen_aggr_curr1st_coll (% %)', v_pen_aggr_curr1st_demand, v_pen_aggr_curr1st_coll;
		...
		and dr.id_installment=v_curr2ndinst
		and dd.id_demand=v_iddemand;
		--raise notice 'egptWardWiseUpdateMatView got Current 1st penalty details : v_pen_aggr_curr2nd_demand, v_pen_aggr_curr2nd_coll (% %)', v_pen_aggr_curr2nd_demand, v_pen_aggr_curr2nd_coll;
		...
		and dr.id_installment=v_curr2ndinst
		and dd.id_demand=v_iddemand;
		--raise notice 'egptWardWiseUpdateMatView got Advance collection details : v_advance (%)', v_advance;
   ...
   FROM ( SELECT v_basicpropertyid as basicproperty, dr.id_installment,
            ...
            CASE drm.code WHEN 'GEN_TAX'::text THEN det.amount END AS generaltax,
            CASE drm.code WHEN 'LIB_CESS'::text THEN det.amount END AS libcesstax,
            CASE drm.code WHEN 'EDU_TAX'::text THEN det.amount END AS educesstax,
            CASE drm.code WHEN 'UNAUTH_PENALTY'::text THEN det.amount END AS unauthpenaltytax,
            CASE drm.code WHEN 'PENALTY_FINES'::text THEN det.amount END AS penaltyfinestax,
            CASE drm.code WHEN 'SEW_TAX'::text THEN det.amount END AS sewtax,
            CASE drm.code WHEN 'VAC_LAND_TAX'::text THEN det.amount END AS vacantlandtax,
            CASE drm.code WHEN 'PUB_SER_CHRG'::text THEN det.amount END AS pubserchrgtax,
            CASE drm.code WHEN 'GEN_TAX'::text THEN det.amt_collected END AS generaltaxcoll,
            CASE drm.code WHEN 'LIB_CESS'::text THEN det.amt_collected END AS libcesstaxcoll,
            CASE drm.code WHEN 'EDU_TAX'::text THEN det.amt_collected END AS educesstaxcoll,
            CASE drm.code WHEN 'UNAUTH_PENALTY'::text THEN det.amt_collected END AS unauthpenaltytaxcoll,
            CASE drm.code WHEN 'PENALTY_FINES'::text THEN det.amt_collected END AS penaltyfinestaxcoll,
            CASE drm.code WHEN 'SEW_TAX'::text THEN det.amt_collected END AS sewtaxcoll,
            CASE drm.code WHEN 'VAC_LAND_TAX'::text THEN det.amt_collected END AS vacantlandtaxcoll,
            CASE drm.code WHEN 'PUB_SER_CHRG'::text THEN det.amt_collected END AS pubserchrgtaxcoll,
	    	CASE drm.code WHEN 'GEN_TAX'::text THEN det.amount END AS gen_tax,
            CASE drm.code WHEN 'SCAVENGE_TAX'::text THEN det.amount END AS scavengetax,
            CASE drm.code WHEN 'DRAINAGE_TAX'::text THEN det.amount END AS drainagetax,
            CASE drm.code WHEN 'LIGHT_TAX'::text THEN det.amount END AS lighttax,
            CASE drm.code WHEN 'WATER_TAX'::text THEN det.amount END AS watertax,
	    	CASE drm.code WHEN 'GEN_TAX'::text THEN det.amt_collected END AS gen_tax_coll,
            CASE drm.code WHEN 'SCAVENGE_TAX'::text THEN det.amt_collected END AS scavengetaxcoll,
            CASE drm.code WHEN 'DRAINAGE_TAX'::text THEN det.amt_collected END AS drainagetaxcoll,
            CASE drm.code WHEN 'LIGHT_TAX'::text THEN det.amt_collected END AS lighttaxcoll,
            CASE drm.code WHEN 'WATER_TAX'::text THEN det.amt_collected END AS watertaxcoll
	...
	--updating general tax as total of scavengetax, drainagetax, lighttax, watertax
		...
		FOR r in (select id_installment from egpt_mv_inst_dem_coll where id_basic_property=v_basicpropertyid)
		    LOOP
			begin
			select sum(COALESCE(generaltax,0) + COALESCE(scavengetax,0) + COALESCE(drainagetax,0) + COALESCE(lighttax,0) + COALESCE(watertax,0)), sum(COALESCE(generaltaxcoll,0) + COALESCE(scavengetaxcoll,0) + COALESCE(drainagetaxcoll,0) + COALESCE(lighttaxcoll,0) + COALESCE(watertaxcoll,0)) into v_agg_gentax, v_agg_gentaxcoll from egpt_mv_inst_dem_coll where id_basic_property=v_basicpropertyid and id_installment=r.id_installment;

			update egpt_mv_inst_dem_coll set generaltax = v_agg_gentax, generaltaxcoll= v_agg_gentaxcoll, balance=(generaltax + libcesstax + educesstax + unauthPenaltyTax + penaltyFinesTax + sewtax + vacantLandTax + pubSerChrgTax)-(GENERALTAXCOLL + libCessTaxColl + eduCessTaxColl + unauthPenaltyTaxColl + penaltyFinesTaxColl + sewTaxColl + vacantLandTaxColl + pubSerChrgTaxColl) where id_basic_property=v_basicpropertyid and id_installment=r.id_installment; 
	
		END;
		END LOOP;

		--raise notice 'egptWardWiseUpdateMatView : latest data (% % % % % % % % % %)',v_aggregate_arrear_demand,v_annualdemand,v_pen_aggr_arrear_demand,v_pen_aggr_curr1st_demand,v_pen_aggr_curr2nd_demand, v_arrearcollection,v_annualcoll,v_pen_aggr_arr_coll,v_pen_aggr_curr1st_coll,v_pen_aggr_curr2nd_coll;
		--Calculate total due
		...
		--raise notice 'egptWardWiseUpdateMatView v_totaldue : (%)', v_totaldue;

		if (v_advance = 0 and v_totaldue > 0) then
			select inst.description into v_frominstallment from egpt_mv_inst_dem_coll idc, eg_installment_master inst where idc.id_installment = inst.id and idc.id_basic_property = v_basicpropertyid and idc.balance>0 order by inst.start_date asc limit 1;
		...
		--raise notice 'egptWardWiseUpdateMatView : updated mv for v_upicno (%)', v_upicno;
		
		EXCEPTION
		WHEN OTHERS THEN
		raise notice 'egptWardWiseUpdateMatView : % %', SQLERRM, SQLSTATE;
		END;
	END LOOP;

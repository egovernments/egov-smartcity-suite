...
declare
	...
	v_writeoff_amount			double precision default 0;
	v_courtverdict_amount			double precision default 0;
BEGIN
	...
	begin
		...
		--write off amount
		select sum(COALESCE(ddv.dramount, 0)) into v_writeoff_amount 
                from eg_demand_detail_variation ddv 
                join eg_demand_details dd on dd.id=ddv.demand_detail
                join eg_demand_reason_master drm on drm.id=ddv.demand_reason_master
                where drm.code='WRITE_OFF' and dd.id_demand=v_iddemand;
                
                --court verdict amount
		select sum(COALESCE(ddv.dramount, 0)) into v_courtverdict_amount 
                from eg_demand_detail_variation ddv 
                join eg_demand_details dd on dd.id=ddv.demand_detail
                join eg_demand_reason_master drm on drm.id=ddv.demand_reason_master
                where drm.code='COURT_CASE' and dd.id_demand=v_iddemand;

...
declare
	...
	v_inst_writeoff				double precision default 0;
	v_inst_courtverdict			double precision default 0;
BEGIN
   ...
   FROM ( SELECT v_basicpropertyid as basicproperty, dr.id_installment,
	...
	--updating general tax as total of scavengetax, drainagetax, lighttax, watertax
		...
		--installment wise write off amount
		select COALESCE(sum(COALESCE(ddv.dramount, 0)), 0) into v_inst_writeoff 
                from eg_demand_detail_variation ddv, eg_demand_details dd, eg_demand_reason_master drmvar, eg_demand_reason dr
                where dd.id = ddv.demand_detail
                and drmvar.id = ddv.demand_reason_master
                and dr.id = dd.id_demand_reason
                and drmvar.code = 'WRITE_OFF' 
                and dd.id_demand = v_iddemand
                and dr.id_installment = r.id_installment;
                
                --installment wise court verdict amount
                select COALESCE(sum(COALESCE(ddv.dramount, 0)), 0) into v_inst_courtverdict 
                from eg_demand_detail_variation ddv, eg_demand_details dd, eg_demand_reason_master drmvar, eg_demand_reason dr
                where dd.id = ddv.demand_detail
                and drmvar.id = ddv.demand_reason_master
                and dr.id = dd.id_demand_reason
                ...
                update egpt_mv_inst_dem_coll set generaltax = v_agg_gentax, generaltaxcoll= v_agg_gentaxcoll, courtverdict_amt = v_inst_courtverdict, writeoff_amt = v_inst_writeoff where id_basic_property=v_basicpropertyid and id_installment=r.id_installment;

...
public class ModifyPropertyAction extends PropertyTaxBaseAction {
	...
	public String view() {
		if (getModelId() != null) {
			...
			if (propertyModel.getState() != null) {
				endorsementRequired = propertyTaxCommonUtils.getEndorsementGenerate(
						securityUtils.getCurrentUser().getId(), propertyModel.getCurrentState());
	...
	public String approve() {
		...
		if ("Y".equalsIgnoreCase(appConfigValue)) {
			Map<String, Map<String, Object>> voucherData = propService.prepareDemandVoucherData(propertyModel,
					oldProperty, false);
	...
	private void createPropertyStatusValues() {
		...
		final PropertyTypeMaster proptypeMstr = propertyTypeMasterDAO
				.findById(propertyModel.getPropertyDetail().getPropertyTypeMaster().getId(), false);
		if (!proptypeMstr.getCode().equalsIgnoreCase(OWNERSHIP_TYPE_VAC_LAND))
			propCompletionDate = propService
	...
	public String reject() {
		...
		if (wfInitiator != null) {
			...
			if (SOURCE_SURVEY.equalsIgnoreCase(propertyModel.getSource())) {
				SurveyBean surveyBean = new SurveyBean();
				if (isThirdPartyCheckbox() && PropertyTaxConstants.WF_STATE_UD_REVENUE_INSPECTOR_APPROVAL_PENDING
	...
	public void prepare() {
		...
		if (getModelId() != null && !getModelId().isEmpty())
			...
			setBasicProp(basicPropertyDAO.getBasicPropertyByPropertyID(indexNumber));
	...
	private void populateDropdowns() {
		...
		final List<StructureClassification> structureList = structureClassificationService.getAllActiveStructureTypes();
		...
		final List<Apartment> apartmentsList = apartmentService.getAllApartments();
	...
	public void validate() {
		...
		validateProperty(propertyModel, areaOfPlot, eastBoundary, westBoundary, southBoundary, northBoundary,
				propTypeId, modifyRsn, propCompletionDate, vacantLandPlotAreaId, layoutApprovalAuthorityId, null);
		validateBoundaries();
		validateApproverDetails();
	...
	private void validateBoundaries() {
		...
		if ((electionWardId == null || electionWardId == -1)
				&& !basicProp.getPropertyID().getElectionBoundary().isActive())
	...
	public String printAck() {
		...
		if (modifyRsn.equals(PROPERTY_MODIFY_REASON_GENERAL_REVISION_PETITION)) {
			reportParams.put("noOfDays", ((PtApplicationType) entityManager.createNamedQuery(PtApplicationType.BY_CODE)
					.setParameter("code", "REVISION_PETETION").getSingleResult()).getResolutionTime().toString());
	...
	private void setBoundaryDetailsFlag(final BasicProperty basicProperty) {
		...
		if (basicProperty.getPropertyID().getLocality() != null
				&& basicProperty.getPropertyID().getLocality().isActive())

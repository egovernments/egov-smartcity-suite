...
public class GisSearchPropertyAction extends BaseFormAction {
	...
	public void gisFormRedirect() {
		...
		try {
			response.sendRedirect(response.encodeRedirectURL(
					GISVERSION + GISCITY + "/ajaxtiledviewersample.jsp?DomainName=" + GISCITY + "&mode=PTIS"));
	...
	@SkipValidation
	@Action(value = "/gisSearchProperty-gisSearchForm", results = {
			@Result(name = NEW, location = "/gisSearchProperty-new.jsp") })
	...
	public String srchByBndry() {
		...
		if (zoneId != null && zoneId != -1)
			try {
				...
				setSearchValue(
						"Zone Num: " + strZoneNum + ", Ward Num: " + strWardNum + ", Plot No/House No: " + houseNum);
		...
		if (zoneId != null && zoneId != -1)
			try {
				...
				if (areaId != null && areaId != -1)
					query.setInteger("AreaID", areaId);
				if (propTypeId != null && propTypeId != -1)
					query.setLong("propType", propTypeId);
				final List<PropertyMaterlizeView> propertyList = query.list();
				if (propertyList.size() < 0) {
					setSESSION(getSESSION());
					setMode("propType");
				...
				setSearchValue(
						"Zone Num: " + strZoneNum + ", Ward Num: " + strWardNum + ", Property Type: " + propTypeName);
				LOGGER.debug("Search Criteria : " + getSearchCreteria());
				LOGGER.debug("Search Value : " + getSearchValue());
				setSESSION(getSESSION());
				setGisCity(GISCITY);
	...
	public void prepare() {
		...
		final List<Boundary> zoneList = (List<Boundary>) entityManager.unwrap(Session.class)
				.createQuery(
						"from BoundaryImpl BI where BI.boundaryType.name=:type and BI.boundaryType.heirarchyType.name=:hierarchy "
								+ "and BI.isHistory='N' order by BI.id")
				.setParameter("type", ZONE_BNDRY_TYPE).setParameter("hierarchy", ELECTION_HIERARCHY_TYPE);
	...
	private void prepareWardDropDownData(final boolean zoneExists, final boolean wardExists) {
		...
		if (zoneExists && wardExists) {
			...
			wardNewList = (List<Boundary>) entityManager.unwrap(Session.class)
					.createQuery(
							"from BoundaryImpl BI where BI.boundaryType.name=:type and BI.parent.id = :parent and BI.isHistory='N' order by BI.name ")
					.setParameter("type", WARD_BNDRY_TYPE).setParameter("parent", getZoneId());
	...
	private void prepareAreaDropDownData(final boolean wardExists, final boolean areaExists) {
		...
		if (wardExists && areaExists) {
			...
			areaNewList = (List<Boundary>) entityManager.unwrap(Session.class)
					.createQuery(
							"from BoundaryImpl BI where BI.boundaryType.name=:type and BI.parent.id =:parent and BI.isHistory='N' order by BI.name ")
					.setParameter("type", AREA_BNDRY_TYPE).setParameter("parent", getWardId());
	...
	public void setPropTypeId(final Long propTypeId) {

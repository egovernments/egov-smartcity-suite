...
@Controller
@RequestMapping("/integratedDemandBill")
public class IntegratedDemandBillController {
    ...
    public String inventoryNotice(@ModelAttribute("egBill") final EgBill egBill, final Model model,
        ...
        intgDemandBillService.validate(egBill, errors);
        if (errors.hasErrors()) {
            model.addAttribute("integrateddemandbill", new EgBill());
            return INTEGRATEDDEMANDBILL_FORM;
        } else
    ...
    public @ResponseBody ResponseEntity<byte[]> generateDemandBill(final Model model, @PathVariable final String assessment) {
        return intgDemandBillService.generateIntgDemandBill(assessment);
    }

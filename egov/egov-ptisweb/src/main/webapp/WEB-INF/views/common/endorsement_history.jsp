...
<table class="table table-bordered">
	<s:if test="%{!endorsementNotices.isEmpty()}">
		<input type="hidden" id="endorsementNotices"
			value="${endorsementNotices}" />
		<c:choose>
			<c:when test="${!endorsementNotices.isEmpty()}">
				<tbody>
					<tr>
						<th><spring:message code="lbl.endorsement" /></th>
					</tr>
				</tbody>
				<c:forEach items="${endorsementNotices}" var="ptNotice"
					varStatus="serialNo">
					<tbody>
						<tr>
						<td>Endorsement Notice</td>
							<td>
								<a
								href="javascript:viewDocument('<c:out value="${ptNotice.fileStore.fileStoreId}"/>')">
									<c:out value="${ptNotice.noticeNo}" />
							</a>
							</td>
							<td>Notice Generated Date:</td>
							<td><c:out value="${ptNotice.noticeDate}" /></td>
						</tr>
					</tbody>
				</c:forEach>
			</c:when>

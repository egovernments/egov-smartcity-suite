...
<c:choose>
	<c:when test="${!documentsList.isEmpty()}">
		...
		<div id="choultries">
			<c:forEach var="docs" items="${documentsChoultries}"
				varStatus="status">
						...
						<tr>
							<td>
								...
								<div class="col-sm-3 add-margin text-center">
									<c:choose>
										<c:when test="${choultryDocs.isEmpty()}">
											<c:choose>
												<c:when test="${docs.mandatory}">
										...
										<c:otherwise>
											<c:choose>
												<c:when test="${!choultryDocs[index].files.isEmpty()}">
													<c:forEach items="${choultryDocs[index].files}" var="file">

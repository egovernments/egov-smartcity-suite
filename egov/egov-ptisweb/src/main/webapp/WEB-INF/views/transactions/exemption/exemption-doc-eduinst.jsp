...
<c:choose>
	<c:when test="${!documentsList.isEmpty()}">
		...
		<div id="eduinst">
			<c:forEach var="docs" items="${documentsEduInst}" varStatus="status">
						...
						<tr>
							<td>
								...
								<div class="col-sm-3 add-margin text-center">
									<c:choose>
										<c:when test="${eduinstDocs.isEmpty()}">
											<c:choose>
												<c:when test="${docs.mandatory}">
										...
										<c:otherwise>
											<c:choose>
												<c:when test="${!eduinstDocs[index].files.isEmpty()}">
													<c:forEach items="${eduinstDocs[index].files}" var="file">

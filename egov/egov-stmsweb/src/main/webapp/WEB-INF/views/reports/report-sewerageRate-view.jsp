...
<div class="row">
	<div class="col-md-12">
		<form:form method="post" id="sewerageRateDCBReportView"
			...
			<div class="panel panel-primary" data-collapsed="0">
				...
				<div class="panel-body">
					...
					<div class="form-group">
						...
						<div class="col-xs-3 col-md-3 col-sm-3 view-content">
							<c:choose>
								<c:when
									test="${not empty sewerageApplicationDetails.connection.shscNumber}">
									<c:out
						...
						<c:forEach items="${propertyOwnerDetails.ownerNames}" var="owner">
							<div class="col-md-3 col-sm-3 col-xs-3">
								<spring:message code="lbl.owner.name" />
							</div>
							<div class="col-md-3 col-sm-3 col-xs-3 view-content" id="applicantname">
								<c:out value="${owner.ownerName}" />
							</div>
							<div class="col-md-3 col-sm-3 col-xs-3">
								<spring:message code="lbl.locality" />
							</div>
							<div class="col-md-3 col-sm-3 view-content" id="locality">
								<c:out
									value="${propertyOwnerDetails.boundaryDetails.localityName}" />
							</div>
							<div class="col-xs-3 col-md-3 col-sm-3">
								<spring:message code="lbl.revenue.ward" />
							</div>
							<div class="col-xs-3 col-md-3 col-sm-3 view-content" id="zonewardblock">
							...
							<div id="propertyType" class="col-md-3 col-sm-3 col-xs-6 view-content">
								<c:out
					...
					<table
						...
						role="grid" id="sewerageRateReportTable" sortable="sortable">
						<thead>
							<tr role="row">
								<th colspan="2"></th>
							...
							<tr>
								...
								<c:forEach var="dcb" items="${dcbResultList}" varStatus="status">
									<tr class="odd" role="row">
										...
										<c:set var="pendingArrearAmount"
											value="${dcb.arrearAmount-dcb.collectedArrearAmount}" />
										<c:set var="pendingDemandAmount"
											value="${dcb.demandAmount-dcb.collectedDemandAmount}" />
										<c:set var="pendingPenaltyAmount"
											value="${dcb.penaltyAmount-dcb.collectedPenaltyAmount}" />

										<td align="right">${pendingArrearAmount}</td>
										<td align="right">${pendingDemandAmount}</td>
										<td align="right">${pendingPenaltyAmount}</td>

										<c:set var="totalArrearAmount"
											value="${totalArrearAmount+dcb.arrearAmount}" />
										<c:set var="totalDemandAmount"
											value="${totalDemandAmount+dcb.demandAmount }" />
										<c:set var="totalPenaltyAmount"
											value="${totalPenltyAmount+dcb.penaltyAmount}" />
										<c:set var="totalAdvanceAmount"
											value="${dcb.collectedAdvanceAmount}" />
										<c:set var="totalCollectedArrearAmount"
											value="${totalCollectedArrearAmount+dcb.collectedArrearAmount}" />
										<c:set var="totalCollectedDemandAmount"
											value="${totalCollectedDemandAmount+dcb.collectedDemandAmount}" />
										<c:set var="totalCollectedPenaltyAmount"
											value="${totalCollectedPenaltyAmount+dcb.collectedPenaltyAmount}" />
										<c:set var="totalPendingArrearAmount"
											value="${totalArrearAmount-totalCollectedArrearAmount}" />
										<c:set var="totalPendingDemandAmount"
											value="${totalDemandAmount-totalCollectedDemandAmount}" />
										<c:set var="totalPendingPenaltyAmount"
											value="${totalPenaltyAmount-totalCollectedPenaltyAmount}" />
									</tr>
								</c:forEach>
							<tr>
						</tbody>
					...
					<table align="center" style="max-width: 12cm" cellpadding="10"
						class="table table-bordered datatable dt-responsive multiheadertbl"
						id="totalDCBReportTable" role="grid">
						<thead>
							<tr role="row">
						...
						<tfoot>
							...
							<c:set var="totalDemandSum"
								value="${totalArrearAmount+totalDemandAmount+totalPenaltyAmount}" />
							<c:set var="totalCollectionSum"
								value="${totalCollectedArrearAmount+totalCollectedDemandAmount+totalCollectedPenaltyAmount}" />
							<c:set var="totalBalanceSum"
								value="${totalPendingArrearAmount+totalPendingDemandAmount+totalPendingPenaltyAmount-totalAdvanceAmount}" />
					...
					<table align="center" style="max-width: 12cm;" cellpadding="10"
						class="table table-bordered datatable dt-responsive multiheadertbl"
						role="grid" id="dcbReportTable" sortable="sortable">
						<tbody>
							<c:forEach var="dcb" items="${dcbResultList}" varStatus="counter">
								<c:if test="${not empty dcb.receipts}">
									<thead>
										<tr role="row">
											<th><spring:message
													code="lbl.sewerageRateReport.receipt" /></th>
									...
									<c:forEach var="receiptApplMap" items="${dcb.receipts}">
										<c:forEach var="egdmCollectedReceipts"
											items="${receiptApplMap.value}">
											<tr>
												<td align="center"><a class="open-popup"
													href="/collection/citizen/onlineReceipt-viewReceipt.action?receiptNumber=${egdmCollectedReceipts.key}&consumerCode=${receiptApplMap.key}&serviceCode=STAX">${egdmCollectedReceipts.key}</a></td>
												<c:forEach var="receiptObject"
													items="${egdmCollectedReceipts.value}">
													<td align="center">${receiptObject.value}</td>
													<td align="center">${receiptObject.key}</td>
													<c:set var="totalReceiptAmount"
														value="${totalReceiptAmount+receiptObject.value}" />
												</c:forEach>
											</tr>
										</c:forEach>
									</c:forEach>

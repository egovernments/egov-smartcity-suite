...
<div class="row"> 
	<div class="col-md-12">
		<form:form  id="sewarageChangeClosetsSuccess" method ="post" class="form-horizontal form-groups-bordered" modelAttribute="sewerageApplicationDetails" >
		<div class="panel panel-primary" data-collapsed="0">
			<div class="panel-heading">
				<div class="panel-title text-center">
					<c:if test="${sewerageApplicationDetails.applicationType.code == 'CHANGEINCLOSETS'}">
						<c:choose>
							...
							<c:when test="${sewerageApplicationDetails.state.value == 'Rejected'}">
								<span><spring:message code="msg.changeincloset.rejected" arguments="${sewerageApplicationDetails.applicationNumber}"/>${approverName}~${nextDesign}</span>
							</c:when>
							<c:when test="${sewerageApplicationDetails.status.code == 'CANCELLED'}">
								<span><spring:message code="msg.changeincloset.cancelled" arguments="${sewerageApplicationDetails.applicationNumber}" /></span>
							</c:when>
						</c:choose>
					</c:if>
				</div> 
	...
	<div class="add-margin">
		<c:if
			...
			<a href="javascript:void(0)" class="btn btn-primary"
				onclick="renderWorkOrderPdf()">Preview Work Order</a>
		</c:if>
	</div>
	<div class="add-margin">

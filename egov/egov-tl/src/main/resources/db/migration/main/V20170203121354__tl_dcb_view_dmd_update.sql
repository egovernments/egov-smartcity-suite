CREATE OR REPLACE VIEW egtl_mv_dcb_view AS 
 SELECT mv.licensenumber,
   ...
   FROM ( SELECT lic.license_number AS licensenumber,
          ...
          WHERE lic.is_active = true AND drm.isdemand = true AND im.id =currdmd.id_installment AND m.name::text = 'Trade License'::text AND im.installment_type::text = 'Yearly'::text
        UNION
         SELECT lic.license_number AS licensenumber,
           ...
           FROM egtl_license lic
             ...
             LEFT JOIN eg_installment_master im ON im.id = dr.id_installment
             LEFT JOIN eg_module m ON m.id = im.id_module
          WHERE lic.is_active = true AND drm.isdemand = true AND m.name::text = 'Trade License'::text AND im.id <> arrdmd.id_installment) mv
  GROUP BY mv.licensenumber, mv.licaddress, mv.licenseid, mv.username, mv.wardid, mv.locality;

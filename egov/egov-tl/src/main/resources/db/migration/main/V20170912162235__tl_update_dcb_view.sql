...
CREATE OR REPLACE VIEW egtl_mv_dcb_view AS 
 SELECT mv.licensenumber,
   ...
   FROM ( SELECT lic.license_number AS licensenumber,
            ...
            (select name from eg_boundary where id=lic.id_parent_bndry) AS wardName,
  ...
  GROUP BY mv.licensenumber, mv.licaddress, mv.licenseid, mv.username, mv.wardName,mv.wardid, mv.locality, mv.active;

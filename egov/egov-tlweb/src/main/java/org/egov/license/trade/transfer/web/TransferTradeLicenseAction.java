package org.egov.license.trade.transfer.web;
...
public class TransferTradeLicenseAction extends BaseLicenseAction {
	...
	public String newForm() {
		...
		Boundary licenseeboundary = (new BoundaryDAO()).getBoundary(this.tl.getLicensee().getBoundary().getId().intValue());
	...
	public String create() {
		...
		if (this.tl.getLicenseeZoneId() != null && this.tl.getLicenseTransfer().getBoundary() == null) {
			Boundary licenseeboundary = (new BoundaryDAO()).getBoundary(this.tl.getLicenseeZoneId().intValue());
	...
	private void setMessages() {
		...
		if (this.workflowBean.getActionName().equalsIgnoreCase(Constants.BUTTONSAVE)) {
			this.userManager.getUserByID(this.license().getCreatedBy().getId());
			...
			final User userByID = this.userManager.getUserByID(this.license().getCreatedBy().getId());
			...
			final User userByID = this.userManager.getUserByID(this.workflowBean.getApproverUserId());
	...
	public String beforeEdit() {
		...
		cityZoneList = this.licenseUtils.getAllZone();
		this.tl.setLicenseZoneList(cityZoneList);
		Boundary licenseeboundary = (new BoundaryDAO()).getBoundary(this.tl.getLicenseTransfer().getBoundary().getId().intValue());
		if (licenseeboundary.getName().contains("Zone")) {
			this.addDropdownData(Constants.DROPDOWN_DIVISION_LIST_LICENSEE, Collections.EMPTY_LIST);
		} else
		{

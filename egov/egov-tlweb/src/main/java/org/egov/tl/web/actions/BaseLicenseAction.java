...
@Results({
		...
		@Result(name = "tl_generateRejCertificate", type = "redirectAction", location = "viewTradeLicense", params = {
				"namespace", "/viewtradelicense", "method",
...
public abstract class BaseLicenseAction extends GenericWorkFlowAction {
	...
	public String createDraftItems() {
		try {
			this.setCheckList();
			// service().create(license()); //mani when made changes from
	 ...
	 * service().endWorkFlowForLicense(license()); }
	 */

	// create workflow and pushes to drafts
	/*
	 * public void initiateWorkFlowForLicenseDraft() {
	 * service().initiateWorkFlowForLicenseDraft(license(), workflowBean);
	 * addActionMessage(this.getText("license.saved.in.draft")); }
	 */

	/*
	 * public void initiateWorkFlowForRenewLicense() {
	 * service().initiateWorkFlowForRenewLicense(license(), workflowBean);
	 * addActionMessage(this.getText("license.saved.in.inbox")); }
	 */
	...
	public String getCollectedDemandAmountInWords() {
		...
		final LicenseDemand currentYearDemand = service().getCurrentYearDemand(
				license());

...
function submitform() {
    if ($('#digSignDetailsTab').find('input[type=checkbox]:checked').length == 0) {
        bootbox.alert('Please select atleast one document to sign');
        return false;
    } else {
        var licenseids = [];
        $('#digSignDetailsTab tr').each(function (i, row) {
            var $row = $(row),
                $checkedBoxes = $row.find('input:checked');
            $checkedBoxes.each(function (i, checkbox) {
                var licenseid = $(checkbox.id);
                if (licenseid.selector != "selectAll")
                    licenseids.push(licenseid.selector);
            })
        })

...
@Result(name=BaseFormAction.SUCCESS,type="StreamResult.class",location="egBillRegisterPDF", params={"inputName","egBillRegisterPDF","contentType","application/pdf","contentDisposition","no-cache"})
@ParentPackage("egov")
public class ContractorBillPDFAction extends BaseFormAction{ 
	...
	public String execute(){
		if(egbillRegisterId!=null){		
			...
			mBHeader=(MBHeader)getPersistenceService().find("from MBHeader mbHeader where mbHeader.egBillregister.id = ?",egbillRegisterId);
			}

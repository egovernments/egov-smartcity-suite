...
public class SearchEstimateForContractorAdvanceAction extends SearchFormAction { 
	...
	protected void setTenderType() {
		...
		while(i.hasNext()){
			WorkOrderEstimate woe = (WorkOrderEstimate) i.next();
			TenderResponse tenderResponse = tenderResponseService.find("from TenderResponse tr where tr.negotiationNumber =? " +
					"and tr.egwStatus.code = ?",woe.getWorkOrder().getNegotiationNumber(), TenderResponse.TenderResponseStatus.APPROVED.toString());
	...
	public Map<String,Object> getContractorForApprovedWorkOrder() {
		...
		List<Contractor> approvedContractorList = contractorService.findAllByNamedQuery("getApprovedContractorsWithWO");

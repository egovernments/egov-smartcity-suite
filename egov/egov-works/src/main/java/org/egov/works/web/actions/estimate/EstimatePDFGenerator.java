...
public class EstimatePDFGenerator extends AbstractPDFGenerator  { 
		...
		try {
			...
			if (approvaldetailsTable.getRows().size()!=1 ) {
				...
				{						
					...
					document.add(spacer());			
					Paragraph headingPara1 = new Paragraph(new Chunk("NOTE FOR ADMINISTRATIVE SANCTION AS PER RULE 78 OF ", new Font(Font.UNDEFINED, LARGE_FONT,Font.BOLD)));
					headingPara1.setAlignment(Element.ALIGN_CENTER);
					document.add(headingPara1);
					Paragraph headingPara2 = new Paragraph(new Chunk("MCMC ACT 1919 ", new Font(Font.UNDEFINED, LARGE_FONT,Font.BOLD)));
					headingPara2.setAlignment(Element.ALIGN_CENTER);
					document.add(headingPara2);
					
					document.add(spacer());
					PdfPTable estimateDetailsTable1 = createEstimateDetailsTable1(estimate);
					...
					PdfPTable estimateDetailsTable2 = createBalanceAmtCalculationTable(estimate);
					document.add(estimateDetailsTable2);
					document.add(spacer());					
					Paragraph endTextPara = new Paragraph(new Chunk("** END **", new Font(Font.UNDEFINED, LARGE_FONT,Font.BOLD)));
	...
	{
		List<AppConfigValues> configList = abstractEstimateService.getAppConfigValue("Works", "ESTIMATE_APPROVAL_NUMBER_DATE");
...
*/		int isReject=-1;
		...
		abstractEstimateAppropriationList=estimateAppropriationService.findAllBy("from AbstractEstimateAppropriation aea where aea.abstractEstimate.id=? and aea.depositWorksUsage.id is not null order by id, aea.depositWorksUsage.financialYearId asc",estimate.getId());
		...
		if(estimate.getBudgetRejectionNo()!=null){
			String budgetRejectionNo=estimate.getBudgetRejectionNo();
			...
			for(FinancialDetail financialDetail:financialdetails){
				if(financialDetail.getCoa()!=null) {
					System.out.println("depositWorksUsage.getAppropriationDate()>>>>"+depositWorksUsage.getAppropriationDate());
					 totalUtilizedAmt=depositWorksUsageService.getTotalUtilizedAmountForDepositWorks(financialDetail,depositWorksUsage.getCreatedDate());
					 System.out.println("totalUtilizedAmt.doubleValue()>>>"+totalUtilizedAmt.doubleValue());
...
*/				     addRow(depositWorksAppropriationTable, true,centerPara("Deposit Code"),centerPara(estimate.getDepositCode().getCode()));
					 addRow(depositWorksAppropriationTable, true,centerPara("Account Code"),centerPara(estimate.getFinancialDetails().get(0).getCoa().getGlcode()+"-"+estimate.getFinancialDetails().get(0).getCoa().getName()));	 
					 addRow(depositWorksAppropriationTable, true,makePara("Function Center"),centerPara(estimate.getFinancialDetails().get(0).getFunction().getName()));
					 addRow(depositWorksAppropriationTable, true,makePara("Department"),centerPara(estimate.getUserDepartment().getName()));
					 addRow(depositWorksAppropriationTable, true,makePara("Amount of the Estimate "),rightPara(toCurrency(estimate.getTotalAmount())));
...
*/			 // }
		  // }
			...
			addRow(appropriationDetailTable, true,makePara(7f,"Appropriation Number"),makePara(7f,"Total Deposit Amount"),makePara(7f,"Amount Appropriated so far"),makePara(7f,"Amount Appropriated"),makePara(7f,"Balance on Hand"),makePara(7f,"Balance After Appropriation"));
			for(AbstractEstimateAppropriation abstractEstimateAppropriation:abstractEstimateAppropriationList){
				if(abstractEstimateAppropriation.getDepositWorksUsage().getConsumedAmount().doubleValue()!=0){
					...
					totalUtilizedAmt=depositWorksUsageService.getTotalUtilizedAmountForDepositWorks(abstractEstimateAppropriation.getAbstractEstimate().getFinancialDetails().get(0),abstractEstimateAppropriation.getDepositWorksUsage().getCreatedDate());
					if(totalUtilizedAmt==null){
						totalUtilizedAmt=BigDecimal.ZERO;
					}
					amtAppropriatedsofar=totalUtilizedAmt.subtract(abstractEstimateAppropriation.getDepositWorksUsage().getConsumedAmount());
					...
					addRow(appropriationDetailTable, true,makePara(7f,abstractEstimateAppropriation.getDepositWorksUsage().getAppropriationNumber()),rightPara(7f,toCurrency(totalDepositAmt.doubleValue())),rightPara(7f,toCurrency(amtAppropriatedsofar.doubleValue())),rightPara(7f,toCurrency(amtAppropriated.doubleValue())),rightPara(7f,toCurrency(balOnHand.doubleValue())),rightPara(7f,toCurrency(balanceAvailable.doubleValue())));
				}
			}
			...
			PdfPCell appDetailRightHeader=new PdfPCell(makePara("Financail Year Wise Appropriation Details"));
	...
	private PdfPTable createBudgetaryAppropriationTable(AbstractEstimate estimate)throws DocumentException,EGOVException,ValidationException {		  
		...
		estimateAppropriationService=abstractEstimateService.getEstimateAppropriationService();
		abstractEstimateAppropriationList=estimateAppropriationService.findAllBy("from AbstractEstimateAppropriation aea where aea.abstractEstimate.id=? and aea.budgetUsage.id is not null order by aea.budgetUsage.id,aea.budgetUsage.financialYearId asc",estimate.getId());
	...
	private Department getDeptFromBudgtAppropriationNo(String number)
	{
		if(StringUtils.isBlank(number))
		{
			...
			else
			{
				...
				Department dept = (Department) persistenceService.find(" from Department where deptCode=?",deptCode);
				if(dept==null)
				{
	  ...
	  private PdfPTable createApprovalDetailsTable(AbstractEstimate estimate)	throws DocumentException {
		try {
			...
			if(shouldShowApprovalNumber)
				addRow(approvaldetailsTable, true, makePara("Approval Step"),	centerPara("Name"), centerPara("Designation"), centerPara("Approved on"), centerPara("Approval Number"), centerPara("Remarks"));
			else
				addRow(approvaldetailsTable, true, makePara("Approval Step"),	centerPara("Name"), centerPara("Designation"), centerPara("Approved on"),  centerPara("Remarks"));
	  ...
	  private PdfPTable createEstimateDetailsTable1(AbstractEstimate estimate)	throws DocumentException {
			try {
				...
				addRow(estimateDetailsTable1, true, 	centerPara("Department"),	centerPara(estimate.getExecutingDepartment().getName()));
	  ...
	  private PdfPTable createBudgetDetailsForEstimateTable(AbstractEstimate estimate)	throws DocumentException {
			try {
					...
					if(abstractEstimateAppropriationList!=null && !abstractEstimateAppropriationList.isEmpty()){
						latestAbstractEstimateAppropriation=abstractEstimateAppropriationList.get(abstractEstimateAppropriationList.size()-1);
						...
						if(latestAbstractEstimateAppropriation!=null){
							if(estimate.getDepositCode()==null){
									if(latestAbstractEstimateAppropriation.getBudgetUsage().getConsumedAmount()!=0){
										Department dept = getDeptFromBudgtAppropriationNo(latestAbstractEstimateAppropriation.getBudgetUsage().getAppropriationnumber());
										totalGrant=abstractEstimateService.getTotalGrantForYearAsOnDate(estimate.getFinancialDetails().get(0),latestAbstractEstimateAppropriation.getBudgetUsage().getFinancialYearId().longValue(),Integer.parseInt(dept.getId().toString()),latestAbstractEstimateAppropriation.getBudgetUsage().getUpdatedTime());
										BigDecimal planningBudgetPerc = abstractEstimateService.getPlanningBudgetPercentage(estimate.getFinancialDetails().get(0),latestAbstractEstimateAppropriation.getBudgetUsage().getFinancialYearId().longValue(),Integer.parseInt(dept.getId().toString()));
										if(planningBudgetPerc!=null && !planningBudgetPerc.equals(0)) {
											totGrantafterMultiFactor=totalGrant.multiply(planningBudgetPerc.divide(new BigDecimal(100)));  
											appValue = planningBudgetPerc.divide(new BigDecimal(100)).toString();
										}
										...
										balOnHand=budgetAvailable.add(new BigDecimal(latestAbstractEstimateAppropriation.getBudgetUsage().getConsumedAmount()));
										amtAppropriated=totGrantafterMultiFactor.subtract(balOnHand);
									}
							}
							else{
								if(latestAbstractEstimateAppropriation.getDepositWorksUsage().getConsumedAmount().doubleValue()!=0){
									totalUtilizedAmt=depositWorksUsageService.getTotalUtilizedAmountForDepositWorks(latestAbstractEstimateAppropriation.getAbstractEstimate().getFinancialDetails().get(0),latestAbstractEstimateAppropriation.getDepositWorksUsage().getCreatedDate());
									if(totalUtilizedAmt==null){
										totalUtilizedAmt=BigDecimal.ZERO;
									}
					...
					addRow(estBudgetDetailsTable, true, centerPara("Estimate Date"),centerPara(DateUtils.getFormattedDate( estimate.getEstimateDate(),"dd/MM/yyyy")),centerPara("Fund"),centerPara((estimate.getFinancialDetails().isEmpty())?"":estimate.getFinancialDetails().get(0).getFund().getName()));
					if(estimate.getDepositCode()==null){
						addRow(estBudgetDetailsTable, true, centerPara("Function "),centerPara((estimate.getFinancialDetails().isEmpty())?"":estimate.getFinancialDetails().get(0).getFunction().getName()),centerPara("Budget Head"),centerPara((estimate.getFinancialDetails().isEmpty())?"":estimate.getFinancialDetails().get(0).getBudgetGroup().getName()));
						addRow(estBudgetDetailsTable, true, centerPara("Amount Appropriated so far"),centerPara((amtAppropriated==null?"":toCurrency(amtAppropriated.doubleValue()))),centerPara("Estimate Amount"),centerPara(toCurrency(estimate.getTotalAmount().getValue())));
					}
					else{
						...
						addRow(estBudgetDetailsTable, true, centerPara("Amount Appropriated so far"),centerPara((amtAppropriatedsofar==null?"":toCurrency(amtAppropriatedsofar.doubleValue()))),centerPara("Estimate Amount"),makePara(toCurrency(estimate.getTotalAmount().getValue()),Element.ALIGN_RIGHT));
					}
	  ...
	  private PdfPTable createBalanceAmtCalculationTable(AbstractEstimate estimate)	throws DocumentException {
		try {
				...
				if(abstractEstimateAppropriationList!=null && !abstractEstimateAppropriationList.isEmpty()){
					AbstractEstimateAppropriation latestAbstractEstimateAppropriation=abstractEstimateAppropriationList.get(abstractEstimateAppropriationList.size()-1);
						if(latestAbstractEstimateAppropriation!=null){
							if(estimate.getDepositCode()==null){
								if(latestAbstractEstimateAppropriation.getBudgetUsage().getConsumedAmount()!=0){
										budgetAvailable=latestAbstractEstimateAppropriation.getBalanceAvailable();
							...
							if(latestAbstractEstimateAppropriation.getDepositWorksUsage().getConsumedAmount().doubleValue()!=0){
								totalDepositAmt=depositWorksUsageService.getTotalDepositWorksAmount(estimate.getDepositCode().getFund(), latestAbstractEstimateAppropriation.getAbstractEstimate().getFinancialDetails().get(0).getCoa(), accountdetailtype, estimate.getDepositCode().getId(), latestAbstractEstimateAppropriation.getDepositWorksUsage().getAppropriationDate());
								totalUtilizedAmt=depositWorksUsageService.getTotalUtilizedAmountForDepositWorks(latestAbstractEstimateAppropriation.getAbstractEstimate().getFinancialDetails().get(0),latestAbstractEstimateAppropriation.getDepositWorksUsage().getCreatedDate());
								if(totalUtilizedAmt==null){
									totalUtilizedAmt=BigDecimal.ZERO;
								}
				...
				if(estimate.getDepositCode()==null){
					addRow(estimateDetailsTable2, true, 	centerPara("Balance Amount Available"),	centerPara(budgetAvailable==null?"":toCurrency(budgetAvailable.doubleValue())));
				}
				else{
					addRow(estimateDetailsTable2, true, 	centerPara("Balance Amount Available"),	centerPara(balanceAvailable==null?"":toCurrency(balanceAvailable.doubleValue())));
				}
	...
	public void displayHistory(StateHistory ad,PdfPTable approvaldetailsTable) throws Exception{
		if(!ad.getValue().equals("NEW") && !ad.getValue().equals("END")){
			...
			if(shouldShowApprovalNumber) 
				addRow(approvaldetailsTable, true,	makePara(state), makePara(emp.getEmployeeName()),makePara(desgName), makePara(DateUtils.getFormattedDate(ad.getCreatedDate(),"dd/MM/yyyy" )), makePara(ad.getExtraInfo()),rightPara(ad.getComments()));
			else
				addRow(approvaldetailsTable, true,	makePara(state), makePara(emp.getEmployeeName()),makePara(desgName), makePara(DateUtils.getFormattedDate(ad.getCreatedDate(),"dd/MM/yyyy" )), rightPara(ad.getComments()));
		}
	}
	...
	private PdfPTable createActivitiesTable(AbstractEstimate estimate)
		...
		for (Activity activity : activities) {
			...
			addRow(activitiesTable, true, makePara(index++),rightPara(activity.getQuantity()), makePara(activity.getSchedule().getDescription()), centerPara(activity.getSchedule().getCode()), 
					 centerPara(estimateUom), rightPara(toCurrency(activity.getSORCurrentRate())), rightPara(toCurrency(activity.getAmount())));
		}
		activities = estimate.getNonSORActivities();
		for (Activity activity : activities) {
			addRow(activitiesTable, true, makePara(index++),rightPara(activity.getQuantity()),makePara(activity.getNonSor().getDescription()),centerPara(""), 
	...
	private String getSORYear() {
		...
		String estimateDept=estimate.getExecutingDepartment().getName();
		...
		for(AppConfigValues configValue:appConfigList){
			...
			if(ELECTRICAL_DEPARTMENT.equalsIgnoreCase(estimateDept) && value[1].equalsIgnoreCase(ELECTRICAL_DEPARTMENT)){
			...
			else if(!ELECTRICAL_DEPARTMENT.equalsIgnoreCase(estimateDept) && !value[1].equalsIgnoreCase(ELECTRICAL_DEPARTMENT)){ 
				String date[] = value[0].split("-");
				df = new SimpleDateFormat("dd/MM/yyyy",new Locale("en","IN"));
				try {
					Date startDate=df.parse(date[0]);

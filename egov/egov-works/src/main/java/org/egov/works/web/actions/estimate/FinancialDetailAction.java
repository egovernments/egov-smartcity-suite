...
@Result(name=FinancialDetailAction.PRINT,type="StreamResult.class",location="budgetFolioPDF", params={"inputName","budgetFolioPDF","contentType","application/pdf","contentDisposition","no-cache"})
public class FinancialDetailAction extends BaseFormAction
{
	...
	public String save()
	{
		...
		if(financialDetail.getAbstractEstimate()!=null && financialDetail.getAbstractEstimate().getMultiYearEstimates()!=null 
	...
	public String saveAndSubmit(){	
		...
		if(!isSkipBudgetCheck()) {
			for(MultiYearEstimate multiYearEstimate:financialDetail.getAbstractEstimate().getMultiYearEstimates()) {
				if(multiYearEstimate!=null && multiYearEstimate.getFinancialYear().getId().compareTo(finYearId)==0 && multiYearEstimate.getPercentage()>0){
		...
		try{
			   abstractEstimate = (AbstractEstimate) estimateWorkflowService.transition(actionName, abstractEstimate, approverComments);
				...
				List<ValidationError> errorList=exp.getErrors();
	...
	private void persistFinancialDetail(){
		...
		if(getMaxFinancingSource(financingSourceList).getFundSource()!=null && commonsService.fundsourceById(getMaxFinancingSource(financingSourceList).getFundSource().getId())!=null){
				abstractEstimate.setFundSource(commonsService.fundsourceById(getMaxFinancingSource(financingSourceList).getFundSource().getId()));
		}
	...
	protected void populateFinancingSourceDetails(){
	 ...
	 protected void populateMultiYearEstimates() {
		 ...
		 List<ValidationError> multiYearErrors=new ArrayList<ValidationError>();
		 ...
		 for(MultiYearEstimate multiYearEstimate: actionMultiYearEstimateValues) {
			 if(multiYearEstimate!=null) {
				 if(multiYearEstimate.getFinancialYear()!=null && multiYearEstimate.getFinancialYear().getId()!=null && multiYearEstimate.getFinancialYear().getId()!=-1) {
				 ...
				 if(!isPercError && multiYearEstimate!=null && multiYearEstimate.getPercentage()<=0){
				  ...
				  if(!isFinYearError && (multiYearEstimate.getFinancialYear()==null  || (multiYearEstimate.getFinancialYear()!=null && (multiYearEstimate.getFinancialYear().getId()==null || multiYearEstimate.getFinancialYear().getId()==-1)))) {				  
	...
	public List getEstimateStatuses() {
		return persistenceService.findAllBy("from EgwStatus s where moduletype=? and s.code<> 'BUDGETARY_APPR_VALIDATED' " +
	...
	public String viewDepositFolio(){
		Fund fund=commonsService.fundById(fundId);
		DepositCode depositCode=depositCodeService.findById(depositCodeId, false);
		CChartOfAccounts coa=commonsService.getCChartOfAccountsById(glcodeId);
	...
	public String viewDepositWorksFolio(){
		...
		AbstractEstimateAppropriation astractEstimateAppropriation=abstractEstimateService.getEstimateAppropriationService().findByNamedQuery("getLatestDepositWorksUsageForEstimate", abstractEstimate.getId());
...
*/		return SEARCH_DEPOSIT_WORKS_FOLIO;
	...
	public void search(String src)
	{
		...
		if(APP.equalsIgnoreCase(src) && financialDetail!=null && financialDetail.getAbstractEstimate().getUserDepartment()!=null){
		...
		if(APP.equalsIgnoreCase(src) && abstractEstimate!=null && abstractEstimate.getLeastFinancialYearForEstimate()!=null
			...
			if(!DateUtils.compareDates(new Date(),getReportDate())) throw new ValidationException(Arrays.asList(new ValidationError("greaterthan.currentDate.reportDate",getText("greaterthan.currentDate.reportDate"))));
			CFinancialYear finyear=null;
			try{
			finyear = abstractEstimateService.getCurrentFinancialYear(getReportDate());
			}catch (EGOVRuntimeException noFinYearExp) {
	...
	public String viewBudgetFolio(){
		...
		AbstractEstimateAppropriation astractEstimateAppropriation=abstractEstimateService.getEstimateAppropriationService().findByNamedQuery("getLatestBudgetUsageForEstimate", abstractEstimate.getId());
		if(astractEstimateAppropriation!=null){
	...
	public Map<String, Object> getDepositFolioDetails(
		...
		if(approvedBudgetFolioDetailsMap.isEmpty()){
			...
			latestCumulativeExpense=(Double) approvedBudgetFolioDetailsMap.get("totalCumulativeExpensesIncurred").get(0);
		}
	...
	public void validate(){
		...
		if (parameters.get("actionName") != null && parameters.get("actionName")[0] != null
			...
			if ((financialDetail.getBudgetGroup() != null && financialDetail.getBudgetGroup().getId() != null && financialDetail
					.getBudgetGroup().getId() != -1)) {
				if (financialDetail.getBudgetGroup().getMaxCode().getGlcode()
					...
					if (financialDetail.getScheme() == null
							|| (financialDetail.getScheme() != null && financialDetail.getScheme().getId() == null)
							|| (financialDetail.getScheme() != null && financialDetail.getScheme().getId() == -1)) {
						addActionError(getText("mandatory.scheme"));
					}
					if (financialDetail.getSubScheme() == null
							|| (financialDetail.getSubScheme() != null && financialDetail.getSubScheme().getId() == null)
							|| (financialDetail.getSubScheme() != null && financialDetail.getSubScheme().getId() == -1)) {
		...
		if("searchdetails".equalsIgnoreCase(option)){
			...
			if(!StringUtils.isBlank(mandatoryFields.get("department")) && (userDepartment==null 
					|| userDepartment ==-1)){

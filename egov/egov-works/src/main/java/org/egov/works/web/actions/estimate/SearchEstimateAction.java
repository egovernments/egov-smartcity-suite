...
public class SearchEstimateAction extends SearchFormAction { 
	...
	public void prepare(){
		...
		List<Department> values = (List<Department>) getPersistenceService().findAllBy("from Department dt");
		addDropdownData("executingDepartmentList",values);
		List<NatureOfWork> worktypeList = (List<NatureOfWork>) getPersistenceService().findAllBy("from NatureOfWork dt");

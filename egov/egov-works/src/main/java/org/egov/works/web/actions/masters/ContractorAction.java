...
public class ContractorAction extends SearchFormAction{  
	...
	*/
	...
	private String EDIT_ENABLE_ROLE_NAME="Edit Contractor Bank Info";
	...
	public String save(){ 
		...
		if(mode.equalsIgnoreCase("") && !hasRoleMapped){
			if(StringUtils.isNotBlank(contractor.getPanNumber()) && contractor.getBank()!=null && 
					StringUtils.isNotBlank(contractor.getBankAccount()) && StringUtils.isNotBlank(contractor.getIfscCode())){
		...
		contractor.getContractorDetails().clear(); 
		 for(ContractorDetail contractorDetail: actionContractorDetails) {
			 if (validContractorDetail(contractorDetail)) {
				 contractorDetail.setDepartment((Department) getPersistenceService().find("from Department where id = ?", contractorDetail.getDepartment().getId()));
...
//	}
	...
	public void setHasRoleMapped(boolean hasRoleMapped) {

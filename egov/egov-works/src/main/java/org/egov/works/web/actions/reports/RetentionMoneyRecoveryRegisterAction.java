...
@ParentPackage("egov")
@Results({
@Result(name=RetentionMoneyRecoveryRegisterAction.EXPORTPDF,type="StreamResult.class",location="pdfInputStream", params={"inputName","pdfInputStream","contentType","application/pdf","contentDisposition","no-cache;filename=RetentionMoneyRecoveryRegister.pdf"}),
@Result(name=RetentionMoneyRecoveryRegisterAction.EXPORTEXCEL,type="StreamResult.class",location="excelInputStream", params={"inputName","excelInputStream","contentType","application/xls","contentDisposition","no-cache;filename=RetentionMoneyRecoveryRegister.xls"})
})
public class RetentionMoneyRecoveryRegisterAction extends SearchFormAction { 
	...
	public void prepare() {		
		...
		addDropdownData("budgetHeadList", getPersistenceService().findAllBy("select distinct(bg) from FinancialDetail fd , BudgetGroup bg where " +
				"fd.abstractEstimate.egwStatus.code = ? and fd.abstractEstimate.projectCode.id in (select bpd.accountDetailKeyId from EgBillPayeedetails bpd where bpd.accountDetailTypeId = (" +
					...
					" and expendituretype='Works' ) and bg=fd.budgetGroup order by bg.name ",AbstractEstimate.EstimateStatus.ADMIN_SANCTIONED.toString(),ContractorBillRegister.BillStatus.APPROVED.toString()));
		
		addDropdownData("depositCOAList", getPersistenceService().findAllBy("select distinct(fd.coa) from FinancialDetail fd where " +
				"fd.abstractEstimate.egwStatus.code = ? and fd.abstractEstimate.projectCode.id in (select bpd.accountDetailKeyId from EgBillPayeedetails bpd where bpd.accountDetailTypeId = (" +
				" select id from Accountdetailtype where name='PROJECTCODE') and bpd.egBilldetailsId.egBillregister.status.code=? " +
				" and expendituretype='Works') order by glcode ",AbstractEstimate.EstimateStatus.ADMIN_SANCTIONED.toString(),ContractorBillRegister.BillStatus.APPROVED.toString()));
	}	
	
	@Override
	public SearchQuery prepareQuery(String sortField, String sortOrder) {
		String query = getSelectQuery()+getSearchQuery();
	...
	public String searchList() 	{
		...
		if(searchResult!=null && searchResult.getList()!=null && !searchResult.getList().isEmpty()) {
				List<RetentionMoneyRecoveryRegisterBean> resultList = setBeanValues(searchResult.getList());
	...
	private List<RetentionMoneyRecoveryRegisterBean> setBeanValues(List searchList) {
		List<RetentionMoneyRecoveryRegisterBean> resultList = new ArrayList<RetentionMoneyRecoveryRegisterBean>(); 
		for(Object[] object : (List<Object[]>)searchList) {
			...
			if(object[12] != null){
				//If the isFinalBill flag is not set for the project code, then set the refund date based on the bill status
	...
	private String getSearchQuery() { 
		...
		Map<String,Object> whereClauseMap = formSearchConditionsQuery();
		...
		String query1 =" (SELECT dp.dept_name AS \"Bill Department\", cont.code AS \"Contractor Code\", cont.name AS \"Contractor Name\","+
		...
		" and bd1.debitamount>0 and bd1.billid=br.id) and not exists (select cbr.id from egw_contractorbill cbr where cbr.id = br.id ) "+whereClauseBfr+estimateStartQueryCondition;
		
		if(retentionMoneyRefPeriod!=null && retentionMoneyRefPeriod!=-1){
			Date currentDate = new Date();
			Long period = new Long(retentionMoneyRefPeriod) * new Long(24) * new Long(3600) * new Long(1000);
			Date toDate = new Date(currentDate.getTime() + period.longValue());
		...
		String query2 = "(SELECT dp.dept_name AS \"Bill Department\", cont.code AS \"Contractor Code\", cont.name AS \"Contractor Name\","+
				...
				" WHERE dp.id_dept = bmis.departmentid AND br.id = bd.billid AND bmis.billid = br.id AND bd.id = bpd.billdetailid "+
				" AND cont.id = bpd.ACCOUNTDETAILKEYID AND mbh.WORKORDER_EST_ID = woe.id AND mbh.billregister_id = br.id AND bpd.ACCOUNTDETAILTYPEID=(select id from accountdetailtype where name='contractor') "+
				" AND bd.creditamount > 0 AND br.EXPENDITURETYPE = 'Works' AND br.STATUSID IN (select id from egw_status where code='APPROVED' and moduletype='CONTRACTORBILL') " +
				" and bd.glcodeid in(select coa1.id from chartofaccounts coa1 " +
				" where coa1.purposeid = (select id from egf_accountcode_purpose where name = 'RETENTION_MONEY'))" + 
				" and pc.id in(select bpd1.accountdetailkeyid from eg_billpayeedetails bpd1,eg_billdetails bd1 where " +
		...
		String groupByQuery = " ) GROUP BY \"Bill Department\", \"Contractor Code\",\"Contractor Name\",\"Bill Number\", \"Bill Type\", " +
	...
	private Map<String, Object> formSearchConditionsQuery() {
		...
		if(StringUtils.isNotBlank(contractorCodeName)) { 
			titleBuffer.append(" "+getText("retentionMoneyRecoveryRegister.title.contractorcodeorname")+contractorCodeName);
			String[] contractorDetails = contractorCodeName.split("~");
		...
		if(billDepartment != null && billDepartment != 0 && billDepartment != -1) {
			...
			Department department = (Department) departmentService.getDepartmentById(billDepartment);
			titleBuffer.append(" "+getText("retentionMoneyRecoveryRegister.title.billdepartment")+department.getName());
		}
		...
		if(billDateFrom != null && billDateTo != null) 
			titleBuffer.append(" "+getText("retentionMoneyRecoveryRegister.title.date")+DateUtils.getFormattedDate(billDateFrom,"dd/MM/yyyy")+" to "+DateUtils.getFormattedDate(billDateTo,"dd/MM/yyyy"));
		...
		if(retentionMoneyAmountFrom != null && retentionMoneyAmountTo != null 
			...
			titleBuffer.append(" "+getText("retentionMoneyRecoveryRegister.report.retentionmoney.amount.range.lebel")+String.format("%.2f", retentionMoneyAmountFrom)+" - "+String.format("%.2f", retentionMoneyAmountTo)); 
		}
		...
		if(StringUtils.isNotBlank(estimateNumber) || (budgetHeads != null && !budgetHeads.isEmpty()) || (depositCOA != null && !depositCOA.isEmpty())) {
			...
			if(StringUtils.isNotBlank(estimateNumber)) {
				...
				titleBuffer.append(" "+getText("retentionMoneyRecoveryRegister.title.estimatenumber")+estimateNumber);
			}
			
			if(budgetHeads != null && !budgetHeads.isEmpty() && budgetHeads.get(0) != null && budgetHeads.get(0) != -1
					&& depositCOA != null && !depositCOA.isEmpty() && depositCOA.get(0) != null && depositCOA.get(0) != -1) {
					estimateStartQueryCondition = estimateStartQueryCondition + " and est.id in (select abstractestimate_id from egw_financialdetail" +
						" where "+ getInSubQuery(new ArrayList<Object>(budgetHeads), " BUDGETGROUP_ID ");
					estimateStartQueryCondition =  estimateStartQueryCondition +" or "+ getInSubQuery(new ArrayList<Object>(depositCOA), " COA_ID ")+ ")";
					titleBuffer.append(" "+getText("retentionMoneyRecoveryRegister.title.budgethead")+subHeaderBudgetHeads); 
					titleBuffer.append(" "+getText("retentionMoneyRecoveryRegister.title.depositcoa")+subHeaderDepositCOA);
			}
			else if(budgetHeads != null && !budgetHeads.isEmpty() && budgetHeads.get(0) != null && budgetHeads.get(0) != -1) {
				estimateStartQueryCondition = estimateStartQueryCondition + " and est.id in (select abstractestimate_id from egw_financialdetail" +
						" where "+ getInSubQuery(new ArrayList<Object>(budgetHeads), " BUDGETGROUP_ID ")+ ")";
				titleBuffer.append(" "+getText("retentionMoneyRecoveryRegister.title.budgethead")+subHeaderBudgetHeads);  				     
			}			
			else if(depositCOA != null && !depositCOA.isEmpty() && depositCOA.get(0) != null && depositCOA.get(0) != -1) {
				estimateStartQueryCondition = estimateStartQueryCondition + " and est.id in (select abstractestimate_id from egw_financialdetail" +
						" where "+ getInSubQuery(new ArrayList<Object>(depositCOA), " COA_ID ")+ ")";
				titleBuffer.append(" "+getText("retentionMoneyRecoveryRegister.title.depositcoa")+subHeaderDepositCOA);
			}
	...
	private List getReportData() { 
		String orderByClause =" ORDER BY \"Bill Date\"";
		String reportQuery = getSelectQuery()+getSearchQuery()+orderByClause;
		Query sqlQuery = getPersistenceService().getSession().createSQLQuery(String.valueOf(reportQuery));
		int count=0;
		...
		List<RetentionMoneyRecoveryRegisterBean> reportList = setBeanValues(resultList);
		return reportList;
	}
	...
	public String exportToPdf() {
		...
		ReportRequest reportRequest = new ReportRequest("RetentionMoneyRecoveryRegister",reportData, reportParams);
	...
	public String exportToExcel(){
		Map<String,Object> reportParams = new HashMap<String,Object>();
		List<AbstractEstimate> reportData = getReportData();
		reportParams.put("reportSubTitle", reportSubTitle);
		ReportRequest reportRequest = new ReportRequest("RetentionMoneyRecoveryRegister",reportData, reportParams);

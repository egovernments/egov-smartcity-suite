...
@ParentPackage("egov")
@Results({
@Result(name = WorkProgressRegisterAction.PRINT_PDF, type = "StreamResult.class", location = "workProgressRegisterStream", params = {"inputName", "workProgressRegisterStream", "contentType", "application/pdf","contentDisposition", "no-cache;filename=WorkProgressRegisterReport.pdf"}),
@Result(name = WorkProgressRegisterAction.PRINT_EXCEL, type = "StreamResult.class", location = "workProgressRegisterStream", params = {"inputName", "workProgressRegisterStream", "contentType", "application/xls","contentDisposition", "no-cache;filename=WorkProgressRegisterReport.xls"})
})
public class WorkProgressRegisterAction extends SearchFormAction {
	...
	private Map<String,String> milestoneStatuses=new LinkedHashMap<String,String>();
	...
	public void prepare(){
		...
		addDropdownData("preparedByList", getPersistenceService().findAllBy("select distinct wo.workOrderPreparedBy from WorkOrder wo where wo.workOrderPreparedBy.employeeName is not null"));
	...
	public String searchDetails(){
		...
		List<Object> objects=searchResult.getList();
		ArrayList<WorkProgressRegister> workProgressRegisterList=(ArrayList<WorkProgressRegister>) getWorkProgressRegisterList(objects);
	...
	private List getWorkProgressRegisterList(List workPorgressRegisterList){
		...
		List<WorkProgressRegister> tempList=new ArrayList<WorkProgressRegister>();
		while(iter.hasNext()) {
		...
		if(workOrderEstimate!=null){	
			...
			workProgress.setAdminSanctionDate(DateUtils.getFormattedDate(estimate.getState().getCreatedDate().toDate(), dateFormat));
				...
				for(AbstractEstimateAppropriation estimateApp : estimate.getAbstractEstimateAppropriations()){
					...
					if(estimateApp.getBudgetUsage()!=null){
						if(estimateApp.getBudgetUsage().getConsumedAmount()!=0){
							String finyearRange=commonsService.getFinancialYearById(estimateApp.getBudgetUsage().getFinancialYearId().longValue()).getFinYearRange();
							if(apprDetails!=null){
								...
								apprDetails=count+")"+finyearRange+", "+NumberUtil.formatNumber(new BigDecimal(estimateApp.getBudgetUsage().getConsumedAmount()));
							}
						}
					}
					else{
			...
			Map tenderDetail=getTenderDetails(estimate.getId());
			if(tenderDetail.get("tenderDate")!=null){
				workProgress.setTenderDate(DateUtils.getFormattedDate((Date)tenderDetail.get("tenderDate"),dateFormat));
			}
			if(tenderDetail.get("tenderFinalizationDate")!=null){
				workProgress.setTenderFinalizationDate(DateUtils.getFormattedDate((Date)tenderDetail.get("tenderFinalizationDate"),dateFormat));
			}
			if(tenderDetail.get("aggreementDate")!=null){
				workProgress.setTenderAgreementDate(DateUtils.getFormattedDate((Date)tenderDetail.get("aggreementDate"),dateFormat));
			}
			if(mbHeaders!=null){
				Map paymentDetails=getPaymentDetail(mbHeaders);
			...
			if(workOrder!=null){
				SetStatus objStatusForWorkComncd = worksStatusService.findByNamedQuery(STATUS_OBJECTID,workOrder.getId(), 
						WO_OBJECT_TYPE,WORK_COMMENCED);
				SetStatus objStatusForSiteHandOver = worksStatusService.findByNamedQuery(STATUS_OBJECTID,workOrder.getId(), 
						WO_OBJECT_TYPE,WorksConstants.WO_STATUS_WOSITEHANDEDOVER);
				if(objStatusForWorkComncd!=null)
					workProgress.setWorkCommencementDate(DateUtils.getFormattedDate(objStatusForWorkComncd.getStatusDate(),dateFormat));
				if(objStatusForSiteHandOver!=null)
					workProgress.setSiteHandedOverDate(DateUtils.getFormattedDate(objStatusForSiteHandOver.getStatusDate(), dateFormat));
			}
			...
			if(trackMilestone!=null && "APPROVED".equalsIgnoreCase(trackMilestone.getEgwStatus().getCode())){
				...
				if(milestone!=null && "APPROVED".equalsIgnoreCase(milestone.getEgwStatus().getCode())){
					List<TrackMilestoneActivity> trackList=new LinkedList<TrackMilestoneActivity>();
			...
			if(workProgress.getIsFinalBillCreated()){
				...
				if("END".equalsIgnoreCase(workOrder.getCurrentState().getValue()) && workOrder.getEgwStatus().getCode().equalsIgnoreCase("CANCELLED")){
  ...
  private Map getTenderDetails(Long estimateId){
	  ...
	  String query="select wpkg,tr from TenderResponse tr,WorksPackage wpkg left outer join wpkg.worksPackageDetails wpkgd where tr.tenderEstimate.worksPackage.id=wpkg.id and wpkgd.estimate.id=?";
	  	...
	  	List<Object> result = (List<Object>)persistenceService.findAllBy(query, paramList.toArray());
		Object[] objects;
		Iterator iterator = result.iterator();
		HashMap<String, Object> tenderDates=new HashMap<String, Object>();
		while (iterator.hasNext()) {
			objects = (Object[]) iterator.next();
			if(objects[0]!=null){
				WorksPackage worksPackage=(WorksPackage) objects[0];
				for(SetStatus status:worksPackage.getSetStatuses()){
			...
			if(objects[1]!=null){
				TenderResponse tenderResponse=(TenderResponse) objects[1];
				if(tenderResponse!=null){
					SetStatus objStatusForSite = worksStatusService.findByNamedQuery(STATUS_OBJECTID,tenderResponse.getTenderResponseContractors().get(0).getId(),"TenderResponseContractors",TENDER_AGGREEMENT_ORDER);
  ...
  public Map<String,Object> getPaymentDetail(Set<MBHeader> mbHeaders){
	  	...
	  	List<PaymentDetail> paymentDetailList=new LinkedList<PaymentDetail>();
	  	for(MBHeader mbHeader:mbHeaders){
	  		PaymentDetail paymentDetail=new PaymentDetail();
	  		EgBillregister egBillRegister=mbHeader.getEgBillregister();
	  		BigDecimal netPayableAmt = BigDecimal.ZERO;
	  		
	  		if(egBillRegister!=null){
	  			if(egBillRegister.getStatus() != null && egBillRegister.getStatus().getCode().equalsIgnoreCase("APPROVED")) {
	  		  		...
	  		  		if(egBillRegister.getBilltype().equalsIgnoreCase((String)contractorBillService.getBillType().get(1)) 
	...
	public String viewWorkProgressRegister(){
		HashMap<String,Object> queryMap=getQueryForWorkProgressRegister();
		String finalQuery=(String) queryMap.get("finalQuery");
		ArrayList<Object> paramList=(ArrayList<Object>)queryMap.get("params");
		List workPorgressRegisterList=persistenceService.findAllBy(finalQuery,paramList.toArray());
		Map<String, Object> reportParams = new HashMap<String, Object>();
		reportParams.put("searchCriteria", searchCriteria);
		ReportRequest reportInput = new ReportRequest("workProgressRegister",getWorkProgressRegisterList(workPorgressRegisterList), reportParams);
	...
	public HashMap<String,Object> getQueryForWorkProgressRegister(){
		...
		if(execDept!=-1){
			Department dept = departmentService.getDepartmentById(execDept);
		...
		if(fromDate!=null && toDate!=null && getFieldErrors().isEmpty()){
			srchCrit.append(" for date range "+ DateUtils.getFormattedDate(fromDate,dateFormat) +" - "+DateUtils.getFormattedDate(toDate,dateFormat));
		...
		if(contractorId!=null) {
			Contractor contractor = (Contractor) getPersistenceService().find("from Contractor where id=?", contractorId);
		...
		if(fund!=-1){
			Fund f = (Fund) getPersistenceService().find("from Fund where id=?", fund);
		...
		if(function!=-1){
			CFunction fun = (CFunction) getPersistenceService().find("from CFunction where id=?", function);
		...
		if(parentCategory!=-1){
			EgwTypeOfWork tow = (EgwTypeOfWork) getPersistenceService().find("from EgwTypeOfWork etw where etw.parentid is null and id=?",parentCategory);
		...
		if(category!=-1){
			EgwTypeOfWork subtow = (EgwTypeOfWork) getPersistenceService().find("from EgwTypeOfWork etw where id=? and parentid.id=?", 
		...
		if(preparedBy!=-1){
			PersonalInformation prepBy = (PersonalInformation) getPersistenceService().find("from PersonalInformation where id=?",preparedBy);
		...
		if(getScheme()!=null && getScheme()!= -1){
			Scheme sch = (Scheme) getPersistenceService().find("from Scheme where isactive=1 and id=?",getScheme());
		...
		if(getSubScheme()!=null && getSubScheme()!= -1){
			SubScheme subsch = (SubScheme) getPersistenceService().find("from SubScheme where id=?",getSubScheme());
		...
		if(budgetHead!=-1){
			BudgetGroup bh = (BudgetGroup) getPersistenceService().find("from BudgetGroup where id=?",budgetHead);
		...
		if(wardId!=null){
			Boundary wardObj = boundaryService.getBoundaryById(wardId);
		...
		if(engineerIncharge!=null && engineerIncharge != -1){
			PersonalInformation engInc1 = (PersonalInformation) getPersistenceService().find("from PersonalInformation where id=?",engineerIncharge);
		...
		if(engineerIncharge2!=null && engineerIncharge2 != -1){
			PersonalInformation engInc2 = (PersonalInformation) getPersistenceService().find("from PersonalInformation where id=?",engineerIncharge2);
		...
		if(sourcePage!=null && (sourcePage.equalsIgnoreCase("deptWiseReport") || sourcePage.equalsIgnoreCase("deptWiseReportForWP"))){
			if(woId!=null){

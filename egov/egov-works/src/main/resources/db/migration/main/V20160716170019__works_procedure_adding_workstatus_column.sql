alter table egw_mv_work_progress_register add column   estimatestatuscode character varying(50);
alter table egw_mv_work_progress_register add column   estimatevalue double precision;
alter table egw_mv_work_progress_register add column   workvalue double precision;
alter table egw_mv_work_progress_register add column   workstatus character varying(50);
...
BEGIN
	...
	select estimatestatus.code,estimate.estimatevalue,estimate.workvalue into v_estimatestatuscode ,v_estimatevalue,v_workvalue from egw_abstractestimate as estimate,egw_status estimatestatus where estimate.status = estimatestatus.id and estimate.estimatenumber = v_estimatenumber and estimatestatus.code in ('ADMIN_SANCTIONED','TECH_SANCTIONED');
	...
	IF (v_boqpresent) THEN 
		...
		ELSE
		     IF v_workstatus = '' THEN 
		    	v_workstatus = 'Work Not Commenced'; 
		     END IF;
		...
		IF (v_workcommenced) and (v_workcompleted!=true) THEN 
		     IF v_workstatus = '' THEN 
		    	v_workstatus = 'Work In Progress'; 
		     END IF;
		END IF;

		IF (v_workcompleted) THEN 
		    v_workstatus = 'Work Completed'; 
		END IF;

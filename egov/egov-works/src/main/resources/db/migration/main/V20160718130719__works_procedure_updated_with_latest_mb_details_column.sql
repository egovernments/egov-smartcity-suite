--------------Procedure updated with latest mb ref and mb date----------------
...
BEGIN
	...
	select estimatestatus.code,estimate.estimatevalue,estimate.workvalue, le.technicalsanctionby, led.estimateamount, pc.code into v_estimatestatuscode ,v_estimatevalue,v_workvalue, v_technicalsanctionby, v_estimateamount, v_wincode from egw_abstractestimate as estimate,egw_status estimatestatus, egw_lineestimate_details as led, egw_lineestimate as le,egw_projectcode pc where estimate.status = estimatestatus.id and estimate.estimatenumber = v_estimatenumber and estimatestatus.code in ('ADMIN_SANCTIONED','TECH_SANCTIONED') and estimate.estimatenumber = led.estimatenumber and pc.id = led.projectcode and le.id = led.lineestimate;
	...
	IF (v_boqpresent) THEN
        select mb.mb_refno, mb.mb_date into v_latestmbnumber, v_latestmbdate from egw_mb_header as mb where
        	mb.createddate = (select max(mbheader.createddate) from egw_mb_header as mbheader where 
        		mbheader.workorder_estimate_id = (select id from egw_workorder_estimate as woe where 
...
select COALESCE(sum(br.billamount),0) into v_totalbillamount from egw_workorder_estimate woe,eg_billregister as br
 ...
 INSERT INTO egw_mv_billdetail(
	...
	END IF;
        ...
        INSERT INTO egw_mv_work_progress_register(id, ward, location, workcategory, typeofslum, beneficiary, nameofwork, wincode,fund, function, budgethead, typeofwork, subtypeofwork, adminsanctionby,adminsanctiondate, adminsanctionamount, technicalsanctionby,           technicalsanctiondate, estimateamount, modeofallotment, agreementnumber,agreementdate, contractor, agreementamount, latestmbnumber, latestmbdate,latestbillnumber, latestbilldate, billtype, billamount, totalbillamount,totalbillpaidsofar, balancevalueofworktobill, spilloverflag, department,scheme,subscheme,natureofwork,leid,ledid,lineestimatestatus,departmentName,wostatuscode,	    workordercreated,workcompleted,typeofworkname,subtypeofworkname,milestonepercentagecompleted,createdby, createddate, lastmodifiedby,lastmodifieddate,estimatestatuscode,estimatevalue,workvalue,workstatus, estimatenumber) VALUES (nextval('seq_egw_mv_work_progress_register'), lineestimatedetails.ward, lineestimatedetails.location,lineestimatedetails.workcategory, lineestimatedetails.typeofslum, lineestimatedetails.beneficiary,lineestimatedetails.nameofwork, v_wincode,lineestimatedetails.fund, lineestimatedetails.function, lineestimatedetails.budgethead, lineestimatedetails.typeofwork,lineestimatedetails.subtypeofwork, lineestimatedetails.adminsanctionby,            lineestimatedetails.adminsanctiondate, v_estimateamount,v_technicalsanctionby,            lineestimatedetails.technicalsanctiondate,lineestimatedetails.actualestimateamount,lineestimatedetails.modeofallotment,v_agreementnumber,v_agreementdate, v_contractor, v_agreementamount, v_latestmbnumber, v_latestmbdate,v_latestbillnumber, v_latestbilldate, v_billtype, v_billamount, v_totalbillamount,v_totalbillpaidsofar, v_balancevalueofworktobill, lineestimatedetails.spilloverflag, lineestimatedetails.executingdepartment,lineestimatedetails.scheme,lineestimatedetails.subscheme,lineestimatedetails.natureofwork,lineestimatedetails.leid,lineestimatedetails.ledid,lineestimatedetails.lineestimatestatus, lineestimatedetails.departmentName,v_wostatuscode,lineestimatedetails.workordercreated,v_workcompleted,lineestimatedetails.typeofworkname,lineestimatedetails.subtypeofworkname,
v_milestonepercentagecompleted,1, now(), 1, now(),v_estimatestatuscode,v_estimatevalue,v_workvalue,v_workstatus, v_estimatenumber);

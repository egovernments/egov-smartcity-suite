...
BEGIN
	...
	select estimatestatus.code,estimate.estimatevalue,estimate.workvalue, techsan.technicalsanctionby, 
		led.estimateamount, pc.code into v_estimatestatuscode ,v_estimatevalue,v_workvalue, v_technicalsanctionby,
			v_estimateamount, v_wincode from egw_abstractestimate as estimate join egw_estimate_technicalsanction techsan
				on estimate.id = techsan.abstractestimate,egw_status estimatestatus, egw_lineestimate_details as led, egw_lineestimate
					as le,egw_projectcode pc where estimate.status = estimatestatus.id and estimate.estimatenumber = v_estimatenumber
						and estimatestatus.code in ('ADMIN_SANCTIONED','TECH_SANCTIONED') and estimate.estimatenumber = led.estimatenumber
							and pc.id = led.projectcode and le.id = led.lineestimate and techsan.createddate = (select max(createddate)

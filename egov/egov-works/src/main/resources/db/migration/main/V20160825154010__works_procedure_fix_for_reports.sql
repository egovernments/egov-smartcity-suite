...
BEGIN
	...
	v_spilloverbillscreatedflag := lineestimatedetails.billscreated;
        ...
        select br.billnumber, br.billdate, br.billtype, br.billamount, mb.mb_refno, mb.mb_date into v_latestbillnumber, v_latestbilldate, v_billtype, v_billamount, v_latestmbnumber, v_latestmbdate from egw_workorder_estimate woe,eg_billregister as br
            join egw_contractorbill as con on br.id = con.id join egw_mb_header as mb on br.id = mb.billregister_id where con.workorderestimate = woe.id and woe.workorder_id = (select id from egw_workorder where workorder_number = v_agreementnumber and status_id = (
                select id from egw_status where moduletype = 'WorkOrder' and code = 'APPROVED')) and con.billsequencenumber = (
                    select max(conn.billsequencenumber) from egw_workorder_estimate woe,egw_contractorbill as conn
                        join eg_billregister as brr on brr.id = conn.id where
                            conn.workorderestimate = woe.id and woe.workorder_id = (select id from egw_workorder where workorder_number = v_agreementnumber and status_id = (
                                select id from egw_status where moduletype = 'WorkOrder' and code = 'APPROVED')) and brr.billstatus = 'APPROVED');
 ...
 INSERT INTO egw_mv_billdetail(
	...
	END IF;
        ...
        INSERT INTO egw_mv_work_progress_register(
            id, ward, location, workcategory, beneficiary, nameofwork, wincode,
	    ...
	    scheme,subscheme,natureofwork,leid,ledid,lineestimatestatus,departmentName,wostatuscode,
	    workordercreated,workcompleted,typeofworkname,subtypeofworkname,milestonepercentagecompleted,createdby, createddate, lastmodifiedby,
		lastmodifieddate)
...
v_milestonepercentagecompleted,1, now(), 1, now());

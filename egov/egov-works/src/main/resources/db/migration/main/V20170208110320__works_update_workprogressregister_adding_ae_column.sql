ALTER TABLE EGW_MV_WORK_PROGRESS_REGISTER ADD COLUMN aeid BIGINT;
...
select count(ea.id) into v_checkboqpresent from egw_estimate_activity ea,egw_abstractestimate as ae,egw_status aestatus where ea.abstractestimate = ae.id and ae.status = aestatus.id and  aestatus.code = 'APPROVED' and ae.estimatenumber = v_estimatenumber;
	...
	IF (v_boqpresent) THEN 
		...
		select wo.workorder_number into v_checkworkordernumber2 from egw_workorder_estimate  as woe,egw_workorder wo,egw_abstractestimate ae  where woe.abstractestimate_id = ae.id and ae.status in (select id from egw_status where moduletype = 'AbstractEstimate' and code = 'APPROVED') and  woe.workorder_id = wo.id and status_id = (select id from egw_status where moduletype = 'WorkOrder' and code = 'APPROVED') and wo.workorder_number = v_agreementnumber and woe.workorder_id = (select distinct(os.object_id) from egw_offline_status  as os,egw_status egwstatus where os.status_id = egwstatus.id and os.id = (select max(status.id) from egw_offline_status  status where status.object_type = 'WorkOrder' and status.object_id = woe.workorder_id) and os.object_id = woe.workorder_id and lower(egwstatus.code) = 'work_commenced' and os.object_type = 'WorkOrder' );
	...
	ELSE
		...
		select wo.workorder_number into v_checkworkordernumber2 from egw_workorder_estimate  as woe,egw_workorder wo,egw_abstractestimate ae  where woe.abstractestimate_id = ae.id and ae.status in (select id from egw_status where moduletype = 'AbstractEstimate' and code = 'APPROVED') and  woe.workorder_id = wo.id and status_id in (select id from egw_status where moduletype = 'WorkOrder' and code not in ('APPROVED','CREATED')) and wo.workorder_number = v_agreementnumber;

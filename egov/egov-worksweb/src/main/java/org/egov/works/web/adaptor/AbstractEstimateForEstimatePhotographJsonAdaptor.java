...
@Component
public class AbstractEstimateForEstimatePhotographJsonAdaptor implements JsonSerializer<AbstractEstimate> {
    @Override
    public JsonElement serialize(final AbstractEstimate abstractEstimate, final Type type,
        ...
        if (abstractEstimate.getCreatedDate() != null)
            jsonObject.addProperty("estimateDate",
                    DateUtils.getFormattedDate(abstractEstimate.getCreatedDate(), "dd/MM/yyyy"));
        else
            jsonObject.addProperty("estimateDate", StringUtils.EMPTY);
        jsonObject.addProperty("natureOfWork", abstractEstimate.getNatureOfWork().getName());

...
public class SearchTypeOfWorkJsonAdaptor implements JsonSerializer<EgwTypeOfWork> {
    ...
    @Override
    public JsonElement serialize(final EgwTypeOfWork typeOfWork, final Type type, final JsonSerializationContext jsc) {
        final JsonObject jsonObject = new JsonObject();
        if (typeOfWork != null) {
            jsonObject.addProperty("code", typeOfWork.getCode());
            jsonObject.addProperty("name", typeOfWork.getName());
            if (typeOfWork.getDescription() != null && !typeOfWork.getDescription().isEmpty())

...
@Component
public class SearchWorkOrderForCRJsonAdaptor implements JsonSerializer<WorkOrderEstimate> {

    @Override
    public JsonElement serialize(final WorkOrderEstimate workOrderEstimate, final Type typeOfSrc,
            final JsonSerializationContext context) {
        final JsonObject jsonObject = new JsonObject();
        jsonObject.addProperty("workOrderNumber", workOrderEstimate.getEstimate().getEstimateNumber());
        jsonObject.addProperty("nameOfWork", workOrderEstimate.getEstimate().getName());
        jsonObject.addProperty("estimateNumber", workOrderEstimate.getWorkOrder().getEstimateNumber());
        jsonObject.addProperty("agreementValue", workOrderEstimate.getWorkOrder().getWorkOrderAmount());
        jsonObject.addProperty("contractorName", workOrderEstimate.getWorkOrder().getContractor().getName() + " - "
                + workOrderEstimate.getWorkOrder().getContractor().getCode());

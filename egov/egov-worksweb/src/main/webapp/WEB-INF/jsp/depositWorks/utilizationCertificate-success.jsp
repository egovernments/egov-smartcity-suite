	...
	<tr>
		<td align="center">
			<s:if test="%{currentState != null && currentState.previous.value == 'APPROVED'}">
		            <s:property value="%{model.applicationDetails.applicationRequest.applicationNo}"/>&nbsp; <s:text name="dw.utilizationCertificate.approved" />	             
		       </s:if>        
		       <s:elseif test="%{currentState != null && currentState.value == 'CHECKED'}"> 
		               <s:property value="%{model.applicationDetails.applicationRequest.applicationNo}"/>  <s:text name="dw.utilizationCertificate.checked" />
		               <br>
		       ...
		       <s:elseif test="%{currentState != null && currentState.value == 'REJECTED'}">
		               <s:property value="%{model.applicationDetails.applicationRequest.applicationNo}"/>  <s:text name="dw.utilizationCertificate.rejected" />
		                <br>
		               <s:text name="common.forwardmessage" />  <s:property value="%{employeeName}" />(<s:property value="%{designation}" />)
		       </s:elseif>                                         
		       <s:else>

...
function afterSORResults(sType,results){
    ...
    if(results[2].length==0){
     	showMessage('sor_error','<s:text name="estimate.sor.notavailable.ason.estiimatedate"/>');    	
	}
}

function sorSearchParameters(){
	if(dom.get('scheduleCategory').value!=-1){
	   	return "estimateDate="+dom.get('estimateDate').value+"&scheduleCategoryId="+dom.get('scheduleCategory').value+"&showRCRelatedSORs=true";
	}
	else{
		return "estimateDate="+dom.get('estimateDate').value+"&showRCRelatedSORs=true";
	}
}
...
function checkForCategorySelected(){
	if(dom.get("scheduleCategory").value==-1){
		dom.get("sor_error").style.display='';
		document.getElementById("sor_error").innerHTML='<s:text name="validate.select.schedule.category"/>';
		return false;
	}
	return true;
}
...
<table id="baseSORTable" width="100%" border="0" cellspacing="0" cellpadding="0">
	<tr>        
        ...
        <td class="whitebox2wk" colspan="2">
        	<s:select onchange="clearCategoryMessage();" headerKey="-1" headerValue="%{getText('estimate.default.select')}" name="scheduleCategory" id="scheduleCategory" cssClass="selectwk" list="dropdownData.scheduleCategoryList" listKey="id" listValue="code+' : '+description"/>
        </td>
	</tr>
    <tr>

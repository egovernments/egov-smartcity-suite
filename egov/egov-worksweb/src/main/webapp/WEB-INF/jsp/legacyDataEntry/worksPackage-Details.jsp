...
function goToPath(){
	if(dom.get("packageDate").value==""){
	  	 dom.get("wp_error").innerHTML='<s:text name="lde.wp.estimateSearch.packageDate.is.null"/>'; 
	...
	window.open("${pageContext.request.contextPath}/estimate/searchEstimate.action?execDept="+
			dom.get("department").value+"&source=wp"+"&wpdate="+dom.get("packageDate").value,"",
...
function update(elemValue) {
	...
	<s:if test="%{sourcepage=='inbox' && (model.currentState.value=='REJECTED' || model.currentState.value=='NEW')}">

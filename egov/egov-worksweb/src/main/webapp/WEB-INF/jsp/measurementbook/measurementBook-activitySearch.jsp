...
function validateAndSubmit() {
	document.activitySearchForm.action='${pageContext.request.contextPath}/measurementbook/measurementBook!searchActivitiesForMB.action';
 	document.activitySearchForm.submit();
}
</script>
<html>
	<head>
	    <title><s:text name="measurementbook.title.activity.search" /></title>
	</head>
	<body>
		<s:if test="%{hasErrors()}">
		...
		<s:form action="measurementBook" theme="simple"	name="activitySearchForm" onsubmit="validateAndSubmit();">
			...
			<s:hidden name="activitySearchMode" id="activitySearchMode" />
			<s:hidden id="workOrder" name="workOrderId"/>
				...
				<tr>
					<td>
						<table id="activitySearchTable" width="100%" border="0"
							...
							<tr>
								<td colspan="4">
									<div class="estimateno">Work Order No: 
									<s:if test="%{not workorderNo}">&lt; Not Selected &gt;</s:if>
							...
							<tr>
								<td width="19%" class="greyboxwk">
									<s:text name='measurementbook.item.code' />:
								</td>
								...
								<td width="14%" class="greyboxwk">
									<s:text name='measurementbook.itemdesc' />:
								</td>
														...
														style="border:1px;width:100%;empty-cells:show;border-collapse:collapse;">
																...
																<display:column headerClass="pagetableth" class="pagetabletd"
																	...
																	<s:elseif test="%{#attr.currentRow.activity.nonSor}">
																		...
																		<a href="#" class="hintanchor" onMouseover="showhint('<s:property value="%{#attr.currentRow.activity.nonSor.description}" />', this, event, '300px')"><img src="${pageContext.request.contextPath}/image/help.png"	alt="Help" width="16" height="16" border="0" align="absmiddle"/></a>
																	</s:elseif>
																</display:column>
																<display:column headerClass="pagetableth" class="pagetabletd"
			...
			function returnBackToParent() { 
				...
				if(value.length>0) {
					...
					if(wind==undefined){
						wind=window.opener;
						data=value;
						<s:if test="%{activitySearchMode=='nonTendered'}">
							window.opener.updateOtherActivity(data,'nonTendered');
						</s:if>
						<s:if test="%{activitySearchMode=='lumpSum'}">
							window.opener.updateOtherActivity(data,'lumpSum');
						</s:if>
						<s:if test="%{activitySearchMode!='nonTendered' && activitySearchMode!='lumpSum'}">

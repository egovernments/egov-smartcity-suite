...
<html>
<title><s:text name='page.title.workprogressabstract.showWPEstimates' /><s:property value="departmentName" /></title>
<style type="text/css">
th.pagetableth {
...
<script type="text/javascript">
	...
	jQuery(document).ready(function() {
	    jQuery('.table-header-fix').fixheadertable({
	         ...
	         minColWidth	 : 10 , 
	         resizeCol	 : true, 
	         colratio    : [40,150,100,130,330,100,100,100]
	    });
	});
	function viewWP(id) {
	...
	function viewWPDetailsPdf(){
		...
		window.open("${pageContext.request.contextPath}/reports/workProgressAbstract!viewWPEstimatePDF.action?"+parameter,'','height=650,width=980,scrollbars=yes,left=0,top=0,status=yes,resizable=yes');
	}
	...
	function viewWPDetailsExcel(){
		...
		window.open("${pageContext.request.contextPath}/reports/workProgressAbstract!viewWPEstimateXLS.action?"+parameter,'','height=650,width=980,scrollbars=yes,left=0,top=0,status=yes,resizable=yes');
	}
	function openWorkPrgReport(estId){
		window.open("${pageContext.request.contextPath}/reports/workProgressRegister!searchDetails.action?"+
						 	...
						 	<s:text id="adminSancDate" name="%{getText('label.estimate.admin.sanction.date')}"></s:text>
	  				        <display:table name="pagedResults" uid="currentRow" pagesize="30" 
								...
								<display:column headerClass="pagetableth" class="pagetabletd"    title="Work Progress Estimate Link"  style="text-align:center;">
						 				<a href="Javascript:openWorkPrgReport(<s:property  value='%{#attr.currentRow.estimate.id}' />)">

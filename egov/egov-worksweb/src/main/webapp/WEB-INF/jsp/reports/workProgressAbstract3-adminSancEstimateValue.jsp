...
<html>
<title>
	<s:if test="%{estStatus=='currentYearEstimates'}">
...
div.t_fixed_header div.headtable {
	...
	function viewEstimatePDF(){
		...
		window.open("${pageContext.request.contextPath}/reports/workProgressAbstract3!viewAdminSancEstimatesExport.action?"+parameter,'','height=650,width=980,scrollbars=yes,left=0,top=0,status=yes,resizable=yes');
	}
						...
						<tr id="resultRow" ><td colspan="4"> 
						 <s:if test="%{paginatedList.fullListSize!=0}">
						    ...
						    <display:table name="paginatedList" uid="currentRowObject" pagesize="30" 
								...
								<s:if test="%{estStatus=='spillOverEstimates'}">

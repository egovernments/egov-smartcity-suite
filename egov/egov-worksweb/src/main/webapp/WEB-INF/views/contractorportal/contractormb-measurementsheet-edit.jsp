<%--
  ...
  --%>
<c:if test="${!details.workOrderActivity.workOrderMeasurementSheets.isEmpty() }">
	            ...
	            <table class=" table table-bordered msheet-table" id="contractorMBDetails[${item.index }].mstable">
	                ...
	                <tbody>
	                	<c:forEach begin="0" end="${details.workOrderActivity.workOrderMeasurementSheets.size() - 1 }" varStatus="index">
	                		<tr>
			                	<td hidden="true">
			                        <form:input path="contractorMBDetails[${item.index }].measurementSheets[${index.index }].id" class="clearthis" id="contractorMBDetails_${item.index }_measurementSheets_${index.index }_id" />
			                        <form:input path="contractorMBDetails[${item.index }].measurementSheets[${index.index }].woMeasurementSheet" id="contractorMBDetails_${item.index }_measurementSheets_${index.index }_woMeasurementSheet" value="${details.workOrderActivity.workOrderMeasurementSheets.get(index.index).id}" />
			                    </td>
			                    ...
			                    <td>
			                        <form:textarea path="contractorMBDetails[${item.index }].measurementSheets[${index.index }].remarks" id="contractorMBDetails[${item.index }].measurementSheets[${index.index }].remarks" class="form-control text-left patternvalidation runtime-update"
			                               data-pattern="alphanumeric" maxlength="1024" ></form:textarea>
			
			                    </td>
			                    <td>
			                        <form:input path="contractorMBDetails[${item.index }].measurementSheets[${index.index }].no" onkeyup="limitCharatersBy3_2(this);" id="contractorMBDetails[${item.index }].measurementSheets[${index.index }].no" class="form-control text-right patternvalidation runtime-update"
			                               data-pattern="decimalvalue" data-idx="0" />
			
			                    </td>
			                    <td>                                                                     
			                        <form:input path="contractorMBDetails[${item.index }].measurementSheets[${index.index }].length" onkeyup="limitCharatersBy10_4(this);" id="contractorMBDetails[${item.index }].measurementSheets[${index.index }].length" class="form-control text-right patternvalidation runtime-update"
			                               data-pattern="decimalvalue" data-idx="0" />
			
			                    </td>
			                    <td>
			                        <form:input path="contractorMBDetails[${item.index }].measurementSheets[${index.index }].width" onkeyup="limitCharatersBy10_4(this);" id="contractorMBDetails[${item.index }].measurementSheets[${index.index }].width" class="form-control text-right patternvalidation runtime-update"
			                               data-pattern="decimalvalue"  data-idx="0" />
			
			                    </td>
			                    <td>
				                    <form:input path="contractorMBDetails[${item.index }].measurementSheets[${index.index }].depthOrHeight" onkeyup="limitCharatersBy10_4(this);" id="contractorMBDetails[${item.index }].measurementSheets[${index.index }].depthOrHeight" class="form-control text-right patternvalidation runtime-update"
				                           data-pattern="decimalvalue"  data-idx="0" />
			
			                	</td>
			                	<td>
				                    <form:input path="contractorMBDetails[${item.index }].measurementSheets[${index.index }].quantity" id="contractorMBDetails[${item.index }].measurementSheets[${index.index }].quantity" class="form-control text-right patternvalidation runtime-update"
		                ...
		                <tr>
		                    ...
		                    <td colspan="5" class="text-right">
		                        ...
		                        <input type="button" value="Submit" id="contractorMBDetails[${item.index }].mssubmit" class="btn btn-xs btn-primary ms-submit mssubmit_${item.index }"/> 
		                    </td>
		                    <td class="text-right"><spring:message code="lbl.total" /></td>

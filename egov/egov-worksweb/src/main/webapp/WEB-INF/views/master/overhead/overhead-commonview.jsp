		...
		<div id="overheadTable" class="panel panel-primary" data-collapsed="0"
			...
			<div class="panel-body">
				<div class="row add-border">
					...
					<div class="col-xs-3 add-margin view-content">
						<c:out value="${overhead.description}" />
					</div>
				</div>
				<div class="row add-border">
					<div class="col-xs-3 add-margin">
						<spring:message code="lbl.account" />
					</div>
					<div class="col-xs-5 add-margin view-content">
						<c:out value="${overhead.accountCode.glcode}" />
						:
						<c:out value="${overhead.accountCode.name}" />
					</div>
				</div>
			</div>
		</div>
				...
				<table class="table table-bordered">
					<thead>
						<tr>
							...
							<th><spring:message code="lbl.startdate" /></th>
							<th><spring:message code="lbl.enddate" /></th>
						</tr>
					</thead>
					<tbody>
						<c:forEach items="${overhead.getOverheadRates()}"
							var="overheadDtls">
							<tr>
								<td align="right"><fmt:formatNumber groupingUsed="false"
										maxFractionDigits="2" minFractionDigits="2"
										value="${overheadDtls.lumpsumAmount}" /></td>
								<td align="right"><fmt:formatNumber groupingUsed="false"
										maxFractionDigits="2" minFractionDigits="2"
										value="${overheadDtls.percentage}" />
								<td><fmt:formatDate
										value="${overheadDtls.validity.startDate}"
										pattern="dd/MM/yyyy" /></td>
								<td><fmt:formatDate
										value="${overheadDtls.validity.endDate}" pattern="dd/MM/yyyy" />
								</td>
							</tr>
						</c:forEach>
					</tbody>

    ...
    <table>
	    <tr class='msheet-tr'>
	        <td colspan="16"><!--only for validity head end -->
	            ...
	            <table class=" table table-bordered msheet-table" id="nonTenderedMbDetails[${item.index }].mstable">
	                <thead>
	                	<tr>
		                    <th colspan="1"></th>
	                ...
	                <tbody>
	                	<c:forEach begin="0" end="${details.workOrderActivity.workOrderMeasurementSheets.size() - 1 }" varStatus="index">
	                		<tr>
			                    ...
			                    <td id="nontenderedmsrowidentifier_${item.index }_${index.index }">
			                    	<c:choose>
			                    ...
			                    <td>
			                        <form:textarea path="nonTenderedMbDetails[${item.index }].measurementSheets[${index.index }].remarks" id="nonTenderedMbDetails[${item.index }].measurementSheets[${index.index }].remarks" class="form-control text-left patternvalidation runtime-update"
			                               data-pattern="alphanumeric" maxlength="1024" ></form:textarea>
			
			                    </td>
			                    <td>
			                        <form:input path="nonTenderedMbDetails[${item.index }].measurementSheets[${index.index }].no" onkeyup="limitCharatersBy3_2(this);" id="nonTenderedMbDetails[${item.index }].measurementSheets[${index.index }].no" class="form-control text-right patternvalidation runtime-update"
			                               data-pattern="decimalvalue" data-idx="0" />
			
			                    </td>
			                    <td>                                                                     
			                        <form:input path="nonTenderedMbDetails[${item.index }].measurementSheets[${index.index }].length" onkeyup="limitCharatersBy10_4(this);" id="nonTenderedMbDetails[${item.index }].measurementSheets[${index.index }].length" class="form-control text-right patternvalidation runtime-update"
			                               data-pattern="decimalvalue" data-idx="0" />
			
			                    </td>
			                    <td>
			                        <form:input path="nonTenderedMbDetails[${item.index }].measurementSheets[${index.index }].width" onkeyup="limitCharatersBy10_4(this);" id="nonTenderedMbDetails[${item.index }].measurementSheets[${index.index }].width" class="form-control text-right patternvalidation runtime-update"
			                               data-pattern="decimalvalue"  data-idx="0" />
			
			                    </td>
			                    <td>
				                    <form:input path="nonTenderedMbDetails[${item.index }].measurementSheets[${index.index }].depthOrHeight" onkeyup="limitCharatersBy10_4(this);" id="nonTenderedMbDetails[${item.index }].measurementSheets[${index.index }].depthOrHeight" class="form-control text-right patternvalidation runtime-update"
				                           data-pattern="decimalvalue"  data-idx="0" />
			
			                	</td>
			                	<td>
				                    <form:input path="nonTenderedMbDetails[${item.index }].measurementSheets[${index.index }].quantity" id="nonTenderedMbDetails[${item.index }].measurementSheets[${index.index }].quantity" class="form-control text-right patternvalidation runtime-update"
		                ...
		                <tr>
		                    ...
		                    <td colspan="6" class="text-right">
		                        ...
		                        <input type="button" value="Submit" id="nonTenderedMbDetails[${item.index }].mssubmit" class="btn btn-xs btn-primary ms-submit mssubmit_${item.index }"/> 
		                    </td>
		                    <td class="text-right"><spring:message code="lbl.total" /></td>
		                    <td id="nonTenderedMbDetails[${item.index }].msnet" class="text-right">${net}</td>
		                </tr>
	                </tbody>
	            </table>

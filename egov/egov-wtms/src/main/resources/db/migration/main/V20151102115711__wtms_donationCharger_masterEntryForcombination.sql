...
(nextval('SEQ_EGWTR_DONATION_DETAILS'),(select id from egwtr_donation_header where category in(select id from egwtr_category where code='GENERAL' ) and 
 usagetype in(select id from egwtr_usage_type where code='COMMERCIAL') and
 minpipesize in(select id from egwtr_pipesize where code='1/4 Inch') and  propertytype in(select id from egwtr_property_type where code='NON-RESIDENTIAL')),'2015-08-18','2016-11-27',1500.0,0);

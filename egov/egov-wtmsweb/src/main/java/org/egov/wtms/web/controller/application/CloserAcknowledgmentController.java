...
public class CloserAcknowledgmentController {
    ...
    @GetMapping(value = "/acknowlgementNotice", produces = APPLICATION_PDF_VALUE)
    public @ResponseBody ResponseEntity<InputStreamResource> generateEstimationNotice(final HttpServletRequest request) {
        ...
        final ReportOutput reportOutput = reportGenerationService.generateClosureConnectionReport(waterConnectionDetails, null);
        return ReportUtil.reportAsResponseEntity(reportOutput);
    }

    @GetMapping(value = "/acknowlgementNotice/view/{applicationNumber}", produces = APPLICATION_PDF_VALUE)

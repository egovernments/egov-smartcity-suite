...
$(document)
		.ready(
				function() {
					...
					var approvalPositionExist = $('#approvalPositionExist')
							.val();
					...
					function validateDateRange(fromDate, toDate) {
						if (fromDate != "" && toDate != "") {
							...
							startDate = Date.parse(stsplit[1] + "/"
									+ stsplit[0] + "/" + stsplit[2]);
					...
					$(".btn-primary")
							.click(
									function() {
										...
										if (status == 'RECONNECTIONINITIATED'
												&& action == 'Reject') {
										...
										if (status == 'CREATED'
											...
											$('#approvalComent').removeAttr(
													'required');
											if ($('form').valid()) {
												var estimationCharge = $(
														'#estimationCharges')
														.val();
					...
					function changeCategory() {
						if ($('#connectionCategorie :selected').text()
								.localeCompare("BPL") == 0) {
							$("#cardHolderDiv").show();
							$("#bplCardHolderName")

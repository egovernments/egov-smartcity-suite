...
jQuery(document).ready(
		function() {
			$('#report-footer').hide();
			jQuery('#searchid').click(
					function(e) {
						if ($('#waterSearchRequestForm').valid()
								&& validatedate() && validategeneratebill())
							loadingReport();
					});
...
function merge() {
	...
	window.open("/wtms/report/notice/search/mergeAndDownload"
			+ obj_to_query(params), "_self");
...
function zipanddownload() {
	...
	window.open("/wtms/report/notice/search/zipAndDownload"
			+ obj_to_query(params), "_self");
		...
		var oDataTable = oTable
				.dataTable({
						...
						"aButtons" : [
								{
									...
									"sPdfMessage" : "Generate Report as on "
											+ today + "",
					...
					ajax : {
						...
						dataSrc : function(d) {
							if (d.recordsCount > 4000) {
								...
								$('#search-notice-table_wrapper').addClass(
										'hide');
							} else {
								$('#warning-msg').addClass('hide');
							}
					...
					"columns" : [
							{
								...
								"render" : function(data, type, full, meta) {
									...
									return oTable.fnPagingInfo().iStart
											+ meta.row + 1;
								}
							},
							{
							...
							},
							{
								...
								render : function(data, type, full) {
									if (data != undefined && data != "") {
										...
										return regDateSplit[2].substring(0, 2)
												+ "/" + regDateSplit[1] + "/"
												+ regDateSplit[0];
									} else
										return "";
					...
					"fnInitComplete" : function() {
						if (oDataTable) {
							oDataTable.fnSort([ [ 7, 'desc' ], [ 3, 'asc' ] ]);
						}
					},
...
function populateNoticeType() {
	if ($('#applicationType').val() === "NEWCONNECTION"
			|| $('#applicationType').val() === "ADDNLCONNECTION") {
		$('#noticetype').find('option').remove();
		$('#noticetype').append($("<option />").val('').text(''));
		$('#noticetype').append(
				$("<option />").val('Sanction Order').text('Sanction Order'));
		$('#noticetype').append(
				$("<option />").val('Estimation Notice').text(
						'Estimation Notice'));
		...
		$('#noticetype').append($("<option />").val('').text(''));
		$('#noticetype').append(
				$("<option />").val('Proceeding for Closure of Connection')
						.text('Proceeding for Closure of Connection'));
	} else if ($('#applicationType').val() === "RECONNECTION") {
		$('#noticetype').find('option').remove();
		$('#noticetype').append($("<option />").val('').text(''));
		$('#noticetype').append(
				$("<option />").val('Proceeding for Re-connection').text(
						'Proceeding for Re-connection'));

...
// TODO REWRITE USIGN NEW SCHEDULER - SEE http://118.102.160.130:8000/projects/erp_3_0/cards/96
public class UpdateDishonoredInstrumentsJob  // implements EgovJob 
{
	 ...
	 */
	private void updateReceiptHeaderStatus(List<ReceiptHeader> receiptHeaderList, 
			EgwStatus status,boolean isReconciled) {
		//receiptPayeeDetails.clear();
		Set<ReceiptPayeeDetails> receiptPayeeDetails = new HashSet<ReceiptPayeeDetails>();
		for (ReceiptHeader receiptHeader : receiptHeaderList) {
			...
			receiptPayeeDetails.add(receiptHeader.getReceiptPayeeDetails());
		}
		receiptPayeeDetailsService.persist(receiptPayeeDetails);
	}
	...
	private Boolean updateDetailsToBillingSystems(List<ReceiptHeader> receiptHeaders) {
		...
		for (Entry<String, HashSet<BillReceiptInfo>> entry : serviceBillReceiptInfoMap
			...
			try {
				...
				flag = billingService.updateReceiptDetails(entry.getValue());
			} catch (Exception e) {
	...
	public Set<BillReceiptInfo> getBillReceipts() {
		return billReceipts;
	}

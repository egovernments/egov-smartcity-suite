...
public class CollectionCommon {
    ...
    public ReceiptDetail addDebitAccountHeadDetails(BigDecimal debitAmount, ReceiptHeader receiptHeader,
        ...
        if (chequeInstrumenttotal.toString() != null
            ...
            newReceiptDetail.setAccounthead(commonsService.getCChartOfAccountsByGlCode(chequeInHandGLCode));
        ...
        if (otherInstrumenttotal.toString() != null
            ...
            if (instrumentType.equals(CollectionConstants.INSTRUMENTTYPE_CASH)) {
                ...
                newReceiptDetail.setAccounthead(commonsService.getCChartOfAccountsByGlCode(cashInHandGLCode));
     ...
     */
    public void updateBillingSystemWithReceiptInfo(ReceiptPayeeDetails payeeDetails) {
        ...
        if (receiptPayeeDetailsService.updateBillingSystem(serviceCode, billReceipts)) {
            for (ReceiptHeader receiptHeader : payeeDetails.getReceiptHeaders()) {
                receiptHeader.setIsReconciled(true);
            }
            // the receipts should be persisted again
            receiptPayeeDetailsService.persist(payeeDetails);
        }
    }
     ...
     */
    public List<ReceiptPayeeDetails> initialiseReceiptModelWithBillInfo(BillInfo collDetails, Fund fund,
            DepartmentImpl dept) {
        List<ReceiptPayeeDetails> modelPayeeList = new ArrayList<ReceiptPayeeDetails>();
        ...
        for (BillPayeeDetails billPayee : collDetails.getPayees()) {
            ReceiptPayeeDetails receiptPayee = new ReceiptPayeeDetails(billPayee.getPayeeName(), billPayee
                    .getPayeeAddress());
            ...
            for (BillDetails billDetail : billPayee.getBillDetails()) {
                ...
                ReceiptHeader receiptHeader = new ReceiptHeader(billDetail.getRefNo(), billDetail.getBilldate(),
                ...
                Boundary boundary = boundaryDAO.getBoundary(Integer.valueOf(billDetail.getBoundaryNum()), billDetail
                ...
                Fundsource fundSource = commonsService.getFundSourceByCode(collDetails.getFundSourceCode());
                ...
                for (BillAccountDetails billAccount : billDetail.getAccounts()) {
                    CChartOfAccounts account = commonsService.getCChartOfAccountsByGlCode(billAccount.getGlCode());
                    CFunction function = commonsService.getFunctionByCode(billAccount.getFunctionCode());
    ...
    public InstrumentHeader validateAndConstructBankInstrument(PaymentInfoBank paytInfoBank) {
        ...
        if (paytInfoBank.getBankAccountId() == null) {
            ...
            account = commonsService.getBankaccountById(paytInfoBank.getBankAccountId().intValue());
    ...
    public InstrumentHeader validateAndConstructChequeDDInstrument(PaymentInfoChequeDD paytInfoChequeDD) {
        ...
        if (paytInfoChequeDD.getBankId() != null) {
            bank = commonsService.getBankById(paytInfoChequeDD.getBankId().intValue());
            if (bank == null) {
     ...
     * bank=commonsService.getBankById(paytInfoATM.getBankId().intValue());

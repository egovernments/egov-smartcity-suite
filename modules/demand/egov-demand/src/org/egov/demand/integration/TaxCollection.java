...
/**
 * This class is used to persist Bills with Collection Details(i.e
 * received from ErpCollection) .This is used for the
...
public abstract class TaxCollection implements BillingIntegrationService {
	 ...
	 * the receipt status i.e is_cancelled to true,but in case of cheque
	 * Bounce the status of the receipt will not be changed.
	 *
	...
	BillReceipt updateBillReceiptForCancellation(BillReceiptInfo bri, EgBill egBill,
			BigDecimal totalCollectedAmt) {
		BillReceipt billRecpt = null;
		if (bri == null) { throw new EGOVRuntimeException(" BillReceiptInfo Object is null "); }
		if (egBill != null && totalCollectedAmt != null) {
			billRctDao = DCBDaoFactory.getDaoFactory().getEgBillReceiptDao();
			billRecpt = billRctDao.getBillReceiptByEgBill(egBill);
			if (billRecpt == null) { throw new EGOVRuntimeException(
					" Bill receipt Object is null for the EgBill " + egBill.getId()); }
	 ...
	 * In case of Payment ,new EgBill is created and in case of cheque bounce only
	 * the EgBill is updated with necessary details(done for the
	 * DCB implementation)
	 *
	 ...
	 *
	 *@exception org.egov.exceptions.EGOVRuntimeException
	 */
    ...
    public EgDemandDetails getDemandDetail(EgDemand egDemand, Installment instl, String code) {
        ...
        List<EgDemandDetails> dmdDetList = dmdGenericDao.getDmdDetailList(egDemand, instl, module(),
	...
	protected EgdmCollectedReceipt persistCollectedReceipts(
		...
		EgdmCollectedReceiptDao egdmCollectedReceiptDao = DCBDaoFactory.getDaoFactory()
				.getEgdmCollectedReceiptsDao();
	 ...
	 * "&lt;enablebillapportioning&gt; as true to Collection System in the bill-xml.
	 *   
	 * @param billRefNo

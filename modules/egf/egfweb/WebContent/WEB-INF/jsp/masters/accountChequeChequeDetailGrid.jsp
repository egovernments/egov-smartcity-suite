...
<script>
   ...
   var chqDetailsIndex = 0;
		...
		chequeDetailsGridTable.on('cellClickEvent',function (oArgs) {
			...
			if (column.key == 'Delete') { 	
					...
					if(record.getData("isExhustedL") =="Yes" || record.getData("nextChqPresent")=="Yes"){
						...
						var index = chequeRangeArray.indexOf(record.getData("fromChqNo")+"-"+record.getData("toChqNo")+"-"+record.getData("deptId"));
	...
	<s:iterator value="chequeDetailsList" status="stat">
		...
		var chequeRange = '<s:property value="fromChqNo"/>'+"-"+'<s:property value="toChqNo"/>'+"-"+'<s:property value="deptId"/>';

...
<html>
<head>
	<title>Dishonored Cheque Entries</title>
<script>
<%
...
function ButtonPress(arg)
{
		...
		if(arg == "process")
		{
			...
			//document.getElementById('paramTxnDate').disabled="true";
		...
		if(arg != "view" && arg != "process")
		{
			...
			if(vType=='Receipt' || vType=='Journal Voucher')
			{
				...
				/*
				if(document.getElementById("glcodeChId").value=="" || document.getElementById("accCodedescCh").value=="")
				{
				alert("Please Select a Account Code First !!!");
				document.getElementById("glcodeChList").focus();
				return false;
				}
				*/
				...
				if(isRowsEmptyReceiptRev())return;
				
				if(!checkValidAmountReceiptRev("gridReceiptReversalCheque","glAmt","debitAmount","creditAmount")) return;
				if(!checkTotal("gridReceiptReversalCheque","debitAmount","creditAmount")) return;
			...
			if(vType=='Payment')
			{
				...
				/*
				if(document.getElementById("glcodeChIdP").value=="" || document.getElementById("chartOfAccounts_glCode").value=="")
				{
				alert("Please Enter Account Code First !!!");
				document.getElementById("chartOfAccounts_glCode").focus();
				return false;
				}
				*/
				...
				if(!checkTotal("gridPaymentReversalCheque","creditAmount",null)) return;
			...
			document.DishonoredChequeForm.action = "../brs/DishonoredChequeEntries.do?submitType=saveDetails";		
			document.DishonoredChequeForm.submit();
		} // savenew
		...
		if(arg == "view")
		{
			...
			{
				alert("Please Enter Cheque Number First !!!");
			...
			*/
			...
			{	
				alert("Please Enter Cheque From Date !!!");
			...
			/*
			if(valTo == "" && valTo.length==0)
			{	
				alert("Please Enter Cheque To Date !!!");
				document.DishonoredChequeForm.bankToDate.focus();
				return false;
			}
					
			if(valTo != null && valTo != "" && valTo.length>0)
			{	
				var dat=validateDate(document.DishonoredChequeForm.bankToDate.value);	
				if (!dat){
				alert('Invalid date format : Enter Date as dd/mm/yyyy');
				document.DishonoredChequeForm.bankToDate.focus();
				return;
...
function clearValues(neibrObj,neibrObj1,neibrObj2,neibrObj3,neibrObj4,neibrObj5,neibrObj6){
	if(neibrObj != null){
		neibrObj.value ="";
	}
	if(neibrObj2 != null){
...
var vType=document.getElementById("voucherTypeParam").value; 	
	...
	var temp = obj.value;
	 ...
	 if(temp.length==1)
	 {
		...
		clearValues(neibrObj,neibrObj1,neibrObj2,neibrObj3,neibrObj4,neibrObj5,neibrObj6);
	 }
	 else{
	//obj.value=temp[0];
	var currRow=getRow(obj);
...
function checkTotal(tableId,ctlToSearch1,ctlToSearch2)
 {
 	...
 	for(var i=1;i<table.rows.length;i++)
	{
	chqAmount=chqAmount+eval((parseFloat(getControlInBranch(table.rows[i],ctlToSearch1).value,2)*10/10).toFixed(2));
	} // for
 ...
 {	
	...
	%>
		           ...
		           if(searchResults!=null && searchResults.size()>0)
					...
					payCheque="";
						...
						<tr>
							...
							<td class="tdStlyle" style="display: none">
								<div id="voucherId" name="voucherId"><%= voucherId %></div>
								<html:hidden property="passVoucherId" value=""  styleId="passVoucherId"/>
							</td>
							<td class="tdStlyle">
					...
					String chqDate = formatter.format(dt);
					%>
							...
							<td class="smallfieldcell4">
								<html:text property="bankChargeAmt" style="text-align:right"
									value="" onblur="checkValidAmt();" maxlength="5" />
							</td>
							<td class="smallfieldcell">
								<html:text property="refNo" value="" maxlength="10" />
		...
		-->
							...
							<tr>
								<td class="smallfieldcell" align="center" width="25%">
									<html:select property="glcodeChList" styleId="glcodeChList"
										onchange="splitChqGlCode(this,'accCodedescCh');"
										styleClass="bigcombowidth">
										<option value='0'>
											----choose----
										</option>
										<c:forEach var="glCode"
											...
											<html:option value="${glCode}">${glCode}</html:option>
										</c:forEach>
									</html:select>
									<input type="hidden" name="glcodeR" id="glcodeR">
									<input type="hidden" name="glcodeChId" id="glcodeChId">
								</td>

								<td class="fieldcelldesc">
									<html:text property="accCodedescCh" style="text-align:left"
										value="" readonly="true" />
								</td>
								<!--
		...
		<td class="fieldcell" >
		 -->
								<input type="hidden" name="glAmt" id="glAmt" value=""
									style="text-align: right" readonly="true" />
								<!--</td> 
		
		-->
								<td class="fieldcell">
									<html:text property="debitAmount" value=""
										style="text-align:right" size="12" maxlength="16" />
								</td>
								<td class="fieldcell">
									<html:text property="creditAmount" value=""
										style="text-align:right"  size="12" maxlength="16" />
								</td>
						...
						<table width="70%" align="center" style="display: none"
							...
							<tr>
								...
								<td class="smallfieldcell" align="center" width="25%">
									...
									<input type="hidden" name="glcodeChIdP" id="glcodeChIdP">
									
								</td>
								<td class="fieldcelldesc">
									<html:text property="accCodedescChP" style="text-align:left"
										value="" readonly="true" />
								</td>
								<input type="hidden" name="glAmt" id="glAmt" value=""
									style="text-align: right" readonly="true" />
								...
								<td class="fieldcell">
									<html:text property="debitAmount" value="" styleId="debitAmount"
										style="text-align:right" size="12" maxlength="16" />
								</td>

								<td class="fieldcell">
									<html:text property="creditAmount" value="" styleId="creditAmount"
										style="text-align:right" size="12" maxlength="16" />
								</td>

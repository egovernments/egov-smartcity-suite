...
function BALANCESHEET(vtodt date,vmajor number,vminor number,vsubminor number,vdiv number) return egf_report.vGetResultSet;
...
function BSSCHEDULE(vtodt date,vminor number,vsubminor number,vdiv number,vscheduleid number) return egf_report.vGetResultSet;
...
cursor fund_c is select distinct id,name,code from fund order by code;
...
cursor fund_c is select id,name from fund order by code;
...
function BALANCESHEET(vtodt date,vmajor number,vminor number,vsubminor number,vdiv number) return egf_report.vGetResultSet is
...
cursor fund_c is select distinct id,code,name from fund where id in (Select distinct fundid from voucherheader) order by code;
...
where (substr(coa.glcode,1,'||vminor||')=coa1.glcode or substr(coa.glcode,1,'||vsubminor||')=coa1.glcode) and coa1.scheduleid in (sm.id) and sm.reporttype=''BS'' and coa.type in (''A'',''L'') and subs.subschname=sm.repsubtype
...
cursor fund_c is select id,name from fund where id in (Select distinct fundid from voucherheader) order by code;
...
function BSSCHEDULE(vtodt date,vminor number,vsubminor number,vdiv number,vscheduleid number) return egf_report.vGetResultSet is
...
query_part0:='select MINORCODE  as "Account Code", MINORNAME as "Particulars",';
...
where coa.scheduleid='||vscheduleid||' and (substr(coa1.glcode,1,'||vminor||')=coa.glcode or substr(coa1.glcode,1,'||vsubminor||')=coa.glcode) ) group by MINORCODE,MINORNAME,COATYPE

...
private String postInVoucherHeader(String cgNo,String newVcNo,String effDate,String cgvNo,Connection con,String uId) throws Exception{
	...
	LOGGER.debug("vhobj type "+ vhObj.getType());
	...
	LOGGER.debug("INSERTING VOUCHER STATUS");
...
//	VoucherDetail diffVoucherDetail=new VoucherDetail();
	...
	CChartOfAccounts bankChargsAccCode = getBankChargeAccCode();
	...
	while(it.hasNext()){
		...
		if(Double.parseDouble(vdObj.getDebitAmount())>0){
			...
			if(diffDbAmount>0){
				diffVoucherDetail.setGLCode(bankChargsAccCode.getGlcode());
				diffVoucherDetail.setAccountName(bankChargsAccCode.getName());
...
private void postInGeneralLedger(String newVdId,String oldVdId,String newVhId,double diffDbAmount,int usage,Connection con)throws TaskFailedException,SQLException{
	...
	if(Double.parseDouble(glObj.getdebitAmount())>0){
		...
		glObj.setglCode(bankChargsAccCode.getGlcode());
		glObj.setglCodeId(String.valueOf(bankChargsAccCode.getId()));
...
private CChartOfAccounts getBankChargeAccCode(){
	
	
	Session session = HibernateUtil.getCurrentSession();
	Criteria purposeCriteria = session.createCriteria(AccountCodePurpose.class);
	purposeCriteria.add(Restrictions.eq("name", "Bank Charges"));
	AccountCodePurpose purpose = (AccountCodePurpose)purposeCriteria.list().get(0);
	
	Criteria accountCriteria = session.createCriteria(CChartOfAccounts.class);
	accountCriteria.add(Restrictions.eq("purposeId", purpose.getId()));
	return (CChartOfAccounts)accountCriteria.list().get(0);
}
private boolean isReconciled(String cgNo,Connection con) throws SQLException{
	boolean valid=false;
	String sql="select isreconciled from BANKRECONCILIATION bc ,voucherheader vh where bc.voucherheaderid=vh.id and vh.cgn='"+cgNo+"'";
	Statement st=null;
	ResultSet rset=null;
	try{

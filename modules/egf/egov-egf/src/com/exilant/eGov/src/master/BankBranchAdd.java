...
public class BankBranchAdd extends AbstractTask {
	...
	public void execute (String nameTask,
		...
		//The below line is to Get DataBase Date and asigning that to Global Variable dbDate;
	  	String[][] accountGrid =(String[][])dataCollection.getGrid("bankAccountGrid");
        try{
		   ...
		   // else
			...
			postInBankAcount();
            dataCollection.addMessage("userSuccess",dataCollection.getValue("bankBranch_branchName").toString(),"Branch Inserted Successfully");
			ChartOfAccounts.getInstance().reLoadAccountData();
		}catch(Exception ex){
			dataCollection.addMessage("userFailure","Branch Insertion Failure");
			throw taskExc;
			}
	}
	...
	private void postInBankBranch() throws TaskFailedException,Exception{
		...
		if(bank_code ==null || bank_code.length()==0){
			dataCollection.addMessage("userMessege","Bank Code is must required field");
		...
		rs.close();
		st.close();
		String isActive=dataCollection.getValue("bankBranch_isActive").toString();
		if(isActive!=null && (isActive.compareToIgnoreCase("ON")==0 || isActive.equals("1")))
			isActive="1";
		else
			isActive="0";

		String bankBranch_branchCode=dataCollection.getValue("bankBranch_branchCode").toString();
		if(bankBranch_branchCode!=null && bankBranch_branchCode.length()!=0){
			table.setBranchCode(bankBranch_branchCode);
		}
			
		else{
			dataCollection.addMessage("userMessege","Please Fill the Branch Code");
			throw new TaskFailedException("Branch Code cannot be empty.");
		}
		String bankBranch_branchName=dataCollection.getValue("bankBranch_branchName").toString();
		if(bankBranch_branchName!=null && bankBranch_branchName.length()!=0){
			table.setBranchName(bankBranch_branchName);
		}
			
		else{
			dataCollection.addMessage("userMessege","Please Fill the Branch Name");
			throw new TaskFailedException("Bank Name cannot be empty.");
		}
		String bankBranch_branchAddress1=dataCollection.getValue("bankBranch_branchAddress1").toString();
		if(bankBranch_branchAddress1!=null && bankBranch_branchAddress1.length()!=0){
			table.setBranchAddress1(bankBranch_branchAddress1);
		}
			
		else{
			dataCollection.addMessage("userMessege","Please Fill the Branch Address");
			throw new TaskFailedException("Bank Address cannot be empty.");
		}

		String bankBranch_branchAddress2=dataCollection.getValue("bankBranch_branchAddress2").toString();
		if(bankBranch_branchAddress2!=null && bankBranch_branchAddress2.length()!=0)
			table.setBranchAddress2(bankBranch_branchAddress2);

		String bankBranch_branchCity=dataCollection.getValue("bankBranch_branchCity").toString();
		...
		String bankBranch_branchPin=dataCollection.getValue("bankBranch_branchPin").toString();
		if(bankBranch_branchPin!=null && bankBranch_branchPin.length()!=0)
			table.setBranchPin(bankBranch_branchPin);

		String bankBranch_branchPhone=dataCollection.getValue("bankBranch_branchPhone").toString(); 
		if(bankBranch_branchPhone!=null && bankBranch_branchPhone.length()!=0)
			table.setBranchPhone(bankBranch_branchPhone);

		String bankBranch_branchFax=dataCollection.getValue("bankBranch_branchFax").toString();
		if(bankBranch_branchFax!=null && bankBranch_branchFax.length()!=0)
			table.setBranchFax(bankBranch_branchFax);
		
		table.setBankId(bankId);
		String bankBranch_contactPerson=dataCollection.getValue("bankBranch_contactPerson").toString();
		...
		String bankBranch_narration=dataCollection.getValue("bankBranch_narration").toString();
	...
	private void postInBankAcount() throws TaskFailedException,Exception{
		...
		String[][] accountGrid =(String[][])dataCollection.getGrid("bankAccountGrid");
       	for(int i=0;i<accountGrid.length;i++){
			if(accountGrid[i][4].length()>0 && accountGrid[i][5].length()>0){
			String accountStatus="0";
			newGLCode="";coaID="";
			String code=EGovConfig.getProperty("egf_config.xml","glcodeMaxLength","","AccountCode");
			LOGGER.info("code:"+code);
            newGLCode=bankModify.prepareBankAccCode(con,accountGrid[i][4],code);
            LOGGER.info("newGLCode ---->"+newGLCode);
   			coaID=postInChartOfAccounts(newGLCode,accountGrid[i][4],accountGrid[i][0]);
   			LOGGER.info("coaID ---->"+coaID);
			
			if(accountGrid[i][0]!=null && accountGrid[i][0].length()!=0 && accountGrid[i][1]!=null && accountGrid[i][1].length()!=0){
				if(accountGrid[i][3]!=null && (accountGrid[i][3].compareToIgnoreCase("ON")==0 || accountGrid[i][3].equals("1")))	accountStatus="1";
	...
	public String postInChartOfAccounts(String glCode,String parentId,String accNumber) throws Exception{
        ...
        LOGGER.info("entrd postInChartOfAccounts----");
		chart.setGLCode(glCode);
		chart.setName(bankName+" "+dataCollection.getValue("bankBranch_branchName")+" "+accNumber);
		chart.setDescription(bankName+" "+dataCollection.getValue("bankBranch_branchName")+" "+accNumber);
    ...
    public void postInBank(Connection con)throws TaskFailedException,SQLException
    {
        String code="";
        Bank bnk=new Bank();

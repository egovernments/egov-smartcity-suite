...
//      Checking If data has to be Edited  -------SL
    ...
    private void createVoucher(DataCollection dc,Connection con)throws TaskFailedException
    {
        ...
        try
        {
           ...
           //  verifyUniqueness(con,dc.getValue("worksDetail_code"));
            ...
            LOGGER.info("CREATED WORKS DETAIL ID IS ***************************"+workOrderId);
            //added by iliyaraja
    ...
    {
        LOGGER.info("Inside postInWorksDetail Method");
        ...
        try
        {
            ...
            LOGGER.info("oorderdate1: "+orderdate1);
       ...
       // wd.setCode(dc.getValue("worksDetail_code"));
        ...
        LOGGER.info("Wrd info"+dc.getValue("worksDetail_wardId"));
    ...
    {
        LOGGER.info("Inside postInWorksmis Method");
        ...
        LOGGER.debug(sid+"sub scheme"+ssid);
        ...
        LOGGER.info("Inside postInWorksmis Method var : "+strworkOrderId);
        try{
            if(dc.getValue("modeOfExec").equalsIgnoreCase("new"))
            { LOGGER.info("new");
                wm.insert(con);
    ...
    private void postInEgw_Works_Deductions(Connection con,DataCollection dc)throws SQLException,TaskFailedException
	    {
	        LOGGER.info("Inside postInEgw_Works_Deductions Method");
	        ...
	        String tdsList[]=dc.getValueList("tds_List");
	         ...
	         int rcount=0;
	         LOGGER.info("Inside postInEgw_Works_Deductions modeOfExec is--->"+dc.getValue("modeOfExec"));



	        if(tdsList==null)
	        {
	        	LOGGER.info("<-------------Inside tdsList NULL--------------------->");
	        ...
	        LOGGER.info("tdsList length is------>"+tdsList.length);
	         LOGGER.info("tdsList[0] value is------>"+tdsList[0]);


	        for(int i=0;i<tdsList.length;i++)
	        {
	        	//LOGGER.info("<-------Inside tdsList NOT NULL  tdsList[i] value is-->"+tdsList[i]);
	        	...
	        	if(tdsList[i]=="" || tdsList[i]==null)
	        	{
					if(dc.getValue("modeOfExec").equalsIgnoreCase("edit"))
					{
						if(booleanDelA)
						{
								LOGGER.info("Inside Modify Mode 11111111");
								LOGGER.info("If No TDS for that particular Work Order then delete that old record from Egw_Works_Deductions");
								...
								// st=con.createStatement();
								 ...
								 if(rcount>0)
								LOGGER.info("deleted "+rcount+" row from table: Egw_Works_Deductions");
	        	...
	        	egwDed.setTdsId(tdsList[i]);
	               	try{
		            if(dc.getValue("modeOfExec").equalsIgnoreCase("new"))
		            { LOGGER.info("INSERT ROW IN Egw_Works_Deductions");
		            egwDed.insert(con);

		            }
		            else
		            {
		                LOGGER.info("Inside Modify Mode 2222222222222");

		              	if(booleanDelB)
		             	{
		             	 	LOGGER.info("Modify Mode Delete ROW IN Egw_Works_Deductions");
		             	 	LOGGER.info("workOrderId"+workOrderId);
		                ...
		                egwDed.insert(con);
		               	LOGGER.info("Modify Mode Insert ROW IN Egw_Works_Deductions");
		            }
    ...
    */
    ...
    private void deleteEgfAssetPO(Connection con)throws TaskFailedException
    {
        ...
        try
        {
            ...
            if(rs>0)
                LOGGER.info("deleted "+rs+" from table: eg_asset_po");
        }
        catch(Exception e){
    ...
    {
        LOGGER.info("validateAdvanceAmt:");
        Statement st=con.createStatement();
        ResultSet rs=null;
        ...
        if(rs.next())
        {
            ...
            LOGGER.info("advAmt:"+advAmt+" advAmtPay: "+advAmtPay);
            if(advAmtPay<advAmt)
            {

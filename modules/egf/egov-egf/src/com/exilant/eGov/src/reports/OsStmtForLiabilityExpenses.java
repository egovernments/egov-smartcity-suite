...
public class OsStmtForLiabilityExpenses 
{
	...
	private static final Logger LOGGER = Logger.getLogger(OsStmtForLiabilityExpenses.class);
	
	public HashMap<String,Object> getReport(final String asOnDate,final String type,final String relationId,final String param_fromDate,
			final String param_toDate,final String param_fund)
	{
		
		return HibernateUtil.getCurrentSession().doReturningWork(new ReturningWork<HashMap<String, Object>>() {

			@Override
			public HashMap<String, Object> execute(Connection con)
					throws SQLException {

				LOGGER.debug("Inside OsStmtForLiabilityExpenses getReport method");
...
//					String date = formatter.format(asOn);
					...
					LOGGER.info("fromdate:"+param_fromDate);
					LOGGER.info("todate:"+param_toDate);
					if(param_fromDate != null && !param_fromDate.trim().equals("")){
						...
						dateCondition = dateCondition + " and v.VOUCHERDATE >= '"+fromDate+"' ";
					}
					if(param_toDate != null && !param_toDate.trim().equals("")){
						date=null;
						date = sdf.parse(param_toDate);
						LOGGER.info("to date:"+date);
						String toDate = formatter.format(date);
						dateCondition = dateCondition + " and v.VOUCHERDATE <= '"+toDate+"' ";
					}
					String fundCondition= " ";
					if(param_fund != null && !param_fund.trim().equals("")){
						fundCondition = fundCondition + " and v.fundid = '"+param_fund+"' ";
					}
					String whereCondition="";
					
					
					if(!relationId.equals(""))
						whereCondition = " and r.id='"+relationId+"' ";
					...
					if(type.equalsIgnoreCase("0"))  // both (contractor/supplier)
					{
						...
						query="SELECT  c.BILLNUMBER||' / '||v.vouchernumber as \"Bill Number\",c.contractorid,r.NAME AS \"NAME\",(c.PASSEDAMOUNT-(c.PAIDAMOUNT+c.otherrecoveries)) AS \"Bill Amount\",v.VOUCHERDATE AS \"Bill DATE\", " +
								...
								" WHERE c.contractorid=r.ID AND v.ID=c.voucherheaderid AND v.status=0 " +
								 dateCondition + fundCondition +" AND gl.voucherheaderid=v.ID AND gl.DEBITAMOUNT>0 " +
					...
					else if(type.equalsIgnoreCase("1"))  // supplier
					{
							query="SELECT s.BILLNUMBER||' / '||v.vouchernumber as \"Bill Number\",s.supplierid,r.NAME AS \"NAME\",(s.PASSEDAMOUNT-(s.PAIDAMOUNT+s.otherrecoveries)) AS \"Bill Amount\",v.VOUCHERDATE AS \"Bill DATE\", " +
								" gl.GLCODE AS \"Code of Account\",v.ID AS \"VHID\",'Supplier' AS \"Nature of Payable\",wd.NAME AS \"Work Order\",f.NAME AS \"FUND\", v.cgn" +
								" FROM supplierbilldetail s,relation r,voucherheader v,generalledger gl,worksdetail wd,fund f" +
								" WHERE s.supplierid=r.ID AND v.ID=s.voucherheaderid AND v.status=0 and r.relationtypeid=1 "+whereCondition+" " +
								dateCondition + fundCondition +" AND gl.voucherheaderid=v.ID AND gl.DEBITAMOUNT>0" +
								" AND wd.ID=s.WORKSDETAILID AND f.ID=wd.fundid " +
								" GROUP BY s.ID,s.supplierid,r.NAME,s.PASSEDAMOUNT,s.PAIDAMOUNT,s.otherrecoveries,s.tdsamount,s.advadjamt,v.VOUCHERDATE,gl.GLCODE,v.ID,wd.NAME,f.NAME,s.BILLNUMBER,v.vouchernumber, v.cgn" +
								//" HAVING (s.PASSEDAMOUNT-(s.PAIDAMOUNT+s.otherrecoveries))>0 " +
					...
					else if(type.equalsIgnoreCase("2"))  // contractor
					{
							query = " SELECT c.BILLNUMBER||' / '||v.vouchernumber as \"Bill Number\",c.contractorid,r.NAME AS \"NAME\",(c.PASSEDAMOUNT-(c.PAIDAMOUNT+c.otherrecoveries)) AS \"Bill Amount\",v.VOUCHERDATE AS \"Bill DATE\", " +
								" gl.GLCODE AS \"Code of Account\",v.ID AS \"VHID\",'Contractor' AS \"Nature of Payable\",wd.NAME AS \"Work Order\", f.NAME AS \"FUND\", v.cgn " +
								" FROM contractorbilldetail c,relation r,voucherheader v,generalledger gl,worksdetail wd,fund f" +
								" WHERE c.contractorid=r.ID AND v.ID=c.voucherheaderid AND v.status=0 and r.relationtypeid=2 "+whereCondition+" " +
								dateCondition + fundCondition +" AND gl.voucherheaderid=v.ID AND gl.DEBITAMOUNT>0" +
								" AND wd.ID=c.WORKSDETAILID AND f.ID=wd.fundid" +
								" GROUP BY c.ID,c.contractorid,r.NAME,c.PASSEDAMOUNT,c.PAIDAMOUNT,c.otherrecoveries,c.tdsamount,c.advadjamt,v.VOUCHERDATE,gl.GLCODE,v.ID,wd.NAME,f.NAME,c.BILLNUMBER,v.vouchernumber, v.cgn" +
								//" HAVING (c.PASSEDAMOUNT-(c.PAIDAMOUNT+c.otherrecoveries))>0" +
					...
					LOGGER.info("Query for O/S report="+query);
					...
					while(rs.next())
					{
						...
						if(tempVHID.equals(rs.getString("VHID")) || tempVHID.equals(""))
						{
							if(!codeOfAccount.equals(""))
								codeOfAccount = codeOfAccount +", "+rs.getString("Code of Account");
							else
								codeOfAccount = rs.getString("Code of Account");
							
							natureOfPay	= (rs.getString("Nature of Payable")==null?"":rs.getString("Nature of Payable"))+"/ "+(rs.getString("Work Order")==null?"":rs.getString("Work Order"));
							billDate = formatter.format(rs.getDate("Bill DATE"));
							billAmt  = cf.numberToString(rs.getBigDecimal("Bill Amount").toString()).toString();
					...
					if(tempTotal!=0) // printing the last record
					{
						...
						amtInWords = NumberToWord.amountInWords(total);
						
						osMap = new HashMap<String,String>();
						osMap.put("srlNo", "<hr noshade color=black size=1>&nbsp;<hr noshade color=black size=1>");
						osMap.put("csName", "<hr noshade color=black size=1><b>Total : </b><hr noshade color=black size=1>");
						osMap.put("natureOfPayable","<hr noshade color=black size=1>&nbsp;<hr noshade color=black size=1>");
						osMap.put("codeOfAccount", "<hr noshade color=black size=1>&nbsp;<hr noshade color=black size=1>");
						osMap.put("billNumber", "<hr noshade color=black size=1>&nbsp;<hr noshade color=black size=1>");
						osMap.put("billDate","<hr noshade color=black size=1>&nbsp;<hr noshade color=black size=1>");
						osMap.put("billAmount", "<hr noshade color=black size=1>"+cf.numberToString(total.toString()).toString()+"<hr noshade color=black size=1>");
						osMap.put("fund", "<hr noshade color=black size=1>&nbsp;<hr noshade color=black size=1>");
						osMap.put("remarks", "<hr noshade color=black size=1>&nbsp;<hr noshade color=black size=1>");
						osMap.put("cgn","&nbsp;");
						osStmtList.add(osMap);
						...
						osMap = new HashMap<String,String>();
						osMap.put("srlNo", "<hr noshade color=black size=1>&nbsp;<hr noshade color=black size=1>");
						osMap.put("csName", "<hr noshade color=black size=1><b>Prepared By :</b><hr noshade color=black size=1>");
						osMap.put("natureOfPayable","<hr noshade color=black size=1>&nbsp;<hr noshade color=black size=1>");
						osMap.put("codeOfAccount", "<hr noshade color=black size=1>&nbsp;<hr noshade color=black size=1>");
						osMap.put("billNumber","<hr noshade color=black size=1><b>&nbsp;</b><hr noshade color=black size=1>");
						osMap.put("billDate","<hr noshade color=black size=1><b>&nbsp;</b><hr noshade color=black size=1>");
						osMap.put("billAmount", "<hr noshade color=black size=1><b>Checked By : </b><hr noshade color=black size=1>");
						osMap.put("fund", "<hr noshade color=black size=1>&nbsp;<hr noshade color=black size=1>");
						osMap.put("remarks", "<hr noshade color=black size=1>&nbsp;<hr noshade color=black size=1>");
						osMap.put("cgn","&nbsp;");
						osStmtList.add(osMap);
				...
				{
					LOGGER.error("Exception while getting the O/S stmt for liability expenses report="+e.getMessage());

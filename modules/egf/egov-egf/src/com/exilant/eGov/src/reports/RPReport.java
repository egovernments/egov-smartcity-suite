...
{	
	...
	public ArrayList getRPReport(final ReceiptPaymentBean reportBean)throws TaskFailedException	   
	{
		...
		return HibernateUtil.getCurrentSession().doReturningWork(new ReturningWork<ArrayList>() {
			ArrayList rpList;
			@Override
			public ArrayList execute(Connection conn) throws SQLException {
				...
				try {
					...
					LOGGER.info("satrtDate:" + startDate + " endDate:" + endDate+ " prevStartDate:" + prevStartDate + " prevEndDate:"+ prevEndDate);
					...
					try
					{
						...
						//LOGGER.debug("reqFundId length==============="+reqFundId.length);
						...
						//LOGGER.debug("reqFundId length second ==============="+reqFundId.length);
						...
						String rcptQuery="select distinct RECEIPTSCHEDULEID from chartofaccounts where glcode="+cashCode[0].substring(0,minorCodeLength);
						LOGGER.info("rcptQuery-->"+rcptQuery);
						Statement stmt = conn.createStatement();
						ResultSet rsRcpt = stmt.executeQuery(rcptQuery);
						...
						{
							String schQuery="select schedule, schedulename from schedulemapping where id="+cashOBScheduleId;
							LOGGER.info("schQuery-->"+schQuery);
							Statement stmt1 = conn.createStatement();
							ResultSet rsSch= stmt1.executeQuery(schQuery);			
							if(rsSch.next())
							{
							...
							String query1="select distinct rowconcat('select distinct glcode from chartofaccounts coa1,schedulemapping sm1 where "
										...
										+" OR coa.paymentscheduleid = sm.ID) AND sm.ID ="+cashOBScheduleId+" GROUP BY sm.schedule, sm.ID";
						...
						while (rs.next())
						{
							...
							if(rs.getString("repsubtype").equalsIgnoreCase("ROP"))
							{
								...
								rcptQuery="select distinct glcode from chartofaccounts where RECEIPTSCHEDULEID="+rcptScheduleId;
								LOGGER.info("rcptQuery-->"+rcptQuery);
								Statement stmt2 = conn.createStatement();
								rsRcpt = stmt2.executeQuery(rcptQuery);
								if(rsRcpt.next())
							...
							else if(rs.getString("repsubtype").equalsIgnoreCase("POP"))
							{
								...
								String pymntQuery="select distinct glcode from chartofaccounts where PAYMENTSCHEDULEID="+pymntScheduleId;
								LOGGER.info("pymntQuery-->"+pymntQuery);
								stmt = conn.createStatement();
								ResultSet rsPymnt = stmt.executeQuery(pymntQuery);
								if(rsPymnt.next())
									pymntAccountCode=rsPymnt.getString("glcode").substring(0,maxCodeLength);
												
								String remissionQuery="select isremission from schedulemapping where id="+pymntScheduleId;
								LOGGER.info("remissionQuery-->"+remissionQuery);
								ResultSet rsRemission = stmt.executeQuery(remissionQuery);
					...
					LOGGER.error("Exception Occured in RPReport | getRPReport "+e);
	...
	public void getFundList(Connection conn,String fromDate,String toDate) throws SQLException
	{
		try
		{
			...
			String query = " SELECT distinct id,name,code FROM FUND WHERE id IN (SELECT fundid FROM VOUCHERHEADER WHERE voucherdate>='"+fromDate+"' AND voucherdate<='"+toDate+"' AND status<>4) order by code ";
	 ...
	 public void getOpeningBalanceForCash(Connection conn,String fundId, String type1,String type2,String cashCode, String substringVal, String startDate, String endDate, int classification,String reqFundId1[],HashMap openingBal)throws Exception
	 {
	     ...
	     if(!startDate.equalsIgnoreCase(endDate))
	     {
             ...
             +" and vh.status<>4 and vh.VOUCHERDATE >='"+startDate+ "' and vh.VOUCHERDATE<='"+endDate+"'"+ fundCondition +" and "
	 ...
	 public void getPrevYearReport(Connection conn,String prevStartDate, String prevEndDate, String[] cashCode,int maxCodeLength, int minorCodeLength, String amountIn, String[] reqFundId)
	 	...
	 	try
	 	{
			...
			cstmt1.registerOutParameter(1,Types.OTHER);
	 ...
	 public void getOtherReceiptsOrPayments(final String type, final String cashMajorCode, final String substringVal, 
			 final String startDate,final String endDate,final HashMap otherAmt)throws Exception
	 {
		 ...
		 HibernateUtil.getCurrentSession().doWork(new Work() {
			...
			public void execute(Connection conn) throws SQLException {
			     ...
			     String query="SELECT f.id as \"fundid\", "+amountType+" as \"amount\" FROM GENERALLEDGER gl, VOUCHERHEADER vh, CHARTOFACCOUNTS coa, FUND f"
			     			+" WHERE vh.ID=gl.voucherheaderid AND SUBSTR(gl.glcode,1,"+substringVal+")=coa.glcode AND vh.voucherdate>='"+startDate+"' AND vh.voucherdate<='"+endDate+"' "	               
	 ...
	 List getRPSchedule(final String amountIn,final String scheduleNo)throws TaskFailedException,SQLException,ParseException{
		 ...
		 return HibernateUtil.getCurrentSession().doReturningWork(new ReturningWork<List>() {
 ...
 */
	 List getRPSingleSchedule(final String amountIn, final String scheduleNo)throws TaskFailedException,SQLException,ParseException{
				 ...
				 try{
					...
					// BigDecimal tempAmt = BigDecimal.ZERO;;
					 ...
					 while(resultset.next()){
						 ...
						 //LOGGER.info("add to dataList:"+dataList.size());
						 dataList.add(map);
					 }
					 //totals- last row
					 Map map = new HashMap();
	 ...
	 String getRPScheduleSQL(final String scheduleNo)throws TaskFailedException,SQLException,ParseException{
		 ...
		 return HibernateUtil.getCurrentSession().doReturningWork(new ReturningWork<String>() {
			...
			public String execute(Connection conn) throws SQLException {
				 ...
				 try{
					statement=conn.createStatement();
					resultset=statement.executeQuery("select repsubtype from schedulemapping where schedule='"+scheduleNo+"'");
					if(resultset.next()){
 ...
 */
	 ...
	 List getRP_R01_R37_Schedule(final String amountIn, final String scheduleNo)throws TaskFailedException,SQLException,ParseException{
			...
			public List execute(Connection conn) throws SQLException {
				 ...
				 try {
				   ...
				   //prev year obbalance ends
					 String sql="select coa.glcode glcode ,coa.name coaname, sm.schedule schedule,sm.schedulename schedulename, sm.REPSUBTYPE repsubtype from chartofaccounts coa ,schedulemapping sm where sm.id in (receiptscheduleid, paymentscheduleid) and sm.schedule='"+scheduleNo+"' order by sm.schedule";
					 LOGGER.info("getRP_"+scheduleNo+"_Schedule SQL:"+sql);
					 statement =conn.createStatement();
					 resultset=statement.executeQuery(sql);	
					 BigDecimal[] fundTotals = new BigDecimal[reqFundId.length];

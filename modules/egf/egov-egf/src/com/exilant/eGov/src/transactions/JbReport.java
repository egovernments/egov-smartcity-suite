...
public class JbReport 
{
    ...
    public LinkedList getJbReport(final GeneralLedgerBean reportBean)
    throws TaskFailedException
    {
    	...
    	return HibernateUtil.getCurrentSession().doReturningWork(new ReturningWork<LinkedList>() {
			...
			public LinkedList execute(Connection connection) throws SQLException {
		        ...
		        try { statement=connection.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);}catch(Exception e){LOGGER.debug("Exception in creating statement:"+statement);}
...
//		          
		        ...
		        {
		            //LOGGER.debug("Financial Year Not Valid");
		        ...
		        if((fundId == null || fundId.equalsIgnoreCase("")))
		        {
		            try
		            {
		                resultset = statement.executeQuery("SELECT max(id) as \"max\"  FROM fund");
		                if(resultset.next())
		            ...
		            {
		                LOGGER.debug("Error getting max fund: " + ex.toString());
		        ...
		        LOGGER.info("fundSourceId:"+fundSourceId);
		        if((fundSourceId == null || fundSourceId.trim().equalsIgnoreCase("")))
		        {
		            try
		            {
		                resultset = statement.executeQuery("SELECT max(id) as \"max\"  FROM fundsource");
		                if(resultset.next())
		            ...
		            {
		                LOGGER.debug("Error getting max fundSource: " + ex.toString());
		             
		            }
		        }
		        else
		        {
		        	LOGGER.info("inside esle addTableToQuery:"+addTableToQuery);
		            toFundSource=fundSourceId;
		            fromFundSource=fundSourceId;
		            addTableToQuery=", vouchermis vmis ";
		            fundSourceCond=" and	vmis.fundsourceid=d.id AND vmis.voucherheaderid=a.id ";   
		        ...
		        try
		        {
		             ...
		             **/
		            //LOGGER.debug("RS: " + resultset.isBeforeFirst() + " " + resultset.isAfterLast());

		            ArrayList data = new ArrayList();
		            String voucherNo="", vcDate="", name="", cgn="", glcode="", accName="", desc="", curVou="",isconfirmed="", rowNumber="1",vname="";
		            Double dr, cr;           
		            ...
		            String vn2="";
		                ...
		                //  LOGGER.debug("isconfirmed -->   "+isconfirmed);
		                    ...
		                    //LOGGER.debug("isConfirmedCount -->   "+isConfirmedCount);
		            ...
		            String gridData[][] = new String[data.size()+1][9];            
	...
	private String getQuery(){
		//if(revEntry.equalsIgnoreCase("on")){
			...
			" b.accountname as \"accountname\",b.narration as \"desc\",a.isconfirmed as \"isconfirmed\",debitamount as \"debit\",creditamount as \"credit\",a.name as \"vname\",1 as \"dbOrCr\" "+
			" from voucherheader a,voucherdetail b,fund c,fundsource d " + addTableToQuery +
			"  where a.id=b.voucherheaderid "+ fundSourceCond + voucherNameCond +
			...
			" glcode as \"glcode\",b.accountname as \"accountname\",b.narration as \"desc\",a.isconfirmed as \"isconfirmed\",debitamount as \"debit\",creditamount as \"credit\",a.name as \"vname\", "+
			" 2 as \"dbOrCr\" "+
			" from voucherheader a,voucherdetail b,fund c,fundsource d "+ addTableToQuery +
			" where b.voucherheaderid=a.id "+ fundSourceCond + voucherNameCond+ 
	...
	private String getFYID(final String sDate) throws TaskFailedException
    {
		...
		return HibernateUtil.getCurrentSession().doReturningWork(new ReturningWork<String>() {
			...
			public String execute(Connection connection) throws SQLException {
		        ...
		        try{
					...
					//resultset = statement.executeQuery("SELECT id FROM financialYear WHERE startingDate<='"+sDate+"' AND endingDate>='"+eDate+"'");
		            ...
		            LOGGER.info("SELECT id FROM financialYear " +
		                    "WHERE startingDate<='"+sDate+"' AND endingDate>='"+sDate+"'");
	...
	private void setDates(final GeneralLedgerBean reportBean) throws TaskFailedException{
		...
		HibernateUtil.getCurrentSession().doWork(new Work() {
			...
			public void execute(Connection connection) throws SQLException {
				...
				ResultSet rs = null;
				ResultSet rs1 = null;
				//startDate = dc.getValue("startDate");
				//endDate = dc.getValue("endDate");
				String formstartDate="";
				String formendDate="";
				try
		   		{
					...
					formstartDate = formatter1.format(dt);
					LOGGER.debug("formstartDate"+formstartDate);
					endDate=(String)reportBean.getEndDate();
					dt = sdf.parse(endDate );
					formendDate = formatter1.format(dt);
					LOGGER.debug("within try transactionDate"+startDate);
				...
				if((startDate == null || startDate.equalsIgnoreCase("")) && (endDate == null || endDate.equalsIgnoreCase("")))
				{
				   try{
					...
					{LOGGER.error("eGovFailure"+ "," +"setDates");}
				}
				if((startDate == null || startDate.equalsIgnoreCase("")) && (endDate != null || !endDate.equalsIgnoreCase("")))
				{
					try{
						...
						{
	                        LOGGER.error("eGovFailure"+ "," +"setDates1");
				...
				if((endDate == null || endDate.equalsIgnoreCase("")) && (startDate != null || !startDate.equalsIgnoreCase("")))
				{
					try{
						...
						rs.close();
						st.close();
					}catch(SQLException  ex){LOGGER.error("eGovFailure"+ "," +"setDates2");}
				}
				...
				//LOGGER.debug("******Dates: "+ startDate + ", " + endDate + ", " + preYrSDate + ", " + preYrEDate);
			}
		});

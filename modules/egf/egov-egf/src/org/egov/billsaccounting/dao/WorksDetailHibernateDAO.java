...
public class WorksDetailHibernateDAO extends GenericHibernateDAO
{
	...
	{
		Query qry = getSession().createQuery("from Worksdetail wd where upper(wd.code) =:code");
	...
	{
		Query qry = getSession().createQuery("from Worksdetail wd order by code");
		return qry.list();
	}
	/**
	 *
	 ...
	 */
	public List<Worksdetail> getWorksDetailFilterBy(String workCode,UserImpl estimatePreparedBy,Date estimatePreparedOn,ArrayList<Integer> statusId)
	{
		Query qry;
    ...
    public List<Worksdetail> getWorksDetailFilterBy(String billType, String workCode,UserImpl estimatePreparedBy,Date estimatePreparedOn,ArrayList<Integer> statusId)
    {
        Query qry;
        StringBuffer qryStr = new StringBuffer();
        List<Worksdetail> worksDetailList=null;
     ...
     public Worksdetail findByWorkOrderId(String workOrderId)
        {
            Query qry = getSession().createQuery("from Worksdetail wd where wd.id =:id");
     ...
     {
         Query qry = getSession().createQuery("from Worksdetail wd where wd.id in(select mb.worksdetail from EgwMBHeader mb where mb.egwStatus=11 and mb.voucherHeaderId is null)  and wd.id not in (select cb.worksdetail From Contractorbilldetail cb where cb.billtype='Final') order by upper(wd.name)");
         return qry.list();
     }
    /* public List findAllCheckedWorksDetail()
     {
         Query qry = getSession().createQuery("from Worksdetail wd where statusid=11 order by upper(name)");
         return qry.list();
     }*/
     ...
     public List<Worksdetail> getWOForRevisionFilterBy(String workCode,UserImpl estimatePreparedBy,Date estimatePreparedOn,ArrayList<Integer> statusId)
 	{
 		Query qry;
 		StringBuffer qryStr = new StringBuffer();
 		List<Worksdetail> worksDetailList=null;
 	...
 	{
 		Query qry = getSession().createQuery("from Worksdetail wd where wd.parentid =:parentId order by id");
	...
	qry = getSession().createQuery(qryStr);
	...
	public List getTotalBillValue(Worksdetail wd)
	{
		...
		qry = getSession().createQuery(billRegqry);

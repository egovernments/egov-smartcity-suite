...
public class BudgetDetailsHibernateDAO extends GenericHibernateDAO implements BudgetDetailsDAO
{
	...
	private BigDecimal getDetails(Map<String,Object> detailsMap) throws ValidationException
	{
		...
		try
		{
			LOGGER.debug("financialyearid=="+financialyearid+",moduleid=="+moduleid+",referencenumber=="+referencenumber+",departmentid=="+departmentid+",functionid=="+functionid
			...
			if(amtavailable!=null && amtavailable.compareTo(BigDecimal.ZERO)>=0)
			{
				...
				Query q = persistenceService.getSession().createQuery(" from BudgetDetail bd where  bd.budget.financialYear.id=:finYearId and bd.budgetGroup.id in (:bgId)" +query);
			...
			for(int i=0;i<values.length;i++)
			{
				if(values[i].equals("department"))
				{
					if(departmentid==null || departmentid==0)
						...
						query = query+getQuery(DepartmentImpl.class, departmentid," and bd.executingDepartment=");
				}
				...
				else if(values[i].equals("boundary"))
				{
					if(boundaryid==null || boundaryid==0)
						...
						query = query+getQuery(BoundaryImpl.class, boundaryid," and bd.boundary=");
				}
				else
			...
			Integer functionaryid, Integer schemeid, Integer subschemeid,Integer boundaryid, List<Long> budgetheadid,Integer fundid) throws ValidationException
	{
		try
		{
			LOGGER.debug("financialyearid==="+financialyearid+",departmentid==="+departmentid+",functionid==="+functionid
			...
			LOGGER.debug("RE query="+finalquery);
			Query q = persistenceService.getSession().createQuery(finalquery);
			...
			if(obj==null)
			{
				LOGGER.debug("BE query="+finalquery);
				q = persistenceService.getSession().createQuery(finalquery);
	...
	public BigDecimal getActualBudgetUtilized(Map<String,Object> paramMap) throws ValidationException
	{
         ...
         try
		{
			...
			LOGGER.debug("deptid="+deptid+",functionid="+functionid+",functionaryid="+functionaryid+",schemeid="+schemeid+",subschemeid="+subschemeid+",boundaryid="+boundaryid+",budgetheadid="+budgetheadid+",asondate="+asondate);
	       ...
	       // String finid = findao.getFinancialYearByDate(asondate);
	       // CFinancialYear finyear = findao.getFinancialYearById(Long.valueOf(finid));
	        ...
	        query = query +getQuery(DepartmentImpl.class, deptid," and vmis.departmentid=");
	        ...
	        query = query +getQuery(BoundaryImpl.class, boundaryid," and vmis.divisionid=");
	...
	public BigDecimal getActualBudgetUtilizedForBudgetaryCheck(Map<String,Object> paramMap) throws ValidationException
	{
        ...
        try
		{
			...
			SimpleDateFormat sdf = new SimpleDateFormat("dd-MMM-yyyy",Constants.LOCALE);
	        FinancialYearHibernateDAO findao = new FinancialYearHibernateDAO(CFinancialYear.class,HibernateUtil.getCurrentSession());
	       // String finid = findao.getFinancialYearId(sdf.format(asondate));
	        CFinancialYear finyear = findao.getFinancialYearByDate(asondate);
	        if(finyear==null)
	        	throw new ValidationException(EMPTY_STRING,"Financial year is not fefined for this date ["+sdf.format(asondate)+"]");
	        fromdate = finyear.getStartingDate();
			...
			if("I".equals(coa.getType()) || "L".equals(coa.getType()))
				select=" SELECT SUM(gl.creditAmount)-SUM(gl.debitAmount) ";
			else
				select=" SELECT SUM(gl.debitAmount)-SUM(gl.creditAmount) ";
	...
	public BigDecimal getBudgetedAmtForYear(Map<String,Object> paramMap) throws ValidationException{
 		...
 		try
		{
			...
			LOGGER.debug("deptid "+deptid+",functionid "+functionid+",functionaryid "+functionaryid+",schemeid "+schemeid+",subschemeid "+subschemeid+",boundaryid "+boundaryid+",budgetheadids "+budgetHeadList+",financialyearid "+financialyearid);
			...
			String finalquery = " from BudgetDetail bd where bd.budget.isbere='RE' "+query;
			
			LOGGER.debug("RE query="+finalquery);
			 //Query hibQuery = HibernateUtil.getCurrentSession().createQuery(finalquery);
			 Query hibQuery = persistenceService.getSession().createQuery(finalquery);
			 ...
			 if(bdList==null || bdList.size()==0)
			{
				...
				LOGGER.debug("BE query="+finalquery);
				hibQuery = getService().getSession().createQuery(finalquery);
				 hibQuery.setParameterList("budgetHeadList", budgetHeadList);  
				 bdList =  (List<BudgetDetail>)hibQuery.list();
	...
	private boolean checkCondition(Map<String,Object> paramMap) throws ValidationException
	{
		...
		try
		{
			...
			//get budgettinh type for first BG object
				...
				LOGGER.debug("billAmt amount"+billAmt);
		...
		try
		{
			...
			else  // Accural based budgeting
			{
				...
				if(coa.getBudgetCheckReq()!=null && coa.getBudgetCheckReq().intValue()==1)
				{
					...
					if(!isBudgetCheckingRequiredForType(txnType,budgetHeadListByGlcode.get(0).getBudgetingType().toString()))
					{
						LOGGER.debug("No need to check budget for :"+glCode+" as the transaction type is "+txnType+"so skipping budget check");
						return true;
					}
					
					// get the financialyear from asondate
					FinancialYearHibernateDAO finDAO = new FinancialYearHibernateDAO(CFinancialYear.class,HibernateUtil.getCurrentSession());
					//String finyearid = finDAO.getFinancialYearId(new SimpleDateFormat("dd-MMM-yyyy",Constants.LOCALE).format(asondate));
					//if(EMPTY_STRING.equals(finyearid))
					//	throw new ValidationException(EMPTY_STRING,"Financial Year is not defined for-"+asondate);
					CFinancialYear finyear = finDAO.getFinancialYearByDate(asondate);
					if(finyear==null)
						throw new ValidationException(EMPTY_STRING,"Financial year is not fefined for this date ["+asondate+"]");
	...
	public BigDecimal getBillAmountForBudgetCheck(Map<String,Object> paramMap) throws ValidationException
	{
        ...
        try
		{
			...
			LOGGER.debug("deptid="+deptid+",functionid="+functionid+",functionaryid="+functionaryid+",schemeid="+schemeid+",subschemeid="+subschemeid+",boundaryid="+boundaryid+",glcodeid="+glcodeid+",asondate="+asondate);
				...
				for(int i=0;i<values.length;i++)
				{
					if(values[i].equals("department"))
					{
						if(deptid==null || deptid==0)
							...
							query = query +getQuery(DepartmentImpl.class, deptid," and bmis.egDepartment.id=");
					}
					else if(values[i].equals("function"))
					{
						if(functionid==null || functionid==0)
					...
					else if(values[i].equals("boundary"))
					{
						if(boundaryid==null || boundaryid==0)
							...
							query = query +getQuery(BoundaryImpl.class, boundaryid," and bmis.fieldid.id=");
					}
					else
						throw new ValidationException(EMPTY_STRING,"budgetaryCheck_groupby_values is not matching="+values[i]);
				}
			...
			query= "select sum(decode(bd.debitamount,null,0,bd.debitamount)-decode(bd.creditamount,null,0,bd.creditamount))  " +
					" from EgBillregister br, EgBilldetails bd, EgBillregistermis bmis  " +
					" where br.id=bd.egBillregister.id and br.id=bmis.egBillregister.id and (bmis.budgetCheckReq is null or bmis.budgetCheckReq=true)  and bmis.voucherHeader is null and upper(br.billstatus) not in ('CANCELLED') " +
					"    "+query;
			
			LOGGER.debug("getBillAmountForBudgetCheck query============"+query);
			Object ob=null;
			if(fromdate!=null)
			{
			ob= persistenceService.find(query, asondate,fromdate);
			}
			else
			{
				ob= persistenceService.find(query, asondate);   
			}
			if(ob==null)
				return BigDecimal.ZERO;
			else
	...
	public BigDecimal getBudgetedAmtForYearRegardingBEorRE(Map<String,Object> paramMap, String typeBeRe) throws ValidationException
	{
		...
		String query=EMPTY_STRING;
 		...
 		try {
			...
			LOGGER.debug("deptid "+deptid+",functionid "+functionid+",functionaryid "+functionaryid+",schemeid "+schemeid+",subschemeid "+subschemeid+",boundaryid "+boundaryid+",budgetheadid "+budgetheadid+",financialyearid "+financialyearid);

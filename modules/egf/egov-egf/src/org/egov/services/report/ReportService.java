...
public abstract class ReportService extends PersistenceService{
	...
	public List<Fund> getFunds() {
		Criteria voucherHeaderCriteria = getSession().createCriteria(CVoucherHeader.class);
	...
	public String getGlcodeForPurposeCode(Integer purposeId){
		Query query = getSession().createSQLQuery("select majorcode from chartofaccounts where purposeid="+purposeId);
	...
	protected String getFilterQuery(Statement balanceSheet){
		...
		if(balanceSheet.getFunctionary()!=null && balanceSheet.getFunctionary().getId()!=null && balanceSheet.getFunctionary().getId()!=0)
			query = query + " and mis.functionaryid="+balanceSheet.getFunctionary().getId().toString();
		if(balanceSheet.getField()!=null && balanceSheet.getField().getId()!=null && balanceSheet.getField().getId()!=0)
			query = query + " and mis.divisionid="+balanceSheet.getField().getId().toString();
		return query;
	}
	...
	protected String getTransactionQuery(Statement balanceSheet){
		...
		if(balanceSheet.getFunctionary()!=null && balanceSheet.getFunctionary().getId()!=null && balanceSheet.getFunctionary().getId()!=0)
			query = query + " and ts.functionaryid="+balanceSheet.getFunctionary().getId().toString();
		if(balanceSheet.getField()!=null && balanceSheet.getField().getId()!=null && balanceSheet.getField().getId()!=0)
			query = query + " and ts.divisionid="+balanceSheet.getField().getId().toString();
		return query;
	}
	...
	void addFundAmount(List<Fund> fundList, Statement type,BigDecimal divisor, StatementResultObject row) {
		for(int index=0;index< type.size();index++) {
			...
			if(type.get(index).getGlCode() != null && row.getGlCode().equals(type.get(index).getGlCode())) 
				type.get(index).getFundWiseAmount().put(getFundNameForId(fundList,Integer.valueOf((row.getFundId()))), amount);
		}
	}
	...
	protected List<StatementResultObject> getAllGlCodesFor(String scheduleReportType){
		Query query = getSession().createSQLQuery("select distinct coa.majorcode as glCode,s.schedule as scheduleNumber," +
				"s.schedulename as scheduleName,coa.type as type from chartofaccounts coa, schedulemapping s " +
				"where s.id=coa.scheduleid and coa.classification=2 and s.reporttype = '"+scheduleReportType+"' order by coa.majorcode")
				.addScalar("glCode")
	...
	List<StatementResultObject> getTransactionAmount(String filterQuery, Date toDate, Date fromDate,String coaType,String subReportType) {
		...
		Query query = getSession().createSQLQuery("select c.majorcode as glCode,v.fundid as fundId,c.type as type,sum(debitamount)-sum(creditamount) as amount" +
				" from generalledger g,chartofaccounts c,voucherheader v ,vouchermis mis where v.id=mis.voucherheaderid and " +
				"v.id=g.voucherheaderid and c.type in("+coaType+") and c.id=g.glcodeid and v.status not in("+voucherStatusToExclude+")  AND v.voucherdate <= '"+getFormattedDate(toDate)
				+"' and v.voucherdate >='"+getFormattedDate(fromDate)+"' and substr(c.glcode,0,"+minorCodeLength+") in " +
				"(select distinct coa2.glcode from chartofaccounts coa2, schedulemapping s where s.id=coa2.scheduleid and " +
				"coa2.classification=2 and s.reporttype = '"+subReportType+"') "+filterQuery+" group by c.majorcode,v.fundid,c.type order by c.majorcode")
				.addScalar("glCode")
				.addScalar("fundId")
				.addScalar("type")
				.addScalar("amount")
	...
	protected Map<String, String> getSubSchedule(String subReportType) {
		...
		List<Object[]> rows = getSession().createSQLQuery("select s.schedule,sub.subschedulename from egf_subschedule sub,schedulemapping s " +
				"where sub.reporttype='"+subReportType+"' and sub.SUBSCHNAME=s.REPSUBTYPE").list();
		for (Object[] row : rows) {
	...
	public Date getFromDate(Statement statement){
		...
		if("Date".equalsIgnoreCase(statement.getPeriod()) && statement.getAsOndate()!=null) {
			 ...
			 financialYear = financialYearDAO.getFinancialYearByDate(statement.getAsOndate());
	...
	public Date getToDate(Statement statement){
		...
		if("Half Yearly".equalsIgnoreCase(statement.getPeriod())){
			String halfYearly = getAppConfigValueFor("EGF","bs_report_half_yearly");
			String[] halfYearComponents = halfYearly.split("/");
			Calendar fin = Calendar.getInstance();
			fin.setTime(statement.getFinancialYear().getStartingDate());
			Calendar calendar = Calendar.getInstance();
			calendar.set(statement.getFinancialYear().getStartingDate().getYear(), Integer.parseInt(halfYearComponents[1]) - 1, Integer.parseInt(halfYearComponents[0]));
	...
	void removeFundsWithNoData(Statement statement) {
		...
		for (Iterator<Fund> fund = statement.getFunds().iterator(); fund.hasNext();) {
			Fund next = fund.next();
			for (StatementEntry balanceSheetEntry : statement.getEntries()) {
				if(balanceSheetEntry.getFundWiseAmount().containsKey(next.getName()))
	...
	protected void populateSchedule(Statement statement,String reportSubType) {
		Query query = getSession().createSQLQuery("select c.majorcode,s.schedulename,s.schedule from chartofaccounts c,schedulemapping s " +
				"where s.id=c.scheduleid and s.reporttype = '"+reportSubType+"' and c.type in(:coaType) group by c.majorcode,s.schedulename,s.schedule")
		...
		for(Object[] obj :scheduleList){
			for (int index=0;index< statement.size();index++) {
				if(obj[0]==null) obj[0]="";
				if(statement.get(index).getGlCode()!=null && obj[0].toString().equals(statement.get(index).getGlCode())){
	...
	protected void computeCurrentYearTotals(Statement statement,String type1,String type2) {
		for (StatementEntry balanceSheetEntry : statement.getEntries()) {
			if(type1.equals(balanceSheetEntry.getAccountName()) || type2.equals(balanceSheetEntry.getAccountName()) || balanceSheetEntry.isDisplayBold())
				continue;
			BigDecimal currentYearTotal = BigDecimal.ZERO;
			for (Entry<String, BigDecimal> entry : balanceSheetEntry.getFundWiseAmount().entrySet()) {

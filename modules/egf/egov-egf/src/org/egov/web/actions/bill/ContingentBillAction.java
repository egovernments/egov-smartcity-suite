...
public class ContingentBillAction extends BaseBillAction {
	...
	public static final SimpleDateFormat  DDMMYYYYFORMATS= new SimpleDateFormat("dd/MMM/yyyy",LOCALE);
	...
	private void getNetPayableCodes() {
		...
		{
			try {
				accountCodeByPurpose = commonsService.getAccountCodeByPurpose(Integer.valueOf(configValuesByModuleAndKey.get(i).getValue()));
	...
	private  List<Action> getValidActions(String purpose , EgBillregister cbill) {
		...
		Script validScript = (Script) scriptExecutionService.findAllByNamedQuery(Script.BY_NAME,"cbill.validation").get(0);
		List<String> list = (List<String>) scriptExecutionService.executeScript(validScript,scriptExecutionService.createContext("eisManagerBean", employeeService,"userId",Integer.valueOf(EGOVThreadLocals.getUserId().trim()),"date",new Date(),"purpose",purpose,"wfitem",cbill));
		for(Object s:list) 
		{
			if("invalid".equals(s))
				break;
	...
	public String create()  
	{
		...
		try {
			...
			//LOGGER.info("---------After  validateLedgerAndSubledger---------------------");
			cbill =(Cbill) createBill();
			//LOGGER.info("---------After  createBill---------------------");
			createCheckList(cbill);
			//LOGGER.info("---------After  createCheckList---------------------");
	...
	private void beforeViewWF(EgBillregister cbill){
			Map<String, Object>  map = voucherService.getDesgBYPassingWfItem("cbill.nextUser",cbill,
	...
	private EgBillregister prepareForViewModifyReverse() throws ClassNotFoundException {
		...
		for(EgBilldetails detail:egBilldetailes)
		{
			//getAll Credits incuding net pay 
				...
				{
				CFunction functionById = commonsService.getCFunctionById(detail.getFunctionid().longValue());
	...
	private void getHeadersFromBill(EgBillregister cbill) {
		...
		if(cbill.getStatus().getDescription().equalsIgnoreCase(FinancialConstants.CONTINGENCYBILL_APPROVED_STATUS))
		{
			String amountInWords=NumberToWord.amountInWords(Double.valueOf(cbill.getPassedamount().toString()));
	...
	private  Cbill createBill() {		
		final HashMap<String, Object> headerDetails = createHeaderAndMisDetails();
		//LOGGER.info("------------After seting header filed");
		headerDetails.put(VoucherConstant.SOURCEPATH, "/EGF/bill/contingentBill!beforeView.action?billRegisterId=");
	...
	private Cbill setBillDetailsFromHeaderDetails(Cbill bill,EgBillregistermis mis,boolean generateBill) 
	{
		...
		if(generateBill)
		{
		//	LOGGER.info("------generateBill number");
		if(isBillNumberGenerationAuto())
		{
	...
	//	LOGGER.info("Inside getNextBillNumber ---------------");

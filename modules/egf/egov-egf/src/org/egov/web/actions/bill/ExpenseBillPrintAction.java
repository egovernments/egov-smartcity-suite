...
public class ExpenseBillPrintAction extends BaseFormAction{
	...
	private void populateVoucher() {
		if(!StringUtils.isBlank(parameters.get("id")[0])){
			...
			CVoucherHeader voucherHeader = (CVoucherHeader) persistenceService.getSession().get(CVoucherHeader.class,id);
	...
	private Map<String,Object>  getBudgetDetails(CChartOfAccounts coa, EgBilldetails billDetail,String functionName) {
		...
		{
			actualAmtFromBill=actualAmtFromBill.subtract(billDetail.getDebitamount());
			currentBillAmount=billDetail.getDebitamount();
			
		}
		else
		{
			actualAmtFromBill=actualAmtFromBill.subtract(billDetail.getCreditamount());
			currentBillAmount=billDetail.getCreditamount();
		}
		BigDecimal balance=budgetedAmtForYear;
		if(actualAmtFromVoucher!=null){
			balance = balance.subtract(actualAmtFromVoucher);
			soFarAppropriated=soFarAppropriated.add(actualAmtFromVoucher);
		}
		if(actualAmtFromBill!=null)
		{
			balance = balance.subtract(actualAmtFromBill);
			soFarAppropriated=soFarAppropriated.add(actualAmtFromBill);
		}
	...
	private void getRequiredDataForBudget(EgBillregister cbill) {
		...
		CFinancialYear financialYear = commonsService.getFinancialYearByDate(cbill.getBilldate());

...
@Result(name=Action.SUCCESS, type="redirect", location = "budgetGroup.action")
@ParentPackage("egov")
@Validation
public class BudgetGroupAction extends BaseFormAction{
	...
	public void prepare()
	{
		...
		addDropdownData("majorCodeList", getPersistenceService().findAllBy(LENGTHQUERY,Long.valueOf(majorcodelength)));
		addDropdownData("minCodeList", getPersistenceService().findAllBy(LENGTHQUERY,Long.valueOf(rangecodelength)));
		addDropdownData("maxCodeList", getPersistenceService().findAllBy(LENGTHQUERY,Long.valueOf(rangecodelength)));

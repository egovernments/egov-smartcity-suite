...
public class PayInSlipAction extends BaseVoucherAction {
	...
	{
		LOGGER.debug("Return the New Pay in slip Form");
		voucherHeader.reset();
		contraBean.reset();
	...
	public void prepare() {
		...
		LOGGER.debug("PayInSlipAction |prepare | Start");
		super.prepare();
	...
	public String create() {
		LOGGER.debug("PayinSlipAction | Create | start");
		createOrEdit("create");
	...
	public String edit(){
		LOGGER.debug("PayinSlipAction | Edit | start");
		voucherHeader.setType("Contra");
		createOrEdit("edit");
	...
	public String reverse(){
		LOGGER.debug("PayinSlipAction | Reverse | start");
		loadSchemeSubscheme();
	...
	public String search() {
		LOGGER.debug("PayInSlipAction | search | Start");
		loadSchemeSubscheme();
		loadBankBranchForFund();
		loadBankAccountNumber(contraBean);
		...
		query.append("select distinct iHeader.id ,iHeader.instrumentNumber,iHeader.instrumentDate,iHeader.instrumentAmount,vh.voucherNumber,vh.voucherDate")
				...
				append(" and iHeader.statusId.id = ").append(commonsService.getStatusByModuleAndCode("Instrument",FinancialConstants.INSTRUMENT_CREATED_STATUS).getId());
		...
		LOGGER.debug(" Instrument List Size = "+iSearchList.size());
	...
	private void createOrEdit(String mode){
		...
		LOGGER.debug("Serial numbers of instruments to be deposited "+selectedInstr);
		...
		try{
			if(mode.equalsIgnoreCase("create")){
			 ...
			 for (String instHeaderId : iHeaderId) {
				 ...
				 commonsService.getBankaccountById(Integer.valueOf(contraBean.getAccountNumberId())).getChartofaccounts().getGlcode(), instrumentHeader);
	...
	private CVoucherHeader callCreateVoucher() {
		try {
			 Bankaccount bankaccount = commonsService.getBankaccountById(Integer.valueOf(contraBean.getAccountNumberId()));

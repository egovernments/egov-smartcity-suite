	...
	public String save(){
		...
		for (ChequeDetail chequeDetail : chequeDetailsList) {
			...
			if(chequeDetail.getNextChqPresent().equalsIgnoreCase("No") && chequeDetail.getIsExhusted().equalsIgnoreCase("No")){
				...
				if(null == chequeMap.get(chequeDetail.getFromChqNo()+chequeDetail.getToChqNo())){
						...
						chequeMap.put(accountCheques.getFromChequeNumber()+accountCheques.getToChequeNumber(), accountCheques);
					}else{
					...
					DepartmentImpl dept = (DepartmentImpl)persistenceService.find("from DepartmentImpl where id="+chequeDetail.getDeptId());

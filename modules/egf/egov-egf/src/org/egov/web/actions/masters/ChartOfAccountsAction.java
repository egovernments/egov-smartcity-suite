...
public class ChartOfAccountsAction extends BaseFormAction{
		...
		try {
			for (Accountdetailtype row : accountDetailType) {
				...
				while(iterator.hasNext()){
					...
					if(hasReference(row.getId(),next.getGlCodeId().getGlcode())){
			...
			populateAccountDetailTypeList();
			String message = accountDetail.concat(" ").concat(getText("chartOfAccount.accDetail.cannot.delete"));
	...
	boolean hasReference(Integer id,String glCode){
		SQLQuery query = persistenceService.getSession().createSQLQuery("select * from chartofaccounts c,generalledger gl,generalledgerdetail gd " +
	...
	public String saveDetailCode() throws Exception {
		if(glCode != null){

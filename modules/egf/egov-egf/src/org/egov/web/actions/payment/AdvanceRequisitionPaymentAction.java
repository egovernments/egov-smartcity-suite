...
public class AdvanceRequisitionPaymentAction extends BaseVoucherAction{
	...
	void loadApproverUser(){
		...
		String type = "Payment" + "|";  
		EgovMasterDataCaching masterCache = EgovMasterDataCaching.getInstance();
		Map<String, Object>  map = getVoucherService().getDesgByDeptAndType(type, scriptName);
	...
	private void populateHeaderDetails(HashMap<String, Object> headerdetails)throws ParseException {
		...
		if(egAdvanceReqMises!=null){
			...
			if(egAdvanceReqMises.getEgDepartment()!=null && egAdvanceReqMises.getEgDepartment().getId()!=null){
				voucherHeader.getVouchermis().setDepartmentid((DepartmentImpl) persistenceService.find("from DepartmentImpl where id=?",egAdvanceReqMises.getEgDepartment().getId()));
			...
			if(egAdvanceReqMises.getSubFieldId()!=null && egAdvanceReqMises.getSubFieldId().getId()!=null){
				voucherHeader.getVouchermis().setDivisionid((BoundaryImpl) persistenceService.find("from BoundaryImpl where id=?",egAdvanceReqMises.getSubFieldId().getId()));
	...
	private void populateAccountCodeDetails(List<HashMap<String, Object>> accountcodedetails) {
		...
		Set advRequisitionDetailsSet = advanceRequisition.getEgAdvanceReqDetailses();
		Iterator itr = advRequisitionDetailsSet.iterator();	
		CFunction function = null;
		while(itr.hasNext())
		{
			EgAdvanceRequisitionDetails advanceReqDetail = (EgAdvanceRequisitionDetails)itr.next(); 
			if (advanceReqDetail != null)
			{
				function = advanceReqDetail.getFunction();				
			}
		}
		if(function != null)
			accdetailsMap.put(VoucherConstant.FUNCTIONCODE,function.getCode());
		logger.info("After setting FunctionCode->accdetailsMap :"+accdetailsMap);
		...
		for (EgAdvanceRequisitionDetails row : advanceRequisition.getEgAdvanceReqDetailses()) {
			...
			accdetailsMap1.put(VoucherConstant.FUNCTIONCODE,row.getFunction().getCode());
		}

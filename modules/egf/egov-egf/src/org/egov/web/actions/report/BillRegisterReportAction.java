...
public class BillRegisterReportAction   extends SearchFormAction{
		...
		for(Object[] object : list) {
			try {
				...
				if(!StringUtils.isEmpty(billRegReport.getVoucherNumber())){
					List<Miscbilldetail> miscBillList =(List<Miscbilldetail>)persistenceService.findAllBy(" from Miscbilldetail where billnumber=?", billRegReport.getBillNumber());
	...
	protected String getQueryByExpndType(String expndType,String whereQuery){
		...
		append("  and vh.status=0 ").append(whereQuery).append(" group by b.billnumber, vh.vouchernumber,mis.payto, b.passedamount, s.description,b.billdate");
		
		query.append(" UNION ");
		
		// query to get bills for which vouchers are not fully approved.
		...
		append(" where b.id= bd.billid and b.id=mis.billid and mis.voucherheaderid =vh.id  and s.id= b.statusid and bd.creditamount > 0").
		append("  and bd.glcodeid in(").append(netPayCodes.toString()).append(")").append(" and b.expendituretype='").append(expndType).append("'").
		append("  and vh.status!=0").append(whereQuery).append(" group by b.billnumber, null,mis.payto, b.passedamount, s.description,b.billdate");
		...
		for(EgBillregister billRegister : list) {
			try {
				...
				if(null != netAmount ){ // for a bill if wrong net payable code is used , then filter out that bill.
				 ...
				 LOGGER.debug( "Failed while processing bill number :"+ billRegister.getBillnumber());
				 throw e;
			}

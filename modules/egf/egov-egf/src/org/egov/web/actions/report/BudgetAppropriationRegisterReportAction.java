...
public class BudgetAppropriationRegisterReportAction  extends BaseFormAction {
	...
	public String search() {
		if(parameters.get("asOnDate")[0] != null) {
			...
			try {
				...
				CFinancialYear financialYear = financialYearDAO.getFinancialYearByDate(dtAsOnDate);
		...
		if(budgetService.hasApprovedReForYear(Long.parseLong(financialYearId)))
		{
			...
			// -- No additional appropriations for REs.
			totalGrant = reAmount;
		}
			...
			for(BudgetAppDisplay budgetAppropriationRegisterDisp : budgetAppropriationRegisterList) {
				...
				if(cumulativeAmt == null) {
					if(BudgetingType.ALL.equals(budgetGroup.getBudgetingType())){ 
						...
						cumulativeAmt = budgetAppropriationRegisterDisp.getBillAmount().abs();
					}
					...
					if(BudgetingType.ALL.equals(budgetGroup.getBudgetingType())){ 
						if(budgetAppropriationRegisterDisp.getDebitAmount() != null && budgetAppropriationRegisterDisp.getDebitAmount().compareTo(BigDecimal.ZERO) == 1) {
							budgetAppropriationRegisterDisp.setCumulativeAmount(budgetAppropriationRegisterDisp.getBillAmount().abs().add(cumulativeAmt));
						} else {
							budgetAppropriationRegisterDisp.setCumulativeAmount(cumulativeAmt.subtract(budgetAppropriationRegisterDisp.getBillAmount().abs()));
						}
					}else{
						budgetAppropriationRegisterDisp.setCumulativeAmount(budgetAppropriationRegisterDisp.getBillAmount().abs().add(cumulativeAmt));
					}
					cumulativeAmt = budgetAppropriationRegisterDisp.getCumulativeAmount();
				}
	...
	private Map<String, Object> getParamMapForReportFile() {
		...
		if(isBeDefined)
		{
			...
			paramMapForReportFile.put("rAddiAppr", addtionalAppropriationForBe.toString() );
		}
		else
		{
			paramMapForReportFile.put("rBE", rBEorREAmountForm+"Budget Not Defined ");
			paramMapForReportFile.put("rAddiAppr", "" );
		}
		if(isReDefined)

...
@Deprecated
	...
	protected void loadSchemeSubscheme(){
		...
		if(headerFields.contains("subscheme") && null != voucherHeader.getVouchermis() && null !=  voucherHeader.getVouchermis().getSchemeid()){
	...
	protected void loadFundSource(){
		if(headerFields.contains("fundsource") && null != voucherHeader.getVouchermis() && null !=  voucherHeader.getVouchermis().getSchemeid()){
			addDropdownData("subschemeList", getPersistenceService().findAllBy("from SubScheme where scheme.id=? and isActive=1 order by name", voucherHeader.getVouchermis().getSchemeid().getId()));
		}
	} 
...
protected HashMap<String, Object> createHeaderAndMisDetails() throws ValidationException
	{
		...
		if(voucherHeader.getVouchermis().getDepartmentid()!=null)
			headerdetails.put(VoucherConstant.DEPARTMENTCODE, voucherHeader.getVouchermis().getDepartmentid().getDeptCode());

...
public class PositionService extends PersistenceService<Position, Integer> {  
	...
	{
		DetachedCriteria detachAssignmentPrd=DetachedCriteria.forClass(AssignmentPrd.class, "assignmentPeriod");
		...
		detachAssignment.createAlias("assignment.assignmentPrd", "assprd").add(Subqueries.propertyIn("assprd.id", detachAssignmentPrd));
		...
		{
			criteria.add(Restrictions.eq("position.desigId.designationId", designationMasterId));
		}

...
public class AssignmentHibernateDAO extends GenericHibernateDAO implements AssignmentDAO
{
	 ...
	 * @param session
	 */
	public AssignmentHibernateDAO(Class persistentClass, Session session)
	{
	...
	{
		LOGGER.info("qry"+id);
			Query qry = getSession().createSQLQuery("select id  from EG_EMP_ASSIGNMENT e   where E.ID in(select ASSIGNMENT_ID from EG_EMPLOYEE_DEPT where HOD is not null)    and E.ID = :id ").addScalar("id",IntegerType.INSTANCE);
			qry.setInteger("id", id);
			LOGGER.info("qry"+qry);
			LOGGER.info("qry"+qry.list());

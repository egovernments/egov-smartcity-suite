...
public class PersonalInformationService extends PersistenceService<PersonalInformation, Integer> implements EntityTypeService 
{
	...
	private DetachedCriteria getDisciplinaryForStatus(Date asOfDate) 
	{
		DetachedCriteria disciplinaryCriteria=DetachedCriteria.forClass(DisciplinaryPunishment.class,"disciplinary");
		disciplinaryCriteria.createAlias("disciplinary.employeeId", "empId").createAlias("disciplinary.status","status");
		disciplinaryCriteria.add(Restrictions.and(Restrictions.eq("disciplinary.whetherSuspended", '1'),
				Restrictions.and(Restrictions.le("disciplinary.dateOfSuspension", asOfDate), 
						Restrictions.or(Restrictions.ge("disciplinary.dateOfReinstatement", asOfDate), Restrictions.isNull("disciplinary.dateOfReinstatement"))))).
						add(Restrictions.and(
								...
								setProjection(Projections.property("empId.idPersonalInformation")); 
		return disciplinaryCriteria;
	}
	private Criteria getCriteriaForEmpSearchByStatus(Integer statusid ,Date fromDate,Date toDate)
	{
		...
		{
			detachCriteriaPersonalInfo.createAlias("emp.egpimsAssignmentPrd", "assPrd").
			...
			add(Subqueries.propertyNotIn("emp.idPersonalInformation", getDisciplinaryForStatus(toDate)));


		}
		else if(egwStatus.getModuletype().equals("Employee") && egwStatus.getDescription().equalsIgnoreCase(EisConstants.STATUS_TYPE_SUSPENDED))
		{
			detachCriteriaPersonalInfo.add(Subqueries.propertyIn("emp.idPersonalInformation", getDisciplinaryForStatus(toDate)));  

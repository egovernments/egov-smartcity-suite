...
public class PostCreationWorkflowAction extends EisCommonWorkflowAction{
	...
	public String save()
	{
		...
		empPosition.setDesigId((DesignationMaster)persistenceService.getSession().load(DesignationMaster.class, Integer.valueOf(getDesignationId())));
		fireWorkFlow();
	...
	protected void saveToOriginal()
	{
			DeptDesig deptDesig=(DeptDesig)persistenceService.find(
					" from DeptDesig where deptId.id=? and desigId.designationId=? ",empPosition.getDeptId().getId(),Integer.valueOf(getDesignationId()));
			...
			else
			{
				...
				deptDesig.setDesigId((DesignationMaster)getPersistenceService().getSession().load(DesignationMaster.class, Integer.valueOf(getDesignationId())));
			}
			Position position = new Position();
			position.setBillNumber(null);
			position.setName(empPosition.getPostName());
			position.setDesigId(empPosition.getDesigId());

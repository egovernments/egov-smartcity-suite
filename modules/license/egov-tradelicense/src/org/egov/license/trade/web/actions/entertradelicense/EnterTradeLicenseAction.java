/*
 * @(#)EnterTradeLicenseAction.java 3.0, 25 Jul, 2013 4:57:53 PM
 * Copyright 2013 eGovernments Foundation. All rights reserved. 
 * eGovernments PROPRIETARY/CONFIDENTIAL. Use is subject to license terms.
 */
package org.egov.license.trade.web.actions.entertradelicense;
...
@ParentPackage("egov")
@Result(name = "viewlicense", type = "redirect", location = "/viewtradelicense/viewTradeLicense!view.action", params = {"model.id", "${model.id}" })
public class EnterTradeLicenseAction extends BaseLicenseAction {
	...
	@Override
	@Validations(requiredFields = { @RequiredFieldValidator(fieldName = "oldLicenseNumber", message = "", key = Constants.REQUIRED), @RequiredFieldValidator(fieldName = "dateOfCreation", message = "", key = Constants.REQUIRED),
			@RequiredFieldValidator(fieldName = "licensee.applicantName", message = "", key = Constants.REQUIRED), @RequiredFieldValidator(fieldName = "licenseeZoneId", message = "", key = Constants.REQUIRED),
			...
			key = "Please enter the valid Email Id") }, stringLengthFields = { @StringLengthFieldValidator(fieldName = "oldLicenseNumber", maxLength = "30", message = "", key = "Maximum Length for Old License Number is 30"),
		...
		if (this.ts.getTps().findAllBy("from License where oldLicenseNumber = ?", this.tradeLicense.getOldLicenseNumber()).isEmpty()) {
			if (this.tradeLicense.getInstalledMotorList() != null) {
				...
				while (motorDetails.hasNext()) {
					...
					if ((installedMotor != null) && (installedMotor.getHp() != null) && (installedMotor.getNoOfMachines() != null) && (installedMotor.getHp().compareTo(BigDecimal.ZERO) != 0)

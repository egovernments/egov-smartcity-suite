/*
 * @(#)SearchTradeAction.java 3.0, 25 Jul, 2013 4:57:53 PM
 * Copyright 2013 eGovernments Foundation. All rights reserved. 
 * eGovernments PROPRIETARY/CONFIDENTIAL. Use is subject to license terms.
 */
package org.egov.license.trade.web.actions.search;
...
import org.egov.license.trade.web.actions.search.form.SearchForm;
...
public class SearchTradeAction extends BaseFormAction {
	...
	@Override
	public void prepare() {
		this.noticelist.add(Constants.DROPDOWN_PRENOTICE);
		this.noticelist.add(Constants.DROPDOWN_SCNOTICE);
		this.noticelist.add(Constants.DROPDOWN_SUSNOTICE);
		this.noticelist.add(Constants.DROPDOWN_CANNOTICE);
		super.prepare();
		...
		this.addDropdownData(Constants.DROPDOWN_NOTICE_LIST, this.noticelist);
	...
	public String search() {
		...
		if (this.page == 0) {
			...
			this.reportSize = ((Long)criteria.uniqueResult()).intValue();
		}
	...
	private Criteria createSearchQuery() {
		Criteria criteria;
		if (this.searchForm != null && this.searchForm.getNoticeNumber().equals("") && this.searchForm.getDocNumber().equals("") && this.searchForm.getNoticeType().equals("-1") && this.searchForm.getNoticeFromDate() == null
				&& this.searchForm.getNoticeToDate() == null) {
			...
			if (StringUtils.isNotBlank(this.searchForm.getLicenseNumber())) {
				...
				if (StringUtils.isNotBlank(this.searchForm.getZone()) && !this.searchForm.getZone().equals("-1")) {
					...
					final Criterion ct1 = Restrictions.eq("parentBndry.id", Integer.valueOf(this.searchForm.getZone()));
					final Criterion ct2 = Restrictions.eq("bndry.id", Integer.valueOf(this.searchForm.getZone()));
	...
	public boolean isRenewable(final Long licenseId) {
		final License license = (License) this.persistenceService.find("from License where id=?", licenseId);

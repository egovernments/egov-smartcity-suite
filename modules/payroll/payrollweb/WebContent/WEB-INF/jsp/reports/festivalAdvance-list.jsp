...
<%@ taglib uri="http://displaytag.sf.net" prefix="display"%> 
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
         ...
         <s:if test="%{searchResult.fullListSize != 0}">
            <display:table name="searchResult" pagesize="30"
				...
				<display:caption style="text-align: center">${billNumber}
				</display:caption >
                    ...
                    <display:column title="FEST ADV."
						titleKey="report.fa.amount"
						style="width:10%;text-align:right" >
   	 						<s:property  value='%{#attr.currentRow.amount}' />
					</display:column>
                                      
                    <display:column title="INSTALLMENT NO."
						titleKey="report.fa.installmentNo"
						style="width:10%;text-align:right" >
   	 						<s:property  value='%{#attr.currentRow.installmentNo}' />
					</display:column>
 ...
 function exportPDF() {
	...
	var url="${pageContext.request.contextPath}/reports/festivalAdvanceReport!exportFestivalAdvancePDF.action?year="+year+"&department="+deptId+"&month="+month+"&billNumber="+billNumber+"&billNumberId="+billNumberId;	
...
function exportExcel() {
	 ...
	 var url="${pageContext.request.contextPath}/reports/festivalAdvanceReport!exportFestivalAdvanceExcel.action?year="+year+"&department="+deptId+"&month="+month+"&billNumber="+billNumber+"&billNumberId="+billNumberId;
	 window.open(url,'','height=650,width=980,scrollbars=yes,left=0,top=0,status=yes');
}

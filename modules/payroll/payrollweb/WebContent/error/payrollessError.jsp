<%@ include file="/includes/taglibs.jsp" %>
<%@ page import="org.apache.struts.action.ActionMessage,
				 org.apache.struts.action.ActionMessages" %>
<html>
<head>
<title>Error Page</title>
		<meta http-equiv="Content-Type" content="text/html; charset=UTF-8"/>
		<title>WelCome to eGov PTIS</title>
		<script language="JavaScript" src="/dhtml.js" type="text/JavaScript"></script>
		<LINK REL=stylesheet HREF="/css/egov.css" TYPE="text/css">



</head>

<body>

<!-- Header Section Begins -->

<!-- Header Section Ends -->


<table align='center'>
<tr>
<td>
<!-- Tab Navigation Begins -->
<center>

</center>
<!-- Tab Navigation Ends -->

<!-- Body Begins -->

<form name="signup" id="signup" method="post" action="#">
<!--img  align=absmiddle src="img/c_top.gif"-->
 <div align="center">
 <b><font color="#000080">



 <table align='center' width="600" height="83">
<tr >
        <td class="labelcell" style="width:650">
	<p><font size="2"><b><center>You are currently not an employee in the system or do not have a current post to perform this operation. Please check with your administrator and try again.</center></b></font>
     	</p>
     	<br/>
     	  	<center><html:messages id="message" message="true">
			<li><%= message %></li>
	        </html:messages></center>
		            
     	</td>
     	</tr>
     	
     	
 </table>
</div>
</form>
<!--img align=absmiddle src="img/c_bot.gif"-->

</td></tr>
</table>
</body>
</html>

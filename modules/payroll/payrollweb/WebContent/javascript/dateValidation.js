...
* Check for the valid characters in the date fields 
**/
function checkForValidCharsInDate(obj)
{
	...
	if(obj.readOnly==false)
	{
		if(dt!="" && dt!=null)
		{
			...
			if(invalid==true)
			{
				alert("Please enter the valid characters in Date Field");
				obj.value="";
				obj.focus();
				return false;
			}
...
function validateDateFormat(obj)
{
	...
	if(obj.readOnly==false)
	{
		if(dtStr!="" && dtStr!=null)
		{
	    	if(checkForValidCharsInDate(obj))
	    	{
				if(validDate && checkDateNew(obj))  // checkDate(obj) from jsCommonMethods.js
		    	{

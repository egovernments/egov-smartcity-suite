...
function btnbgcolorprintpage(obj,tablename1,tablename2,color)
{
	tablename1.style.background = 'white';
	obj.style.visibility = "hidden";
	tablename2.style.visibility="hidden";
	window.print();
}
...
// used by BoundaryPlugin
//onchange resets the all the combo boxes from  third one from the current  to size  1
//note second one is done by loadcombo itself
function clearAdminChilds(ob)
{
for(var i=0;i<10;i++)
	{
if("adminBndry"+i==ob.name)
		{	
			bndry=i;
			break;
		}
...
//used by boundary plugin
// this function adds tool tip to boundary plugin combo boxes
// on  change of parent child combo is populated then its text value is added as title for 
// tool tip 
//this applies for only direct child combo
...
if(comboBox!=undefined && comboBox!=null)
	...
	for(var j=0;j<comboBox.options.length;j++)
			{
		var toolTip=comboBox.options[j].text;
		comboBox.options[j].title=toolTip;
		  }

...
import static org.egov.ptis.nmc.constants.NMCPTISConstants.AREA_CONSTANT_FOR_RESIDENTIAL_BUILDING;
...
import static org.egov.ptis.nmc.constants.NMCPTISConstants.STANDARD_DEDUCTION_PERCENTAGE;
...
public class ResdAndNonResdUnitTaxCalculator {
	...
	public UnitTaxCalculationInfo calculateUnitTax(FloorImpl floor, Boundary propertyArea,
			List<String> applicableTaxes, Installment installment, Long categoryId, Boolean isExemptedFromTax,
			Property property) {
		...
		Boolean isResidentialUnit = USAGES_FOR_RESD.contains(floor.getPropertyUsage().getUsageName()) ? true : false;
		Boolean isNonResidentialUnit = USAGES_FOR_NON_RESD.contains(floor.getPropertyUsage().getUsageName()) ? true : false;
		Boolean isOpenPlotUnit = USAGES_FOR_OPENPLOT.contains(floor.getPropertyUsage().getUsageName()) ? true : false;
		
		if (isExemptedFromTax == null) {
			if (isOpenPlotUnit) {
		...
		if (baseRent == ZERO && (floor.getManualAlv() == null || floor.getManualAlv().equals(""))) {
			...
			// ALV
			BigDecimal grossAnnualRentAfterDeduction = annualRentUnitWiseBeforeDeduction.subtract(
					applicableDeducationAmount).setScale(0, ROUND_HALF_UP);
	...
	private UnitTaxCalculationInfo addResidentialUnitInfo(FloorImpl floorImpl,
		...
		if (floorImpl.getUnitType() != null
				& floorImpl.getUnitType().getCode().equalsIgnoreCase(NMCPTISConstants.UNITTYPE_OPEN_PLOT)) {

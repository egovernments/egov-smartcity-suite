 ...
 */
@ParentPackage("egov")
@Results( { @Result(name = "invalidUser", type="redirect", location = "workflow", params = {
...
public class RecoveryAction extends BaseRecoveryAction {
	...
	public String generateNotice155() {
		...
		if (reportOutput != null && reportOutput.getReportOutputData() != null) {
			...
			PtNotice ptNotice = noticeService.saveNoticeToDMS(Integer.valueOf(EGOVThreadLocals.getUserId()),
					Notice155PDF, noticeNo, NMCPTISConstants.NOTICE155, recovery.getBasicProperty());
	...
	public String generateWarrantApplicaton() {
		...
		if (reportOutput != null && reportOutput.getReportOutputData() != null) {
			...
			/*
			 * saveNoticeToDMS(recovery.getBasicProperty().getProperty(),
			 * recovery.getWarrant().getNotice(), NoticePDF);
			 */
			PtNotice ptNotice = noticeService.saveNoticeToDMS(Integer.valueOf(EGOVThreadLocals.getUserId()),
					warrantApplPDF, noticeNo, NMCPTISConstants.WARRANT_APPLICATION, recovery.getBasicProperty());
	...
	public String generateWarrantNotice() {
		...
		if (reportOutput != null && reportOutput.getReportOutputData() != null) {
			...
			PtNotice ptNotice = noticeService.saveNoticeToDMS(Integer.valueOf(EGOVThreadLocals.getUserId()),
					Notice156PDF, noticeNo, NMCPTISConstants.NOTICE156, recovery.getBasicProperty());
	...
	public String generateCeaseNotice() {
		...
		if (reportOutput != null && reportOutput.getReportOutputData() != null) {
			...
			PtNotice ptNotice = noticeService.saveNoticeToDMS(Integer.valueOf(EGOVThreadLocals.getUserId()),
					Notice159PDF, noticeNo, NMCPTISConstants.NOTICE159, recovery.getBasicProperty());
	...
	private void updateWfstate(String value) {
		...
		if (WFLOW_ACTION_STEP_SAVE.equalsIgnoreCase(workflowBean.getActionName())) {
			...
			Position position = eisCommonsService.getPositionByUserId(recovery.getCreatedBy().getId());

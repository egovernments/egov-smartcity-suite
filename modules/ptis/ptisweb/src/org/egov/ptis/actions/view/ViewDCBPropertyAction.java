...
public class ViewDCBPropertyAction extends BaseFormAction implements ServletRequestAware {
	...
	public String displayPropInfo() {
		...
		Map<String, BigDecimal> demandCollMap = ptDemandDao.getDemandCollMap(getBasicProperty().getProperty());
		...
		try {
			if (getBasicProperty() == null) {
				...
				setOwnerName(ptisCacheMgr.buildOwnerFullName(basicProperty.getProperty().getPropertyOwnerSet()));
				...
				setPropertyType(basicProperty.getProperty().getPropertyDetail().getPropertyTypeMaster().getType());
	...
	private void prepareDisplayInfo() {
		...
		/* Commented out because of no Collection module */
		...
		reasonList.addAll(NMCPTISConstants.DEMAND_REASON_ORDER_MAP.keySet());

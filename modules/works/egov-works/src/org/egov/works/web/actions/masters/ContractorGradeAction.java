...
public void prepare() {
	...
	List<BigDecimal> tempMaxAmountList=(List<BigDecimal>) persistenceService.findAllByNamedQuery("getContractorGradeMaxAmountList");
	List<BigDecimal> tempMinAmountList=(List<BigDecimal>) persistenceService.findAllByNamedQuery("getContractorGradeMinAmountList");
...
public String searchGradeDetails() {
		...
		if (grade != null && !grade.equals("")) {
			hasNoErrors = Pattern.matches(
					ValidatorConstants.alphaNumericwithSpace, grade);
...
public SearchQuery prepareQuery(String sortField, String sortOrder){
	...
	if (getGrade() != null && !getGrade().trim().equals("")) {
		contractorGradeSql.append(" and UPPER(cg.grade) like '%").append(getGrade().trim().toUpperCase()).append("%'");
	}
   
	if(getMinAmount() != -1){
		contractorGradeSql.append(" and cg.minAmount = ").append(getMinAmount());
	}
	
	if(getMaxAmount() != -1){
		contractorGradeSql.append(" and cg.maxAmount = ").append(getMaxAmount());
	}
	...
	return new SearchQueryHQL(contractorGradeStr, countQuery, null);

	...
	public void prepare(){
		...
		if(woEstimateId != null){
			if(getTrackedMilestone(woEstimateId)==null || mode.equalsIgnoreCase("view")){
				...
				Milestone milestone=(Milestone)getPersistenceService().find("from Milestone m where m.id=(select max(m1.id) from Milestone m1 where m1.workOrderEstimate.id=?)",woEstimateId);
				
				if(mode.equalsIgnoreCase("view") && milestone.getTrackMilestone()!=null && milestone.getTrackMilestone().size()!=0){
					TrackMilestone tempTrackMilestone=(TrackMilestone)getPersistenceService().find("from TrackMilestone tm where tm.egwStatus.code<>? and tm.id=(select max(tm1.id) from TrackMilestone tm1 where tm1.milestone.id=?)","NEW",milestone.getId());
...
/*		if(trackMilestone.getCurrentState()!=null &&
				trackMilestone.getCurrentState().getValue().equals(State.NEW)){
...
*/		
	...
	public String save(){
		...
		if(id==null){
			trackMilestone.setEgwStatus(commonsService.getStatusByModuleAndCode(TRACK_MILESTONE_MODULE_KEY,"APPROVED"));
		}
...
/*	public void getDesignation(TrackMilestone trackMilestone){
		if(trackMilestone.getCurrentState()!= null 
				&& !"NEW".equalsIgnoreCase(trackMilestone.getCurrentState().getValue())) {
			String result = worksService.getEmpNameDesignation(trackMilestone.getState().getOwner(),trackMilestone.getState().getCreatedDate());
	...
	public void getDesignation(TrackMilestone trackMilestone){
		if(trackMilestone.getCurrentState()!= null 
				&& !"NEW".equalsIgnoreCase(trackMilestone.getCurrentState().getValue())) {
			String result = worksService.getEmpNameDesignation(trackMilestone.getState().getOwner());
			if(result != null && !"@".equalsIgnoreCase(result)) {
				String empName = result.substring(0,result.lastIndexOf('@'));
				String designation =result.substring(result.lastIndexOf('@')+1,result.length());
				setNextEmployeeName(empName);
...
*/
	...
	@SkipValidation
	public String edit(){
/*		if(((SOURCE_INBOX.equalsIgnoreCase(sourcepage) || MODE_MODIFY.equalsIgnoreCase(mode)) && (trackMilestone.getCurrentState()!=null && 
				!trackMilestone.getCurrentState().getValue().equals(State.END))) || (trackMilestone.getCurrentState()!=null &&
						trackMilestone.getCurrentState().getValue().equals(State.NEW))){
			User user=userDao.getUserByID(Integer.valueOf(EGOVThreadLocals.getUserId()));
			boolean isValidUser=worksService.validateWorkflowForUser(trackMilestone,user);
			if(isValidUser){

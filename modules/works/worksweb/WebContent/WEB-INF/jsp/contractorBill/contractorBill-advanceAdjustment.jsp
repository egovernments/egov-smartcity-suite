...
function setupAdvanceAdjustmentValues(){
	...
	totalPenBalLoadHandler = function(req,res){
	  ...
	  document.getElementById('bal').style.display='';	
	   ...
	   document.getElementById("currenbilldeduction").value=roundTo(dom.get("currenbilldeduction").value);
	...
	function calculateAdvanceAdjustment(){
		...
		var billId = '<s:property value="%{id}" />'
		billDate = dom.get("billdate").value;
		makeJSONCall(["totalPendingBalance"],'${pageContext.request.contextPath}/contractorBill/ajaxContractorBill!calculateAdvanceAdjustment.action',
	    {workOrderId:workOrderId,billDate:billDate,workOrderEstimateId:workOrderEstimateId,billId:billId},totalPenBalLoadHandler,totalPenBalLoadFailureHandler) ;

...
<script>
   ...
   function getNumbers(value){	
	...
	function validateEgBillPayeedetails(elem,recordId){	
		...
		if(elem.value=='' || isNaN(elem.value) || getNumber(elem.value)<= 0){		
		 ...
		 if(document.getElementById("billamount")!=null){
		 	...
		 	if(totalStatAmt > getNumber(document.getElementById("workRecordedAmount").value)){
		 	    dom.get('error'+elem.id).style.display='';
	 			dom.get("contractorBill_error").style.display='';
	...
	var statAmountTextBoxFormatter = createStatAmountTextBoxFormatter(10,8);
...
function createDropdownFormatterForRecovery(listObj,postfixFieldValue, prefixkey){
	...
	var makeStatutoryDeductionsDataTable = function() {
			var statutoryDeductionsColumnDefs = [ 
				...
				{key:"creditAmount",label:'Amount <span class="mandatory">*</span>', formatter:statAmountTextBoxFormatter,sortable:false, resizeable:false},
   ...
   function calculateStatutoryDeduction(){
	...
	<tr>
		...
		<td align="right" class="headingwk" style="border-left-width: 0px">
	      <a id="statutorydeductionrow" href="#" onclick="statutoryDeductionsDataTable.addRow({SlNo:statutoryDeductionsDataTable.getRecordSet().getLength()+1});return false;"><img height="16" border="0" width="16" alt="Add Statutory-Deductions" src="${pageContext.request.contextPath}/image/add.png" /></a>
	    </td>
	</tr>
	<tr>
      <td colspan="4">

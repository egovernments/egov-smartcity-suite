...
var textboxFormatter = function(el, oRecord, oColumn, oData) {
    ...
    markup="<input type='text' id='"+id+"' name='"+fieldName+"' size='"+size+"' maxlength='"+maxlength+"' value='"+value+"' class='selectamountwk' onblur='calculateNonSOR(this,\""+oRecord.getId()+"\");' /><span id='error"+id+"' style='display:none;color:red;font-weight:bold'>&nbsp;x</span>";
    el.innerHTML = markup;
}
...
var textboxDescFormatter = function(el, oRecord, oColumn, oData) {
 var value = (YAHOO.lang.isValue(oData))?oData:"";
   var fieldName = "nonSorActivities[" + oRecord.getCount() + "]." + "nonSor.description";
	markup="<input type='text' id='"+oColumn.getKey()+oRecord.getId()+"' class='selectmultilinewk' value='"+value+"' size='90' maxlength='4000' name='"+fieldName+"' onblur='validateNonSORDescription(\""+oRecord.getId()+"\");'/>"
	el.innerHTML = markup;	 	
}
...
function createNonSorIdHiddenFormatter(el, oRecord, oColumn, oData){
var hiddenFormatter = function(el, oRecord, oColumn, oData) {
    var value = (YAHOO.lang.isValue(oData))?oData:"";
    var id=oColumn.getKey()+oRecord.getId();
    var fieldValue;
    var fieldName = "nonSorActivities[" + oRecord.getCount() + "]." + "nonSor.id";
  	fieldValue=value;
   	markup="<input type='hidden' id='"+id+"' name='"+fieldName+"' value='"+fieldValue+"' />";
    el.innerHTML = markup;
}
...
var hiddenNonSorIdFormatter = createNonSorIdHiddenFormatter(10,10);

function createRCNonSorHiddenFormatter(el, oRecord, oColumn, oData){
var hiddenFormatter = function(el, oRecord, oColumn, oData) {
    var value = (YAHOO.lang.isValue(oData))?oData:"";
    var id=oColumn.getKey()+oRecord.getId();
    var fieldValue;
...
var hiddenRCIdNonSorFormatter = createRCNonSorHiddenFormatter(10,10);
...
var makeNonSORDataTable = function() {
	...
	var nonSorColumnDefs = [ 
		{key:"NonSorId", hidden:true,formatter:hiddenNonSorIdFormatter,sortable:false, resizeable:false} ,
		{key:"NonSorHiddenId", hidden:true,sortable:false, resizeable:false} ,
        ...
        {key:"rcId", hidden:true,formatter:hiddenRCIdNonSorFormatter,sortable:false, resizeable:false},
   ...
   <s:if test="%{model.id==null || (sourcepage=='inbox' && (model.currentState.value=='NEW' || model.currentState.value=='REJECTED')) }" >    
		if (column.key == 'NonSorDelete') { 			
				...
				removeDetails("",record.getData('SlNo'));	
	...
	if(nonSorDataTable.getRecordSet().getLength()=="" || nonSorDataTable.getRecordSet().getLength()<1){
		...
		<table id="nonSorHeaderTable" width="100%" border="0" cellspacing="0" cellpadding="0">              	
              	<tr>
                	...
                	<td  align="right" id="addnonSorButtn" class="headingwk" style="border-left-width: 0px">
	...
	<s:if test="%{estimateRateContractList.isEmpty()}"> 
		<s:iterator id="nonsoriterator" value="NonSORActivities" status="row_status"> 
            nonSorDataTable.addRow({NonSorId:'<s:property value="nonSor.id"/>',
            						NonSorHiddenId:'<s:property value="nonSor.id"/>',
            						Sl_No:'<s:property value="#row_status.count"/>',
                                    ...
                                    nonSorMSheetLength:'<s:property value="measurementSheetList.size"/>',
                                    Delete:'X'});
        ...
        dom.get("nonsor"+column.getKey()+record.getId()).value = '<s:property value="quantityString"/>';
     ...
     <s:else> 
		var count=1;
		<s:iterator id="rcdetailNonSorIterator" value="estimateRCDetails" status="row_status">
			<s:if test="%{activity.nonSor!=null}"> 
				 nonSorDataTable.addRow({NonSorId:'<s:property value="activity.nonSor.id"/>',
            						NonSorHiddenId:'<s:property value="activity.nonSor.id"/>',
            						Sl_No:count,
                                    SlNo:count,
                                    nonSordescription:'<s:property value="activity.nonSor.descriptionJS" escape="false"/>',
                                    Uom:'<s:property value="activity.nonSor.uom.id"/>',
                                    nonSorUom:'<s:property value="activity.nonSor.uom.id"/>',
                                    rate:'<s:property value="activity.rate"/>',								   	 
												   
                                    quantity:'<s:property value="activity.quantity"/>',
                                    NonSorEstdAmt:roundTo('<s:property value="activity.amount"/>'),
                                    serviceTaxPerc:'<s:property value="activity.serviceTaxPerc"/>',
                                    NonSorTaxAmt:roundTo('<s:property value="activity.taxAmount"/>'),
                                    NonSorTotal:roundTo('<s:property value="activity.amountIncludingTax"/>'),
                                    nonSorMSheetLength:'<s:property value="activity.measurementSheetList.size"/>',
                                    Delete:'X',
		        ...
		        var column = nonSorDataTable.getColumn('nonSordescription');
		        // Important to use escape=false. Otherwise struts will replace double quotes with &quote;  
		        dom.get(column.getKey()+record.getId()).value = '<s:property value="activity.nonSor.descriptionJS" escape="false"/>';
		        
		        var column = nonSorDataTable.getColumn('rate');  
		        dom.get("nonsor"+column.getKey()+record.getId()).value = '<s:property value="activity.rate"/>';
		        
		        var column = nonSorDataTable.getColumn('quantity');  
		        dom.get("nonsor"+column.getKey()+record.getId()).value = '<s:property value="activity.quantity"/>';
		    
		        column= nonSorDataTable.getColumn('serviceTaxPerc');
		        dom.get("nonsor"+column.getKey()+record.getId()).value = '<s:property value="activity.serviceTaxPerc"/>';
		        
		        column = nonSorDataTable.getColumn('Uom');
		        for(i=0; i < uomDropdownOptions.length; i++) {
		            if (uomDropdownOptions[i].value == '<s:property value="activity.nonSor.uom.id"/>') {
		                 nonSorDataTable.getTdEl({record:record, column:column}).getElementsByTagName("select").item(0).selectedIndex = i;
		                 nonSorDataTable.updateCell(record,nonSorDataTable.getColumn('nonSorUomValue'),uomDropdownOptions[i].label);		                
		            }
		        }
		      
		        dom.get('nonSordescription'+record.getId()).readonly=true;    
		        dom.get('nonSordescription'+record.getId()).disabled=true;        	
            	dom.get('nonsorrate'+record.getId()).readonly=true;
            	dom.get('nonsorrate'+record.getId()).disabled=true;
            	nonSorDataTable.getTdEl({record:record, column:nonSorDataTable.getColumn('Uom')}).getElementsByTagName("select").item(0).disabled=true;

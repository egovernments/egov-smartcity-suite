...
function setMultipleEstimateIds(elem,fundId){
	var flag = true;
	if(estimateArr.length == 0){
		selectedFund = fundId;
	}else if(fundId != selectedFund){
		alert('Estimates should be of Same Fund');
		flag = false;
		elem.checked = false;
	}
	...
	if(flag){
		if(elem.checked == true){
			estimateArr.push(elem.value);
		}else{
			var index = estimateArr.indexOf(elem.value);
			estimateArr.splice(index,1);
		}
	}
...
<display:table name="searchResult" pagesize="30" uid="currentRow"
	...
	<s:if
		test="%{source=='createServiceOrderNew'}">
	...
	<display:column headerClass="pagetableth" class="pagetabletd"
		...
		<input name="checkbox" type="checkbox" id="checkbox"
			...
			onClick="setMultipleEstimateIds(this,<s:property value='%{#attr.currentRow.financialDetails[0].fund.id}'/>);" />
	</display:column>
	</s:if><s:else> 
	<display:column headerClass="pagetableth" class="pagetabletd"
		title="Select" titleKey="column.title.select"

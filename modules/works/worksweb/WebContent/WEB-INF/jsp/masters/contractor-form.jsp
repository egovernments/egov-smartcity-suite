...
function validateContractorFormAndSubmit() {
    ...
    if(errors) {
    	...
    	var lenFrm=document.contractor.elements.length;
		for(i=0;i<lenFrm;i++){
			document.contractor.elements[i].readonly=false;	
			document.contractor.elements[i].disabled=false;	
		} 
    	return true;
   	}
}
...
var contractorIdHiddenFormatter= createHiddenFormatter(5,5);
...
var makeContractorDataTable = function() {
	...
	var contractorColumnDefs = [ 
		{key:"contractor", hidden:true, formatter:contractorIdHiddenFormatter, sortable:false, resizeable:false},
       ...
       <tr>
         <td>
		   ...
		   <tr>
            	...
            	<td class="whitebox2wk"><s:textfield name="email" id="email" maxlength="120" cssClass="selectwk" value = "%{email}" /></td>
           </tr>
		   <tr>
           ...
           <tr>
            	...
            	<td class="whitebox2wk"><s:textfield name="bankAccount" id="bankAccount" maxlength="15" cssClass="selectwk" value = "%{bankAccount}" /></td>
		        ...
		        contractorDataTable.addRow(
			        ...
			        for(i=0; i < departmentDropdownOptions.length; i++) {
			            if (departmentDropdownOptions[i].value == '<s:property value="department.id"/>') {
			            	selIndex=contractorDataTable.getTdEl({record:record, column:column}).getElementsByTagName("select").item(0).selectedIndex = i;
			            }
			        }
			        ...
			        <s:if test="%{getDisableContractorDepartments().contains(department.deptCode)}">
				        column = contractorDataTable.getColumn('registrationNumber');
						...
						column = contractorDataTable.getColumn('departmentName');
						contractorDataTable.getTdEl({record:record, column:column}).getElementsByTagName("select").item(0).disabled=true;
						column = contractorDataTable.getColumn('gradeName');
						contractorDataTable.getTdEl({record:record, column:column}).getElementsByTagName("select").item(0).disabled=true;
			        </s:if>
			       </s:iterator>         

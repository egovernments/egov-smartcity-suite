...
<html>
	...
	<body>
		<s:form name="searchContractorForm" action="searchContractorAction" theme="simple" >
			...
			<s:hidden name="est_department" id="est_department" />
			<s:hidden name="est_zone" id="est_zone" />
			<s:hidden name="est_date" id="est_date" />
			<s:hidden name="rcType" id="rcType" />
			<s:hidden name="est_function" id="est_function" />
			<s:hidden name="est_fund" id="est_fund" />
			<s:hidden name="est_budgetGroup" id="est_budgetGroup" />
								...
								<tr>
									<td>
										<table width="100%" border="0" cellspacing="0" cellpadding="0">
											...
											<tr>
											<s:if test="%{sourcepage=='estimate'}">
												<td width="19%" class="greyboxwk">
													<s:text name='contractor.search.label.rcNumber' />:
												</td>
												<td width="37%" class="greybox2wk">
													<s:textfield name="rcNumber" id="rcNumber" cssClass="selectboldwk"/>
												</td>
											</s:if>	
											<s:else>
												<td width="19%" class="greyboxwk">
		...
		<script type="text/javascript">
			...
			function returnBackToParent(contractorId,contractorCode,contractorname) {
				...
				if(wind==undefined){
					wind=window.opener;
					if(dom.get('sourcepage').value=='estimate')
						data='loadContractor'+'`~`'+row_id+ '`~`' +contractorId+ '`~`' + contractorCode+ '`~`' + contractorname;
					else
					...
					wind=window.dialogArguments;
					if(dom.get('sourcepage').value=='estimate')
						wind.result='loadContractor'+'`~`'+row_id+ '`~`' +contractorId+ '`~`' + contractorCode+ '`~`' + contractorname;
					else

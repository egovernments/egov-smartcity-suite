...
<script type="text/javascript" src="/jsutils/prototype/prototype.js"></script>
...
var descriptionFormatter = function(el, oRecord, oColumn, oData) {
    ...
    markup="<span class='yui-dt-liner'>"+value+"</span>"+hint.replace(/@fulldescription@/g,oRecord.getData('FullDescription'))
    el.innerHTML = markup;
}
...
function validateDetailsBeforeSubmit(tenderNegotiationForm){
	 if(document.tenderNegotiationForm.tenderType.value==dom.get("tenderType")[1].value){
		...
		<s:if test="%{ model.id==null || (model.currentState!=null && model.currentState.value!=null && (model.currentState.value=='NEW'|| model.currentState.value=='REJECTED'))}">
   ...
   <s:else>	
   		itemRateDataTable.addRow({
			...
			activity:'<s:property value="id"/>',
                        ...
                        schNo:'',
                        schCode:'',
                        description:'<s:property value="nonSor.description"/>',
                        estimatedQuantity:'<s:property value="quantity"/>',
                         estimatedQty:'<s:property value="quantity"/>',
                        ...
                        FullDescription:'<s:property value="nonSor.description"/>',
                        SORDiff:''});
   </s:else>
    var record = itemRateDataTable.getRecord(parseInt('<s:property value="#row_status.index"/>'));
  
    dom.get("estTotal").innerHTML=roundTo(getNumericValueFromInnerHTML("estTotal") - 0.0 + getNumber(record.getData("estimatedAmount")));  
    dom.get("negotiatedTotal").innerHTML=roundTo(getNumericValueFromInnerHTML("negotiatedTotal") - 0.0 + getNumber(record.getData("negotiatedAmount")));
    dom.get("marketRateTotal").innerHTML=roundTo(getNumericValueFromInnerHTML("marketRateTotal") - 0.0 + getNumber(record.getData("marketRateAmount")));
    calculateQuotedTotalForContractors();
</s:iterator>
</s:if>  
	...
	<s:if test="%{code!=''}">
	 	itemRateDataTable.addRow({schId:'<s:property value="activity.schedule.id"/>',
			activity:'<s:property value="activity.id"/>',
                        ...
                        description:'<s:property value="summary"/>',
                        estimatedQuantity:'<s:property value="quantity"/>',
                        estimatedQty:'<s:property value="quantity"/>',
                        ...
                        marketRate:'',
                        marketRateAmount:'',                       
                        FullDescription:'<s:property value="description"/>',
                        SORDiff:''});	
	...
	<s:if test="%{activity.schedule!=null}">
    	itemRateDataTable.addRow({schId:'<s:property value="activity.schedule.id"/>',
			activity:'<s:property value="activity.id"/>',
                        ...
                        marketRate:'',
                        marketRateAmount:'',                       
                        FullDescription:'<s:property value="activity.schedule.description"/>',
                        SORDiff:''});
   </s:if>
   <s:else>
   		itemRateDataTable.addRow({schId:'',
			activity:'<s:property value="activity.id"/>',
                        ...
                        FullDescription:'<s:property value="activity.nonSor.description"/>',
                        SORDiff:''});
   </s:else>
    var record = itemRateDataTable.getRecord(parseInt('<s:property value="#row_status.index"/>')); 
    var column = itemRateDataTable.getColumn('schId');  

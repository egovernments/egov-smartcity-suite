...
class Stores_TC_Modify_UOM_category< Test::Unit::TestCase 
...
def test_0010_modify_uom_category_valid_values

  tc = '0010'
  hash_array = get_input_data_oo_hasharray(tc, MODIFY_UOM_CATEGORY_SHEET, Stores_constants::INPUT_OO)
  attributes = hash_array[1]
  test_data = hash_array[2]

  
 create_uom_category(tc, test_data, attributes) 
 modify_uom_category(tc, test_data, attributes)
 view_uom_category_success(tc, test_data, attributes)
 
end
  
  
def test_0020_modify_uom_category_with_special_char_for_uomcategory_description

...
public  boolean checkPos(Integer posId,Date fromDate,Date toDate,Integer empId,String isPrimary)
{
	...
	try
	{
		...
		if(fromDate != null && toDate != null)
		{
			...
			main +=	"((ev.toDate is null ) or " +
					" (ev.fromDate <= :fromDate and ev.toDate >= :toDate) or " +
		...
		{
			qry = getCurrentSession().createQuery("from Assignment ev  where ev.position.id = :posId and ((ev.toDate is null ) or (ev.fromDate <= :fromDate AND ev.toDate >= :fromDate))");
 ...
 */
	...
	public List<Integer> getAssignmentsForEmp(Integer empId,Date givenDate)throws EGOVException
	{
		...
		try
		{
			...
			query = sessionFactory.getCurrentSession().createSQLQuery(stringbuffer.toString()).addScalar("ASS_ID", IntegerType.INSTANCE);
	...
	public  List searchEmployee(Integer status,Date fromDate,Date toDate)throws Exception
	{
		...
		try
		{
				...
				/*else if(statusType.getModuletype().equals("Employee") && statusType.getDescription().equals("Suspended"))
				{
					...
					if(fromDate != null && toDate!=null)
					{
						mainStr += " and ev.employee.idPersonalInformation in " +
	...
	public  boolean isFilterByDept()
	  {
	      ...
	      AppConfigValues configValue = appConfigValuesDAO.getAppConfigValueByDate("EIS-PAYROLL","FILTERBYDEPT",new Date());
	      if (null != configValue){
